...
// 121201
// attempt to eliminate severe persistent error: string handles locked in UFmd2_CfsCFS...ihc->IHC2 disposehandle->IHCdisposehandle2 (NO SUCCESS)
...
// 091002 q&d (not thoroughly tested) clip strings one character shorter than previously in UFmd2_CfsSetVarVal()
...
int UFmd2_CfsCreateFile(			void * );  // double Channels; double BlockSize; Handle Comment; Handle FName; Handle sRes; }* p)
int UFmd2_CfsOpenFile(				void * );  // ...Handle FName; Handle sRes; }* p)
int UFmd2_CfsCloseFile(			void * );  // double hnd;    Handle sRes; }* p)
int UFmd2_CfsCommitFile(			void * );  // double hnd;    Handle sRes; }* p)
int UFmd2_CfsGetGenInfo(        void * );  // ...
int UFmd2_CfsGetFileInfo(       void * );  // ...
int UFmd2_CfsGetFileChan(       void * );  // ...
int UFmd2_CfsSetFileChan(       void * );  // double Other;  double Spacing;     double DataKind; double DataType; Handle xUnits; 
int UFmd2_CfsSetDSChan(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsGetDSChan(         void * );  // ....
int UFmd2_CfsGetChanData(       void * );  // ....
int UFmd2_CfsInsertDS(          void * );  // double FlagSet; double DataSection; double hnd;    Handle sRes; }* p)
int UFmd2_CfsWriteData(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsReadData(          void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsSetVarVal(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsGetVarVal(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsGetVarType(        void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsGetVarDesc(        void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFmd2_CfsSetDescriptor(		void * );  // nType, nChans, FiveElements, //...int maxDSVar, int maxFileVar
  ...
  //  Funktion          or Message method	   Used that often
{
   ...
   { UFmd2_CfsCommitFile					},
   { UFmd2_CfsGetGenInfo					},   
   { UFmd2_CfsGetFileInfo				},   
   { UFmd2_CfsGetFileChan				},   
   { UFmd2_CfsSetFileChan				},   
   { UFmd2_CfsSetDSChan					},   
   { UFmd2_CfsGetDSChan					},   
   { UFmd2_CfsGetChanData				},   
   { UFmd2_CfsInsertDS					},   
   { UFmd2_CfsWriteData					},   
	{ UFmd2_CfsReadData					},   
	{ UFmd2_CfsSetVarVal					},   
...
int UFmd2_CfsCreateFile( struct { double ErrMode;   double maxFileVar; double maxDSVar; double Channels; 
...
//   Handle CFileName  = IHC2( p->FileName, &hState1 );
   ...
   char buf[200];
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (UFmd2_CfsCreateFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
 ...
 // WORKAROUND for the above error: check accessibility of desired path before erroneous CreateCFSFile() is called 
   ...
   if ( code )
   {
      sprintf( buf, "++++Error: Cannot open '%s' , (UFmd2_CfsCreateFile / XOPCreateFile return code=%d/0x%x) \r", nativePath, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
      DisposeHandleIHC( CComment );		// 050204
      DisposeHandleIHC( CFileName );	// 050204
      p->res = code < 0 ? code : -code; // ...as a positive code would be interpreted as a valid CFS file handle
   ...
   {                                    // the file has been created: delete it again...
      ...
      {
         sprintf( buf, "\t\tUFmd2_CfsCreateCFS...receives '%s' '%s' %d %d ptr ptr %d %d,  returns code=%d \r",
...
int UFmd2_CfsOpenFile( struct { double ErrMode;   double memoryTable; double enableWrite; Handle FileName; double res; }* p)
...
//   Handle CFileName  = IHC2( p->FileName, &hState );
   ...
   short  code;
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (UFmd2_CfsOpensFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
   ...
   { 
      ...
      if ( (int)p->ErrMode & MSGLINE )
      {
         sprintf( buf, "\t\tUFmd2_CfsOpenCFS...receives '%s' %d %d ,  returns code=%d \r",
...
int UFmd2_CfsCloseFile( struct { double ErrMode; double hnd; double res; }* p)
...
int UFmd2_CfsCommitFile( struct { double ErrMode; double hnd; double res; }* p)
...
int UFmd2_CfsGetGenInfo( struct { double ErrMode;  double hnd; Handle sRes; }* p)
{
...
// Flaw / Limitation: Neither 'time' nor 'date' nor 'comment' may contain the character 'CFSSEP' 
   ...
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetGenInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
	...
	//sprintf( errbuf, "UFmd2_CfsGetGenInfo [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
...
int UFmd2_CfsGetFileInfo( struct { double ErrMode; double hnd; Handle sRes; }* p)
{
...
// Difference to CFS: as IGOR doesn't know pointers the 4 CFS variable pointers are returned as string list   
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetFileInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFmd2_CfsGetFileInfo [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFmd2_CfsGetFileChan( struct { double ErrMode; double Channel;  double hnd; Handle sRes; }* p)
...
// Flaw / Limitation: Neither 'Units' nor 'Description' may contain the character 'CFSSEP' 
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetFileChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFmd2_CfsGetFileChan [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFmd2_CfsSetFileChan( struct { double ErrMode; double Other;  double Spacing;     double DataKind; double DataType; Handle xUnits; 
...
int UFmd2_CfsSetDSChan( struct { double ErrMode; double xOffset;     double xScale;      double yOffset; double yScale; double Points;
...
int UFmd2_CfsGetDSChan( struct { double ErrMode; double DataSection; double Channel; double hnd; Handle sRes; }* p)
...
// Difference to CFS: the 6 CFS variabls are returned as string list   
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetDSChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFmd2_CfsGetDSChan [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFmd2_CfsGetChanData( struct { double ErrMode;	double AreaSize;	waveHndl wDataADS;	double NumElements;
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   int		 pts;
	...
	// DebugPrintWaveProperties( "UFmd2_CfsGetChanData", p->wDataADS ); 	// 050128

	if ( p->wDataADS == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd2_CfsGetChanData() received nonvalid wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd2_CfsGetChanData() received non integer wave \r" );
		XOPNotice( errBuf );
...
int UFmd2_CfsInsertDS( struct { double ErrMode; double FlagSet; double DataSection; double hnd; double res; }* p)
...
int UFmd2_CfsWriteData( struct { double ErrMode; waveHndl wDataADS; double Bytes; double StartOffset; double DataSection; double hnd; double res; }* p)
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   void *Raw;
	...
	// DebugPrintWaveProperties( "UFmd2_CfsWriteData  ", p->wDataADS ); 	// 050128

	if ( p->wDataADS == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd2_CfsWriteData() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );			           // return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if (   WaveType(p->wDataADS) != NT_I16 ) // check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFmd2_CfsWriteData() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );				        // return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   ...
   if ( ErrMode & MSGLINE )
	{
	   sprintf( errBuf, "\t\t\tUFmd2_CfsWriteData() has written %d bytes \r", Bytes );
		XOPNotice( errBuf );
	}
...
int UFmd2_CfsReadData( struct { double ErrMode; waveHndl wDataADS; double Bytes; double StartOffset; double DataSection; double hnd; double res; }* p)
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   void *Raw;
	...
	// DebugPrintWaveProperties( "UFmd2_CfsReadData    ", p->wDataADS ); 	// 050128
...
int UFmd2_CfsSetVarVal( struct { double ErrMode; Handle VarAsString; double DataSection; double VarKind; double VarNo; double hnd; double res; }* p)
...
*/
   ...
   char      errbuf[10000];
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\tUFmd2_CfsSetVarVal(string) \t hnd:%d  varno:%2d \r", hnd, VarNo );   
         XOPNotice( errbuf );
      }
   ...
   if ( VarType == LSTR )                          // String
   {
		if ( PassedSize >= VarSize )
		{
	      ...
	      sprintf ( errbuf, "++++Error: UFmd2_CfsSetVarVal() String too long, truncated from %d to %d chars: '%s'\r", PassedSize, VarSize-1, *CVarAsString );   
			XOPNotice( errbuf );
      } 
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\tUFmd2_CfsSetVarVal(string) \t type:%d  len:%2d (max:%d)  string:'%s' \r", VarType, PassedSize, VarSize, *CVarAsString );   
         XOPNotice( errbuf );
      }
   }
// 050203
...
int UFmd2_CfsGetVarVal( struct { double ErrMode; double DataSection; double VarKind; double VarNo; double hnd; Handle sRes; }* p)
...
//? two sorts of errors can occur: memory and CFS ...not all are handled correctly...
{
   ...
   // sprintf ( errbuf, "UFmd2_CfsGetVarVal( any  )  expected vartype:%d  varsize:%d  \r", VarType, VarSize ); XOPNotice( errbuf );
   ...
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetVarVal() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;											// out of memory
   }
   else
   ...
   if ( (int)p->ErrMode & MSGLINE ) {
      sprintf ( errbuf, "\t\t\t\t\tUFmd2_CfsGetVarVal(  any  )\t type:%d  len:%2d  varsize:%d  string:'%s'\r", VarType, strlen(sValOrTxt), VarSize, sValOrTxt ); 
      XOPNotice( errbuf );
	}
	err = 0;
...
int UFmd2_CfsGetVarType( struct { double ErrMode; double VarKind; double VarNo; double hnd; double res; }* p)
...
int UFmd2_CfsGetVarDesc( struct { double ErrMode; double VarKind; double VarNo; double hnd; Handle sRes; }* p)
...
//030401	
//sprintf( tmp, "UFmd2_CfsGetVarDesc() hnd:%d VarSize:%d %s VarType:%d %s Units:%s %s Description:%s \r", hnd, VarSize, CFSSEP, VarType, CFSSEP, Units, CFSSEP, Description ); XOPNotice( tmp );

   sprintf( tmp, "%d%s%d%s%s%s%s", VarSize, CFSSEP, VarType, CFSSEP, Units, CFSSEP, Description );
   len = strlen( tmp );
   if (( str1 = NewHandle( len )) == NIL ) // get output handle , do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFmd2_CfsGetVarDesc() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				               // out of memory
   }
   else                                      // string length is OK so return string 
   ...
   //sprintf( buf, "UFmd2_CfsGetVarDesc:  '%s' \r", *str1 ); XOPNotice( buf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}

int UFmd2_CfsSetDescriptor( struct { Handle sEle; double Chan; double Type; double res; }* p)
