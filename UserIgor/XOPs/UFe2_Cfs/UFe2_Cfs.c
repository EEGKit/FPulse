...
// 121201
// attempt to eliminate severe persistent error: string handles locked in UFe2_CfsCFS...ihc->IHC2 disposehandle->IHCdisposehandle2 (NO SUCCESS)
...
// 091002 q&d (not thoroughly tested) clip strings one character shorter than previously in UFe2_CfsSetVarVal()
...
int UFe2_CfsCreateFile(			void * );  // double Channels; double BlockSize; Handle Comment; Handle FName; Handle sRes; }* p)
int UFe2_CfsOpenFile(				void * );  // ...Handle FName; Handle sRes; }* p)
int UFe2_CfsCloseFile(			void * );  // double hnd;    Handle sRes; }* p)
int UFe2_CfsCommitFile(			void * );  // double hnd;    Handle sRes; }* p)
int UFe2_CfsGetGenInfo(        void * );  // ...
int UFe2_CfsGetFileInfo(       void * );  // ...
int UFe2_CfsGetFileChan(       void * );  // ...
int UFe2_CfsSetFileChan(       void * );  // double Other;  double Spacing;     double DataKind; double DataType; Handle xUnits; 
int UFe2_CfsSetDSChan(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsGetDSChan(         void * );  // ....
int UFe2_CfsGetChanData(       void * );  // ....
int UFe2_CfsInsertDS(          void * );  // double FlagSet; double DataSection; double hnd;    Handle sRes; }* p)
int UFe2_CfsWriteData(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsReadData(          void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsSetVarVal(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsGetVarVal(         void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsGetVarType(        void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsGetVarDesc(        void * );  // double xOffset;     double xScale;      double yOffset; double yScale; double Points;
int UFe2_CfsSetDescriptor(		void * );  // nType, nChans, FiveElements, //...int maxDSVar, int maxFileVar
...
int UFe2_CfsCreateFile( struct { double ErrMode;   double maxFileVar; double maxDSVar; double Channels; 
...
//   Handle CFileName  = IHC2( p->FileName, &hState1 );
   ...
   char buf[200];
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (UFe2_CfsCreateFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
 ...
 // WORKAROUND for the above error: check accessibility of desired path before erroneous CreateCFSFile() is called 
   ...
   if ( code )
   {
      sprintf( buf, "++++Error: Cannot open '%s' , (UFe2_CfsCreateFile / XOPCreateFile return code=%d/0x%x) \r", nativePath, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
      DisposeHandleIHC( CComment );		// 050204
      DisposeHandleIHC( CFileName );	// 050204
      p->res = code < 0 ? code : -code; // ...as a positive code would be interpreted as a valid CFS file handle
   ...
   {                                    // the file has been created: delete it again...
      ...
      {
         sprintf( buf, "\t\tUFe2_CfsCreateCFS...receives '%s' '%s' %d %d ptr ptr %d %d,  returns code=%d \r",
...
int UFe2_CfsOpenFile( struct { double ErrMode;   double memoryTable; double enableWrite; Handle FileName; double res; }* p)
...
//   Handle CFileName  = IHC2( p->FileName, &hState );
   ...
   short  code;
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (UFe2_CfsOpensFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
   ...
   { 
      ...
      if ( (int)p->ErrMode & MSGLINE )
      {
         sprintf( buf, "\t\tUFe2_CfsOpenCFS...receives '%s' %d %d ,  returns code=%d \r",
...
int UFe2_CfsCloseFile( struct { double ErrMode; double hnd; double res; }* p)
...
int UFe2_CfsCommitFile( struct { double ErrMode; double hnd; double res; }* p)
...
int UFe2_CfsGetGenInfo( struct { double ErrMode;  double hnd; Handle sRes; }* p)
{
...
// Flaw / Limitation: Neither 'time' nor 'date' nor 'comment' may contain the character 'CFSSEP' 
   ...
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetGenInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
	...
	//sprintf( errbuf, "UFe2_CfsGetGenInfo [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
...
int UFe2_CfsGetFileInfo( struct { double ErrMode; double hnd; Handle sRes; }* p)
{
...
// Difference to CFS: as IGOR doesn't know pointers the 4 CFS variable pointers are returned as string list   
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetFileInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFe2_CfsGetFileInfo [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFe2_CfsGetFileChan( struct { double ErrMode; double Channel;  double hnd; Handle sRes; }* p)
...
// Flaw / Limitation: Neither 'Units' nor 'Description' may contain the character 'CFSSEP' 
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetFileChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFe2_CfsGetFileChan [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFe2_CfsSetFileChan( struct { double ErrMode; double Other;  double Spacing;     double DataKind; double DataType; Handle xUnits; 
...
int UFe2_CfsSetDSChan( struct { double ErrMode; double xOffset;     double xScale;      double yOffset; double yScale; double Points;
...
int UFe2_CfsGetDSChan( struct { double ErrMode; double DataSection; double Channel; double hnd; Handle sRes; }* p)
...
// Difference to CFS: the 6 CFS variabls are returned as string list   
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetDSChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
	...
	//sprintf( errbuf, "UFe2_CfsGetDSChan [len:%d]:  '%s' \r", len, tmp ); XOPNotice( errbuf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}
...
int UFe2_CfsGetChanData( struct { double ErrMode;	double AreaSize;	waveHndl wDataADS;	double NumElements;
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   int		 pts;
	...
	// DebugPrintWaveProperties( "UFe2_CfsGetChanData", p->wDataADS ); 	// 050128

	if ( p->wDataADS == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFe2_CfsGetChanData() received nonvalid wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFe2_CfsGetChanData() received non integer wave \r" );
		XOPNotice( errBuf );
...
int UFe2_CfsInsertDS( struct { double ErrMode; double FlagSet; double DataSection; double hnd; double res; }* p)
...
int UFe2_CfsWriteData( struct { double ErrMode; waveHndl wDataADS; double Bytes; double StartOffset; double DataSection; double hnd; double res; }* p)
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   void *Raw;
	...
	// DebugPrintWaveProperties( "UFe2_CfsWriteData  ", p->wDataADS ); 	// 050128

	if ( p->wDataADS == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFe2_CfsWriteData() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );			           // return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if (   WaveType(p->wDataADS) != NT_I16 ) // check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFe2_CfsWriteData() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );				        // return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   ...
   if ( ErrMode & MSGLINE )
	{
	   sprintf( errBuf, "\t\t\tUFe2_CfsWriteData() has written %d bytes \r", Bytes );
		XOPNotice( errBuf );
	}
...
int UFe2_CfsReadData( struct { double ErrMode; waveHndl wDataADS; double Bytes; double StartOffset; double DataSection; double hnd; double res; }* p)
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   void *Raw;
	...
	// DebugPrintWaveProperties( "UFe2_CfsReadData    ", p->wDataADS ); 	// 050128
...
int UFe2_CfsSetVarVal( struct { double ErrMode; Handle VarAsString; double DataSection; double VarKind; double VarNo; double hnd; double res; }* p)
...
*/
   ...
   char      errbuf[10000];
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\tUFe2_CfsSetVarVal(string) \t hnd:%d  varno:%2d \r", hnd, VarNo );   
         XOPNotice( errbuf );
      }
   ...
   if ( VarType == LSTR )                          // String
   {
		if ( PassedSize >= VarSize )
		{
	      ...
	      sprintf ( errbuf, "++++Error: UFe2_CfsSetVarVal() String too long, truncated from %d to %d chars: '%s'\r", PassedSize, VarSize-1, *CVarAsString );   
			XOPNotice( errbuf );
      } 
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\tUFe2_CfsSetVarVal(string) \t type:%d  len:%2d (max:%d)  string:'%s' \r", VarType, PassedSize, VarSize, *CVarAsString );   
         XOPNotice( errbuf );
      }
   }
// 050203
...
int UFe2_CfsGetVarVal( struct { double ErrMode; double DataSection; double VarKind; double VarNo; double hnd; Handle sRes; }* p)
...
//? two sorts of errors can occur: memory and CFS ...not all are handled correctly...
{
   ...
   // sprintf ( errbuf, "UFe2_CfsGetVarVal( any  )  expected vartype:%d  varsize:%d  \r", VarType, VarSize ); XOPNotice( errbuf );
   ...
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetVarVal() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;											// out of memory
   }
   else
   ...
   if ( (int)p->ErrMode & MSGLINE ) {
      sprintf ( errbuf, "\t\t\t\t\tUFe2_CfsGetVarVal(  any  )\t type:%d  len:%2d  varsize:%d  string:'%s'\r", VarType, strlen(sValOrTxt), VarSize, sValOrTxt ); 
      XOPNotice( errbuf );
	}
	err = 0;
...
int UFe2_CfsGetVarType( struct { double ErrMode; double VarKind; double VarNo; double hnd; double res; }* p)
...
int UFe2_CfsGetVarDesc( struct { double ErrMode; double VarKind; double VarNo; double hnd; Handle sRes; }* p)
...
//030401	
//sprintf( tmp, "UFe2_CfsGetVarDesc() hnd:%d VarSize:%d %s VarType:%d %s Units:%s %s Description:%s \r", hnd, VarSize, CFSSEP, VarType, CFSSEP, Units, CFSSEP, Description ); XOPNotice( tmp );

   sprintf( tmp, "%d%s%d%s%s%s%s", VarSize, CFSSEP, VarType, CFSSEP, Units, CFSSEP, Description );
   len = strlen( tmp );
   if (( str1 = NewHandle( len )) == NIL ) // get output handle , do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFe2_CfsGetVarDesc() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				               // out of memory
   }
   else                                      // string length is OK so return string 
   ...
   //sprintf( buf, "UFe2_CfsGetVarDesc:  '%s' \r", *str1 ); XOPNotice( buf );  
   PrintFileError( (int)p->ErrMode ); // We must handle the errors here because we cannot return the error code to the calling function
   p->sRes = str1;
   return err;
}

int UFe2_CfsSetDescriptor( struct { Handle sEle; double Chan; double Type; double res; }* p)
