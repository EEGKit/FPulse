//
//	UFmd1_Utils.c -- Utility routines 
//
...
// 070223
//int UFmd1_UtilGetSystemDirectory(void * );  //

int UFmd1_UtilConvolve(				void * );  // 
int UFmd1_UtilWaveCopy(				void * );  // 
int UFmd1_UtilWaveExtract(			void * );  // 
int UFmd1_UtilRealWaveSet(			void * );  // 
int UFmd1_UtilRealWaveMultiply(	void * );  // 
int UFmd1_UtilRealWaveMultiplyAdd(void * );  // 
//int UFmd1_UtilCorrelation_(		void * );  // 
int UFmd1_UtilCorrelation(			void * );  // 
int UFmd1_UtilFileDialog(			void * );  // 
//int UFmd1_UtilIsValidString(		void * );  //
int UFmd1_UtilMemoryLoad(			void * );  //
int UFmd1_UtilTotalPhys(			void * );  //
int UFmd1_UtilAvailPhys(			void * );  //
int UFmd1_UtilTotalVirtual(		void * );  //
int UFmd1_UtilAvailVirtual(		void * );  //
int UFmd1_UtilContiguousMemory(	void * );  //
int UFmd1_UtilHeapCompact(			void * );  //
int UFmd1_UtilGetAllUsersDirectory(void * );  //
int UFmd1_UtilGetSystemDirectory(void * );  //


/////////////////////////////////////////////////////////////////////////////////////////

// SAME ORDER HERE IN  '(*sFunc[])' AND IN UFmd1_UtilsWinCustom.RC
...
{
// 070223
//	{ UFmd1_UtilGetSystemDirectory		},		

	{ UFmd1_UtilConvolve						},
	{ UFmd1_UtilWaveCopy						},
	{ UFmd1_UtilWaveExtract					},
	{ UFmd1_UtilRealWaveSet					},
	{ UFmd1_UtilRealWaveMultiply			},
	{ UFmd1_UtilRealWaveMultiplyAdd		},
//	{ UFmd1_UtilCorrelation_				},
	{ UFmd1_UtilCorrelation					},
	{ UFmd1_UtilFileDialog					},
//	{ UFmd1_UtilIsValidString				},
	{ UFmd1_UtilMemoryLoad					},		
	{ UFmd1_UtilTotalPhys					},		
	{ UFmd1_UtilAvailPhys					},		
	{ UFmd1_UtilTotalVirtual				},		
	{ UFmd1_UtilAvailVirtual				},		
	{ UFmd1_UtilContiguousMemory			},		
	{ UFmd1_UtilHeapCompact					},
	{ UFmd1_UtilGetAllUsersDirectory	},		
	{ UFmd1_UtilGetSystemDirectory		},		
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
// UFmd1_Util - UTILITIES

int 	UFmd1_UtilGetAllUsersDirectory( struct { Handle sRes; }* p)
// Returns e.g. 'D:\Documente und Einstellungen\All Users' , without trailing '\'
{
   ...
   {
      sprintf( errbuf, "++++Error: UFmd1_UtilGetAllUsersDirectory() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
...
int 	UFmd1_UtilGetSystemDirectory( struct { Handle sRes; }* p)
{
	char			errbuf[MAX_PATH + 100];
	char			sDirPath[MAX_PATH + 20];
   int			len, err	= 0;
   ...
   if (( str1 = NewHandle( len )) == NIL )		// get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFmd1_UtilGetSystemDirectory() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;										// out of memory
   }
   else														// string length is OK so return string 
	...
	sprintf( errbuf, "UFmd1_UtilGetSystemDirectory [len:%d]:  '%s' \r", len, sDirPath ); XOPNotice( errbuf );  
   p->sRes = str1;
   return err;
}
...
int UFmd1_UtilConvolve( struct {	double bStoreIt; double	nPnts; double	nChan; double	nChunk; double	nCompress; 
								  ...
								  
{
   ...
   float    *wBigWave;   
	...
	//DebugPrintWaveProperties( "UFmd1_UtilConvolve   ", p->wRaw );		// 050128
	//DebugPrintWaveProperties( "UFmd1_UtilConvolve   ", p->wBigWave ); // 050128

   if ( p->wBigWave == NIL || p->wRaw == NIL )// check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilConvolve() received nonvalid wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilConvolve() received non integer wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilConvolve() received non float wave (4Byte) for wBigWave \r" );
		XOPNotice( errBuf );
   ...
   wBigWave = WaveData( p->wBigWave );			//  char pointer to IGOR wave data 
	...
	if ( nDirec == DADIREC )
	{
		for ( pt =   begPt;  pt < endPt;   pt += 1 )
		{
			...
			{
				sprintf( buf, "\t\tUFmd1_UtilConvolve() \tDACs \t\tBeg/End\t%8d\t\t\tPt written:\t%8d\t\t\t\t\tDAIndex:\t%8d \r",
							pt, pt1, nRawIdx );
				XOPNotice( buf );
			}
			*/
		...
		if ( c < nTrueADorDA )				// the uncompressed true 'gnCntAD' AD channels
		{
			...
			else		// fill with fixed value to avoid erratic display
			{
				for ( pt =   begPt;  pt < endPt;   pt += 1 )
				{
					...
					/* 050128
					if ( pt==begPt){
						sprintf( buf, "\t\tUFmd1_UtilConvolve() \tTrueADda\tBegPt:\t%8d\t\t\t\t\t\t FirstPt written:\t%8d\t \r", begPt, pt1 );
						XOPNotice( buf );
					}
					*/
				}
			...
			/* 050128
			sprintf( buf, "\t\tUFmd1_UtilConvolve() \tTrueADda\tEndPt:\t%8d\t\t\t\t\t\t Last Pt written:\t%8d\tInt?\t%8.3lf\tRdIndex:\t%8d \r",
		...
		if ( c >= nTrueADorDA )				// the compressed 'gnCntTG' TG channels (ASSUMPTION: they follow the true AD channels)
		{
			if ( bStoreIt )
			{
				for ( pt =   begPt;  pt < endPt;   pt += nCompress )
				{
					...
					{
						sprintf( buf, "\t\tUFmd1_UtilConvolve() \tcompress:%2d\tnPnts:\t%8d\tOrgPt:\t%8d\tPoint:\t%8d\tPt/Co(=written):\t%8d\tRawIdx(=read):\t%8d\tStoreIt:%2d\t  \r",
					...
					*/
					...
					{
						sprintf( buf, "++++Error\t\tUFmd1_UtilConvolve() Indices:   %d <= %d < %d ??? \r", 0, nRawIdx, nRawPts );
						XOPNotice( buf );
					}
					if ( nBigWvIdx < 0 || nBigWvIdx >= nBigWvPts )
					{
						sprintf( buf, "++++Error\t\tUFmd1_UtilConvolve() Indices:   %d <= %d < %d ??? \r", 0, nBigWvIdx, nBigWvPts );
						XOPNotice( buf );
					}
			...
			else		// fill with fixed value to avoid erratic display
			{
				for ( pt =   begPt;  pt < endPt;   pt += nCompress )
				{
					...
					/* 050128
					if ( pt==begPt){
						sprintf( buf, "\t\tUFmd1_UtilConvolve() \tcompress:%2d\tBegPt:\t%8d\t  EPt/Comp:\t%8d\t FirstPt written:\t%8d\t \r",
			...
			/* 050128
			sprintf( buf, "\t\tUFmd1_UtilConvolve() \tcompress:%2d\tEndPt:\t%8d\t  EPt/Comp:\t%8d\t Last Pt written:\t%8d\tInt?\t%8.3lf\tRdIndex:\t%8d \r",
...
int UFmd1_UtilWaveCopy( struct { double scl; double nSourceOfs; double nPnts; waveHndl wFloatSource; waveHndl wIntTarget; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSource;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilWaveCopy  ", p->wFloatSource );	// 050128
	// DebugPrintWaveProperties( "UFmd1_UtilWaveCopy  ", p->wIntTarget );		// 050128

   if ( p->wIntTarget == NIL || p->wFloatSource == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilWaveCopy() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilWaveCopy() received non integer wave ( 2Byte) for wIntTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilWaveCopy() received non float wave (4Byte) for wFloatSource \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFmd1_UtilWaveExtract( struct { double scl; double nStep; double nSourceOfs; double nPnts; waveHndl wFloatSource; waveHndl wFloatTarget; double res; }* p)
// XOP because IGOR itself is too slow..
// 040204 accepts also Float target, additional parameter step
{
   ...
   float    *wFloatSource;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilWaveExtract", p->wFloatSource ); // 050128
	// DebugPrintWaveProperties( "UFmd1_UtilWaveExtract", p->wFloatTarget ); // 050128

   if ( p->wFloatTarget == NIL || p->wFloatSource == NIL )  // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilWaveExtract() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilWaveExtract() received non float wave ( 4Byte) for wFloatTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not 2Byte int
      return( IS_NOT_4BYTE_FLOAT_WAVE );	
   }
   ...
   if ( WaveType(p->wFloatSource )!= NT_FP32 )		// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilWaveExtract() received non float wave (4Byte) for wFloatSource \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wFloatSource	= WaveData( p->wFloatSource );	//  char pointer to IGOR wave data 
	...
	for ( i = 0; i <= nPnts - nStep;	i += nStep ) {
		//sprintf( bf, "\tUFmd1_UtilWaveExtract()\tpts:\t%7d\tos:\t%7d\tstp:\t%7d\tscl:\t%10.4lf\ti:\t%7d\tItg:\t%7d\tIsc:\t%7d\t \r", 
...
int UFmd1_UtilRealWaveSet( struct { double value; double nEnd; double nBeg; waveHndl wFloatTarget; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatTarget;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilRealWaveSet", p->wFloatTarget ); 	// 050128

   if ( p->wFloatTarget == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveSet() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if ( WaveType(p->wFloatTarget )!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveSet() received non float wave (4Byte) for wFloatTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFmd1_UtilRealWaveMultiply( struct { waveHndl wFloatTgt; double nPts; double nBeg2; waveHndl wFloatSrc2; double nBeg1; waveHndl wFloatSrc1; double res; }* p)
...
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSrc1, *wFloatSrc2, *wFloatTgt;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilRealWaveMultiply", p->wFloatSrc1 ); 	// 050128
	// DebugPrintWaveProperties( "UFmd1_UtilRealWaveMultiply", p->wFloatSrc2 ); 	// 050128

   if ( p->wFloatSrc1 == NIL  ||  p->wFloatSrc2 == NIL ||  p->wFloatTgt == NIL )	// check if wave handles are valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveMultiply() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveMultiply() received non float wave (4Byte) for source or target wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFmd1_UtilRealWaveMultiplyAdd( struct { double nPts; double nBeg2; waveHndl wFloatSrc2; double nBeg1; waveHndl wFloatSrc1; double res; }* p)
...
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSrc1, *wFloatSrc2;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilRealWaveMultiplyAdd", p->wFloatSrc1 ); 	// 050128
	// DebugPrintWaveProperties( "UFmd1_UtilRealWaveMultiplyAdd", p->wFloatSrc2 ); 	// 050128

   if ( p->wFloatSrc1 == NIL  ||  p->wFloatSrc2 == NIL )	// check if wave handles are valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveMultiplyAdd() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilRealWaveMultiplyAdd() received non float wave (4Byte) for source wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
// Order of values in wSums .   !!! Must be the same in  UF_MiniDetection.ipf  and in  UFmd1_Utils.c
#define	kMD_SumX		0
#define	kMD_SumX2	1
#define	kMD_SumY		2
#define	kMD_SumY2	3
...
int UFmd1_UtilCorrelation_( struct { double nPrintIt; waveHndl wCorr; waveHndl wSums; double nTPts; waveHndl wTempl; double nDPts; waveHndl wData; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wData, *wTempl, *wSums, *wCorr;   
	...
	// DebugPrintWaveProperties( "UFmd1_UtilCorrelation_  ", p->wData );		// 050128
	// DebugPrintWaveProperties( "UFmd1_UtilCorrelation_  ", p->wTemplate );	// 050128

   if ( p->wData == NIL || p->wTempl == NIL || p->wSums == NIL || p->wCorr == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilCorrelation_() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilCorrelation_() received non float wave (4Byte) for wData, wTempl, wSums or wCorr \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wTempl	= WaveData( p->wTempl );			//  char pointer to IGOR wave data 
	...
	for ( i = 0; i < nTPts; i += 1 ) {
		...
		if ( nPrintIt && ( i < 5 || i % 100 == 0 ) ) {
			sprintf( errBuf, "\tUFmd1_UtilCorrelation_ Initialising\tT:%3d/%6d  D:%6d/%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", i, nTPts, n, nDPts, MaskStart, wSums[ kMD_SumX ], wSums[ kMD_SumY ], SumXY, SP, SQX, SQY, B, A, r );
			XOPNotice( errBuf );
		}
	}
	...
	for ( n = 0; n < nDPts - nTPts; n += 1 ) {
		...
		for ( i = 0; i < nTPts; i += 1) {
			...
			if (  nPrintIt && i < 3 && n%10000 == 0 ) {
				sprintf( errBuf, "\tUFmd1_UtilCorrelation_ Computing..\tT:%3d/%6d  D:%6d/%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", i, nTPts, n, nDPts, MaskStart, wSums[ kMD_SumX ], wSums[ kMD_SumY ], SumXY, SP, SQX, SQY, B, A, r );
				XOPNotice( errBuf );
			}	
		}
...
int UFmd1_UtilCorrelation( struct { double nEnd; double nBeg; double nPrintIt; waveHndl wCorr; waveHndl wSums; double nMaskStart;  double nTPts; waveHndl wTempl; waveHndl wData; double res; }* p)
// XOP because IGOR itself is too slow..
{
   long      nTPts		= (long)p->nTPts;
   ...
   float    *wData, *wTempl, *wSums, *wCorr;   
	...
	double	A	= 0;		// offset
	double	r	= 0;		// correlation
	// DebugPrintWaveProperties( "UFmd1_UtilCorrelation  ", p->wData );		// 050128
	// DebugPrintWaveProperties( "UFmd1_UtilCorrelation  ", p->wTemplate );	// 050128

   if ( p->wData == NIL || p->wTempl == NIL || p->wSums == NIL || p->wCorr == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFmd1_UtilCorrelation() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   if ( WaveType(p->wData)!= NT_FP32 || WaveType(p->wTempl)!= NT_FP32 || WaveType(p->wSums)!= NT_FP32 || WaveType(p->wCorr)!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFmd1_UtilCorrelation() received non float wave (4Byte) for wData, wTempl, wSums or wCorr \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
	...
	for ( n = nBeg; n < nEnd; n += 1 ) {
		...
		for ( i = 0; i < nTPts; i += 1) {
			...
			if (  nPrintIt == 1  && i < 3 && n%10000 == 0 ) {
				sprintf( errBuf, "\tUFmd1_UtilCorrelation Computing..\tT:%3d/%6d  D:%6d...%6d...%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", 
		...
		if (  nPrintIt == 2  &&  57300 < n  &&  n < 57500 ) {
			sprintf( errBuf, "\tUFmd1_UtilCorrelation Computing..\tD:%6d...%6d...%6d  MSt:%d  X:%g  X2:%g\tSY:\t%8.1g\tSY2:\t%10.2g\tSumXY:%g\tSP:\t%g\tSQX:\t%g\tSQY:\t%g\tB:\t%g\tA:\t%g\tr:\t%g \r", 
...
int UFmd1_UtilFileDialog( struct { Handle FilePath; Handle DefExt; Handle InitDir; double Index; Handle Filter; Handle Prompt; Handle ReturnFilePath; }* p)
...
//						To select a directory call from IGOR like : gsDataPath = UFmd1_UtilFileDialog( "Select directory" , "Directories; ;;" ,  1, sPath,  "", "_" )	
...
// Advantage 2 :	custom file filters other than those IGOR provides can be used
{
	...
	*(*(p->Filter)+n+1)	= '\0';
	...
	//sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(2) \tn:%2d lenFilter:%d\t'%s'  \r", n , lenFilter, *(p->Filter)  );	XOPNotice( errbuf );

   strFilter = IHC( p->Filter );
	//sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(3) \tstrFilter: \t\t'%s'   Index:%d \r", *strFilter, Index );	XOPNotice( errbuf );

   strInitDir = IHC( p->InitDir );
	//sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(4) \tstrInitDir: \t\t'%s' \r", *strInitDir );	XOPNotice( errbuf );
	...
	//sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(5) \tnativeInitDir:\t\t'%s' \r", nativeInitDir );	XOPNotice( errbuf );
   strDefExt = IHC( p->DefExt );

	bReadMode	= !strcmp( *strDefExt, csREADMODE );
	//sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(6) \tstrDefExt: \t\t'%s' -> ReadMode:%2d \r", *strDefExt, bReadMode );	XOPNotice( errbuf );

   strFilePath = IHC( p->FilePath );
  //sprintf ( errbuf, "\t\t\tUFmd1_UtilFileDialog(7) \tstrFilePath:\t\t'%s' \r", *strFilePath );	XOPNotice( errbuf );
   ...
   if (( strReturnFilePath = NewHandle( lenReturnFilePath )) == NIL ) // get output handle , do not provide space for '\0' 
	{
		sprintf( errbuf, "++++Error: UFmd1_UtilFileDialog() Not enough memory. Aborted...\r");
	   XOPNotice( errbuf );
		err = NOMEM;											// out of memory
	}
	else															// string length is OK so return string 
	...
	// XOPNotice( "\tUFmd1_UtilFileDialog() Out: strReturnFilePath:'" );	XOPNotice( *strReturnFilePath );	XOPNotice( "'\r" );
...
/*
#int 	UFmd1_UtilIsValidString( struct { Handle IsValidString; double res; }* p)
...
int 	UFmd1_UtilMemoryLoad( struct { double res; }* p)
...
int 	UFmd1_UtilTotalPhys( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFmd1_UtilAvailPhys( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFmd1_UtilTotalVirtual( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFmd1_UtilAvailVirtual( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFmd1_UtilContiguousMemory( struct { double nBytes; double res; }* p)
...
{
	waveHndl		waveHndPtr;
	char			*WaveNm		= "UF_Temp_UFmd1_UtilContiguousMemCheck";
   int			nType			= NT_FP32;					// arbitrary, make 4-byte wave
	...
	// sprintf( errbuf, "UFmd1_UtilContiguousMemory( MegaPts: %.6lf ) returns %d (0=OK, 1=Fail)\r", nPnts * 4 / 1e6, code ); 
	// XOPNotice( errbuf ); 
	p->res = code;
   return   0;		
}
...
int 	UFmd1_UtilHeapCompact( struct { double res; }* p)
{
