...
//#include "C:\Programme\wavemetrics\IgorXOPs\XOPSupport\XOPStandardHeaders.h" //..ANSI headers IgorXOP.h XOP.h XOPSupport.h

#include "XopMain.h"						// 
	
#pragma pack(2)								//  all structures are 2-byte-aligned for Igor Toolkit , 4-byte-aligned for MCTG.


// also defined in UFP_Ced.h
...
*/
	...
	// ..are renumbered into the range 100..211   (see UFP_CedWinCustom.RC)
	if ( -611 <= result && result <= -500 )
		result = ErrorNrCEDtoIGOR( result ); //= FIRST_XOP_ERR - result - 400;
	SetXOPResult( result );
...
#ifdef _DEBUG
	sprintf( sMsg, "Loaded UFP_Ced.xop (debug) \r" );
	XOPNotice( sMsg );
#else
	sprintf( sMsg, "Loaded UFP_Ced.xop \r" );
	XOPNotice( sMsg );
#endif
...
// converts the negative CED error codes to positive codes required by Igor 
//e.g. 'Timeout occurred'= -590   ->  190
{
