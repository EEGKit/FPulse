//
// UFP_MC700Tg.c 	XOP based on Igors sample WindowXOP needed to get Axons MultiClamp700 Telegraph data into FPulse
...
#include "XopMain.h"
#include "..\C_Common\XopUtils.h" 

#include "UFP_MC700Tg.h"			// sets MCTG_IGORMAINFRM: compile and link all code necessary for the MultiClamp telegraph connection	
...
BOOL	gnDebugMsg = 0;				// determines whether MCTg debugging information is printed in Igors history area
											...
											// (can still be enabled  at run-time by calling from Igor UFP_MCTgDebugMsg( 1 )  
...
int		ProcessTimerMsg( HWND hwnd, WPARAM TimerId )	//todo return TRUE,FALSE or break????
{
	...
	if ( nBroadcastTimerEventID == TimerId )
	{
      ...
      //sprintf( sMsg, "Info (UFP_MC700Tg.xop): No AxoPatch MultiClamp found, starting in demo mode. Trying to connect.... \r" );
#ifdef _DEBUG
	   sprintf( sMsg, "Loaded UFP_MC700Tg.xop (debug) : No AxoPatch MultiClamp found. \r" );
#else
	   sprintf( sMsg, "Loaded UFP_MC700Tg.xop : No AxoPatch MultiClamp found. \r" );
#endif
		XOPNotice( sMsg );
...
void		UpdateDisplay( HWND hwnd, UINT nMC )
{
	...
	//	...which can be retrieved by an XOP function, so that 'UFP_MCTgPickupGains( port,bus,ch) can be called.
	...
	{ 
		...
		char	sHd[100], sChg[10], sFlags[100], sLine[400];
...
int	UFP_MCTgDebugMsg( struct { DOUBLE p1; DOUBLE result;	}*p )
{
...
int	UFP_MCTgPickupInfo( struct { Handle sRes;	}*p )
...
// this is faster than a 'request message' but requires an open MC telegraph connection
{
   ...
   {
      sprintf( errbuf, "++++Error: UFP_MCTgPickupInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
...
/*
	...
	{
	   char		buf[4000]; 
		sprintf( buf, " UFP_MCTgPickupInfo():  '%s' \r", *str1 );	// *str1 has no string end 
