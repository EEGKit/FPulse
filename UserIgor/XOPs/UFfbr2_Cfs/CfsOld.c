...
#define NDATAKIND      2             /* number of data kinds defined in CFS */
...
#endif
#define WHOLEFILECHARS 128/* to hold whole of temp file name including path */
...
typedef struct
{
    ...
    TCFSKind  dKind;                               /* equalspaced or matrix */
...
int         g_maUFfbr2_CfsFiles = 0;        /* The length of the array of file info */
...
static void  TransferIn(TpCStr olds,TpStr new,BYTE max);
...
/* 2. Open file for writing */
    ...
    #if defined(_IS_MSDOS_) || defined(_IS_WINDOWS_)
        if (CCreat(fname, rwMode, &pfileInfo->DOSHdl.d) != 0)  /* Open data file */
                         ...
                         short     other)   /* next channel for matrix data */
{
    ...
    ASSERT(handle >= 0);
    ASSERT(handle < g_maUFfbr2_CfsFiles);
    ASSERT(channel >= 0);
    ...
    if ((handle < 0) || (handle >= g_maUFfbr2_CfsFiles))
    {
...
/* If this is the first edit made to a file get it ready for the change */
/* This code strips the lookup table from the file size, which will herlp out! */
    if (pfileInfo->fileHeadP->tablePos != 0)
    {
        short   ecode;
...
/* if the table is in memory free its allocated space */
    ...
    for (index = 0; index < g_maUFfbr2_CfsFiles; index++)  /* Finally, check tables */
        if (g_fileInfo[index].allowed != nothing)    /* Any files still open? */
            break;                         /* If so, we exit the loop early */

    if (index >= g_maUFfbr2_CfsFiles)     /* If no files open, will clean up memory */
        CleanUpCfs();
                       ...
                       TpVoid varADS)             /* for return of variable */
{
    ...
    if ((handle < 0) || (handle >= g_maUFfbr2_CfsFiles))           /* check handle */
    {
        InternalError(handle,Proc,BADHANDLE);
        return;
    }
...
CFSAPI(void) GetFileChan(short   handle,             /* program file handle */
                         ...
                         TpStr   xUnits,   /* EqualSpaced to return x units */
                         TpDType dataTypeP,
                                      /* to return 1 of 8 types for channel */
                         TpDKind dataKindP, /* return EqualSpaced or Matrix */
                         TpShort spacing,
...
short FindUnusedHandle(void)
{
    ...
    if (g_maUFfbr2_CfsFiles <= 0)                   /* Do the initial allocation? */
    {
        ...
        g_maUFfbr2_CfsFiles = INITCEDFILES;  /* Number of files we have space for */
        for (search = 0; search < g_maUFfbr2_CfsFiles; search++)
            g_fileInfo[search].allowed = nothing;   /* Initialise file info */
    }

    search = g_maUFfbr2_CfsFiles - 1;               /* Start index for the search */
    while ((search >= 0) && (g_fileInfo[search].allowed != nothing))
        search--;

    if ((search < 0) && (g_maUFfbr2_CfsFiles < MAXCEDFILES))  /* If table is full */
    {                                              /* but could be enlarged */
        TFileInfo*  pNew;
        int         num;

        num = g_maUFfbr2_CfsFiles * 2;          /* Normally, just double the size */
        if (num > MAXCEDFILES)
...
void CleanUpCfs(void)
{
    ...
    for (i = 0; i < g_maUFfbr2_CfsFiles; i++)
...
void TransferIn(TpCStr olds, TpStr new, BYTE max)
{
    BYTE     lengths;
    short    loop;
    ...
    for (loop = 0;loop < (short)lengths;loop++)
        new[loop+1] = olds[loop];
    new[lengths+1] = '\0';                               /* put NULL on end */
}                                                      /* end of TransferIn */
