...
int         g_maUFmdt_CfsFiles = 0;        /* The length of the array of file info */
                         ...
                         short     other)   /* next channel for matrix data */
{
    ...
    ASSERT(handle >= 0);
    ASSERT(handle < g_maUFmdt_CfsFiles);
    ASSERT(channel >= 0);
    ...
    if ((handle < 0) || (handle >= g_maUFmdt_CfsFiles))
    {
...
/* if the table is in memory free its allocated space */
    ...
    for (index = 0; index < g_maUFmdt_CfsFiles; index++)  /* Finally, check tables */
        if (g_fileInfo[index].allowed != nothing)    /* Any files still open? */
            break;                         /* If so, we exit the loop early */

    if (index >= g_maUFmdt_CfsFiles)     /* If no files open, will clean up memory */
        CleanUpCfs();
                       ...
                       TpVoid varADS)             /* for return of variable */
{
    ...
    if ((handle < 0) || (handle >= g_maUFmdt_CfsFiles))           /* check handle */
    {
        InternalError(handle,Proc,BADHANDLE);
        return;
    }
...
short FindUnusedHandle(void)
{
    ...
    if (g_maUFmdt_CfsFiles <= 0)                   /* Do the initial allocation? */
    {
        ...
        g_maUFmdt_CfsFiles = INITCEDFILES;  /* Number of files we have space for */
        for (search = 0; search < g_maUFmdt_CfsFiles; search++)
            g_fileInfo[search].allowed = nothing;   /* Initialise file info */
    }

    search = g_maUFmdt_CfsFiles - 1;               /* Start index for the search */
    while ((search >= 0) && (g_fileInfo[search].allowed != nothing))
        search--;

    if ((search < 0) && (g_maUFmdt_CfsFiles < MAXCEDFILES))  /* If table is full */
    {                                              /* but could be enlarged */
        TFileInfo*  pNew;
        int         num;

        num = g_maUFmdt_CfsFiles * 2;          /* Normally, just double the size */
        if (num > MAXCEDFILES)
...
void CleanUpCfs(void)
{
    ...
    for (i = 0; i < g_maUFmdt_CfsFiles; i++)
