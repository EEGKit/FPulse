...
// 070223
//int UFp1_UtilGetSystemDirectory(void * );  //

int UFp1_UtilConvolveDA(			void * );  // 
int UFp1_UtilConvolveAD(			void * );  // 
int UFp1_UtilConvolveTG(			void * );  // 
int UFp1_UtilConvolve(				void * );  // 
int UFp1_UtilWaveCopy(				void * );  // 
int UFp1_UtilWaveExtract(			void * );  // 
int UFp1_UtilRealWaveSet(			void * );  // 
int UFp1_UtilRealWaveMultiply(	void * );  // 
int UFp1_UtilRealWaveMultiplyAdd(void * );  // 
//int UFp1_UtilCorrelation_(		void * );  // 
int UFp1_UtilCorrelation(			void * );  // 
int UFp1_UtilFileDialog(			void * );  // 
//int UFp1_UtilIsValidString(		void * );  //
int UFp1_UtilMemoryLoad(			void * );  //
int UFp1_UtilTotalPhys(			void * );  //
int UFp1_UtilAvailPhys(			void * );  //
int UFp1_UtilTotalVirtual(		void * );  //
int UFp1_UtilAvailVirtual(		void * );  //
int UFp1_UtilContiguousMemory(	void * );  //
int UFp1_UtilHeapCompact(			void * );  //
int UFp1_UtilGetAllUsersDirectory(void * );  //
int UFp1_UtilGetSystemDirectory(void * );  //


/////////////////////////////////////////////////////////////////////////////////////////

// SAME ORDER HERE IN  '(*sFunc[])' AND IN UFp1_UtilsWinCustom.RC
...
//	{ UFp1_UtilGetSystemDirectory		},		
	...
	{ UFp1_UtilConvolve					},
	{ UFp1_UtilWaveCopy					},
	{ UFp1_UtilWaveExtract					},
	{ UFp1_UtilRealWaveSet					},
	{ UFp1_UtilRealWaveMultiply			},
	{ UFp1_UtilRealWaveMultiplyAdd		},
//	{ UFp1_UtilCorrelation_				},
	{ UFp1_UtilCorrelation					},
	{ UFp1_UtilFileDialog					},
//	{ UFp1_UtilIsValidString				},
	{ UFp1_UtilMemoryLoad					},		
	{ UFp1_UtilTotalPhys					},		
	{ UFp1_UtilAvailPhys					},		
	{ UFp1_UtilTotalVirtual				},		
	{ UFp1_UtilAvailVirtual				},		
	{ UFp1_UtilContiguousMemory			},		
	{ UFp1_UtilHeapCompact					},
	{ UFp1_UtilGetAllUsersDirectory	},		
	{ UFp1_UtilGetSystemDirectory		},		
...
int 	UFp1_UtilGetAllUsersDirectory( struct { Handle sRes; }* p)
// Returns e.g. 'D:\Documente und Einstellungen\All Users' , without trailing '\'
{
   ...
   {
      sprintf( errbuf, "++++Error: UFp1_UtilGetAllUsersDirectory() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
...
int 	UFp1_UtilGetSystemDirectory( struct { Handle sRes; }* p)
{
	char			errbuf[MAX_PATH + 100];
	char			sDirPath[MAX_PATH + 20];
   int			len, err	= 0;
   ...
   if (( str1 = NewHandle( len )) == NIL )		// get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: UFp1_UtilGetSystemDirectory() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;										// out of memory
   }
   else														// string length is OK so return string 
	...
	sprintf( errbuf, "UFp1_UtilGetSystemDirectory [len:%d]:  '%s' \r", len, sDirPath ); XOPNotice( errbuf );  
   p->sRes = str1;
   return err;
}
...
int UFp1_UtilConvolveDA( struct {	double begTape; double	nPnts; double	SmpF; 
								  ...
								  
{
   ...
   float    *wBigWave;   
	...
	//DebugPrintWaveProperties( "UFp1_UtilConvolve   ", p->wRaw );		// 050128
	//DebugPrintWaveProperties( "UFp1_UtilConvolve   ", p->wBigWave ); // 050128

   if ( p->wBigWave == NIL || p->wRaw == NIL )// check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilConvolveDA() received nonvalid wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveDA() received non integer wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveDA() received non float wave (4Byte) for wBigWave \r" );
		XOPNotice( errBuf );
...
int UFp1_UtilConvolveAD( struct {	double InChkBeg; double begTape; double	nPnts; double	nChan; double	nChunk; double	nCompress; 
										double yscl; double ofs;  double PtsPerChunk;  double endPt; double begPt; 
								  ...
								  
{
   ...
   if ( p->wBigWave == NIL || p->wRaw == NIL )// check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilConvolveAD() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if ( WaveType(p->wRaw)!= NT_I16 )			// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveAD() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   if ( WaveType(p->wBigWave )!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveAD() received non float wave (4Byte) for wBigWave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wBigWave = WaveData( p->wBigWave );			//  char pointer to IGOR wave data 
	...
	for ( pt =   begPt;  pt < endPt;   pt += 1 )
	{
		...
		{
			sprintf( buf, "++++Error\t\tUFp1_UtilConvolveAD() nRawIdx:   %d <= %d < %d ??? [BegPt:%d  pt:%d  EndPt:%d] \r", 0, nRawIdx, nRawPts, begPt, pt, endPt );
			XOPNotice( buf );
			return -1;
		}
		if ( ptBigIO < 0 || ptBigIO >= nBigWvPts )
		{
			sprintf( buf, "++++Error\t\tUFp1_UtilConvolveAD() ptBigIO:   %d <= %d < %d ??? [BegPt:%d  pt:%d  EndPt:%d] \r", 0, ptBigIO, nBigWvPts, begPt, pt, endPt );
			XOPNotice( buf );
			return -1;
		}
...
int UFp1_UtilConvolveTG( struct {	double BegTape; double	nPnts; double	nChan; double	nChunk; double	nCompress; 
										double yscl; double ofs;  double PtsPerChunk;  double endPt; double begPt; 
										double nTG;  double nTrueADorDA;  waveHndl wRaw; waveHndl wBigWave; double res; }* p)
// XOP because IGOR itself is too slow..(2GHz: Igor~3us/pt, XOP~40ns/pt)
// ASSUMPTION: the channel order is at first ALL non-compressed true AD channels, then all telegraph channels which are compressed by the same factor
								  
{
   ...
   if ( p->wBigWave == NIL || p->wRaw == NIL )// check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilConvolveTG() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if ( WaveType(p->wRaw)!= NT_I16 )			// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveTG() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   if ( WaveType(p->wBigWave )!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolveTG() received non float wave (4Byte) for wBigWave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wBigWave = WaveData( p->wBigWave );			//  char pointer to IGOR wave data 
	...
	for ( pt =   begPt;  pt < endPt;   pt += 1 )
	{
		if ( pt % nCompress == 0 )
		{
			...
			if ( nRawIdx < 0  || nRawIdx >= nRawPts )
			{
				sprintf( buf, "++++Error\t\tUFp1_UtilConvolveTG() nRawIndex:   %d <= %d < %d ??? [BegPt:%d  pt:%d  EndPt:%d] \r", 0, nRawIdx, nRawPts, begPt, pt, endPt );
				XOPNotice( buf );
				return -1;
			}
		...
		if ( ptBigIO < 0 || ptBigIO >= nBigWvPts )
		{
			//sprintf( buf, "++++Error\t\tUFp1_UtilConvolveTG() ptBigIO:   %d <= %d < %d ??? [BegPt:%d  pt:%d  EndPt:%d] \r", 0, ptBigIO, nBigWvPts, begPt, pt, endPt );
			//XOPNotice( buf );
			//return -1;
		}
		else {
...
int UFp1_UtilConvolve( struct {	double bStoreIt; double	nPnts; double	nChan; double	nChunk; double	nCompress; 
								  ...
								  
{
   ...
   if ( p->wBigWave == NIL || p->wRaw == NIL )// check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilConvolve() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if ( WaveType(p->wRaw)!= NT_I16 )			// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolve() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   if ( WaveType(p->wBigWave )!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilConvolve() received non float wave (4Byte) for wBigWave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );							// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wBigWave = WaveData( p->wBigWave );			//  char pointer to IGOR wave data 
	...
	if ( nDirec == DADIREC )
	{
		for ( pt =   begPt;  pt < endPt;   pt += 1 )
		{
			...
			{
				sprintf( buf, "\t\tUFp1_UtilConvolve() \tDACs \t\tBeg/End\t%8d\t\t\tPt written:\t%8d\t\t\t\t\tDAIndex:\t%8d \r",
							pt, pt1, nRawIdx );
				XOPNotice( buf );
			}
			*/
		...
		if ( c < nTrueADorDA )				// the uncompressed true 'gnCntAD' AD channels
		{
			...
			else		// fill with fixed value to avoid erratic display
			{
				for ( pt =   begPt;  pt < endPt;   pt += 1 )
				{
					...
					/* 050128
					if ( pt==begPt){
						sprintf( buf, "\t\tUFp1_UtilConvolve() \tTrueADda\tBegPt:\t%8d\t\t\t\t\t\t FirstPt written:\t%8d\t \r", begPt, pt1 );
						XOPNotice( buf );
					}
					*/
				}
			...
			/* 050128
			sprintf( buf, "\t\tUFp1_UtilConvolve() \tTrueADda\tEndPt:\t%8d\t\t\t\t\t\t Last Pt written:\t%8d\tInt?\t%8.3lf\tRdIndex:\t%8d \r",
		...
		if ( c >= nTrueADorDA )				// the compressed 'gnCntTG' TG channels (ASSUMPTION: they follow the true AD channels)
		{
			if ( bStoreIt )
			{
				for ( pt =   begPt;  pt < endPt;   pt += nCompress )
				{
					...
					{
						sprintf( buf, "\t\tUFp1_UtilConvolve() \tcompress:%2d\tnPnts:\t%8d\tOrgPt:\t%8d\tPoint:\t%8d\tPt/Co(=written):\t%8d\tRawIdx(=read):\t%8d\tStoreIt:%2d\t  \r",
					...
					*/
					...
					if ( nRawIdx < 0  || nRawIdx >= nRawPts )
					{
						sprintf( buf, "++++Error\t\tUFp1_UtilConvolve() Indices:   %d <= %d < %d ??? \r", 0, nRawIdx, nRawPts );
						XOPNotice( buf );
					}
					if ( nBigWvIdx < 0 || nBigWvIdx >= nBigWvPts )
					{
						sprintf( buf, "++++Error\t\tUFp1_UtilConvolve() Indices:   %d <= %d < %d ??? \r", 0, nBigWvIdx, nBigWvPts );
						XOPNotice( buf );
					}
			...
			else		// fill with fixed value to avoid erratic display
			{
				for ( pt =   begPt;  pt < endPt;   pt += nCompress )
				{
					...
					/* 050128
					if ( pt==begPt){
						sprintf( buf, "\t\tUFp1_UtilConvolve() \tcompress:%2d\tBegPt:\t%8d\t  EPt/Comp:\t%8d\t FirstPt written:\t%8d\t \r",
			...
			/* 050128
			sprintf( buf, "\t\tUFp1_UtilConvolve() \tcompress:%2d\tEndPt:\t%8d\t  EPt/Comp:\t%8d\t Last Pt written:\t%8d\tInt?\t%8.3lf\tRdIndex:\t%8d \r",
...
int UFp1_UtilWaveCopy( struct { double scl; double nSourceOfs; double nPnts; waveHndl wFloatSource; waveHndl wIntTarget; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSource;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilWaveCopy  ", p->wFloatSource );	// 050128
	// DebugPrintWaveProperties( "UFp1_UtilWaveCopy  ", p->wIntTarget );		// 050128

   if ( p->wIntTarget == NIL || p->wFloatSource == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilWaveCopy() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilWaveCopy() received non integer wave ( 2Byte) for wIntTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilWaveCopy() received non float wave (4Byte) for wFloatSource \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFp1_UtilWaveExtract( struct { double scl; double nStep; double nSourceOfs; double nPnts; waveHndl wFloatSource; waveHndl wFloatTarget; double res; }* p)
// XOP because IGOR itself is too slow..
// 040204 accepts also Float target, additional parameter step
{
   ...
   float    *wFloatSource;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilWaveExtract", p->wFloatSource ); // 050128
	// DebugPrintWaveProperties( "UFp1_UtilWaveExtract", p->wFloatTarget ); // 050128

   if ( p->wFloatTarget == NIL || p->wFloatSource == NIL )  // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilWaveExtract() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilWaveExtract() received non float wave ( 4Byte) for wFloatTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not 2Byte int
      return( IS_NOT_4BYTE_FLOAT_WAVE );	
   }
   ...
   if ( WaveType(p->wFloatSource )!= NT_FP32 )		// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilWaveExtract() received non float wave (4Byte) for wFloatSource \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );									// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wFloatSource	= WaveData( p->wFloatSource );	//  char pointer to IGOR wave data 
	...
	for ( i = 0; i <= nPnts - nStep;	i += nStep ) {
		//sprintf( bf, "\tUFp1_UtilWaveExtract()\tpts:\t%7d\tos:\t%7d\tstp:\t%7d\tscl:\t%10.4lf\ti:\t%7d\tItg:\t%7d\tIsc:\t%7d\t \r", 
...
int UFp1_UtilRealWaveSet( struct { double value; double nEnd; double nBeg; waveHndl wFloatTarget; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatTarget;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilRealWaveSet", p->wFloatTarget ); 	// 050128

   if ( p->wFloatTarget == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilRealWaveSet() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if ( WaveType(p->wFloatTarget )!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilRealWaveSet() received non float wave (4Byte) for wFloatTarget \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFp1_UtilRealWaveMultiply( struct { waveHndl wFloatTgt; double nPts; double nBeg2; waveHndl wFloatSrc2; double nBeg1; waveHndl wFloatSrc1; double res; }* p)
...
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSrc1, *wFloatSrc2, *wFloatTgt;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilRealWaveMultiply", p->wFloatSrc1 ); 	// 050128
	// DebugPrintWaveProperties( "UFp1_UtilRealWaveMultiply", p->wFloatSrc2 ); 	// 050128

   if ( p->wFloatSrc1 == NIL  ||  p->wFloatSrc2 == NIL ||  p->wFloatTgt == NIL )	// check if wave handles are valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilRealWaveMultiply() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilRealWaveMultiply() received non float wave (4Byte) for source or target wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
int UFp1_UtilRealWaveMultiplyAdd( struct { double nPts; double nBeg2; waveHndl wFloatSrc2; double nBeg1; waveHndl wFloatSrc1; double res; }* p)
...
// XOP because IGOR itself is too slow..
{
   ...
   float    *wFloatSrc1, *wFloatSrc2;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilRealWaveMultiplyAdd", p->wFloatSrc1 ); 	// 050128
	// DebugPrintWaveProperties( "UFp1_UtilRealWaveMultiplyAdd", p->wFloatSrc2 ); 	// 050128

   if ( p->wFloatSrc1 == NIL  ||  p->wFloatSrc2 == NIL )	// check if wave handles are valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilRealWaveMultiplyAdd() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilRealWaveMultiplyAdd() received non float wave (4Byte) for source wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
...
// Order of values in wSums .   !!! Must be the same in  UF_MiniDetection.ipf  and in  UFp1_Utils.c
#define	kMD_SumX		0
#define	kMD_SumX2	1
#define	kMD_SumY		2
#define	kMD_SumY2	3
...
int UFp1_UtilCorrelation_( struct { double nPrintIt; waveHndl wCorr; waveHndl wSums; double nTPts; waveHndl wTempl; double nDPts; waveHndl wData; double res; }* p)
// XOP because IGOR itself is too slow..
{
   ...
   float    *wData, *wTempl, *wSums, *wCorr;   
	...
	// DebugPrintWaveProperties( "UFp1_UtilCorrelation_  ", p->wData );		// 050128
	// DebugPrintWaveProperties( "UFp1_UtilCorrelation_  ", p->wTemplate );	// 050128

   if ( p->wData == NIL || p->wTempl == NIL || p->wSums == NIL || p->wCorr == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilCorrelation_() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilCorrelation_() received non float wave (4Byte) for wData, wTempl, wSums or wCorr \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
   ...
   wTempl	= WaveData( p->wTempl );			//  char pointer to IGOR wave data 
	...
	for ( i = 0; i < nTPts; i += 1 ) {
		...
		if ( nPrintIt && ( i < 5 || i % 100 == 0 ) ) {
			sprintf( errBuf, "\tUFp1_UtilCorrelation_ Initialising\tT:%3d/%6d  D:%6d/%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", i, nTPts, n, nDPts, MaskStart, wSums[ kMD_SumX ], wSums[ kMD_SumY ], SumXY, SP, SQX, SQY, B, A, r );
			XOPNotice( errBuf );
		}
	}
	...
	for ( n = 0; n < nDPts - nTPts; n += 1 ) {
		...
		for ( i = 0; i < nTPts; i += 1) {
			...
			if (  nPrintIt && i < 3 && n%10000 == 0 ) {
				sprintf( errBuf, "\tUFp1_UtilCorrelation_ Computing..\tT:%3d/%6d  D:%6d/%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", i, nTPts, n, nDPts, MaskStart, wSums[ kMD_SumX ], wSums[ kMD_SumY ], SumXY, SP, SQX, SQY, B, A, r );
				XOPNotice( errBuf );
			}	
		}
...
int UFp1_UtilCorrelation( struct { double nEnd; double nBeg; double nPrintIt; waveHndl wCorr; waveHndl wSums; double nMaskStart;  double nTPts; waveHndl wTempl; waveHndl wData; double res; }* p)
// XOP because IGOR itself is too slow..
{
   long      nTPts		= (long)p->nTPts;
   ...
   float    *wData, *wTempl, *wSums, *wCorr;   
	...
	double	A	= 0;		// offset
	double	r	= 0;		// correlation
	// DebugPrintWaveProperties( "UFp1_UtilCorrelation  ", p->wData );		// 050128
	// DebugPrintWaveProperties( "UFp1_UtilCorrelation  ", p->wTemplate );	// 050128

   if ( p->wData == NIL || p->wTempl == NIL || p->wSums == NIL || p->wCorr == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: UFp1_UtilCorrelation() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   ...
   if ( WaveType(p->wData)!= NT_FP32 || WaveType(p->wTempl)!= NT_FP32 || WaveType(p->wSums)!= NT_FP32 || WaveType(p->wCorr)!= NT_FP32 )	// check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: UFp1_UtilCorrelation() received non float wave (4Byte) for wData, wTempl, wSums or wCorr \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );								// return NaN if wave is not 4Byte float
      return( IS_NOT_4BYTE_FLOAT_WAVE );
   }
	...
	for ( n = nBeg; n < nEnd; n += 1 ) {
		...
		for ( i = 0; i < nTPts; i += 1) {
			...
			if (  nPrintIt == 1  && i < 3 && n%10000 == 0 ) {
				sprintf( errBuf, "\tUFp1_UtilCorrelation Computing..\tT:%3d/%6d  D:%6d...%6d...%6d  MSt:%d  SumX:%g  SumY:%g  SumXY:%g  %g  %g  %g  %g  %g r:%g \r", 
		...
		if (  nPrintIt == 2  &&  57300 < n  &&  n < 57500 ) {
			sprintf( errBuf, "\tUFp1_UtilCorrelation Computing..\tD:%6d...%6d...%6d  MSt:%d  X:%g  X2:%g\tSY:\t%8.1g\tSY2:\t%10.2g\tSumXY:%g\tSP:\t%g\tSQX:\t%g\tSQY:\t%g\tB:\t%g\tA:\t%g\tr:\t%g \r", 
...
int UFp1_UtilFileDialog( struct { Handle FilePath; Handle DefExt; Handle InitDir; double Index; Handle Filter; Handle Prompt; Handle ReturnFilePath; }* p)
...
//						To select a directory call from IGOR like : gsDataPath = UFp1_UtilFileDialog( "Select directory" , "Directories; ;;" ,  1, sPath,  "", "_" )	
...
// Advantage 2 :	custom file filters other than those IGOR provides can be used
{
	...
	*(*(p->Filter)+n+1)	= '\0';
	...
	//sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(2) \tn:%2d lenFilter:%d\t'%s'  \r", n , lenFilter, *(p->Filter)  );	XOPNotice( errbuf );

   strFilter = IHC( p->Filter );
	//sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(3) \tstrFilter: \t\t'%s'   Index:%d \r", *strFilter, Index );	XOPNotice( errbuf );

   strInitDir = IHC( p->InitDir );
	//sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(4) \tstrInitDir: \t\t'%s' \r", *strInitDir );	XOPNotice( errbuf );
	...
	//sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(5) \tnativeInitDir:\t\t'%s' \r", nativeInitDir );	XOPNotice( errbuf );
   strDefExt = IHC( p->DefExt );

	bReadMode	= !strcmp( *strDefExt, csREADMODE );
	//sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(6) \tstrDefExt: \t\t'%s' -> ReadMode:%2d \r", *strDefExt, bReadMode );	XOPNotice( errbuf );

   strFilePath = IHC( p->FilePath );
  //sprintf ( errbuf, "\t\t\tUFp1_UtilFileDialog(7) \tstrFilePath:\t\t'%s' \r", *strFilePath );	XOPNotice( errbuf );
   ...
   if (( strReturnFilePath = NewHandle( lenReturnFilePath )) == NIL ) // get output handle , do not provide space for '\0' 
	{
		sprintf( errbuf, "++++Error: UFp1_UtilFileDialog() Not enough memory. Aborted...\r");
	   XOPNotice( errbuf );
		err = NOMEM;											// out of memory
	}
	else															// string length is OK so return string 
	...
	// XOPNotice( "\tUFp1_UtilFileDialog() Out: strReturnFilePath:'" );	XOPNotice( *strReturnFilePath );	XOPNotice( "'\r" );
...
/*
#int 	UFp1_UtilIsValidString( struct { Handle IsValidString; double res; }* p)
...
int 	UFp1_UtilMemoryLoad( struct { double res; }* p)
...
int 	UFp1_UtilTotalPhys( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFp1_UtilAvailPhys( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFp1_UtilTotalVirtual( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFp1_UtilAvailVirtual( struct { double res; }* p)
// From MSVC Knowledge base, HOWTO: Determine the Amount of Physical Memory Installed, Article ID: Q117889  
{
	MEMORYSTATUS MemoryStatus;
...
int 	UFp1_UtilContiguousMemory( struct { double nBytes; double res; }* p)
...
{
	waveHndl		waveHndPtr;
	char			*WaveNm		= "UF_Temp_UFp1_UtilContiguousMemCheck";
   int			nType			= NT_FP32;					// arbitrary, make 4-byte wave
	...
	// sprintf( errbuf, "UFp1_UtilContiguousMemory( MegaPts: %.6lf ) returns %d (0=OK, 1=Fail)\r", nPnts * 4 / 1e6, code ); 
	// XOPNotice( errbuf ); 
	p->res = code;
   return   0;		
}
...
int 	UFp1_UtilHeapCompact( struct { double res; }* p)
{
