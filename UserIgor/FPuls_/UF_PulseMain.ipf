...
// 2005-08-16  todo : possible convert  FPulse panel to  Panel3Main()  , the others e.g. preferences, data utilities to Panel2Sub()
...
#define   dDEBUG						// Comment this line for a Release version
...
// 2008-03-01 Newstyle script
#include "UFPE_Script6"
#include "UFPE_StimDisp6"
#include "UF_AcqStimDisp6"
//#include "UFPE_StimDispCurves6"  // file exist but is not called???
...
// 2008-03-01 Newstyle script
#include "UF_AcqCed6"					
#include "UF_AcqCfsWrite6"
#include "UF_AcqDisp6"
#include "UF_AcqDispCurves6"
...
// The test functions in the file  'UF_Test.ipf'  are not to be distributed to the user :  The code in   'UF_Test.ipf'  is automatically deleted in the Release version (but the empty wrapper 'UF_Test.ipf' is supplied)
// Alternate approach to prevent distribution to the user:  Rename 'UF_Test.ipf'  to  'Test.ipf'  to prevent copying to the release directory  and  automatically delete the preceding  line '#include "UF_Test" '  in FPulseMain and in FEvalMain	
#include "UF_AcqTest_4"						// the code in this file is automatically deleted in the Release version   (but the empty wrapper 'UF_Test.ipf' is supplied)
...
static strconstant	klstDEBUGPRINT	= "Timer;ShowLines;ShowKeys;ShowIO;ShowVal;ShowAmpTime;ShowEle;Expand;Digout;OutElems;Telegraph;Ced;AcqDA;AcqAD;CfsWrite;DispDurAcq;"
...
UFCom_PossiblyCreateFolder( "root:uf:" + sFo + ":dispFS" )		// for acquisition display  oldstyle and newstyle
...
// 2005-05-30  was in  :co:  but must now in :Keep: as it must be maintained  ( -> LagTime()  ElapsedTime()  ) , :co:  is killed....  Other possibility: do not kill  :co:  but instead set all variables to 0
// 2009-03-26  removed  Lag  and LagTime()  completely
...
// !!! Do not change the function name  'FPulse()'   as  it is used as an argument to FunctionPath()  (and possibly/probably at other places...)   
...
// 2009-03-26
//	UFCom_EnableSetVar( "pul", 	"root_uf_acq_pul_svLagTime0000",	UFCom_kCo_NOEDIT_SV  )	// Setvariable must only display value
	...
	// print "\tFPulse(a)\tScriptPath  :",  UFCom_Ini_Section( sFo, sSubFoIni, "Script", "Path" ), "      dir:", sscriptdir, "          lstLRU:", lstLRU
	if (  UFCom_PossiblyCreatePath( sScriptDir ) == UFCom_kOK )
		lstLRU	= ScriptPathSet( sFo, ksFPUL, sScriptPath ) 									
		// print "\tFPulse(b)\tScriptPath  :",  UFCom_Ini_Section( sFo, sSubFoIni, "Script", "Path" ), "      dir:", sscriptdir, "          lstLRU:", lstLRU
		 ...
		 PathInfo /S 	$UFPE_ksSCRIPTS_SYMPATH;  printf "\t\tFPulse() \t PathInfo: Symbolic path '%s' does %s exist.    ScriptDir  '%s'  is OK. \r", S_path, SelectString( v_Flag,  "NOT", "" ) , sScriptDir
	...
	// print "\tFPulse(d)\tScriptPath  :",  UFCom_Ini_Section( sFo, sSubFoIni, "Script", "Path" ), "      dir:", sscriptdir, "          lstLRU:", lstLRU
...
Function			NewStyleSet( sFo, bNewStyle )
	string  	sFo
	variable	bNewStyle
	variable	/G	$"root:uf:" + sFo + ":" + "gbNewstyle"	= bNewStyle
	return	bNewStyle
End	
Function			NewStyle( sFo )
	string  	sFo
	nvar   /Z		bNewStyle	     =	$"root:uf:" + sFo + ":" + "gbNewstyle"
	if ( ! nvar_exists( bNewstyle ) )
		variable  	/G			$"root:uf:" + sFo + ":" + "gbNewstyle"
		nvar  	bNewStyle	     =	$"root:uf:" + sFo + ":" + "gbNewstyle"
	endif
	return	bNewStyle
End	
...
Function		PanelPulse()
	...
	UFCom_LstPanelsSet(  ksACQ, ksfACQVARS_, AddListItem( sWin, UFCom_LstPanels( ksACQ, ksfACQVARS_ ) ) )	
...
//	string  	sF								// e.g. 'root:uf:acq:' 
	...
	n += 1;	tPn[ n ] =	"BU:    0:	1:	6:	0:	°:			,:			1,°:			LoadScript:	Load :		:			fLoadScript():	:		:				:			:		Loading a script:	" //	single button
	...
	n += 1;	tPn[ n ] =	"BU:    1:	0:	3:	0:	°:			,:			1,°:			buDiacWnAdd:	Add acq window::			fDiacWndAdd():	:		:				:			:		Add window:		" 
	n += 1;	tPn[ n ] =	"BU:    0:	2:	4:	0:	°:			,:			1,°:			Comment:		Comment +~Comment -::		fComment():	:		fStdColorLst_a():	:			:		Comment:			" 
...
//	n += 1;		tPn[ n ] 	=	"PN_SETVAR;	root:uf:acq:ola:gnPeakAverMS		;average peak over ms; 	15; 	%.2lf ;.01,1000,0;	"			
...
// 2009-03-26
//	n += 1;	tPn[ n ] =	"SV:	   0:	1:	2:	0:	°:			,:			1,°:			svLagTime:	Lag / s:		:			:			40:		%d;-1,10000,0:		~0:			:		Lagging time:				" //  	1 dim  SetVariable( 1 row)
...
//	n += 1;	tPn[ n ] =	"SV:	   0:	11:	14:	2:	°:			,:			1,°:			svSwpsTot:	/:			:			:			32:		%d;-1,10000,0:		~1:			:		Total number of sweeps:		" //  	1 dim  SetVariable( 1 row)
	...
	n += 1;	tPn[ n ] =	"SEP:  1:	0:	1:	0:	°:			,:			,:			dum98:		:			:			:			:		:				:			:							"
   	n += 1;	tPn[ n ] =	"BU:    1:	0:	3:	1:	°:			,:			1,°:			buDbgPrint:	Debug Print +~Debug Print -::	fDbgPrint():	:		fStdColorLst_a():	~1:			:		Debug Printing:			" // ~1 : must be initially on so that variables are constructed
#endif
...
strconstant		ksPUL_HELP_WND	= "FPulseHelp"   
strconstant		ksPUL_HELP_PATH	= "FPulseHelp.txt"
...
Function		fNbPuHelp( s ) 
	...
	if ( state )
		...
		if ( cmpstr( sPath[ 0 ], ":" ) == 0 )
			UFCom_InternalError( "Could not locate file '" + ksPUL_HELP_PATH + "' ." )
		...
		sPath = ParseFilePath( 1, sPath, ":", 1,0 ) + ksPUL_HELP_PATH		// Create path to the help file.
		printf "\t\tfPuHelp( s ) \thelp path: '%s' \r", sPath
	
		OpenNotebook	/K=1	/V=1	/N=$ksPUL_HELP_WND   sPath		// visible, could also use /P=symbpath...
		MoveWindow /W=$ksPUL_HELP_WND	1, 1, 1, 1				// restore from minimised to old size
	else
		if ( WinType( ksPUL_HELP_WND ) == UFCom_WT_NOTEBOOK )
			MoveWindow /W=$ksPUL_HELP_WND	0, 0, 0, 0			// minimise the Notebook  to an icon
		endif
	endif
End
...
Function		fOLACursorsDsp( s ) 
	...
	PanelPulse()										// rebuild THIS main FPulse  showing or hiding the  OLA cursors section
End
...
Function		fAcqStatusDsp( s ) 
	...
	// nvar	state		= $ReplaceString( "_", s.ctrlname, ":" )	// the underlying button variable name is derived from the control name
	// printf "\t\t\t\t%s\tvalue:%2d   \t \r",  s.ctrlname, state	
	PanelPulse()						// rebuild THIS main FPulse  showing or hiding the  Acquisition Status section
End
...
// Reads the gain of each channel from 'wIO' (wIO has been set with the gain as found in the script) and update the panel gain (=svAxogain00x0) with the wIO gain.
	string  	sBaseNm, sF		// e.g. 'root:uf:acq:'
	string  	sWin 
	string 	lstADNames	= "", 	sADCNm	= "", sBase = "Adc"
	variable	cio, nCntAD, nio = UFPE_IOT_ADC

// 2009-12-12
if ( NewStyle( ksACQ ) == kPULSE_OLD )

//	wave/Z/T	wIO	= $"root:uf:" + ksACQ + ":ar:wIO"  						// This  'wIO'	is valid in FPulse ( Acquisition )
//	if ( waveExists( wIO ) )
//		nCntAD	=  UFPE_ioUse( wIO, nio )
////		sADCNm	= ""
//		for ( cio = 0; cio < nCntAD; cio += 1 )
//			if ( UFPE_TGChan( wIO, nio, cio )  != UFCom_kNOTFOUND )								// this is a true Adc channel having a corresponding telegraph channel 
//				sADCNm	=  UFPE_ios( wIO, nio, cio, UFPE_IO_NM ) + " (Tg" 	+ num2str( UFPE_TGChan( 	  wIO, nio, cio ) ) + ")      "
//			elseif ( UFPE_TGMCChan( wIO, nio, cio )  != UFCom_kNOTFOUND )								// this is a true Adc channel having a corresponding MULTICLAMP telegraph channel
//				sADCNm	=  UFPE_ios( wIO, nio, cio, UFPE_IO_NM ) + " (MCTg"	+ num2str( UFPE_TGMCChan( wIO, nio, cio ) ) + ")"
//			else																	// this is a true Adc channel  without  a corresponding telegraph channel
//				sADCNm	=  UFPE_ios( wIO, nio, cio, UFPE_IO_NM ) + "                             " 	// magical number of spaces makes a better vertical alignment of the Setvariable titles  in the panal  //  " (fix)   "
//			endif
...
//		endfor
//	else
//		// printf  "\t\t\tTelegraphGain( LstAllAD() \tlstADNames: '%s'   as   wIO is not yet defined  \r", lstADNames
//	endif
else
	...
	// printf  "\t\t\tTelegraphGain( LstAllAD( \t%s\t%s, %s ) \tNewStyle:%2d\tlstADNames: '%s'   \r", UFCom_pd(sBaseNm,13), sF, sWin, NewStyle( ksACQ ), lstADNames
	return	lstADNames
End
...
// Axogain Setvariable controls: disable the control (=no Edit, display only) if it is a telegraph controlled channel, enable (=allow changing the value) if it is a normal not-telegraph-controlled channel
	string  	sFo
	string  	sCNm	= ""
	variable	cio, nCntAD, nio	= UFPE_IOT_ADC, Gain

// 2009-12-12
if ( NewStyle( sFo ) == kPULSE_OLD )
//	wave/Z/T	wIO	= $"root:uf:" + sFo + ":ar:wIO"  						// This  'wIO'	is valid in FPulse ( Acquisition )
//	if ( waveExists( wIO ) )
//		nCntAD	=  UFPE_ioUse( wIO, nio )
//		for ( cio = 0; cio < nCntAD; cio += 1 )
...
//			// printf "\t\t\t\tTelegraphGain() EnableDisableAxogains()   cio:%2d/%2d   \t%s\tGain:\t%8.2g \tNewStyle:%2d\r", cio, nCntAD, sCNm, Gain,  NewStyle( sFo )
//		endfor
//	endif
else
	nio	= kSC_ADC
...
//	wave  	wG		= $"root:uf:" + sFolder + ":" + UFPE_ksKPwg + ":wG" 					// This  'wG'  	is valid in FPulse ( Acquisition )
