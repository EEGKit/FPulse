//
//   UF_AcqTest_4.ipf		( for Version 3nn)
// 
...
// From  UFPE_PulseConstants.ipf 
//static strconstant	 UFPE_ksACQ			= "acq"			// The subfolder for the  'acquisition'  variables. Do not change as action proc names (and globals in panels) depend on it.
//static constant		ERRLINE			= 2				// DEBUG:    print the error in the command window
//static constant		MSGLINE			= 4				// DEBUG:    print the error in the command window and invoke error message box

// From  UFPE_Misc_3xx.IPF 
static strconstant	ksCOM			= "com"			// th
static strconstant	ksDF_DLG		= "root:uf:dlg:"		// this special folder must contain colon ! Do not change until  ALL occurrences of 'root:uf:dlg:' and of  'root_uf_dlg_' have been replaced by ksDF_DLG
...
// 2008-02-28 
	UFCom_PossiblyCreateFolder( sFBase + sFolder + sPnOptions ) 	

	InitPanelTest( sFolder, sPnOptions )
	UFPE_ConstructOrDisplayPanel(  "PnTestDebug", "Test + Debug" , sFolder, sPnOptions,  98, 99 )
End
...
// 2008-04-14 q&d removed as folder 'root:uf:ola:' does not exist....(code must perhaps be reinserted ???)
//	n += 1;	tPn[ n ] =	"PN_CHKBOX;	root:uf:ola:gOLADoFitOrStartVals;Do OLA fit (or display only Start vals)"		// Flaw: folder  'ola'  is only known in  Acq  , not in  Eval
	...
	n += 1;	tPn[ n ] =	"PN_BUTTON;	buKill_Root_uf_acq			;Kill  folder  'Root_uf_acq' "
	n += 1;	tPn[ n ] =	"PN_BUTTON;	buKillAllTimers				;Kill All Timers"
	n += 1;	tPn[ n ] =	"PN_BUTTON;	buSwitch_Eliminate_Blanks	;Switch 'Eliminate Blanks Mode'"
	...
	n += 1;	tPn[ n ] =	"PN_BUTTON;	buTest14					;Test14  bCRASH_ON_REDIM - AutoLoadScripts"	// 2005-1105
	...
	n += 1;	tPn[ n ] =	"PN_BUTTON;	buTest18					;Test18   Setk ksFONT , UFCom_kFONTSIZE"
...
Function		buKill_Root_uf_acq( ctrlName ) : ButtonControl
	string 	ctrlName	
...
Function		buKillAllTimers( ctrlName ) : ButtonControl
	string 	ctrlName	
...
Function		buSwitch_Eliminate_Blanks( ctrlName ) : ButtonControl
// replaces a constant:
// constant   	UFPE_kELIMINATE_BLANK_ns 	= 0			// very slow: transfers nostore periods to/from Ced  (never use this mode except in debug mode for comparing transfer speeds)
// constant   	UFPE_kELIMINATE_BLANK_ns 	= 1			//  do NOT transfer 'Blank=nostore' periods between computer and host. Improves loading time and acq data rates.  The  'BigIO' waves still contain blanks which clog the memory
// constant  	UFPE_kELIMINATE_BLANK_ns 	= 2			//  do NOT transfer 'Blank=nostore' periods between computer and host and avoid  'BigIO' waves which clog the memory.  'TapeIO' waves containing only store periods are used instead.
	string 	ctrlName	
...
// 2009-06-25   moved to UFPE_SCRIPT6
//Function		ELIMINATE_BLANKS()
//	nvar		/Z	nEBmode	= root:uf:acq:nEliminateBlanksMode
//	if ( ! nvar_exists( nEBmode ) )
//		variable /G		   root:uf:acq:nEliminateBlanksMode	= 2
//		nvar		nEBmode	= root:uf:acq:nEliminateBlanksMode
//	endif
//	return	nEBmode
//End
...
Function		buTest13( ctrlName ) : ButtonControl
	...
	UFP_CedCloseAndOpen(  UFPE_ERRLINE )					// do NOT show error box  ( Parameter1 = n1401 should be 0, see prog int lib 3.20, dec 99, p.5)
	...
	code		= UFP_CedLdErrOut( UFPE_MSGLINE, sCmdDir, sCmds )
...
// 2008-04-22 not used or moveToAcqCed
...
//	CEDSendStringCheckErrors( sBuf, 1 ) 						//(//  UF_AcqCed
//	//variable	code = UFP_CedSendStringErrOut( MSGLINE, sBuf ) 
//End
//
...
//	CEDSendStringCheckErrors( sBuf, 1 ) 						//(//  UF_AcqCed
//	//code	= UFP_CedSendStringErrOut( MSGLINE, sBuf ) 
//End
...
Function	TestSetvarAndPopupmenu( sTxt, xs, bodyw, xp, dy, n )
	...
	UFPE_FormatSetvarPopup_319( sTxt, xs, bodyw )		// references are changed
...
Function 	MyCC_CounterFunc1(s)
	...
	//if (  s.eventCode != UFCom_CCE_mousemoved ) 
		printf "\tMyCC_CounterFunc1() \tnVal:%d \tEventCode:%2d    \t%s \r ", s.nVal,  s.eventCode,  UFCom_pd( StringFromList( s.eventCode, UFCom_CCE_lstEVENTCODES ), 8 )
	endif	
...
Function  		MC700Dlg()
	...
	UFPE_ConstructOrDisplayPanel(  "PnMC700",  "MC700" , sFolder, sPnOptions, 100, 50 )

	 UFPE_EnableButton_319( "PnMC700", "buMC700CreateObject",			UFCom_kCo_ENABLE )
	 UFPE_EnableButton_319( "PnMC700", "buMC700ScanAllMultiClamps",		UFCom_kCo_DISABLE )
	 UFPE_EnablePopup_319( "PnMC700", "root_uf_misc_MC700_gpSelectMCC",UFCom_kCo_DISABLE )
	 UFPE_EnableButton_319( "PnMC700", "buMC700SetModeVC",			UFCom_kCo_DISABLE )
	 UFPE_EnableButton_319( "PnMC700", "buMC700SetModeCC",			UFCom_kCo_DISABLE )
	 UFPE_EnableButton_319( "PnMC700", "buMC700Reset",				UFCom_kCo_DISABLE )
	 UFPE_EnableButton_319( "PnMC700", "buMC700DestroyObject",			UFCom_kCo_DISABLE )
End
...
Function		InitPanelMC700( sFolder, sPnOptions )
	...
	variable	/G				root:uf:acq:misc:MC700:gDebugMsg	= 0
	...
	n += 1;	tPn[ n ] =	"PN_POPUP  ;	root:uf:acq:misc:MC700:gpSelectMCC;; 	200	;   1	;gpSelectMCC_Lst; gpSelectMCC  "	// Sample: Default value (Par4) supplied, reset on panel rebuild.
//	n += 1;	tPn[ n ] =	"PN_POPUP  ;	root:uf:acq:misc:MC700:gpMC700	;; 	200	;	;gpSelectMCC_Lst; gpSelectMCC  "	// Sample: No default value (Par4) supplied, set only at program start. PrintResults needs an explicit action procedure 
...
//Function		gDebugMsg( ctrlName, varNum, varStr, varName ) : SetVariableControl
	...
	variable	varNum
	nvar		gDebugMsg	= root:uf:acq:misc:MC700:gDebugMsg	
...
//	variable	/G				root:uf:acq:misc:MC700:hMCCmsg
	...
	if ( hMCCmsg ) 
		 UFPE_EnableButton_319( "PnMC700", "buMC700CreateObject",		UFCom_kCo_DISABLE )
		 UFPE_EnableButton_319( "PnMC700", "buMC700ScanAllMultiClamps",	UFCom_kCo_ENABLE )
		 UFPE_EnableButton_319( "PnMC700", "buMC700DestroyObject",		UFCom_kCo_ENABLE )
	else
...
Function		buMC700ScanAllMultiClamps( ctrlName ) : ButtonControl
	...
	if ( hMCCmsg )
		...
		ControlUpdate	/W=$"PnMC700"   root_uf_misc_MC700_gpSelectMCC 
		 UFPE_EnablePopup_319( "PnMC700", "root_uf_misc_MC700_gpSelectMCC", UFCom_kCo_ENABLE )
	else
		glstlstAllMCCIds	= ""
		glstPopupMCCs	= ""
		UFCom_Alert( UFCom_kERR_IMPORTANT, "Communication with MultiClamp(s) not ready.  (" + ctrlName + ") " )
		gpSelectMCC	= -1//Nan
...
Function		gpSelectMCC( sControlNm, popNum, popStr ) : PopupMenuControl
	...
	if ( hMCCmsg )
		...
		if ( nCode == 0 )
			UFCom_Alert( UFCom_kERR_IMPORTANT, sControlNm + " failed. " )
		else
			 UFPE_EnableButton_319( "PnMC700", "buMC700SetModeVC",		UFCom_kCo_ENABLE )
			 UFPE_EnableButton_319( "PnMC700", "buMC700SetModeCC",		UFCom_kCo_ENABLE )
			 UFPE_EnableButton_319( "PnMC700", "buMC700Reset",			UFCom_kCo_ENABLE )
		endif		
	endif		
End
...
Function		buMC700Reset( ctrlName ) : ButtonControl
	...
	if ( hMCCmsg )
		...
		if ( nCode == 0 )
			UFCom_Alert( UFCom_kERR_IMPORTANT, ctrlName + " failed. " )
		endif		
	else
		UFCom_Alert( UFCom_kERR_IMPORTANT, "Communication with MultiClamp(s) not ready.  (" + ctrlName + ") " )
	endif
