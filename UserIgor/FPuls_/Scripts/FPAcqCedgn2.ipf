...
//	make  /O /I /N=( MAXDIGOUT ) wDgoCh, wDgoBeg, wDgoDur						// 32bit integer for Dig wave
	make  /O /I /N=( MAXDIGOUT, 3 ) $"root:uf:" + sFolder + ":dig:wDgoChBegDur"	  ;	wave  wDgoChBegDur	= $"root:uf:" + sFolder + ":dig:wDgoChBegDur"	// 32bit integer for Dig wave
						...
						for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
							...
							if ( numtype(  nChan )  !=  NUMTYPE_NAN )									// true channel number 
								...
								if ( ( nChan == 3  ||  nChan == 4 )   &&   ( b == 0  &&  f == 0  &&  s == 0  &&  e == 0 )  )
									...
									nElemCnt = FillDigDispArrays( "AutoTrig", wG, nElemCnt, nChan, nBeg-cTIME_DAC_TO_DIGOUT, nDur, b, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoChBegDur )  
								...
								elseif ( ( nChan == 3  ||  nChan == 4 )   &&     e > 0   )	
									...
									nElemCnt = FillDigDispArrays( "UserTrig", wG, nElemCnt, nChan, nBeg, nDur, b, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoChBegDur  )  
								...
								elseif ( ! ( nChan == 3  ||  nChan == 4 ) )									
									...
									nElemCnt  = FillDigDispArrays( "normal  ", wG, nElemCnt,  nChan, nBeg, nDur, b, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoChBegDur  )  
								endif			
							endif			
						endfor			
					endif
...
//	redimension  /I	/N=( nElemCnt ) wDgoCh, wDgoBeg, wDgoDur 	// for  Digout wave for display
	redimension  /I	/N=( nElemCnt ) wDgoChBegDur 					// for  Digout wave for display
...
//		sprintf sTxt, "DigoutMakeDisplayWaves()  Redimension  %d ?= %d  ?= %d  ?= %d  ", nElemCnt, numPnts( wDgoCh ), numPnts( wDgoBeg ), numPnts( wDgoDur ) 
	if ( nElemCnt != DimSize( wDgoChBegDur, 0 )  ||  nElemCnt != DimSize( wDgoChBegDur, 1 )   ||  nElemCnt != DimSize( wDgoChBegDur, 2 ) )
		sprintf sTxt, "DigoutMakeDisplayWaves()  Redimension  %d ?= %d  ?= %d  ?= %d  ", nElemCnt, DimSize( wDgoChBegDur, 0 ) , DimSize( wDgoChBegDur, 1 ) , DimSize( wDgoChBegDur, 2 ) 
		InternalError( sTxt )
	endif
	...
	for ( n = 0; n < nElemCnt; n += 1)
		if ( gRadDebgSel > 2  &&  PnDebgDigout )
			printf "\t\t\t\tDigoutMakeDisplayWaves(22.) after sorting Beg \tn:%3d/%3d \tch:%3d \tBeg:%7d \tDur:%7d \r", n, nElemCnt,  wDgoChBegDur[ n ][0], wDgoChBegDur[ n ][1], wDgoChBegDur[ n ][2]  
		endif
	endfor
	...
	for ( n = 0; n < nElemCnt; n += 1)
		for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
			...
			if ( wDgoChBegDur[ n ][0] == nDgoCh )
				...
				for ( l = wDgoChBegDur[ n ][1]; l < wDgoChBegDur[ n ][1] + wDgoChBegDur[ n ][2]; l += 1)
					wDOFull[  l ] = 1
				endfor	
			endif
		endfor	
...
Function		FillDigDispArrays( sText, wG, nElemCnt, nChan, nBeg, nDur, b, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoChBegDur  )  
	...
	if ( nBeg < -1  )	
		Alert( cSEVERE, "Digital Out pulse (ch:" + num2str( wDgoChBegDur[ nElemCnt ][0] ) + ") cannot start at times more negative than 1 time slice (= sample interval) . Must start at  -" + num2str( nSmpInt ) + " us or later. (Starts at " + num2str( nBeg ) + " time slices)" )
	endif
	if ( nBeg == -1 &&  !( nChan == 3  ||  nChan == 4 ) )	// do not complain about Events 3 and 4 which always start at time zero - 1 sample interval
		Alert( cIMPORTANT, "Digital Out pulse (ch:" + num2str( wDgoChBegDur[ nElemCnt ][0] ) + ") starting at -" + num2str( nSmpInt ) + " us will only be output in the 1. protocol") 
	endif
	nElemCnt += 1
	return	nElemCnt
End	
