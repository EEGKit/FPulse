...
rnAxis	= YAxisCnt( w )
				...
				sCurve	= BuildCurve( sChan, ra, mo, bAutoscl, YOfs, YZoom, rnAxis, sRGB )
sCurves	= RetrieveCurves( w )
				...
				for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )					
					ExtractCurve( w, nCurve, sChan, ra, mo, bAutoscl, YOfs, YZoom, rnAxis, sRGB )		// break 'sCurve' to get all traces to be displayed in window 'w'...
				...
				for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )								// Rebuild all traces in window
					ExtractCurve( w, nCurve, sChan, ra, mo, bAutoscl, YOfs, YZoom, rnAxis, sRGB )		// break 'sCurve' to get all traces to be displayed in window 'w'...
					Range2Pt( sFolder, ra, pr, bl, fr, sw, BegPt, EndPt, Pts, bIsFirst )
					nAxis		= YAxisIdx( w, nCurve )
					DurAcqDrawTraceAndAxis( sFolder, w, nCurve, nAxis, BegPt, Pts, bIsFirst, ra, nSmpInt, ResultXShift, LastDataPos )
				endfor
...
Function		fLoadDispCfg3( s )
	...
	svar		sScriptPath= root:uf:acq:pul:gsScrptPath0000
	string  	sFolder	= ksACQ
	LoadDisplayCfg( sFolder, sScriptPath, "_3" )				// "_3"  	must keep this naming convention so that future versions can access old display configurations
End
...
variable	nAxis	
	...
	for ( w = 0; w < kMAX_ACDIS_WNDS; w += 1 )
		...
		variable	ra, mo, rAutoscl, rYOfs, rYZoom, rnAxis 
		string		sChan, sRGB
		variable	pr = 0,  bl = 0,  fr = 0,  sw = 0
		...
		variable	LastDataPos	= 0
		for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )
			sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )					// required for debug printing  AND and for  removing the curve if the channel can not be found 
			ExtractCurve( w, nCurve, sChan, ra, mo, rAutoscl, rYOfs, rYZoom, rnAxis, sRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
			...
			// printf "\t\tLoadDispSettings( b  \trow:%2d\tcol:%2d \t <-\t%s\tch:%2d\tmo:%2d\tra:%2d\twnd:%2d\t'%s'\tldp:%8g\tScript: %s\tCv:%d  ClnC:%d\t\t\t%s   \r", row , col, pd(sChan,7), ch, mo, ra, w, sWndAcDis, LastDataPos, pd( lstChans, 23), ItemsInList( sCurves, ksCURVSEP), ItemsInList( sCleanCurves, ksCURVSEP), pd( sCurve, 43)
		endfor
		for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )
			sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )					// required only for debug printing  
			ExtractCurve( w, nCurve, sChan, ra, mo, rAutoscl, rYOfs, rYZoom, rnAxis, sRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
			Range2Pt( sFolder, ra, pr, bl, fr, sw, BegPt, EndPt, Pts, bIsFirst )
	nAxis		= YAxisIdx( w, nCurve )
			DurAcqDrawTraceAndAxis( sFolder, w, nCurve, nAxis, BegPt, Pts, bIsFirst, ra, nSmpInt, ResultXShift, LastDataPos )
			 printf "\t\tLoadDispSettings( c  \trow:%2d\tcol:%2d \t <-\t%s\tch:%2d\tmo:%2d\tra:%2d\twnd:%2d\t'%s'\tldp:%8g\tScript: %s\tCv:%d  ClnC:%d\tnAxis:%2d\t%s   \r", row , col, pd(sChan,7), ch, mo, ra, w, sWndAcDis, LastDataPos, pd( lstChans, 23), ItemsInList( sCurves, ksCURVSEP), ItemsInList( sCleanCurves, ksCURVSEP), nAxis, pd( sCurve, 43)
		endfor
...
static Function		Curve2Mode( sMoRa )
// retrieves the Mode setting  from TWA  and converts the 2-letter-string   into 2 numbers  e.g. 'SM'->0,1   'RS' ->3,0
	string 	sMoRa
	return	ModeNr(   sMora[1,1] )
End

static Function		Curve2Range( sMoRa )
// retrieves the Range setting  from TWA  and converts the 2-letter-string   into 2 numbers  e.g. 'SM'->0,1   'RS' ->3,0
	string 	sMoRa
...
//	// 060608 weg ????WHY is  Range2Pt()   OUTSIDE the curves loop????????? 
...
//	endif	
	...
	for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )			
	  	//sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )
		// printf "\t\t\tDispDurAcq()  prot:%d  block:%2d  frm:%2d  swp:%2d\t [points %6d..\t%6d \t=%6d\t pts]   ( LastDataPos:%7g , bIsFirst:%d ) \r", pr, bl, fr, sw,  BegPt, BegPt+Pts, Pts, LastDataPos, bIsFirst
variable	nAxis		= YAxisIdx( w, nCurve )
		DurAcqDrawTraceAndAxis( sFolder, w, nCurve, nAxis, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift, LastDataPos )
	endfor	
End
...
static  Function		DurAcqDrawTraceAndAxis( sFolder, w, nCurve, nAxis, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift, LastDataPos )	
// If the user changes the AxoPatch Gain the trace in the Acq Display should keep its height but the Y scaling should adjust. This may occur any time during acquisition. The DAT file will reflect the changed gain only at the beginning of thr next data section.
	string  	sFolder
	variable	w, nCurve, nAxis, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift, LastDataPos
...
rnAxis=nAxis//simplify

	 printf "\t\t\tDurAcqDrawTraceAndAxis(1)\t\tw:%2d\tcv:%2d\t\tax:%2d\t\t\t\t\t\tpts:%6d\tra %d\t %s\t%d r\tmo:%2d\tZm:\t%7.2lf\tOs:\t%9.2lf   \tGn:\t%7.1lf\t Rgb:%s  \r", w, nCurve, rnAxis, pts, nRange , SelectString( nRange == ra, "!=", "=="), ra, mo, ryZoom, rYOfs, Gain, rsRGB
	...
	if (  Pts > 0   &&   waveExists( $sFolderTNm )   &&    nRange == ra )
		...
		// 060608
		// compute nAxes = AxisCnt()  only in  DurAcqDrawXPositionOfYAxis()   STORE nAxis  in sCurve ??? 
		...
		GetAxis/W=$sWNm /Q $wAxisName[ rnAxis ]										// check if axis exists
...
//		string  	sCurves	= RetrieveCurves( w )											
//		string  	sCurve 	= StringFromList( nCurve, sCurves, ksCURVSEP )						// only for the following debug print
		 printf "\t\t\tDurAcqDrawTraceAndAxis(2)  bIs1.%2d\tw:%2d\tcv:%2d\t\tax:%2d\t\t\t\t\t\tpts:%6d\tra:%d  \t\t\tmo:%2d\t'%s' \t'%s'\tsmpint: %d  gResultXShift:%7.2lf \tDrawYax:%2d\t \r", bIsFirst, w, nCurve, rnAxis, pts, ra, mo, rsChan, rsRGB, nSmpInt, gResultXShift, bRedrawYAxis
		DurAcqDrawTrace( sFolder, w, rnAxis, sWNm, rsChan, BegPt, Pts, bIsFirst, mo, ra, nSmpInt, rsRGB, Gain ) 
		...
		if ( bRedrawYAxis )															// Construct and draw Y axis only when required : i.e. when there is no axis as its traces have been removed (e.g. when a new block starts)
			variable	nYAxisCnt		= YAxisCnt( w )								// nYAxisCnt   could be less than   nCurves  if an Y axis has been removed perhaps because 2 or more traces share the same Y axis		
			DurAcqDrawXPositionOfYAxis( sFolder, w, rnAxis, nYAxisCnt, sWNm, rsChan, Pts, rsRGB, LastDataPos )	
		endif
//		if ( ! rbAutoScl )
...
static  Function		DurAcqDrawTrace( sFolder, w, nAxis, sWNm, sTNm, BegPt, Pts, bIsFirst, nMode, nRange, nSmpInt, sRGB, Gain ) 
...
// Different  data are displayed under the same name, but any operation (e.g. resize window) destroys the data leaving only the last...
	...
	variable	w, nAxis, BegPt, Pts, bIsFirst, nMode, nRange, nSmpInt, Gain
	variable	PnDebgDispDurAcq = DebugSection() & kDBG_DispDurAcq
	...
	// printf "\t\t\t\tDurAcqDrawTrace(1)  \t  \t \tmode: %d\tbIs1:%d\tWNm: '%s' \tsTNm:%s ->\t%s  \tnAxis[ w:%2d ] = %d \t\t\t\t\t\t\t\t[cnt:%3d]\tTNL1: '%s'  \r", nMode, bIsFirst, sWNm, sTNm, pd(sTNmUsed,22),  w, nAxis, ItemsInList( TraceNameList( sWNm, ";", 1 )  ),  TraceNameList( sWNm, ";", 1 )  
...
//	wave	wAxisExists	= root:uf:disp:wYAxisExists									// ..during acq the traces are regularly erased, so 'AppendToGraph' either actually draws the first trace in a blank window...
	...
	if ( WhichListItem( sTNmUsedNF, TraceNameList( sWNm, ";", 1 ) ) == kNOTFOUND )			// For Redrawing outside acq ( e.g. after changing zoom, ofs ) the traces are not erased, they exist : here  we avoid drawing them over and over #1, #2, #3... when the YOfs slider is moved 
		if ( nMode == kMANYSUPIMP   ||   ( nMode == kCURRENT && bIsFirst ) )				// in CURRENT mode the trace is appended only once : IGOR updates automatically
//			wAxisExists[ w ][ nAxis ] = TRUE											// 141102 mark this Y axis as 'displayed'  as  we can later act only on  'displayed'  Y axis
			...
			if ( nAxis == 0 ) 														// append the first trace with its Y Axis to the left....
				...
				// Here the connection is made between a certain trace and its accompanying axis name !  AxisInfo  will return  the name of the controlling wave 
				 AppendToGraph /Q /R=$wAxisName[ nAxis ] /W=$sWNm  /C=( rnRed, rnGreen, rnBlue ) $sTNmUsed 
			endif
			// printf "\t\t\t\tDurAcqDrawTrace(2) after appending \tmode: %d\tbIs1:%d\tWNm: '%s' \tsTNm:%s ->\t%s  \tnAxis[ w:%2d ] = %d \tvalid for nAxis != 0: AxNm: '%s'\t[cnt:%3d]\tTNL1: '%s'  \r", nMode, bIsFirst, sWNm, sTNm, pd(sTNmUsed,22),  w, nAxis, wAxisName[ nAxis ], ItemsInList( TraceNameList( sWNm, ";", 1 )  ),  TraceNameList( sWNm, ";", 1 )  
		endif
	endif
...
// 060603 new
static  Function		 DurAcqDrawXPositionOfYAxis( sFolder, w, nAxis, nYAxisCnt, sWNm, sTNm, pts, sRGB, LastDataPos )
...
// - drawing the axis should be done as seldom as possible: only when really needed that is right in the very first display update
	variable	w, nAxis, nYAxisCnt, pts
	...
	variable	nRightAxisCnt	= nYAxisCnt - 1
//	wave	wAxisExists	= root:uf:disp:wYAxisExists
	wave  /T	wAxisName	= root:uf:disp:wYAxisName
	variable	rnRed, rnGreen, rnBlue
	ExtractColors( sRGB, rnRed , rnGreen, rnBlue )
	variable	BotAxisBeg	= 0
	...
	string		sAxisNm		= wAxisName[ nAxis ] 									// 030610
	 printf "\t\t\t\tDurAcqDrawXPositionOfYAxis(1)\tw:%2d\t\t\tax:%2d/%2d\t%s\tRiAxCnt:%d\tpts:%6d\tLastDataPos:%g\t\t\t\t\t\t\tBotAxEnd:%g   \r", w, nAxis, nYAxisCnt, pd(sAxisNm,5), nRightAxisCnt, pts, LastDataPos, BotAxisEnd
...
// 	DrawTextboxUnits( sWNm, nAxis, nCurves, ....
// 060606
//	variable	TboxXPos	= nAxis == 0 ? -54 : 50 - 20 			 *  ( nYAxisCnt - ut )		// left, left right,  left mid right...    -54, 50, 20 are magic numbers which  are adjusted  so that the text is at the same  X as the axis. They SHOULD be   #defined.....
	variable	TboxXPos	= nAxis == 0 ? -54 : 46 - 66 * cAXISMARGIN * ( nYAxisCnt - nAxis - 1 )	// left, left right,  left mid right...    -54, 46, 66 are magic numbers which  are adjusted  so that the text is at the same  X as the axis. They SHOULD be   #defined.....
	...
	// printf "\t\t\t\tDurAcqDrawXPositionOfYAxis()  w:%2d\tut:%2d/%2d \tTboxXPos:%d \t'%s'  \tAxNm:%s \t\r", w, nAxis+1, nYAxisCnt, TboxXPos, rsUnits, pd( sAxisNm,10)
...
// 060606
	...
	variable	YAxisXPos	
	if ( nAxis > 0 )
		YAxisXPos	= LastDataPos * ( 1 +  (nAxis - 1) * cAXISMARGIN ) 					// here goes the new Y axis (value is referred to bottom axis data values)
		//ModifyGraph	/W=$sWNm axisEnab( bottom ) = { 0, 1- ( nYAxisCnt  >1 ) * .1 }			//  supplies a small margin to the right of the rightmost axis if there is at least  1 right Y axis for the rightmost Y axis numbers    

		 printf "\t\t\t\tDurAcqDrawXPositionOfYAxis(2)\tw:%2d\t\t\tax:%2d/%2d\t%s\tRiAxCnt:%d\t\t\tLastDataPos:%g\tThisAxPos:%4d\tBotAxisBeg:%g\tBotAxEnd:%g \r", w, nAxis,  nYAxisCnt, pd(sAxisNm,5), nRightAxisCnt, LastDataPos, YAxisXPos, BotAxisBeg, BotAxisEnd
		...
		SetAxis	/W=$sWNm 	bottom, BotAxisBeg, BotAxisEnd							// make bottom axis longer if there are Y axis on the right (=multiple traces) to be drawn
	endif
End


static  Function		DurAcqRescaleYAxis( nAxis, sWNm, YOfs, YZoom, Gain )	
// Similar to  DurAcqRescaleYAxisOld()  but rather than storing the axis end values, checking for changes and possible rescaling  here no values are stored or checked but the rescaling is done every time (this is much simpler but could be slower...)
	...
	wave   /T	wAxisName		= root:uf:disp:wYAxisName
	SetAxis /Z /W=$sWNm $wAxisName[ nAxis ],  NegLimit, PosLimit
End
...
static  Function		YAxisCnt( w )
// Retieve the number of  Y axes for each window.  This is often just the number of curves but it could also be less if some traces share the same Yaxis.
	variable	w
	string  	sCurves			= RetrieveCurves( w )						// all curves in 1 window
	variable	nCurve, nCurves	= ItemsInList( sCurves, ksCURVSEP )
	string  	sCurve
...
// 060614  wrong code:!!!
//		if ( numtype( nAxis ) != kNUMTYPE_NAN  )							// allows 1 possibility to define a 'Hidden' Yaxis: missinG (=Nan)  
//			nAxes	= max( nAxis, nAxes )
//			nAxes	= += 1
//			 printf "\t\t\t\t\tYAxisCnt(1) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurve:\t'%s' \r", nAxes+1, w, ItemsInList( sCurves, ksCURVSEP ), sCurve
//		endif
//	endfor
//	 printf "\t\t\t\t\tYAxisCnt(2) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurves:\t'%s...' \r", nAxes+1, w, ItemsInList( sCurves, ksCURVSEP ), sCurves[0,250]
//	return	nAxes + 1	
		if ( numtype( nAxis ) != kNUMTYPE_NAN  &&  nAxis != kNOTFOUND )		// allows 2 possibilities to define a 'Hidden' Yaxis: missinG (=Nan)  or   -1 (=kNOTFOUND)
			nAxes	+= 1
			// printf "\t\t\t\t\tYAxisCnt(1) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurve:\t'%s' \r", nAxes, w, ItemsInList( sCurves, ksCURVSEP ), sCurve
		endif
	endfor
	// printf "\t\t\t\t\tYAxisCnt(2) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurves:\t'%s...' \r", nAxes, w, ItemsInList( sCurves, ksCURVSEP ), sCurves[0,250]
	return	nAxes 	
End

static  Function		YAxisIdx( w, nThisCurve )
// Retieve the index of the  Y axes.  This is often just the index of the curve  but it could also be less if some traces share the same Yaxis.
	variable	w, nThisCurve
	string  	sCurves			= RetrieveCurves( w )								// all curves in 1 window
	variable	nCurve, nCurves	= ItemsInList( sCurves, ksCURVSEP )
	string  	sCurve
	variable	bAxis, nAxis		= -1											// as indexing starts with 0  this leads to the consequence that the first curve MUST have an axis, which will be axis 0  = 'left'
	for ( nCurve = 0; nCurve <= nThisCurve; nCurve += 1 )
		sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )
		bAxis		= str2num( StringFromList( kCV_AXIS, sCurve ) )
		if ( numtype( bAxis ) != kNUMTYPE_NAN  &&  bAxis != kNOTFOUND )		// allows 2 possibilities to define a 'Hidden' Yaxis: missinG (=Nan)  or   -1 (=kNOTFOUND)
			nAxis	+= 1
			// printf "\t\t\t\t\tYAxisIdx(1) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurve:\t'%s' \r", nAxis, w, ItemsInList( sCurves, ksCURVSEP ), sCurve
		endif
	endfor
	 printf "\t\t\t\t\tYAxisIdx(2) : %d \t\t\tw:%2d\tcvs:\t  %2d\tsCurves:\t'%s...' \r", nAxis, w, ItemsInList( sCurves, ksCURVSEP ), sCurves[0,250]
	return	nAxis 	
End
...
//				ExtractCurve1( sCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )		// The remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB' are extracted from 'sCurve/sCurves' . 
				...
				//ExtractCurve( w, sCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// Now all parameters extracted from 'sCurve' are valid as 'sCurve' is extracted from 'sCurves' which always contains currently valid values
...
// Executed whenever the mouse is moved within one of the acquisition windows. Gets mouse position in pixels and computes wave and cursor values in axis coordinates which are displayed in the Panel.
	...
	// printf "\t\tfAcqWndHook event (except 'mousemoved' ):%s \t in wnd '%s'  (wCnt:%d) [KeyModifier:%d]  Cursors: %g / %g\tyPt:%g \r", pd(s.eventName,11), s.winName, WndCnt(), s.eventMod, gCursX, gCursY, s.YPointNumber
	...
	if ( s.eventCode	== kWHK_resize )
		...
		ExtractChMoRaFromCurve( sCurve, sChan, ra, mo )									// here only 'rsChan, ra, mo'  are extracted from 'sCurve' , the remaining parameters in 'sCurve' contain old invalid values (as 'sCurve' has been set only once when the controlbar was constructed)
//		ExtractCurve( w, nCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )// here only 'rsChan, ra, mo'  are extracted from 'sCurve' , the remaining parameters contain old invalid values (as 'sCurve' has been set only once when the controlbar was constructed)
//		sCurves	= RetrieveCurves( w )
...
//		ExtractCurve1( sCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )				// The remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB' are extracted from 'sCurve/sCurves' . 
		nCurve	= ExtractCurves( w, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// Extract fom 'sCurves' the one and only curve 'nCurve' with matching ' sChan, ra, mo' and from this the remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB' 
...
// NEVER change the ordering as this would break all existing DCF files. Appending entries is OK, though.  The entries  'Units'  and  'Instance'  are no longer used and must be left empty.
...
////constant	kCV_CHAN  = 0, cMORA = 1, kCV_ZOOM  = 2, csUNITS = 3, kCV_RGB = 4, kCV_OFS  = 5, cnINSTANCE = 6, kCV_AUTOSCL = 7
...
static Function		ReplaceOneParameter( w, nCurve, nIndex, sVarString )
	...
	StoreCurves( w, sCurves )
End
//static Function	/S	ReplaceOneParam( sCurves, nCurve, nIndex, sVarString )
//	variable	nCurve, nIndex
//	string 	sCurves, sVarString
//	string		sCurve
//	return	sCurves
//End
...
//	ExtractCurve( w, nCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// here only 'rsChan, ra, mo'  are extracted from the passed 'sCurve' , the remaining parameters contain old invalid values (as 'sCurve' has been set only once when the controlbar was constructed)
//	nCurve	= WhichCurve( sCurves, ksCURVSEP, rsChan, mo, ra )
//	sCurve	= StringFromList( nCurve,  sCurves, ksCURVSEP )								//  'sCurves' contains the parameters of all windows and all traces, which are valid at any time as they are updated whenever any parameter is changed.
//	ExtractCurve1( sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )					// The remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB' are extracted from 'sCurve/sCurves' . 
	...
	endswitch
	if ( nCurve == kNOTFOUND )
		InternalError( "UpdateCurves() : could not find sCurve '" + sCurve + "' in '" + RetrieveCurves( w )[0,200] + "...' . " )
	endif	
...
static Function	  	ExtractChMoRaFromCurve( sCurve, rsChan, rnRange, rnMode )
...
//	//ninstn= nItemCnt <= 6   ? 	0   :	str2num( StringFromList( 6, sCurve ) ) 
...
static Function 	  ExtractCurves( w, sChan, nRange, nMode, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )			
...
// There is only be 1 instance of each channel/mode/range  in 'sCurves' so the found curve is unique.
	...
	if ( nCurve != kNOTFOUND )
		string  	sMsg; sprintf sMsg, "ExtractCurves() : Could not find  '%s'  / '%s'  in  '%s' . \r", sChan,  BuildMoRaName( nRange, nMode ), sCurves;	InternalError( sMsg )
		string  	sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )		
...
static Function	  	ExtractCurve( w, nCurve, rsChan, rnRange, rnMode, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB  )	
// Extracts all 9 entries (rsTNm, rnRange, rnMode, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB)  including the trace name, the mode and the range when  1 curve  defined by window 'w' and index 'nCurve'  is given
	...
	rnMode	= Curve2Mode( sMoRa )								// convert 2-letter string into 2 numbers,  e.g. 'SM'->0,1   'RS' ->3,0
	rnRange	= Curve2Range( sMoRa )								// convert 2-letter string into 2 numbers,  e.g. 'SM'->0,1   'RS' ->3,0
	rsChan	= StringFromList( kCV_CHAN , sCurve ) 
...
//	rsUnits	= StringFromList( 3, sCurve ) 			// As Units are fixed they are taken directly from the script / wIO. Only if they could be changed (e.g. like colors) they would have to be stored/extracted  in 'Curves'
	...
	//		But DCF files written with older FPulse versions do not yet have these entries, so we must supply defaults here.
	//  todo: set defaults when there is not DCF file at all
	rYOfs	= nItemCnt <= kCV_OFS    	? 	0 :  str2num( StringFromList( kCV_OFS , sCurve ) )	// 040103
	...
	if ( nItemCnt <= kCV_AXIS )													// 'nAxis' entry in this old DCF is missing..
		rnAxis	= nCurve
		ReplaceOneParameter( w, nCurve, kCV_AXIS, num2str(0) )								// 0 means DISPLAY : make the supplied default setting for this parameter permanent
//		ReplaceOneParameter( w, nCurve, kCV_AXIS, num2str( rnAxis ) )						// make the supplied default setting for this parameter permanent
	...
	// printf "\t\t\t\tExtractCurveDispPars() ->   \tsChan:\t%s\tR:%d \tM:%d \trYZoom:\t%7.2lf\trsRGB:\t%s\tYOs:\t%7.1lf\tInst: %d\tAS:%d \t  \r", pd(sChan,7), rnRange, rnMode, rYZoom, pd(rsRGB,12), rYOfs, rbAutoscl
End
...
static Function	  /S	BuildCurve( sChan, nRange, nMode, bAuto, yOfs, yZoom, nAxis, sRGB )
	...
	string		sCurve = sChan + ";" + BuildMoRaName( nRange, nMode )  + ";" + num2str( yZoom ) + ";;" 					+ sRGB  	// 060613	 ";;"	sUnits is empty
 	sCurve	+= ";" + num2str( yOfs ) + ";;" + num2str( bAuto ) 	+ ";" + num2str( nAxis ) 										// 060613  ";;"	nInstance is empty
...
//	string  	sCurve	= ExtractCurve1( sCurves, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// sChan/mo/ra are known already so we extract only  rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB....
	...
	string  	sTNmMoRa	= sChan + " " + RangeNmLong( ra ) + " " + ModeNm( mo ) 	// e.g. 'Adc1 Frames M'	needs  cXTRACENMSIZE >= 120
	 printf "\tCreateControlBarInAcqWnd3( sWNm:%s %d ) \t%s\t mo:%d ra:%d\t%g\t%g\tax:%2d\t%s \r",  sWNm, bAcqControlbar, pd( sTNmMoRa, 13), mo, ra , rYOfs, rYZoom, rnAxis, rsRGB 
...
static  Function		ConstructCbCheckboxHideYAx( sWNm, bDisable, bHide )
	...
	Checkbox	CbCheckboxHideYAx,  win = $sWNm, size={ cXCHECKBOXSIZE,20 },	proc=fHideYAxis,  title= "Hide Y axis"
...
Function 		fTraceColors( s )
	...
	wave  /T	wAxisName	= root:uf:disp:wYAxisName							
	string		sAxisNm		= wAxisName[ rnAxis ] 									
...
Function		fHideYAxis( s )
// Executed only when the user changes the 'Hide Y Axis' checkbox : If it is turned on, then ....
	struct	WMCheckboxAction	&s
	string  	rsChan, rsRGB													// parameters are set by  UpdateCurves()
	variable	ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis								// parameters are set by  UpdateCurves()
	variable	w			= WndNr( s.Win )
	...
	variable	nCurves		= ItemsInList( sCurves, ksCURVSEP )
	string  	sCurve 		= GetUserData( 	s.win,  "",  "sCurve" )						// Get UD sCurve 
	 printf "\t\tfHideYAxis(1)\tw:%2d\tnCurve:%2d/%2d\t->\t%s\t[%s]\t   \r", w, -1, nCurves, pd(sCurve,43), sCurves[0,200]
	variable	nCurve		= UpdateCurves( w, kCV_AXIS,  num2str( s.checked-1 ),  sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// -1 = kNOTFOUND means HIDE, 0 means DISPLAY
//	variable	nStartCurv		= UpdateCurves( w, kCV_AXIS,  num2str( s.checked ),  sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	
	 	sCurves		= RetrieveCurves( w )		
		nCurves= ItemsInList( sCurves, ksCURVSEP )
	// printf "\t\tfHideYAxis()\twnd:%2d\tmo:%2d\tra:%2d\t%s\t%s\t'%s'\t%d/%2d\t->\t%s   \r", w, mo, ra,  sWndAcDis, pd(sLbMoRaNm,12), sAcDisTrcNm,  nCurve, nCurves, sCurves
	 printf "\t\tfHideYAxis(2)\tw:%2d\tnCurve:%2d/%2d\t->\t%s\t[%s]\t   \r", w, nCurve, nCurves, pd(sCurve,43), sCurves[0,200]

//	ReplaceOneParameter( w, nCurve, kCV_AXIS, num2str(  s.checked -1) )		// -1 = kNOTFOUND means HIDE, 0 means DISPLAY

//	for ( nCurve = nStartCurv; nCurve < nCurves; nCurve += 1 )					
//		ExtractCurve( w, nCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
//		if ( s.checked )
//			ReplaceOneParameter( w, nCurve, kCV_AXIS, num2str( rnAxis + 1 ) )
////			UpdateCurves( w, kCV_AXIS,  num2str( rnAxis + 1 ),  sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	
//		else
//			ReplaceOneParameter( w, nCurve, kCV_AXIS, num2str( rnAxis - 1 ) )
////			UpdateCurves( w, kCV_AXIS,  num2str( rnAxis - 1 ),  sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rnAxis, rsRGB )	
//		endif
//	endfor
	...
	sCurves	= RetrieveCurves( w )						// only for debug printing
	 printf "\t\tfHideYAxis(3)\tw:%2d\tnCurve:%2d/%2d\t->\t%s\t[%s]\t   \r", w, nCurve, nCurves, pd(sCurve,43), sCurves[0,200]
...
//		ExtractCurve( w, nCurve, sChan, ra, mo, bAutoscl, YOfs, YZoom, rnAxis, sRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
//		Range2Pt( sFolder, ra, pr, bl, fr, sw, BegPt, EndPt, Pts, bIsFirst )
//		DurAcqDrawTraceAndAxis( sFolder,  w, sCurves, nCurve, BegPt, Pts, bIsFirst, ra, nSmpInt, ResultXShift, LastDataPos )
//	endfor
//
//
//
