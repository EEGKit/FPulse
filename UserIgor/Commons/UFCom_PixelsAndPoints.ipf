...
static constant		UFCom_kIGOR_POINTS72		= 72	// needed to convert from screen pixels to points
static constant		UFCom_kIGOR_XMIN_WNDLOC	= 2	// needed to convert from screen pixels to points
constant		UFCom_kIGOR_XMISSINGPIXEL	= 5	// needed to convert from screen pixels to points 
static constant		UFCom_kIGOR_YMIN_WNDLOC	= 37	// needed to convert from screen pixels to points (=GetIgorAppPoints( ->YMinPoints ) )
constant		UFCom_kIGOR_YMISSINGPIXEL	= 26	// needed to convert from screen pixels to points 

constant		UFCom_kIGOR_YMIN_PANELLOC	= 43 // place panel just below the menu line (panel coords in pixel!) 
...
// Convert to IGOR points giving the maximum IGOR window possible (coordinates in points) 
	...
	rxMinPoints	= UFCom_kIGOR_XMIN_WNDLOC
	rxMaxPoints	= UFCom_kIGOR_XMIN_WNDLOC +  ( xMaxPixel - UFCom_kIGOR_XMISSINGPIXEL ) * UFCom_kIGOR_POINTS72 / screenresolution
	ryMinPoints	= UFCom_kIGOR_YMIN_WNDLOC
	ryMaxPoints	= UFCom_kIGOR_YMIN_WNDLOC +  ( yMaxPixel - UFCom_kIGOR_YMISSINGPIXEL ) * UFCom_kIGOR_POINTS72 / screenresolution
End


Function		UFCom_GetIgorAppPointsX()		
// Returns the (right - left) Igor main frame points value. Works only in Igor 5 or higher..  Tested and works on 1024x768, 1280x1024, 1600x1200 , maximized and windowed mode
// ++ takes into account moving the left and right window border
	string  	ctrlName	
	GetWindow kwFrameInner , wsize
	// printf "GetIgorAppPointsX  \tkwFrameInner\t\t\twsize/pts\tleft:\t%7d\tright:\t%7d\ttop:\t%7d\tbot:\t%7d\t  ScreenResolution: %d\treturning:%d \r", V_left, V_right, V_top, V_bottom, ScreenResolution, V_right - V_left
	return	V_right - V_left
End

Function		UFCom_GetIgorAppPixelX()		
// Returns the (right - left) Igor main frame pixel value. Works only in Igor 5 or higher.   Tested and works on 1024x768, 1280x1024, 1600x1200 , maximized and windowed mode
// ++ takes into account moving the left and right window border
	string  	ctrlName	
	GetWindow kwFrameInner , wsize
	...
	variable	left	= V_left	* screenresolution / UFCom_kIGOR_POINTS72 	// Convert to pixels	
	variable	right	= V_right	* screenresolution / UFCom_kIGOR_POINTS72 	
	variable	top	= V_top	* screenresolution / UFCom_kIGOR_POINTS72 	// Convert to pixels
	variable	bot	= V_bottom* screenresolution / UFCom_kIGOR_POINTS72 	
	// printf "GetIgorAppPixelX   \t\t\t\t\t\t\twsize/PIXEL\tleft:\t%7d\tright:\t%7d\ttop:\t%7d\tbot:\t%7d\t  \r", left, right, top, bot
	return	right - left
End


Function		UFCom_GetIgorAppPointsY()		
// Returns the (right - left) Igor main frame points value. Works only in Igor 5 or higher..  Tested and works on 1024x768, 1280x1024, 1600x1200 , maximized and windowed mode
// ++ takes into account moving the top and bottom window border
	string  	ctrlName	
	GetWindow kwFrameInner , wsize
	 // printf "GetIgorAppPointsY  \tkwFrameInner\t\t\twsize/pts\tleft:\t%7d\tright:\t%7d\ttop:\t%7d\tbot:\t%7d\t  ScreenResolution: %d\treturning:%d \r", V_left, V_right, V_top, V_bottom, ScreenResolution, V_bottom -  V_top 
	return	 V_bottom -  V_top 
End

Function		UFCom_GetIgorAppPixelY()		
// Returns the bottom Igor main frame pixel value. Works only in Igor 5 or higher..   Tested and works on 1024x768, 1280x1024, 1600x1200 , maximized and windowed mode
// ++ takes into account moving the top and bottom window border
	string  	ctrlName	
	GetWindow kwFrameInner , wsize
...
Function		UFCom_GetAutoWindowCorners( row, nRows, col, nCols, rnLeft, rnTop, rnRight, rnBot, xL100, xR100 )
	...
	dy =  nRows == 2  ? 	dy * .75	:  dy			// 2004-0122	if there are only 1 or 2 rows of windows, the automatic height using (almost) the complete screen gives very narrow windows...
