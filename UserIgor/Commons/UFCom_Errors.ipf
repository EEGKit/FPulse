...
//   !!!elsewhere also
strconstant		UFCom_kERR_lstWARNING_LEVEL	= "fatal; severe; important; less important; message;"	//  Popmenus need semicolon separators
constant			UFCom_kERR_FATAL = -1, UFCom_kERR_SEVERE = 0, UFCom_kERR_IMPORTANT = 1, UFCom_kERR_LESS_IMPORTANT = 2, UFCom_kERR_MESSAGE = 3//,  UFCom_kERR_NONE = 4 
...
// 2009-06-22 removed from EVAL  during great cleanup,  now also remove from ACQ  or move there
Function		UFCom_WarningLevel( sFolder )
	string  	sFolder
	nvar	 /Z	nWarningLevel	= $"root:uf:" + sFolder + ":" + UFCom_PrefPanelName( sFolder ) + ":gnWarnLevl0000"
//	return	nWarningLevel - 2							// popmenu items are 1-based : kSEVERE (=the first item)  has popup-internally the index 1 but in the program it is -1
	return	nvar_exists( nWarningLevel ) * ( nWarningLevel - 2 )	// Very early the global may not yet exist.  Popmenu items are 1-based : kSEVERE (=the first item)  has popup-internally the index 1 but in the program it is -1
End	

// 2009-06-22 removed from EVAL  during great cleanup,  now also remove from ACQ  or move there
Function		UFCom_WarningBeep( sFolder )
	string  	sFolder
	nvar	 /Z	bWarningBeep	= $"root:uf:" + sFolder + ":" + UFCom_PrefPanelName( sFolder ) + ":gbWarnBeep0000"
//	return	bWarningBeep
	return	! nvar_exists( bWarningBeep )   ||   bWarningBeep	// Allow beeping  if very early in the program the global perhaps does not yet exist.  
End
...
// Simple version without  global 'gWarningLevel'
 Function		UFCom_Alert1( nPrgWarnLevel, sMessage )
	...
	string 	sMessage
	if ( nPrgWarnLevel <= UFCom_kERR_IMPORTANT  )
		Beep
	endif
	if ( nPrgWarnLevel == UFCom_kERR_FATAL )				
		DoAlert	0, sMessage
...
// This called from program parts which can be accessed from both  FPuls and  FEVAL  or  from  completely different programs which do not have a user setting for the warning level.
	variable	nPrgWarnLevel
	string 	sMessage
	if ( nPrgWarnLevel <= UFCom_kERR_IMPORTANT  )
		Beep
	...
	//	if ( nPrgWarnLevel <= UFCom_kERR_MESSAGE )				// 2006-0125  allow avoiding error message completely with UFCom_kERR_NONE ( for BiggestContiguousMemory() )
...
// Display warning message and possibly beep. The user may set the warning level separately in the program parts FPuls (sFolder='acq') and FEval (sFolder='eva') . 
// Depends on the users 'Beep'  and  'Warning level' settings
	variable	nPrgWarnLevel
