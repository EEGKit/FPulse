...
Function  /S	UFCom_Factors( nPnts )	
// build list containing all possible factors
	variable	nPnts	
	string 	lstFct		= ""
	string 	lstPrimes	= ""
	lstPrimes	= UFCom_FPPrimeFactors( nPnts )
	lstFct		= ""
	...
	for ( pr = 0; pr < ItemsInList( lstPrimes ); pr += 1 )
		...
		for ( f = 0; f < nFct; f += 1 )
			...
			if ( Fct / Prime  == trunc( Fct / Prime ) )
				if ( WhichListItem( UFCom_num2strDec( Fct / Prime ), lstFct ) == kNOTFOUND )
...
Function	/S	UFCom_FPPrimeFactors( nNumber )
...
//	// printf "\t\t\tBreakIntoPrimes2( n:\t%9d )  \t\t\t\t\tnMaxPrime:\t\t%8d \t-> factors: %s \r", nNumber, nMaxPrime, lstFactors[ strlen( lstFactors ) - 200, strlen( lstFactors ) ]
	...
	SetDataFolder root:uf:misc 	
	PrimeFactors 	/Q  nNumber		
	wave	W_PrimeFactors	= root:uf:misc:W_PrimeFactors
...
//	string 	lstPrimes	= ""
//
//	make  /O  /B  /N=( nMax+1 )	root:uf:misc:bIsPrime = TRUE	// initially assume every number to be a prime
//	wave	bIsPrime 	= root:uf:misc:bIsPrime 
