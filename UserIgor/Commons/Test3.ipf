...
//// !!! also used elsewhere
//static strconstant		lstWARNING_LEVEL	= "fatal; severe; important; less important; message;"	//  Popmenus need semicolon separators
//static constant			UFCom_kERR_FATAL = -1, UFCom_kERR_SEVERE = 0, UFCom_kERR_IMPORTANT = 1, UFCom_kERR_LESS_IMPORTANT = 2, UFCom_kERR_MESSAGE = 3//,  UFCom_kERR_NONE = 4 
...
//	nCEDMemPts	=  100000;		nPnts	=   2 * 3 * 3 * 3 * 5 * 5 * 19;	nSmpInt=  50;	nDA	= 1;	nAD	= 1;	nTG	= 1;	nSlices	=  1630	// +++030902test to provoke TA > 1MB error
	...
	nTrfAreaPts	= SetPoints( sFolder, wG, nCEDMemPts, nPnts, nSmpInt , nDA, nAD, nTG, nSlices, UFCom_kERR_IMPORTANT, UFCom_kERR_IMPORTANT )	// all params are points not bytes	
...
// Primes
	...
	for ( n =  nStart; n < nEnd; n += 2 )
		lstFactors	= UFCom_FPPrimeFactors( n )
	...
	UFCom_ResetStartTimer( "IPrime" )
	for ( n =  nStart; n < nEnd; n += 2 )
		PrimeFactors /Q  n
	endfor
	UFCom_ReadTimer( "IPrime" )
	UFCom_StopTimer( "IPrime" )

	UFCom_ResetStartTimer( "FPrime" )
	for ( n =  nStart; n < nEnd; n += 2 )
		lstFactors	= UFCom_FPPrimeFactors( n )
	endfor
	UFCom_ReadTimer( "FPrime" )
	UFCom_StopTimer( "FPrime" )
	
	UFCom_PrintAllTimers( UFCom_kON )
	UFCom_KillAllTimers()
End
...
// 060515a	constant	kbKEEP_ACQ_DISP = 0,  kbNEW_ACQ_DISP = 1
	...
	while ( rCode == UFCom_kOK )			//Stop at first error.   User may exit prematurely with  'Abort'  
End
...
Function	TestSetvarAndPopupmenu( sTxt, xs, bodyw, xp, dy, n )
	...
	FormatSetvarPopup_319( sTxt, xs, bodyw )		// references are changed
...
Function		buTest18( ctrlName ) : ButtonControl
	...
	DefaultGUIFont all ={  UFCom_ksFONT, UFCom_kFONTSIZE, 0 }		// effects all controls in all panel except those using 'DrawText' 
	Execute "DefaultFont	 /U  	"+ UFCom_ksFONT_			// effects  'DrawText'  (used in some controls)
End
...
Function  		MC700Dlg()
	...
	EnableButton_319( "PnMC700", "buMC700CreateObject",			UFCom_kCo_ENABLE )
	EnableButton_319( "PnMC700", "buMC700ScanAllMultiClamps",		UFCom_kCo_DISABLE )
	EnablePopup_319( "PnMC700", "root_uf_misc_MC700_gpSelectMCC",UFCom_kCo_DISABLE )
	EnableButton_319( "PnMC700", "buMC700SetModeVC",			UFCom_kCo_DISABLE )
	EnableButton_319( "PnMC700", "buMC700SetModeCC",			UFCom_kCo_DISABLE )
	EnableButton_319( "PnMC700", "buMC700Reset",				UFCom_kCo_DISABLE )
	EnableButton_319( "PnMC700", "buMC700DestroyObject",			UFCom_kCo_DISABLE )
End
...
Function		InitPanelMC700( sFolder, sPnOptions )
	...
	UFCom_ConstructAndMakeCurFoldr( "root:uf:misc:MC700" )
...
Function		buMC700CheckAPIVersion( ctrlName ) : ButtonControl
	string 	ctrlName		
	variable	nCode	= xMCCMSG_CheckAPIVersion()		
...
Function		buMC700CreateObject( ctrlName ) : ButtonControl
	string 	ctrlName		
//	ConstructAndMakeCurFoldr( "root:uf:misc:MC700" )
//	string  	/G				root:uf:misc:MC700:glstlstAllMCCIds 	= ""
//	string  	/G				root:uf:misc:MC700:glstPopupMCCs	= "" 	
//	variable	/G				root:uf:misc:MC700:gpSelectMCC
//	variable	/G				root:uf:misc:MC700:hMCCmsg
	nvar		hMCCmsg	    	= 	root:uf:misc:MC700:hMCCmsg
	hMCCmsg	= xMCCMSG_CreateObject()
	printf	 "\t\t%s :  \t\t0x%08x  %d \r", ctrlName, hMCCmsg, hMCCmsg
	if ( hMCCmsg ) 
		EnableButton_319( "PnMC700", "buMC700CreateObject",		UFCom_kCo_DISABLE )
		EnableButton_319( "PnMC700", "buMC700ScanAllMultiClamps",	UFCom_kCo_ENABLE )
		EnableButton_319( "PnMC700", "buMC700DestroyObject",		UFCom_kCo_ENABLE )
	else
...
Function		buMC700ScanAllMultiClamps( ctrlName ) : ButtonControl
	...
	if ( hMCCmsg )
		glstlstAllMCCIds	= xMCCScanMultiClamps( hMCCmsg )
		...
		EnablePopup_319( "PnMC700", "root_uf_misc_MC700_gpSelectMCC", UFCom_kCo_ENABLE )
	else
		glstlstAllMCCIds	= ""
		glstPopupMCCs	= ""
...
Function		gpSelectMCC( sControlNm, popNum, popStr ) : PopupMenuControl
	...
	if ( hMCCmsg )
		...
		variable	nCode	= xMCCMSG_SelectMultiClamp( hMCCmsg, rnModel, rsSerialNumber, rnCOMPort, rnDevice, rnChannel )
		if ( nCode == 0 )
			UFCom_Alert( UFCom_kERR_IMPORTANT, sControlNm + " failed. " )
		else
			EnableButton_319( "PnMC700", "buMC700SetModeVC",		UFCom_kCo_ENABLE )
			EnableButton_319( "PnMC700", "buMC700SetModeCC",		UFCom_kCo_ENABLE )
			EnableButton_319( "PnMC700", "buMC700Reset",			UFCom_kCo_ENABLE )
		endif		
	endif		
End
...
Function		buMC700Reset( ctrlName ) : ButtonControl
	...
	nvar		hMCCmsg		= root:uf:misc:MC700:hMCCmsg
	if ( hMCCmsg )
		variable	nCode		= xMCCMSG_Reset( hMCCmsg )
		if ( nCode == 0 )
...
Function		MC700SelectMultiClamp( nModel, sSerialNumber, nCOMPort, nDevice, nChannel )
	...
	nvar		hMCCmsg		= root:uf:misc:MC700:hMCCmsg
	if ( hMCCmsg )
		variable	nCode	= xMCCMSG_SelectMultiClamp( hMCCmsg, nModel, sSerialNumber, nCOMPort, nDevice, nChannel )
		if ( nCode == 0 )
...
Function		MC700SetMode( nMode ) 
	...
	nvar		hMCCmsg		= root:uf:misc:MC700:hMCCmsg
	if ( hMCCmsg )
		variable	nCode	= xMCCMSG_SetMode( hMCCmsg, nMode )
		if ( nCode == 0 )
...
Function		MC700SetGain( Gain ) 
	...
	nvar		hMCCmsg		= root:uf:misc:MC700:hMCCmsg
	if ( hMCCmsg )
		variable	nCode	= xMCCMSG_SetPrimarySignalGain( hMCCmsg, Gain )
		if ( nCode == 0 )
...
Function		MC700GetGain()
	...
	variable	Gain	= -1
	if ( hMCCmsg )
		Gain	= xMCCMSG_GetPrimarySignalGain( hMCCmsg )
	else
...
Function		MC700GetSecondaryGain()
	...
	variable	Gain	= -1
	if ( hMCCmsg )
		Gain	= xMCCMSG_GetSecondarySignalGain( hMCCmsg )
	else
