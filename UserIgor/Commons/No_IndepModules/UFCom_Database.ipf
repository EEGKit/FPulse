...
//	endif
	...
	//  printf "\t\txp:%4d\txSc:%4d\t >? xmo:%4d\tnCols:%3d\tc1:%d\tc:%3d\t[=%2d]\t%s\tyMouse:%4d\tnRows:%4d \tr1: %3d\tr:%3d\t[=%2d]\tOldRbM:%4d\t%s\tE:%2d\t%s\tMod:%4d\tkey:%4d \t \r", xp, xp * screenresolution / UFCom_kIGOR_POINTS72, s.mouseLoc.h, nCols, nFirstCol, c, nTgtCol, sColOutOfRange, s.MouseLoc.v, nRows, nFirstRow, r, nTgtRow,rowByMouse,sRowOutOfRange, s.eventCode,sEvTxt, s.eventMod, s.keycode
End
...
// Todo:  Umlaute ?   Pass direction or toggle?
	...
	UFCom_SortByColumn_( wt, PrimSortCol, SecSortCol, TertSortCol, nOrder, UFCom_kSORT_ALPHA_NUM_CASE_I )	//  Parameter is actually for 'SortList' , not for 'MakeIndex' .
End
...
// !!!  Cave   If this proves to be too fragile  instead of  'IsSymbolicPath()'  an additional boolean 'bIsSymb' could be passed to 'SaveDataBase()' .
	string   	sPath
	return	( strsearch( sPath, ":", 0 )  == UFCom_kNOTFOUND )  ?  TRUE  :  FALSE
End	
