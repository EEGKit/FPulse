...
// !!! also used elsewhere
// constant	UFCom_kIGOR_MAXSTRING = 253				//  IGOR help (string too long error)  claims this to be 400!
...
Function		PanelDebugPrint()
	...
	InitPanelDebugPrint(  ksROOTUF_ + ksDLG_,  ksDEBUG )				// normally this code is required here but in this case it has already been executed in  'CreateGlobals()'  above
	UFCom_Panel3Sub(   sWin, 	sPnTitle,	sFBase + sFSub,  0, 0, UFCom_kPANEL_DRAW )	// Compute the location of panel controls and the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
End
...
Function		fDebugSelectAll( s )
	...
	UFCom_PnChkboxSetAll(  s.win, "root_uf_dlg_deb_Section0000",   ON )	// Turn ON all = ShowIO, ShowEle, Expand, CFS...
End
...
//  we must turn EVERYTHING OFF (especially ..SelFunc,..SelLoop, ..SelAll) to gain the speed advantage
	...
	UFCom_PnChkboxSetAll(  s.win, "root_uf_dlg_deb_Section0000",   OFF )	// Turn OFF all = ShowIO, ShowEle, Expand, CFS...
End
