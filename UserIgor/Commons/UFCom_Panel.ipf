...
#define   dDEBUG									// Comment this line for a Release version.  During the 'ReleaseProject' process this is automatically done .
...
static constant		kPN_TYPE=0, 		kPN_NXLN=1,		kPN_XPOS=2,		kPN_MXPO=3,		kPN_OVSZ=4,		kPN_TABS=5,		kPN_BLKS=6, 	kPN_MODE=7,	 	kPN_NAME=8
static constant		kPN_ROWTI=9,	kPN_COLTI=10,	kPN_ACTPROC=11, kPN_XBDYSZ=12,kPN_FORMENTRY=13,	kPN_INITVAL=14,	kPN_VISIB=15,	kPN_HELPTOPIC=16

static constant		kSEP = 0,  kCB = 1,  kRAD = 2,  kSV = 3,  kPM = 4,  kBU = 5,  kSTR = 6,  kBUP = 7,  kSTC = 8,  kVD = 9 
static strconstant	lstTYPE ="SEP;CB;RAD;SV;PM;BU;STR;BUP;STC;VD;"

static constant 	kPANELBORDER	= 3	// 3 is minimum for a flawless display of a tabcontrol (which might include a groupbox)  [tested  for Win2000]

constant		UFCom_kRIGHT1 = 0//,  kLEFT = 1,  kBOTTOM = 0,  kTOP = 1
...
Function		UFCom_NewPanel1_( sWin, xPosPts, yPosPts, xSzPix, ySzPix, nKill, sTitle )
// Construct and position a newly created panel.  Positions are in points, sizes are in pixels.
	string  	sWin, sTitle
	variable	xPosPts, yPosPts	// the panel upper-left position in points as it is stored in preferences
	variable	xSzPix, ySzPix		// the panel size in pixel...!!!
	...
	variable	pt2pix		= screenresolution / UFCom_kIGOR_POINTS72
	variable	xL, xR, yT, yB
	xL	= xPosPts * pt2pix
	xR	= xPosPts * pt2pix + xSzPix
	yT	= yPosPts * pt2pix
	yB	= yPosPts * pt2pix + ySzPix
...
// Position a newly created panel precisely with or without an offset (in pixel) on any border of the Igor main window
	...
	yT	= kMAGIC_Y_OFFSET_PIXEL +	yOfs	+ ( 1 - bTop ) * (	UFCom_GetIgorAppPixelY()  - 	UFCom_kMAGIC_Y_MISSING_PIXEL - ySize	)
	yB	= kMAGIC_Y_OFFSET_PIXEL +	yOfs	+ ( 1 - bTop ) * ( UFCom_GetIgorAppPixelY()  -	UFCom_kMAGIC_Y_MISSING_PIXEL - ySize ) + ySize

	NewPanel /W=( xL, yT, xR, yB ) /K=( nKill)  /N=$sWin  as  sTitle			
End	
	
	 
Function		UFCom_ShiftWindow( sWin, xOfs, yOfs )
// Shift  an existing window left/right or top/down.  Keep ist size, this is important for images.
	string  	sWin
	variable	xOfs, yOfs				// offset .  Positive values will move right or down
	// Version1 : Will possibly position parts of the panel off-screen if panel size increases
	 ...
	 MoveWindow /W=$sWin	V_Left+xOfs,  V_top+yOfs, V_Right+xOfs, V_Bottom+yOfs
//	 MoveWindow /W=$sWin	V_Left ,  V_top , V_Left +  xSize * UFCom_kIGOR_POINTS72/screenresolution ,  V_top + ySize * UFCom_kIGOR_POINTS72/screenresolution
End
...
// Position an existing window precisely with or without an offset (in pixel) on any border of the Igor main window.  Derived from  'NewPanel1()'  and therefore mainly applicable but not limited to  panels 
	...
	variable	Pix2pts	= UFCom_kIGOR_POINTS72 / screenresolution
...
//	yBmax	= kMAGIC_Y_OFFSET_PIXEL +	yOfs	+ ( bTop	?	ySize		:	GetIgorAppPixelY()  -	UFPE_kMAGIC_Y_MISSING_PIXEL	)
//	if ( xR > xRmax )
//		xL	= 						xOfs	+ ( bLeft	?	0		:	GetIgorAppPixelX()  -	xSize - 4 )	
//		xR	= 						xOfs	+ ( bLeft	?	xSize		:	GetIgorAppPixelX()  - 		4 )
//	elseif ( yB > yBmax )
//		yT	= kMAGIC_Y_OFFSET_PIXEL +	yOfs	+ ( bTop	?	0		:	GetIgorAppPixelY()  - 	UFPE_kMAGIC_Y_MISSING_PIXEL - ySize	)
//		yB	= kMAGIC_Y_OFFSET_PIXEL +	yOfs	+ ( bTop	?	ySize		:	GetIgorAppPixelY()  -	UFPE_kMAGIC_Y_MISSING_PIXEL	)
//	endif
...
// Builds and updates the main panels (e.g. FPuls, FEval ) . Checks whether panel exists already or not. The Panel window creation macro and the window itself have the same name
...
// permanent, can not and should not be closed, can not and should not be minimised, are initialised and built in 1 step, often need recomputing/resizing/redrawing when e.g. the number of regions or fits changes  
	...
	string		sPnWvNm = UFCom_PanelWvNm( sF, sWin )								// 2005-1115 required for the Help
	...
	if ( WinType( sWin ) != UFCom_WT_PANEL )											// only if the panel does not yet  exist ..
		...
		xLocMax	= UFCom_GetIgorAppPixelX() -  Xsize - 4 - 2 * kPANELBORDER					// the rightmost panel position 
		...
		MoveWindow /W = $sWin  xLoc / pt2pix,   yLoc / pt2pix,    (xLoc + xSize + 2 * kPANELBORDER ) / pt2pix, (yLoc + ySize) / pt2pix 	// now resize the preliminary panel to the correct position and size
		...
		// printf "\t\tPanel3Main  Update\t%s    left:%4d  \t(>%.0lf) \t>%.1lf\t\ttop :%d \t(>%.0lf) \t->y %.1lf \r", sWin, V_left, V_left * screenresolution / UFCom_kIGOR_POINTS72,  V_right * screenresolution / UFCom_kIGOR_POINTS72*100/GetIgorAppPixelX(),  V_top, V_top * screenresolution / UFCom_kIGOR_POINTS72, V_bottom * screenresolution / UFCom_kIGOR_POINTS72*100/GetIgorAppPixelY()
		MoveWindow /W = $sWin V_left, V_top, V_left + ( xSize + 2 * kPANELBORDER ) / pt2pix,  V_top + ySize / pt2pix 
		PnDraw( sF, sWin, $sPnWvNm, xSize, ySize, lstPosX, lstPosY, lstTabGrp, lstTabcoBeg, lstTabcoEnd, llstTypes, llstCNames, lllstTabTi,  llstBlkTi, lllstRowTi, lllstColTi, lllstVisibility, llstMode )	// reconstruct the panel at its previous position	
...
Function		UFCom_Panel3Sub_( sWin, sPnTitle, sF, xPosPoints, yPosPoints, bVisible, nMode )
// For Igor6 . xPosPoints and yPosPoints are in points. Panel may be hidden (not just minimised). 
...
// can be hidden, can not be closed but are hidden instead, are initialised and built in 2 different steps. They remember their position when minimised and when closed.  Up till now they don't need recomputing/resizing/redrawing....
// TODO  :  simplify / clarify  'bVisible'  and  nMode...
	string  	sWin, sPnTitle, sF
	...
	variable	xPosPoints, yPosPoints									// panel position in points
	variable	bVisible
	...
	string 	lllstTabTi	= "",  llstBlkTi	= "", lllstRowTi	= "", lllstColTi	= "", lllstVisibility = "", llstMode	= ""	
	variable	pt2pix	= screenresolution / UFCom_kIGOR_POINTS72
	variable	hidden	= UFCom_TRUE

	// If the panel does not yet  exist  or if  invisible  'Initialising'  is specified..
	if (  WinType( sWin ) != UFCom_WT_PANEL )							// PANEL does not yet exist
		NewPanel /W=( 5000, -20, 5100, 10 ) /K= 2 /N=$sWin  as  sPnTitle		// K=2 prevents closing,  K=1 allows closing.  We build a nearly invisible panel (very small and mostly off-screen) so that 'PnInitVars()' can construct the globals and the controls...
	endif															//...different approach: 1.check in'PnInitVars' existance of controls  2.pass parameter 'bDoBuildControl'
	UFCom_PnInitVars( sF, sWin, $sPnWvNm )								// when increasing the number of blocks( e.g. =regions) or when loading another setting or script (number of chans=gains changes)  then create the additionally required controls (and their underlying variables) 
...
// 2007-0927 kPANELBORDER like in main panel but untested here.......
//	MoveWindow /W = $sWin  xPosPoints, yPosPoints, xPosPoints +   xSize 					/ pt2pix, yPosPoints + ySize / pt2pix 	// now resize the preliminary panel to the correct position and size
	MoveWindow /W = $sWin  xPosPoints, yPosPoints, xPosPoints + ( xSize + 2 * kPANELBORDER ) / pt2pix, yPosPoints + ySize / pt2pix 	// now resize the preliminary panel to the correct position and size
	...
	// printf "\t\t\tUFCom_Panel3Sub_( mode: %d )\t[Init:%d, Draw:%d]  \t Mode was  INIT   : Drawing  non-existant \tPanel  '%s' .\tRetrieving \tHidden:%2d \txLoc:%3d \tyLoc:%3d\t \r", nMode, UFCom_kPANEL_INIT, UFCom_kPANEL_DRAW, sWin , hidden, xLoc, yLoc
	// ControlUpdate /A /W = $sWin									// avoid that popupmenu and setvariable 'hangs' (=stays hidden)  until some next user action
	variable	bDisplayIt = bVisible  ||   ( nMode & UFCom_kPANEL_DRAW )
	DoWindow /F /HIDE=( ! bDisplayIt )  $sWin								//  hide or display the panel and move it to the front 
	
End


Function		UFCom_Panel3WidthInPoints( sWin )
	string  	sWin
	GetWindow     $sWin , 	wsize					// in points
	return	V_Right - V_Left
End

Function		UFCom_Panel3WidthInPercent( sWin )
	string  	sWin
	variable	nPoints	= UFCom_Panel3WidthInPoints( sWin ) + 4 //  +4 points includes panel margin (to avoid slight overlap when positioning the left border of another window at this position)
	GetWindow kwFrameInner , wsize			
	return 	100 * nPoints / ( V_Right - V_Left )
End

Function		UFCom_Panel3PositionIt( xPosPercent, yPosPercent, xSzPts, ySzPts, rxPosPts, ryPosPts )
// Will adjust window position (usually a panel) and will return its top-left-coordinates in points if the window size (in points) and the desired position in percent of screen area are given.
	variable	xPosPercent, yPosPercent, xSzPts, ySzPts
	variable	&rxPosPts, &ryPosPts
	variable	pt2pix		= screenresolution / UFCom_kIGOR_POINTS72
	variable	xPosMaxPts	= ( UFCom_GetIgorAppPixelX() - 4 ) / pt2pix  -  xSzPts 							// the rightmost panel position in points 
	variable	yPosMaxPts	= ( UFCom_GetIgorAppPixelY() - UFCom_kY_MISSING_PIXEL_BOT ) / pt2pix  - ySzPts	// the   lowest  panel position in points 
	rxPosPts 	= min( max( 0, xPosMaxPts * xPosPercent / 100 ), xPosMaxPts )
	ryPosPts 	= UFCom_kY_MISSING_PIXEL_TOP/ pt2pix  + min( max( 0, yPosMaxPts * yPosPercent / 100 ), yPosMaxPts )
End

//=====================================================================================================================================

// 2008-07-15...2009-02-03  new INI implementation
Function 		UFCom_RestorePanel2Sub( sFo, sSubFo, sWin, sPnTitle, sFBase, sFSub, sSubFoIni, sKey, sControlBase, nMode )
	string  	sFo, sSubFo, sWin, sPnTitle, sFBase, sFSub, sSubFoIni, sKey, sControlBase
	variable	nMode
	variable	left, top, right, bot
	variable	bVisible	= UFCom_WndVisibility( sFo, sSubFoIni, sWin, sKey )
	string	  	sButName	= "root_uf_" + sFo + "_" + sSubFo + "_" + sControlBase + "0000" // will not work in Tabs: must adjust to 1000, 0100 ...
	UFCom_TurnButton( sSubFo, sButName, bVisible )							// if the window has been created in the hidden state  we adjust the button state so that the next press creates the window again
	string  	lstWndInfo	= UFCom_Ini_Section( sFo, sSubFoIni, sWin, sKey )
			lstWndInfo	= UFCom_WndPosition_( sFo, lstWndInfo, left, top, right, bot )
	UFCom_Panel3Sub_( sWin, sPnTitle, sFBase+sFSub,  left, top, bVisible, nMode ) 		// Compute the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
End


// 2009-06-11  FLAW: Quitting an application  deletes the panel coordinates in the INI file,  exiting IGOR does NOT

Function		UFCom_WndUpdateLocationHook( s, sFo, sSubFo, sSubFoIni, sKey, sControlBase, sIniBasePath )
// Generic hook function which stores window position, size and visibility.  The window may be any type e.g. panel, notebook or graph.
// And it adjusts the corresponding show/hide button in the main panel according to the windows show/hide/killed state.
	struct	WMWinHookStruct &s
	string  	sFo						// e.g. 'acq'
	string  	sSubFo					// e.g. 'pul'								or  empty string if there is no corresponding button (only required for button)
	string  	sSubFoIni					// e.g. 'FPuls'	  or	'Scrip'
	string  	sKey						// e.g. 'Wnd' 
	string  	sControlBase 				// e.g. 'gbPnMisc'  or  'DataUtils'   or	'gbShowScrpt'   or   empty string if there is no corresponding button (only required for button)
	string  	sIniBasePath				// derived from the application name (pointing to the applications main function, for the one and only app INi file)   or   derived from a script path (special  INI file for each script) 
	string  	lstWndInfo		= ""
	string  	sWNm		= s.winName	// the panel name e.g. 'Mis' , 'Prfa' ,  'uta'   or  the notebook name e.g. 'Scri' 
	variable	bVisible 		= -1
	
 	if ( s.eventCode != UFCom_WHK_mousemoved )
		// printf "\t\tUFCom_WndUpdateLocationHook  %2d \t%s\t%s\t%s\t%s\t%s\r", s.eventCode,  UFCom_pd(s.eventName,9),  sFo, UFCom_pd(sSubFoIni ,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10)	

		string   sButtonName	 = "root_uf_" + sFo + "_" + sSubFo + "_" + sControlBase + "0000" 		// will not work in Tabs: must adjust to 1000, 0100 ...

		switch( s.eventCode )

			case UFCom_WHK_kill:
// 2009-06-11  FLAW: Quitting an application  deletes the panel coordinates in the INI file,  exiting IGOR does NOT.     TEST / REMOVE:
//				UFCom_IniFile_SectionSetWrite( sFo, sSubFoIni, sWNm, sKey, "" , sIniBasePath )		// clear all window information by setting  'lstWndInfo'  to an empty string   AND  change the settings file (last param : 1 means write to file)	
				...
				if ( strlen( sControlBase ) )												// only if there is a corresponding button...
					UFCom_TurnButton( 	sSubFo, sButtonName, bVisible )						// ...the user kas killed the button-controlled window so we adjust the button state so that the next press brings up the window again
				endif
				break

			// VERSION 1 :  When moving the window do not store changed positions (not even in memory).  Just respond by adjusting the button state if the window is minimised....
			case UFCom_WHK_move:													// Also sent on 'Minimise' (being the only message sent!).  The 'move' message was added in Igor Pro 5.04B07.
				GetWindow $sWNm hide												// retrieve 'hidden=minimised' state 
				bVisible 	= ! V_Value												// store 'visible' state.  V_Value can be 0, 1 or 2.    Evaluates  bit 0  (hidden/visible)   and   bit 1  (minimised/normal size)
				if ( strlen( sControlBase ) )												// only if there is a corresponding button...
					UFCom_TurnButton( sSubFo, sButtonName,  bVisible )						// adjust the button state (only required for the case when the user clicked the windows 'Minimise/Resore' buttons, which he should not do...)
				endif
				// printf "\t\tUFCom_WndUpdateLocationHook  %2d \t%s\t%s\t%s\t%s\t%s\twriting: nothing  \r", s.eventCode,  UFCom_pd(s.eventName,9), sFo, UFCom_pd(sSubFoIni ,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10)
				break

			// ...only when the window is left get its position  and its show/hide state,  store state in memory and write to file. 
			case UFCom_WHK_deactivate:				
				GetWindow $sWNm hide												// retrieve 'hidden' state 
				bVisible 	= ! V_Value												// store 'visible' state.  V_Value can be 0, 1 or 2.    Evaluates  bit 0  (hidden/visible)   and   bit 1  (minimised/normal size)
				GetWindow $sWNm wsize												// in points
				lstWndInfo	= UFCom_WndVisibilitySet_( sFo, sSubFoIni, sWNm, sKey,  bVisible )				
				lstWndInfo	= UFCom_WndPositionSet_( sFo, sSubFoIni, sWNm, sKey, V_Left, V_Top, V_Right, V_Bottom )
				lstWndInfo	= UFCom_IniFile_SectionSetWrite( sFo, sSubFoIni, sWNm, sKey, lstWndInfo, sIniBasePath )// now finally write file 
				// printf "\t\tUFCom_WndUpdateLocationHook  %2d \t%s\t%s\t%s\t%s\t%s\twriting: '%s'  \r", s.eventCode,  UFCom_pd(s.eventName,9), sFo, UFCom_pd(sSubFoIni ,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10), lstWndInfo
				break

//			// VERSION 2 :  On every move/resize store position in memory... 
//			case UFCom_WHK_move:														// Also sent on 'Minimise' (being the only message sent!).  The 'move' message was added in Igor Pro 5.04B07.
//			case UFCom_WHK_resize:	
...
//				if ( strlen( sControlBase ) )													// only if there is a corresponding button...
//					UFCom_TurnButton( 	   sSubFo, sButtonName, bVisible )						// if the window has been created in the hidden state  we adjust the button state so that the next press creates the window again
//				endif
...
//				// printf "\t\tUFCom_WndUpdateLocationHook  %2d \t%s\t%s\t%s\t%s\t%s\tV_left:%3d, V_Right:%3d, V_top:%3d, V_bottom:%3d -> bVisible:%2d  %s\r", s.eventCode,  UFCom_pd(s.eventName,9), sFo, UFCom_pd(sSubFoIni,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10)	, V_left, V_Right, V_top, V_bottom, bVisible, sButtonName
//				break
//
...
// 2009-01-30 old   VERSION 2  including  'bHasBeenMoved'
//		switch( s.eventCode )
//			case UFCom_WHK_move:														// This message was added in Igor Pro 5.04B07.
//			case UFCom_WHK_resize:	
//				GetWindow $sWNm hide													// retrieve 'hidden' state and store in V_Value
//				variable	bVisible 	= ! V_Value											// store 'visible' state 
//				string	  	sButName = "root_uf_" + sFo + "_" + sSubFo + "_" + sControlBase + "0000" 	// will not work in Tabs: must adjust to 1000, 0100 ...
//				UFCom_TurnButton( sSubFo, sButName, bVisible )								// if the window has been created in the hidden state  we adjust the button state so that the next press creates the window again
//				DiS_WndVisibilitySet_(  sFo, sSubFoIni, sWNm, sKey, bVisible )						// do not write to disk 
//				GetWindow $sWNm wsize													// in points
//				DiS_WndPositionSet_( sFo, sSubFoIni, sWNm, sKey, V_Left,  V_Top,  V_Right, V_Bottom )	// do not write to disk on every  'move/resize', but only on 'deactivate' below
//				// printf "\t\tUFCom_WndUpdateLocation( %2d \t%s\t%s\t%s\t%s\t%s\tV_left:%3d, V_Right:%3d, V_top:%3d, V_bottom:%3d -> bVisible:%2d  \r", s.eventCode,  UFCom_pd(s.eventName,9), sFo, UFCom_pd(sSubFoIni,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10)	, V_left, V_Right, V_top, V_bottom, bVisible
//
//				UFCom_SafeGVarSet( "root:uf:" + sFo + ":" + 				sWNm  + ":" +  "bHasBeenMoved", 1 )	// create in global in panel variables subfolder (disadvantage of possible mixup?)
//				break
//			case UFCom_WHK_deactivate:				
//				variable	bHasBeenMoved = UFCom_SafeGVar(  "root:uf:" +sFo+ ":"+ sWNm+ ":"+ "bHasBeenMoved", 0 )	// create in global in panel variables subfolder (disadvantage of possible mixup?)
//				if ( bHasBeenMoved )
//					lstWndInfo	  = UFCom_Ini_Section( sFo, sSubFoIni, sWNm, "Wnd" )		
//					lstWndInfo	  = UFPE_IniFile_SectionSetWrite( sFo, sSubFoIni, sWNm, "Wnd", lstWndInfo )	
//					 printf "\t\tUFCom_WndUpdateLocationHook  %2d \t%s\t   '%s'\t'%s'  \r", s.eventCode,  UFCom_pd(s.eventName,9), sWNm, lstWndInfo
//					UFCom_SafeGVarSet( "root:uf:" + sFo + ":" + 			sWNm  + ":" +  "bHasBeenMoved", 0 )	// create in global in panel variables subfolder (disadvantage of possible mixup?)
//				endif
//				break
...
// 2009-02-04 superseded....obsolete
//Function		UFCom_Wnd_UpdateLocationHook( s, sFo, sSubFoIni, sKey, sIniBasePath )
//// Generic hook function which stores window position and size (but not visibility, currently without controlling show/hide button).  Currently only used for FPuls 'Acq Ced' timing table.
//	struct	WMWinHookStruct &s
//	string  	sFo, sSubFoIni, sKey
//	string  	sIniBasePath
//	string  	lstWndInfo	= ""
//	string  	sWNm		= s.winName	
//	
//	if ( s.eventCode != UFCom_WHK_mousemoved )
//		// printf  "\t\tUFCom_Wnd_UpdateLocation( %2d \t%s\t%s\t%s\t%s\t%s\r", s.eventCode,  UFCom_pd(s.eventName,9),  sFo, UFCom_pd(sSubFoIni ,10), UFCom_pd(s.winName,11), UFCom_pd( sKey ,10)	
//		switch( s.eventCode )
//			// Only when the window is left get its position  (ignore its show/hide state here),   store it in memory and write file.  20090202
//			case UFCom_WHK_deactivate:				
//				GetWindow $sWNm wsize									// in points
//				lstWndInfo	= UFCom_WndPositionSet_( 	sFo, sSubFoIni, sWNm, sKey, V_Left, V_Top, V_Right, V_Bottom )	// do not write to disk...
//				lstWndInfo	= UFCom_IniFile_SectionSetWrite( sFo, sSubFoIni, sWNm, sKey, lstWndInfo, sIniBasePath )					// now finally write file  
//				// printf "\t\tUFCom_Wnd_UpdateLocationHook() %2d\t%s\t%s\t%s\t%s\t%s\twriting: '%s' \r", s.eventCode,  UFCom_pd(s.eventName,9), sFo, UFCom_pd(sSubFoIni ,10), UFCom_pd(sWNm,11), UFCom_pd( sKey ,10), lstWndInfo
//				break
//		endswitch
//	endif
//End
				...
				for ( nCol = 0; nCol < ColCnt( lstColTi ); nCol += 1 )
					...
					if ( nType == kSTR )										// only  for  SetVariable  in string mode 
						...
						if ( ! nvar_exists( var ) )									// keep the settings when going to the next file, initialise only if there are no settings yet
							...
							if ( nType == kBU )	
								var 	= InitValue( sVarNmIdx, lstInitVal, 0 )				// 2006-0510 buttons may be switched between states: they now have an initial value  and they may switch titles and colors depending on the state
...
// 2006-1018 constant  conflict  between  FP_Panel (SecuCheck)   and  UFPPanel_319/UFPDialog_319 ( FPuls/FEval319)    ->  either make static  OR   move to    UFCom_Constants.ipf
...
//  
	...
	for ( n = 0; n < nCnt; n += 1 )			// for each control in wPn[], no tab or any other expansions yet
		nType	= WhichListItem( UFCom_RemoveWhiteSpace( StringFromList( kPN_TYPE, wPn[ n ], UFCom_ksSEP_WPN ) ), lstTYPE )
		sType	= StringFromList( nType, lstTYPE )
		sName	=UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )
		lstTabTi	= ReplaceString( "\t", StringFromList( kPN_TABS,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstBlkTi	= ReplaceString( "\t", StringFromList( kPN_BLKS,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstRowTi	= ReplaceString( "\t", StringFromList( kPN_ROWTI,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstColTi	= ReplaceString( "\t", StringFromList( kPN_COLTI,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstInitVal	= ReplaceString( "\t", StringFromList( kPN_INITVAL,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstVisibility	= ReplaceString( "\t", StringFromList( kPN_VISIB,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		...
		// 2005-1204 Note: A FormatEntry-function cannot be converted to a list right here as  the PopupMenu function executes code and requires parameters (win, controlname)  whereas SetVariable, ValDisplay etc only receive settings but do not execute code.
		// printf "\t\tPnInitVars( %3d/%3d\t'%s %s\t%s\t%s\t%s\t%s\t%s\tIV:%s\tVis:%s\t \r",  n, nCnt, sF,  UFCom_pd( sWin,13),   UFCom_pd( sName,19),  UFCom_pd(lstTabTi,18), UFCom_pd(lstBlkTi, 18), UFCom_pd( lstRowTi,18), UFCom_pd( lstColTi,18) , lstInitVal, lstVisibility
...
// e.g.  execute from the command line   Panel2Helptopics( 'root:uf:eva:', 'sde' )   or  Panel2Helptopics( "root:uf:acq:", "sda" )   or    Panel2Helptopics( "root:uf:" , "rec" )
	...
	for ( n = 0; n < nCnt; n += 1 )											// for each control in wPn[], no tab or any other expansions yet
		sName		= UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )
		sHelpTopic	= ReplaceString( "\t", StringFromList( kPN_HELPTOPIC, 	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		sHelpTopic	= UFCom_RemoveTrailingWhiteSpace( UFCom_RemoveLeadingWhiteSpace( sHelpTopic ) )
// 2006-1011 changed behavior: if the help entry is empty then do NOT supply a help topic
...
//		endif
		...
		if ( strlen( sHelpTopic ) )										// 2006-1011 changed behavior: add only non-empty  help topics 
...
constant  UFCom_kSTACKED_BLOCKS = 2//0 	// Only 0 or 2.  Bit flag, if set in lstMode then the blocks are on top of each other (like the Passes in MiniDet).  If not set  the blocks are 1 after the other like regions in FEval
//static constant  bSTACKED_BLOCKS080222 = 1
...
// 2007-0927 the passed back panel 'width' does NOT include the additional margin 2 * kPANELBORDER
	...
	string  	lstBlkHeight	= ""
	string  	lstBlksInTab	= "", lstModeInTab = ""
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		sType	= StringFromList( nType, lstTYPE )
		sName	= UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )
		bNextLn	= str2num( StringFromList( kPN_NXLN,	wPn[ n ], UFCom_ksSEP_WPN ) )
		xPos		= str2num( StringFromList( kPN_XPOS,	wPn[ n ], UFCom_ksSEP_WPN ) )
		nCiL		= str2num( StringFromList( kPN_MXPO,	wPn[ n ], UFCom_ksSEP_WPN ) )
		nOvSz	= str2num( StringFromList( kPN_OVSZ,	wPn[ n ], UFCom_ksSEP_WPN ) )

		// xBody Sz is also used as height for group box e.g. FPuls/Gain.  2008-05-13: function returning number is allowed in addition to a direct number entry 
		xBodySz	= PossiblyConvertFuncToNumber( sName, sF, sWin, ReplaceString( "\t", StringFromList( kPN_XBDYSZ,	wPn[ n ], UFCom_ksSEP_WPN ) , "" ) ) ; xBodySz	= ( numType( xBodySz ) == UFCom_kNUMTYPE_NAN )  ?  0 : xBodySz	// convert a missing entries's  NaN  into  Zero
		...
		lstColTi	= ReplaceString( "\t", StringFromList( kPN_COLTI,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstVisibility	= ReplaceString( "\t", StringFromList( kPN_VISIB,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
		lstMode	= ReplaceString( "\t", StringFromList( kPN_MODE,	wPn[ n ], UFCom_ksSEP_WPN ) , "" )
...
// 2008-02-20  test....introduced for and seems to work in MiniDet   (although NOT (yet?)  implemented in  UFCom_PnInitVars() ???) 
		lstMode	= PossiblyConvertTitleFuncToList( sName, sF, sWin, lstMode )			
		...
		for ( nTab = 0; nTab < nTabs; nTab += 1 )
			...
			BlkMx 		= max( BlkCnt, BlkMx )	// todo???
			 //print "PnSize() nTab/nTabs ", nTab, nTabs, "\t", UFCom_pd( lstBlksInTab, 15), "\t, BlkCnt", BlkCnt, "\t BlkMx", BlkMx
		endfor
	...
	//todo: insert additional block title line if nMode >=2.....	
		...
		// printf "\t\t\tPnSize 0a\t%2d/%2d\t%s\tlstMode: '%s'  \r", n, nCnt, UFCom_pd(sName, 15),  lstMode
	 	sLongTitle	= GetLongestTitle3( lstBlkTi, lstRowTi, lstColTi, lstMode )	//todo  3 blockview modes..........  The title from which the length is computed here must be the same which is finally drawn in 'PnDraw()' ...
		...
		// Debug Y:
		string  	sLT	= UFCom_pd(sLongTitle,12)
		// printf "\t\t\tPnSize 1 Y\t%2d/%2d\t%d  %s\t%s\tnxl:%d\txpo:%d/%d\tovs:%d\ttbgr:%2d\t%s\t[%2d ]\tblgr:%2d\t%s\t%s\tmo:%d\troCt:%.1lf\t bH:%d  bM:%d\tyr:%2d\tSz:%3d\t%s\t%d\t%d\twP:\t%s\r",n,nCnt,nType,sType,UFCom_pd(sName,9),bNextLn,xPos,nCiL,nOvSz,TabGrp,UFCom_pd(lstTabTi,12), nTabs,BlkGrp,UFCom_pd(lstBlkTi,12),UFCom_pd(lstMode,9),nMode,nRowCnt,BlkHt,BlkMx,yr,xBodySz,sLT,rXSz,rXSzMx,wPn[n]  
		// Debug X:
...
//	 printf "\t\t\tPnSize 1b lllstVisibility\t\t%s \r",		lllstVisibility
...
#ifdef dDEBUG
	if ( UFCom_DebugVar( "com", "PanelWid" ) )				// 2007-0401
		 printf "\t\t\tPnSize 1b  Widest control: \t%s \r",	sLongestControl
	endif
#endif
	...
	for ( n = nCnt-1; n > 0;  n -= 1 )
		...
		if (TabGrpN == TabGrpNminus1  &&  BlkGrpN == BlgGrpNminus1 )
			...
			// printf "\t\tPnSize 2a\t n:%2d\t/%3d/\tTg(n):%2d\t == \tTg(n-1):%2d\t&&\tBg(n):%2d\t == \tBg(n-1):%2d \t\tcopying from n =%2d- >%2d \tlstBlkHeight:%s \r", n, nCnt, TabGrpN, TabGrpNminus1, BlkGrpN, BlgGrpNminus1, n, n-1, lstBlkHeight
		else
			// printf "\t\tPnSize 2b\t n:%2d\t/%3d\tTg(n):%2d\t != \tTg(n-1):%2d\t || \tBg(n):%2d\t != \tBg(n-1):%2d NOT\tcopying from n =%2d ->%2d \tlstBlkHeight:%s \r", n, nCnt, TabGrpN, TabGrpNminus1, BlkGrpN, BlgGrpNminus1, n, n-1, lstBlkHeight
		endif
	endfor


	// Pass 3 : Compute  Y position of each new tab group.  Position is stored in list lstTabcoBeg( index:tabgrp )
	variable	bWasTab = UFCom_FALSE, bTrueTab, TabGrpBeg, TabGrpEnd
	string  	sPrevTabGrp = "-1", sPrevBlkGrp = "-1", sTxt
	variable	BlkHeight, BlkMax
	TabHt = 0
	lstTabcoBeg	= ""
	for ( n = 0; n < nCnt; n += 1 )
		sName	= UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )
		...
		lstMode	= StringFromList( n, llstMode,   UFCom_ksSEP_CTRL )					// 2008-02-22
		TabGrp	= str2num( StringFromList( n, lstTabGrp ) )
		BlkGrp	= str2num( StringFromList( n, lstBlkGrp ) )
		BlkHeight	= str2num( StringFromList( n, lstBlkHeight ) ) 
		BlkMax	= str2num( StringFromList( n, lstBlksMax ) )

		bTrueTab	= IsTrueTab( lstTabTi )											// UFCom_FALSE if there is no title at all = all titles are empty strings.  UFCom_TRUE if there is at least 1 tab with a non-empty title. Blanks are non-empty and considered as tabs!
...
//			 printf "\t\tPnSize 3a\tn:%2d/%2d\t%s\t%s\ttg:%2d\t%s\tbg:%2d\tBHt:%2d\t* BMx:%2d = THt:%2d\t+ bTTb:%2d\t%s\t%2d\t-> \t%s\twTb:%d\t%s\t \r", n, nCnt, UFCom_pd( sName,9), UFCom_pd( lstTabTi,12), TabGrp, UFCom_pd( lstBlkTi,12), BlkGrp, BlkHeight, BlkMax, TabHt, bTrueTab, UFCom_pd(sTxt,9), TabGrpBeg, UFCom_pd(lstTabcoBeg,9), bWasTab, UFCom_pd(lstTabcoEnd,9) 
		else
		...
		lstModeInTab	= StringFromList( 0, lstMode, UFCom_ksSEP_TAB )				// 2008-02-22 fragile and perhaps wrong code: as we do not yet know the splitting into tabs and into blocks ...
		nMode		= str2num( StringFromList( 0, lstModeInTab, UFCom_ksSEP_STD ) )	// 2008-02-22 .. we  arbitrarily use tab 0 and blk 0 for determining the mode which decides whether to use STACKED_BLOCKS or not 
//	printf "\t\tbSTACKED_BLOCKS080222 a  & %d\t  n:%2d  \tnMode:%2d\tBlkCnt:%2d\tlstMode:\t%s\tlstModeInTab:\t%s\tllstMode:\t%s\t \r", nMode & UFCom_kSTACKED_BLOCKS, n, nMode, -1, UFCom_pd(lstMode,9), UFCom_pd(lstModeInTab,9), llstMode
if  ( nMode & UFCom_kSTACKED_BLOCKS) 
		TabHt		= str2num( StringFromList( n, lstBlkHeight ) ) //* str2num( StringFromList( n, lstBlksMax ) )
endif
		...
		sTxt	= "   "
		// printf "\t\tPnSize 3 \tn:%2d/%2d\t%s\t%s\ttg:%2d\t%s\tbg:%2d\tBHt:%2d\t* BMx:%2d = THt:%2d\t+ bTTb:%2d\t%s\t%2d\t-> \t%s\twTb:%d\t%s\t \r", n, nCnt, UFCom_pd( sName,9), UFCom_pd( lstTabTi,12), TabGrp, UFCom_pd( lstBlkTi,12), BlkGrp, BlkHeight, BlkMax, TabHt, bTrueTab, UFCom_pd(sTxt,9), TabGrpBeg, UFCom_pd(lstTabcoBeg,9), bWasTab, UFCom_pd(lstTabcoEnd,9) 
		bWasTab		= bTrueTab
	endfor
	TabGrpBeg	+=  TabHt + bTrueTab
	...
	lstTabcoEnd	+= num2str( TabGrpEnd ) + ";"
	// printf "\t\tPnSize 3ex\tn:%2d/%2d\t%s\t%s\ttg:%2d\t%s\tbg:%2d\tBHt:%2d\t* BMx:%2d = THt:%2d\t+ bTTb:%2d\t%s\t%2d\t-> \t%s\twTb:%d\t%s\t \r", n, nCnt, UFCom_pd( sName,9), UFCom_pd( lstTabTi,12), TabGrp, UFCom_pd( lstBlkTi,12), BlkGrp, BlkHeight, BlkMax, TabHt, bTrueTab, UFCom_pd(sTxt,9), TabGrpBeg, UFCom_pd(lstTabcoBeg,9), bWasTab, UFCom_pd(lstTabcoEnd,9) 
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		sType	= StringFromList( nType, lstTYPE )
		sName	= UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )
		bNextLn	= str2num( StringFromList( kPN_NXLN,	wPn[ n ], UFCom_ksSEP_WPN ) )					// needed only for debug printing
		xPos		= str2num( StringFromList( kPN_XPOS,	wPn[ n ], UFCom_ksSEP_WPN ) )
		nCiL		= str2num( StringFromList( kPN_MXPO,	wPn[ n ], UFCom_ksSEP_WPN ) )
		nOvSz	= str2num( StringFromList( kPN_OVSZ,	wPn[ n ], UFCom_ksSEP_WPN ) )

		// xBody Sz is also used as height for group box e.g. FPuls/Gain.  2008-05-13: function returning number is allowed in addition to a direct number entry 
		//xBodySz	= str2num( StringFromList( kPN_XBDYSZ, 	wPn[ n ], UFCom_ksSEP_WPN ) ) ;	xBodySz	= ( numType( xBodySz ) == UFCom_kNUMTYPE_NAN )  ?  0 : xBodySz	// convert a missing entries's  NaN  into  Zero
		xBodySz	= PossiblyConvertFuncToNumber( sName, sF, sWin, ReplaceString( "\t", StringFromList( kPN_XBDYSZ,	wPn[ n ], UFCom_ksSEP_WPN ) , "" ) ) ; xBodySz	= ( numType( xBodySz ) == UFCom_kNUMTYPE_NAN )  ?  0 : xBodySz	// convert a missing entries's  NaN  into  Zero
		...
		for ( nTab = 0; nTab < nTabs; nTab += 1 )
			lstBlksInTab	= StringFromList( nTab, lstBlkTi, UFCom_ksSEP_TAB )
			lstModeInTab	= StringFromList( nTab, lstMode, UFCom_ksSEP_TAB )			// 2008-02-22
			BlkCnt		= ItemsInList( lstBlksInTab, UFCom_ksSEP_STD )
			...
			variable	nBlk_
			for ( nBlk_ = 0; nBlk_ < BlkCnt; nBlk_ += 1 )
				nMode= str2num( StringFromList( nBlk_, lstModeInTab, UFCom_ksSEP_STD ) )	// 2008-02-22
				yt	= str2num( StringFromList( TabGrp, lstTabcoBeg ) ) 
				nBlk	= nBlk_
if ( nMode & UFCom_kSTACKED_BLOCKS) 
//	printf "\t\tbSTACKED_BLOCKS080222 c & %d\tnTab:%2d\tnMode:%2d\tBlkCnt:%2d\tlstMode:\t%s\tlstModeInTab:\t%s\tllstMode:\t%s\t \r", nMode & UFCom_kSTACKED_BLOCKS, nTab, nMode, BlkCnt, UFCom_pd(lstMode,9), UFCom_pd(lstModeInTab,9), llstMode
				nBlk = 0
endif
				yb	= nBlk * BlkHt 
				yPos = yt + yb + yy + yr
				// printf "\t\tPnSize  4d\t%2d/%2d\t%d  %s\t%s\tnxl:%d\txpo:%d/%d\tovs:%d\t%s\ttbgr:%2d\ttab:%d/%d\t%s\tblgr:%2d\tblk:%2d/%2d\tmo:%d\tblkHt:%d\tyt:%2d\t+ yb:%2d\t+ yy:%2d\t+ yr:%.1lf\t= yPo:%.1lf\r", n,nCnt,nType,sType,UFCom_pd( sName,9), bNextLn,xPos,nCiL, nOvSz, UFCom_pd(lstTabTi,15),TabGrp,nTab,nTabs, UFCom_pd(lstBlkTi,15), BlkGrp, nBlk, BlkCnt, nMode, BlkHt, yt, yb, yy, yr, yPos
...
//  Draw the panel and the controls taking into account  Tab and Block grouping which was computed in  'PnSize()' . 
	...
	for ( n = 0; n < nCnt; n += 1 )											// for all controls (=all lines in 'wPn' )
		...
		sName	= UFCom_RemoveWhiteSpace( StringFromList( kPN_NAME, wPn[ n ], UFCom_ksSEP_WPN ) )		// or from llstCNames
		...
		sActProc	= UFCom_RemoveWhiteSpace( StringFromList( kPN_ACTPROC, wPn[ n ], UFCom_ksSEP_WPN ) )
		nCiL		= str2num( StringFromList( kPN_MXPO,	wPn[ n ], UFCom_ksSEP_WPN ) )
		nOvSz	= str2num( StringFromList( kPN_OVSZ,	wPn[ n ], UFCom_ksSEP_WPN ) )
		...
		// 2005-1204 Note: A FormatEntry-function HAS NOT ALREADY BEEN converted to a list ... as  the PopupMenu function executes code and requires parameters (win, controlname)  whereas SetVariable, ValDisplay etc only receive settings but do not execute code.
		sFormEntry= UFCom_RemoveWhiteSpace( StringFromList( kPN_FORMENTRY, wPn[ n ], UFCom_ksSEP_WPN ) )
		...
		nTabs		= TabCnt( lstTabTi )								// This control  (having index 'n') appears in this number of  tabs 
		...
		nSelectedTab = max( 0, nSelectedTab )							// Avoid   nSelectedTab = -1 ( which is introduced and probably should be avoided in RecallAllFolderVars()  ??? )
		nSelectedTab = min(  nSelectedTab, nTabs -1  )						// 2007-0730 Avoid   nSelectedTab being larger than the last tab index which may occur if  changing to a data file having less channels than the previous file.. 
																// Normally it is desired to keep the active tab when  changing data files but in this case we must clip to stay in the existing tab range.
		// printf "\t\t\tPnDraw  \tn:%d\t%s\t%s\tTabco:%2d\t->\tSelected Tab:%2d\tnTabs:%2d\t'%s'\t    \r", n, UFCom_pd(sType,6),  UFCom_pd( sName,9), nTabco, nSelectedTab, nTabs, sF + sWin

		for ( nTab = 0; nTab < nTabs; nTab += 1 )								// For all tabs in 1 tabcontrol
			lstBlksInTab	= StringFromList( nTab, lstBlkTi, UFCom_ksSEP_TAB )
			lstModeInTab	= StringFromList( nTab, lstMode, UFCom_ksSEP_TAB )
			BlkCnt		= ItemsInList( lstBlksInTab, UFCom_ksSEP_STD )
			// printf "\t\tbSTACKED_BLOCKS080222 e & %d\tnTab:%2d\tnMode:%2d\tBlkCnt:%2d\tlstMode:\t%s\tlstModeInTab:\t%s\tllstMode:\t%s\t \r", nMode & UFCom_kSTACKED_BLOCKS, nTab, nMode, BlkCnt, UFCom_pd(lstMode,9), lstModeInTab, llstMode
			for ( nBlk = 0; nBlk < BlkCnt; nBlk += 1 )
				...
				xPos 	= str2num( StringFromList( nBlk, StringFromList( n, lstPosX, ";" ), "," ) ) + kPANELBORDER
				yPos 	= str2num( StringFromList( nBlk, StringFromList( n, lstPosY, ";" ), "," ) )

if ( nMode & UFCom_kSTACKED_BLOCKS) 
				xPos 	= str2num( StringFromList( 0, StringFromList( n, lstPosX, ";" ), "," ) ) + kPANELBORDER
				yPos 	= str2num( StringFromList( 0, StringFromList( n, lstPosY, ";" ), "," ) )
endif

				variable	bTabDisable	= ! ( nSelectedTab == nTab )
				variable	bDisable		=  bTabDisable  ||  ( (nMode & ~UFCom_kSTACKED_BLOCKS) == 0 )
				// printf"\t\tPnDraw()\t%s\ttab:%d\tBlk:%d/%d\tbTabDisable:%d\tnMode:%d\tnMode&2:%d\t -> disable:%d \tn:%d ->\tyPos:%3.1lf\t'%s'  \r", UFCom_pd( sName, 15), nTab, nBlk, BlkCnt, bTabDisable,  nMode, nMode & ~UFCom_kSTACKED_BLOCKS, bDisable, n, yPos, lstPosY 
				...
				for ( nRow = 0; nRow < mxRow; nRow += 1 )
					...
					for ( nCol = 0; nCol < mxCol; nCol += 1 )
						...
						// printf "\t\t\tPnDraw()  \tNm:\t%s\t%s\t%s\tbVisib:%.0lf\tbDis:%d -> %d\t\tTab:%2d\tsTitle:'%s'+'%s'+'%s'  \r", UFCom_pd( sName,18),  UFCom_pd( sCNmIdx,22), sFoCNmIdx, bVisib, bDisable, bDisable || ! bVisib, nTab, sBlkTi, StringFromList( nRow, lstRowTi, UFCom_ksSEP_STD), StringFromList( nCol, lstColTi, UFCom_ksSEP_STD) 
						...
						if ( 	nType == kSEP )
							...
							yOs		= nRow
							PanelSepar3(  	bDisable, bVisib, xPos + xOs,  yPos + yOs, sWin, sTitle, sFoCNmIdx, xPnSz, nTabs, xBodySz )

						elseif ( nType == kBU )
							xOs		= nCol * xPnSz / nCiL + 2	
							yOs		= nRow
							variable width = xPnSz * ( 1 + nOvSz )  / nCiL - 2 
							// 2006-0510 buttons may be switched between states: they now have an initial value  and they may switch titles and colors depending on the state
							...
							PanelButton3(	bDisable, bVisib, xPos + xOs, yPos + yOs, sTitle, sActProc, sF, sWin, sFoCNmIdx, sCNmIdx, width, lstRowTi, sFormEntry, sHelpTopic )	// 2006-0510 'sFormEntry' colorises the button
 ...
 // printf "\t\tPnDraw [tab]\t%s\ttg:%2d/%2d\tBeg:%2d\tEnd:%2d\tTabs:%2d\t%s\t%s\t%s\t  bl:\t%s\t \r", UFCom_pd(sCleanName,18), nTabco, TabGroups, TabBeg, TabEnd, nTabs, UFCom_pd(lstTabTi,9), UFCom_pd(lstTabcoBeg,13), UFCom_pd(lstTabcoEnd,13), UFCom_pd(llstBlkTi,29)
		if ( TabEnd != UFCom_kNOTFOUND )
			TabControl  $sCleanName,	win = $sWin,	proc 	= UFCom_fTabControl3						// First call a standard  TabControl  procedure which enables/disables buttons groups according to the selected tab...
			...
			TabControl  $sCleanName,	win = $sWin,	pos = { kPANELBORDER-4,	 (TabBeg-1) * UFCom_kPANEL_kYLINEHEIGHT }	// 2007-0927   empirically adjusted for Groupbox within tabcontrol  : -4...
			TabControl  $sCleanName,	win = $sWin,	size = { xPnSz+9, (TabEnd-TabBeg+1) * UFCom_kPANEL_kYLINEHEIGHT} 		// ...and +9
...
static Function		IsTrueTab( lstTabTi )	
	string  	lstTabTi 
	return	strlen( ReplaceString( UFCom_ksSEP_TAB, lstTabTi, "" ) ) > 0					// UFCom_FALSE if there is no title at all = all titles are empty strings.  UFCom_TRUE if there is at least 1 tab with a non-empty title. Blanks are non-empty and considered as tabs!
	// return	strlen( UFCom_RemoveWhiteSpace( ReplaceString( UFCom_ksSEP_TAB, lstTabTi, "" ) ) ) > 0// UFCom_FALSE if there is no title at all = all titles are empty or blank strings.  UFCom_TRUE if there is at least 1 tab with a non-empty-non-blank title.
End
...
// Internal error : If the debugger indicates that  'lstTi'  is the <null> string  then  the function  'lstTi() / fttPrc() '  receives the wrong number of parameters  OR  it is returning a number but it must return a string  
	...
	if ( strsearch( lstTi, "()", 0 ) != UFCom_kNOTFOUND )						// If there  is  ()   in  'lstTi'  then we have a function returning the list but not a direct list...
...
Function	/S	UFCom_fTitlesProc( sBsName, sF, sWin )
	string  	sBsName, sF, sWin							// Dummy function prototype for tabcontrol title lists returning tab, block, row and column titles
	 printf "\t\tUFCom_fTitlesProc()  \t%s\t  '%s'  '%s' \r",   UFCom_pd( sBsName, 11), sF, sWin
End								


static Function		PossiblyConvertFuncToNumber( sBsName, sF, sWin, sNumber )			
// Possibly convert a function into a number.   Returns  number no matter whether we had a function returning the number or the number itself. E.g. used for number of rows in FPuls / Gain groupbox
// Internal error : see above PossiblyConvertTitleFuncToList()
	string	  	 sBsName, sF, sWin, sNumber
	if ( strsearch( sNumber, "()", 0 ) != UFCom_kNOTFOUND )							// If there  is  ()   in  'sNumber'  then we have a function returning the list but not a direct list...
		string  	lstTmp	= ReplaceString( "()", sNumber, "" )
		// printf "\t\tPossiblyConvertFuncToNumber \t%s\t'%s'  \t%s\t%s:\tExists as:%2d     . Exists as user defined function:%2d \r",  UFCom_pd( sBsName, 12), sF, sWin, UFCom_pd( sNumber,15),  exists( lstTmp ), exists( lstTmp )==6 
		FUNCREF  UFCom_fNumberProc    fNumPrc = $ReplaceString( "()", sNumber, "" )		//...so remove the ()  and get the list entries from this function.  
		sNumber			=    fNumPrc( sBsName, sF, sWin )								// Indirect entries in a function are useful if the string list is too long or if the entries must be built on demand.
	endif																	// If there is  no  ()   in  'sNumber'  then assume it is a direct number
	return	str2num( sNumber )
End

Function	/S	UFCom_fNumberProc( sBsName, sF, sWin )
	string  	sBsName, sF, sWin							// Dummy function prototype for tabcontrol title lists returning tab, block, row and column titles
	// printf "\t\tUFCom_fNumberProc()  \t%s\t  '%s'  '%s' \r",   UFCom_pd( sBsName, 12), sF, sWin
End								


static Function	/S	GetLongestTitle3( lstBlkTi, lstRowTi, lstColTi, lstMode )
// todo : Evaluate 'lstMode'  which should contain bit flags  
//				nMode = bit&4: extra leading column with block title but  narrower following columns because they don't contain the block title,  
// 				nMode = bit&8: extra leading row for each block with block title, but narrower columns because they don't contain the block title,  
	// Get the longest title.  
	string  	lstBlkTi, lstRowTi, lstColTi, lstMode
	// print  	"GetLongestTitle3()", lstBlkTi, "\t\t", lstRowTi, "\t\t", lstColTi , "\t\t", sSep, "\t\tlstMode: ", lstMode
	...
	// Todo : extract bit flags and react accordingly....
//	if ( nMode & 4  ||  nMode & 8 )
//		printf "\tGetLongestTitle3()  (and elsewhere:  mode =bit&4   and  mode = bit8 unfinished....),  mode=%d \r", nMode
...
static Function		IsLonger( sThisString, sOldString )	
	string  	sThisString, sOldString	
//	return	strlen( sThisString ) > strlen( sOldString )															// Version1: works but is not very precise
	return	FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sThisString ) > FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sOldString )// Version2: FontSizeStringWidth() should probably be more precise, especially when the specific font is passed (which is not yet done)
End

static  Function	TextLenToPixel( sTxt )
	string 	sTxt
	 // print  "TextLenToPixel() FontSizeStringWidth()" , UFCom_ksFONT,  "->", FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sTxt ), sTxt
	//return  		FontSizeStringWidth( UFCom_ksFONT,  UFCom_kFONTSIZE, 0, sTxt ) 	// 2004-03-22  too small for  panels and too small for online analysis with many  WAx columns    
	return  4 + 1.04 * FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sTxt ) 	// 2004-03-22  OK for panels , still too small for online analysis  
End	
...
// Result: Right justified, left justified, and exact input field width. Without this the left margin is undefined as the title is just in front of the input field...
	string  	&sTxt				// the control title
 	...
 	string  	sPad			= sTxt
	variable	nPixel1Blank	= FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, " " ) 
	variable 	nStringPix		= 4 + FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sPad ) 	
	variable 	nBlankPix		= xs - bodyw	-  nStringPix
	if ( nBlankPix < 0  && strlen( sPad ) > 0 ) 
		do 															// if title + input field exceed control size...
			sPad			= RemoveEnding( sPad )							// ...then truncate the title until it fits
		 	nStringPix		= 4 + FontSizeStringWidth( UFCom_ksFONT, UFCom_kFONTSIZE, 0, sPad ) // 4 is magical....	
		 	nBlankPix		= xs - bodyw	-  nStringPix
		while ( nBlankPix < 0  && strlen( sPad ) > 0 )							// if title + input field exceed control size...
	endif
	variable	bw	= min( bodyw, xs )		// possibly clip input field size to control size
	variable 	nBlanks			= nBlankPix / nPixel1Blank
	
	variable 	nActualBlanks	=  trunc( nBlanks )				// better than round() , empirical 051127
	variable 	nActualBlankPix	 = nActualBlanks * nPixel1Blank
	sPad  		= PadString( sPad, strlen( sPad ) + nActualBlanks, 0x20 )
	// printf "\t\tFormatSetvarPopup(\tPx/Blnk:%3.1lf\tBwIn:%4d\t xs:%4d\t%s\t) strPx:%3d\t+ BlnkPx: [%3d\t >%4.1lf\t >%3d\t>%4d\t] + BwOut:%4d\t = %4d\tFSSW:%4d\t [xs:%4d] \t->pad: '%s'\r", nPixel1Blank, bodyw, xs, UFCom_pd(sTxt,18), nStringPix, nBlankPix, nBlanks, nActualBlanks, nActualBlankPix, bw, nStringPix+nBlankPix+bw, FontSizeStringWidth( UFCom_ksFONT,UFCom_kFONTSIZE,0,sPad ) + bw, xs, sPad
	sTxt		= sPad
	bodyw	= bw	
End
...
// A button event occurs not only when pressing a button, but also when just moving the mouse over a button. We must exclude these events from 'normal' button press handling.
	...
	variable	nCode	=  ( s.eventCode == UFCom_CCE_mouseup )		// 2006-0120 advice from LH , Wavemetrics 
	// printf "\tButtonPressed()\t\t\t%s\tEvent:%2d\t%s   \tMod:%2d\t%s\treturns %d %s \r", UFCom_pd(s.ctrlName,28), s.eventCode, UFCom_pd( StringFromList( s.eventCode, UFCom_CCE_lstEVENTCODES ), 8 ),  s.eventMod, UFCom_pd(s.win,8), nCode, SelectString( nCode , "-", "PRESSED" )
	return 	nCode
End		
...
// Called when a button is pressed.  ALSO CALLED when moving the mouse over a button.
	struct	WMButtonAction	&s
	string  	sEventName	= StringFromList( s.eventCode, UFCom_CCE_lstEVENTCODES )
	 if (  s.eventCode != UFCom_CCE_idle  &&  s.eventCode != UFCom_CCE_mousemoved ) 
		// printf "\tfButton_Struct3 a \tEvent:%2d\t%s  \tMod:%2d\t%s\t%s\tsecs:%d \r", s.eventCode, UFCom_pd( sEventName, 8 ),   s.eventMod,  UFCom_pd(s.win,8) , UFCom_pd(s.ctrlName,24), mod( DateTime,10000)
	 endif	
	if (  UFCom_ButtonPressed( s ) ) 												// exclude the event which is triggered when just moving the mouse over the button. 
		...
		if ( ! UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_BUTTON, -1, sHelpTopic ) )	// anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
...
Function		UFCom_fBuProc_struct3( s )								// dummy function  prototype
	struct	WMButtonAction	&s
	// printf "\t\tUFCom_fBuProc_struct3( s ) \r"
End
...
// 2009-07-02 	My Help implementation does not work with SetVariable   because   THIS  action procedure is not jumped into  if  any key modifier  CTRL, SHIFT, ALT is pressed   or the right mouse.
//			This is in contrast to the button, checkbox and popupmenu behavior, where Igor jumps into the action procedure if key modifiers are pressed in conjunction with the left or right mouse button.
//			Another peculiarity (though probably not a bug) is the fact that the event 6 (=Value changed by dependency update)  is executed many times even if we are only moving the mouse in different windows (e.g. Eval window).

// 2009-07-02
//	string  	sProcNm		= GetUserData(	s.win,  s.ctrlName,  "sProcNm" )
//	string  	sHelpTopic	= GetUserData(	s.win,  s.ctrlName,  "sHelpTopic" )
//	string  	sEventName	= StringFromList( s.eventcode, "none;MouseUp;EnterKey;LiveUpd;ScrollWheelUp;ScrollWheelDown;NONE6;NONE7;NONE8;NONE9;NONE10;" )
//	string  	sTxt	= ""
//	 printf "\t\tfSetvar_struct3  .  CntrlNm:\t%s\tVal:\t%g\tsVarName:\t%s\tevCo:%2d\t%s\tevMo:%2d \r", UFCom_pd(s.ctrlname,28), s.dval, UFCom_pd(s.vname,13),  s.eventcode, UFCom_pd( sEventName, 8 ), s.eventmod
//	variable	bUserWantsHelp= UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_SETVARIABLE, -1, sHelpTopic ) // anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
//	if ( ! bUserWantsHelp )	
//		if ( s.eventcode == UFCom_SVE_mouseup	||  s.eventcode == UFCom_SVE_EnterKey )	// 2009-06-02	React only on documented events. Otherwise e.g. killing the control would also trigger the action procedure.
//			sTxt	= "Normal action.  No Help."
//			FUNCREF   UFCom_fSvProc_struct3  fSvPrc = $( sProcNm )			// Execute the action procedure... 
//			fSvPrc( s )													//...if there is one defined, it can also be missing 
//		else
//			sTxt	= "Cntrl being killed?????  No Help."
//		endif
//	else
//			sTxt	= "No action as user wants help."
//	endif


// 2009-10-26 ??????  why is thjis not required ?  It IS required in FEval321  to prevent an endless loop !!
//	if ( s.eventcode != 6   &&  s.eventcode != -1	) 	// Holzhammer  to prevent that the following code is called many times in succession if only the mouse is moved in another window (e.g. Eval window).  6 means  Value changed by dependency update.
	if ( s.eventcode != 6				 	) 	// Holzhammer  to prevent that the following code is called many times in succession if only the mouse is moved in another window (e.g. Eval window).  6 means  Value changed by dependency update.
		string  	sProcNm		  = GetUserData(	s.win,  s.ctrlName,  "sProcNm" )
		string  	sHelpTopic	  = GetUserData(	s.win,  s.ctrlName,  "sHelpTopic" )
		string  	sEventName	  = StringFromList( s.eventcode, "none;MouseUp;EnterKey;LiveUpd;ScrollWheelUp;ScrollWheelDown;ValChange;NONE7;NONE8;NONE9;NONE10;" )
		variable	bUserWantsHelp = UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_SETVARIABLE, -1, sHelpTopic ) // anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
		string  	sTxt			  = ""
		// printf "\t\tfSetvar_struct3    CntrlNm:\t%s\tVal:\t%g\tsVarName:\t%s\tevCo:%2d\t%s\tevMo:%2d\tbWantsHelp:%2d\t%s\r", UFCom_pd(s.ctrlname,28), s.dval, UFCom_pd(s.vname,13),  s.eventcode, UFCom_pd( sEventName, 8 ), s.eventmod, bUserWantsHelp, sTxt
		if ( ! bUserWantsHelp  &&  ( s.eventcode == UFCom_SVE_mouseup  ||  s.eventcode == UFCom_SVE_EnterKey ) )	// 2009-07-02	React only on documented events. Otherwise e.g. killing the control would also trigger the action procedure.
			sTxt	= "No Help. Normal action, trying to execute : " + sProcNm+"()"
			FUNCREF   UFCom_fSvProc_struct3  fSvPrc = $( sProcNm )			// Execute the action procedure... 
			fSvPrc( s )													//...if there is one defined, it can also be missing 
			// printf "\t\tfSetvar_struct3 .  CntrlNm:\t%s\tVal:\t%g\tsVarName:\t%s\tevCo:%2d\t%s\tevMo:%2d\tbWantsHelp:%2d\t%s\r", UFCom_pd(s.ctrlname,28), s.dval, UFCom_pd(s.vname,13),  s.eventcode, UFCom_pd( sEventName, 8 ), s.eventmod, bUserWantsHelp, sTxt
		endif
...
// 2009-07-02	// 2009-10-26	make independent of future event codes
//	if ( ! UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_POPUPMENU, -1, sHelpTopic ) )	// anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
//		if ( s.eventcode != UFCom_kEV_ABOUT_TO_BE_KILLED )				// without this unloading the application (e.g. UnloadSecuTst())  would  trigger all popupmenus (e.g. Customer, Inspector and PIN)
...
//	endif
	...
	variable	bWantsHelp 	= UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_POPUPMENU, -1, sHelpTopic ) // anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
	if ( ! bWantsHelp  &&  s.eventcode == UFCom_PME_mouseup )				// 2009-07-02	React only on documented events. Otherwise e.g. unloading the application (e.g. UnloadSecuTst())  would  trigger all popupmenus (e.g. Customer, Inspector and PIN)
		string  	sFoPmVar	= ReplaceString( "_", s.ctrlName, ":" ) 	  	//  e.g. root_uf_eva_de_gpopupVar000 -> root:uf:eva:evl:gpopupVar000
		nvar	 /Z	nTmp	= $sFoPmVar 							// set the global variable with the popup value to store state
		if ( nvar_Exists( nTmp ) )	
			nTmp	= s.popNum								// the global variable has already been constructed previously
...
// 2009-07-02	// 2009-10-26	make independent of future event codes
//	if ( ! UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_CHECKBOX, s.checked, sHelpTopic ) )// anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
//		if ( s.eventcode != UFCom_kEV_ABOUT_TO_BE_KILLED )				// 2007-0310		without this killing the control (as in PnDraw) would  trigger the action procedure 
...
//	endif
	...
	variable	bWantsHelp 	= UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_CHECKBOX, s.checked, sHelpTopic ) // anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
	...
	if ( ! bWantsHelp  &&  s.eventcode == UFCom_CBE_mouseup )								// 2009-07-02	React only on documented events. Otherwise e.g. this killing the control (as in PnDraw) would  trigger the action procedure 
		SetTheSingleRadioCBGlobal( s.ctrlname, s.checked )
		FUNCREF   UFCom_fCbProc_struct3  fCbPrc = $( sProcNm ) 		// ...execute the action procedure ...
		fCbPrc( s )												// ...if there is one defined, it can also be missing ( sProcNm =  "" ) 
	endif
...
// 'UFPE_DebugPrintOptions()'  does the same thing but uses 1 visible variable for every option, here those multiple variables also exist but they are automatical and have not to be accessed directly
...
// ...and there is only one action procedure  and there is only one help topic ....................................which is connected separately to each button
	...
	string  	sHelpTopic	= GetUserData(	s.win,  s.ctrlName,  "sHelpTopic" )
	string  	sEventName	= ""
	// printf "\t\tUFCom_fRadio_struct3( \t%s\t )\t-> \t-> [ will call if found: %s  ]  HT:'%s' \tecode:%d  ename:'%s' \r", UFCom_pd( s.ctrlname, 31) , UFCom_pd( sProcNm+"()", 23) , sHelpTopic,  s.eventcode, sEventName


// 2009-10-26	make independent of future event codes.	TO IMPROVE  after this change has been checked in  FP_PANEL
//	if ( ! UFCom_PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, UFCom_kCI_CHECKBOX, s.checked, sHelpTopic ) )// anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
//		if ( s.eventcode != UFCom_kEV_ABOUT_TO_BE_KILLED )					// without this killing the panel (which kills the control) would  trigger the action procedure 
//			RadioCheckUncheck3( s ) 										// check / uncheck all the radio buttons of this group
//			FUNCREF  UFCom_fCbProc_struct3  fCbPrc = $sProcNm	
//		endif
		...
		for ( nRow = 0; nRow < RowCnt( lstRowTi ); nRow += 1 )
			for ( nCol = 0; nCol < ColCnt( lstColTi ); nCol += 1 )
				...
				if ( cmpstr( sFoNmIdx, ctrlName ) == 0 )								// check (=turn on) the one clicked button... 
					...
					// printf "\t\tRadioCheckUncheck3\t%s\tBlk:\t%s\tRo:\t%s\tCol:\t%s\tOne global var:\t%s\tIndex of checked:%d \r", UFCom_pd(ctrlName,31),  lstBlkTi, lstRowTi, UFCom_pd(lstColTi,31), UFCom_pd(sFoRadVar,31), nLinIdx 	
				else
					CheckBox $sFoNmIdx, value = 0									// ...reset all other radio buttons of this group
					// printf "\t\tRadioCheckUncheck3\t%s\tBlk:\t%s\tRo:\t%s\tCol:\t%s\tResetting:\t\t%s \r", UFCom_pd(ctrlName,31),  lstBlkTi, lstRowTi, UFCom_pd(lstColTi,31),  UFCom_pd(sFoNmIdx,31) 	
				endif
			endfor
		endfor
	// endfor
...
static Function		PanelSepar3(  bDisable, bVisib, xPos,  yPos, sWin, sTitle, sFCNm, width, nTabs, nLines )
...
// Do NOT use  'DrawText'  as this is not a control and is not handled correcty (is not erased by killcontrol)  e.g. [DrawText /W = $sWin	xPos, yPos * UFCom_kPANEL_kYLINEHEIGHT + 18, sTitle]
// 2007-0927 Draw a groupbox and not only a horizontal dividing line because the  horizontal dividing line is not drawn correctly with XP-style controls
	variable	bDisable, bVisib, xPos,  yPos, width, nTabs, nLines
	string		sWin, sTitle, sFCNm
	// print "PanelSepar3() Groupbox  nLines: ", nLines, "strlen( sTitle): ", strlen( sTitle), sTitle
// 2008-05-13
//	if ( strlen( sTitle ) ) 
//		Groupbox	$sFCNm, win = $sWin,	disable			= bDisable  ||  ! bVisib				
//		Groupbox	$sFCNm, win = $sWin,	pos				= { xPos-2 , 4+yPos * UFCom_kPANEL_kYLINEHEIGHT }	
//
//		// 2007-0927 The groupbox 'width' must be 5 points less when the groupbox is located in a tabcontrol (=nTabs>1) to make room for the tabcontrol border
//		Groupbox	$sFCNm, win = $sWin,	size				= { width + 5 - (nTabs>1)*5 , (nLines+1)*UFCom_kPANEL_kYLINEHEIGHT-3}	// height=13 give a narrow line only on Win2000-style controls
//	
...
//	endif
	...
	if ( strlen( sTitle )  ||  nLines > 0  ) 					// empty no-title half-line separators  are  NOT  groupboxes,  but multiline groupboxes without title are OK 
		Groupbox	$sFCNm, win = $sWin,	disable			= bDisable  ||  ! bVisib				
		if ( nLines > 0 )
			Groupbox	$sFCNm, win = $sWin,	pos				= { xPos-2 , 4+yPos * UFCom_kPANEL_kYLINEHEIGHT }	
			// 2007-0927 The groupbox 'width' must be 5 points less when the groupbox is located in a tabcontrol (=nTabs>1) to make room for the tabcontrol border
			// 2008-05-13  allow groupbox without title (strlen(sTitle)==0)/2) makes it half a line narrower 
			Groupbox	$sFCNm, win = $sWin,	size				= { width + 5 - (nTabs>1)*5 , (nLines+1- (strlen(sTitle)==0)/2 )*UFCom_kPANEL_kYLINEHEIGHT-3}
		else	
			Groupbox	$sFCNm, win = $sWin,	pos				= { xPos-6, 4+yPos * UFCom_kPANEL_kYLINEHEIGHT }	// xPos-6 is magic number: makes Groupbox ALMOST invisible (small vertival line remains). -7 hides title on _Subsequent_ call !?
			Groupbox	$sFCNm, win = $sWin,	size				= {0, 0 }
		endif	
		Groupbox	$sFCNm, win = $sWin,	title				= sTitle 
		Groupbox	$sFCNm, win = $sWin,	frame			= 1  
		Groupbox	$sFCNm, win = $sWin,	userdata( bVisib )	= num2str( bVisib )
...
static Function		PanelButton3( bDisable, bVisib, xPos,  yPos, sTitle, sProc, sFo, sWin, sFCNm, sName, width, lstRowTi, lstColors, sHelpTopic )
	...
	nvar		state			= $sBuVarNm
	// 2006-0518
	if ( strsearch( lstColors, "()", 0 ) != UFCom_kNOTFOUND )
	...
	Button	$sFCNm, win = $sWin,	size				= { width, UFCom_kPANEL_kYHEIGHT + 2 }
...
//	CheckBox	$sFCNm,	win = $sWin,	value		= FolderGetV( sFo, sName, UFCom_kOFF)	// check/uncheck the checkbox
...
// 2009-07-18		Lift checkbox text so that Checkbox text  is at same height as Setvariable text (and only 1 pixel below Button text)  [on XP, classic look)
	CheckBox	$sFCNm, win = $sWin,	pos				= { xPos,  yPos * UFCom_kPANEL_kYLINEHEIGHT + 1 }		// this is OK for XP, normal look, rounded window corners
	//CheckBox	$sFCNm, win = $sWin,pos				= { xPos,  yPos * UFCom_kPANEL_kYLINEHEIGHT }			// this is OK for XP, classic look, square-cut		 2009-07-18
...
static Function		PanelSetVar3( nType, bDisable, bVisib, xPos, xSize, yPos, sTitle, sProc, sFo, sWin, sFCNm, sName, lstTabTi, lstBlkTi, lstRowTi, lstColTi, nBodyWidth, lstFormatLimits, sHelpTopic )
	...
	// printf "\t\t\tPanelSetVar3\t%s\t%s\tCtrlNm:\t%s\t%s\tBw:%3d\t%s\txSize:%4d\t  \r", UFCom_pd(sFo,19),  UFCom_pd(sName,19),  UFCom_pd(sFCNm,27), UFCom_pd(sProc,15), nBodyWidth, lstFormatLimits, xSize
	...
	// 2005-1204 Note: Could process a function like PanelColorField3 (CustomControl)  or  PanelValDisplay3 (ValDisplay)   .   However,  a PopupMenu is processed differently.
...
// 2005-11-18  this is from  SetVariable - Do we need it here for the color field which resembles a SetVariable WITHOUT user interaction  although it is a CustomControl ????
//	SetVariable  $sFCNm,  win = $sWin,	userdata( sFo )		= sFo	
//	SetVariable  $sFCNm,  win = $sWin,	userdata( lstTabTi )	= lstTabTi
//	SetVariable  $sFCNm,  win = $sWin,	userdata( lstBlkTi )	= lstBlkTi
...
// 2005-11-18  this is from  the  old  Color field :  Do we need to rename the userdata names ????
	...
	CustomControl 	$sFCNm,  win = $sWin,	proc			= UFCom_ccColorFieldProc
...
Function		UFCom_ccColorFieldProc( s )
	...
	if (  s.eventCode != UFCom_CCE_idle  &&  s.eventCode != UFCom_CCE_mousemoved ) 
		...
		// printf "\tccColorFieldProc()\t\t\tnVal:%d \tEventCode:%2d    \t%s   \t'%s'\t'%s'\r ", s.nVal,  s.eventCode, UFCom_pd( StringFromList( s.eventCode, UFCom_CCE_lstEVENTCODES ), 8 ), s.win,  s.ctrlName
	endif	
	if (  s.eventCode == UFCom_CCE_draw ) 
		...
		// Execute "SetDrawEnv  	fname = " + UFCom_ksFONT_		//  special syntax and formatting required e.g  UFCom_ksFONT_ =  "\"Arial\""  or   "\"Ms Sans Serif\""
...
static Function		PanelPopup3( bDisable, bVisib, xPos,  xSize, yPos, sTitle, sProc, sFo, sWin, sFCNm, sName, lstTabTi, lstBlkTi, lstRowTi, lstColTi, nBodyWidth, sFormEntry, sHelpTopic )
	...
	// printf "\t\t\tPanelPopup3a \t%s\t%s\tCtrlNm:\t%s\t%s\tBw:%3d\tIV:%2d\txpo:%3d\txsz:%3d\tbwi:%3d\t   \r", UFCom_pd(sFo,19),  UFCom_pd(sName,19),  UFCom_pd(sFCNm,27), UFCom_pd(sProc,15), nBodyWidth, nInitialValue, xPos, xSize, nBodyWidth
	UFCom_FormatSetvarPopup( sTitle, xSize, nBodyWidth )		// references are changed
	// printf "\t\t\tPanelPopup3b \t%s\t%s\tCtrlNm:\t%s\t%s\tBw:%3d\tIV:%2d\txpo:%3d\txsz:%3d\tbwi:%3d\typos:%d\t   \r", UFCom_pd(sFo,19),  UFCom_pd(sName,19),  UFCom_pd(sFCNm,27), UFCom_pd(sProc,15), nBodyWidth, nInitialValue, xPos, xSize, nBodyWidth, yPos
	...
	PopupMenu $sFCNm,	win = $sWin,	pos				= {  max( 0, xPos+1 ), max( 0, yPos * UFCom_kPANEL_kYLINEHEIGHT - 2 ) }	// clip to 0 as the topmost control cannot start at -2 
...
//	Execute "ValDisplay testEqn value=#\""+expr+"\""
	...
	if ( strsearch( lstParams, "()", 0 ) != UFCom_kNOTFOUND )
...
//	endif
	...
	string  	sValue	= sFo + sWin + ":" + sName 
	 ...
	 Execute "ValDisplay " + sFCNm + ", win = " +sWin + ", value =   " + sValue  	
...
Function	/S	fStdColorLst()
// the standard on/off button colors : light grey and dark grey
...
// 2009-04-28  moved to datafoldersandglobals
...
// Returns the number of blocks in  tab  'nTab' . 
	...
	variable	nTab
//	// printf "\t\t\t\tBlkCnt( \t%s\tnTab:%2d ) \t-> \t'%s'\t%s\t'%s' \t%d \t \r",  UFCom_pd(lstBlkTi,14),  nTab, UFCom_ksSEP_TAB, StringFromList( nTab, lstBlkTi, UFCom_ksSEP_TAB ), UFPE_ksTAB_B_SEP, ItemsInList( StringFromList( nTab, lstBlkTi, UFCom_ksSEP_TAB ), UFPE_ksTAB_B_SEP )
...
// 2009-10-26	make independent of future event codes.	TO IMPROVE  after this change has been checked in  FP_PANEL
//	if ( s.eventcode != UFCom_kEV_ABOUT_TO_BE_KILLED )				// 2007-0310		without this killing the control (as in PnDraw) would  trigger the action procedure 
	if ( s.eventcode == 2 )											// todo define ....UFCom_TAB_mouseup
		// print "UFCom_fTabControl3( s )  Tab:", s.tab, "\r"//, s
...
Function		UFCom_TabControl3( sWin, sCtrlName, nTab )
	...
	// printf "\t\tfTabControl3 b\ttgCnt:%2d\tTb:%d\t Ty:\t%s\t%s\tta:\t%s\tbl:\t%s \tro:\t%s\tco:\t%s\t \r",  tgCnt, nTab, UFCom_pd(llstTypes,13), UFCom_pd(llstCNames,26), UFCom_pd(lllstTabTi,19), UFCom_pd(llstBlkTi,21),  UFCom_pd(lllstRowTi,19),  UFCom_pd(lllstColTi,19)
	...
	UFCom_PnTabcoIndexSet( sF + sWin, nTabco, nTab )			// store the currently selected tab as the active tab, BEFORE ShowHideTabControl3()  whose Popupmenus  access the globally stored active tab
...
// test
//  Todo toThink   Update only those few  popupmenu and setvariable controls which belong to the changed tab 'nTab' .   This reduces screen flicker.    080611 seems unnecessary since UFCom_PnTabcoIndexSet is executed BEFORE ShowHideTabControl3.......... 
//	ControlUpdate /A /W = $sWin	// BAD: makes screen flicker  but without this line  SOME! / MANY!  popupmenu and setvariable  are not displayed  when changing tabs
End
	...
	for ( c = 0; c < nControlsInTab; c += 1 )
		...
		for ( nTab = 0; nTab < TabMax; nTab += 1 )				
			...
			for ( nBlk = 0; nBlk < BlkMax; nBlk += 1 )
				...
				variable	bTabDisable	= ! ( nSelectedTab == nTab )
//				variable	bDisable		= bTabDisable  ||  ( nMode == 0 )		// 2008-02-22
				variable	bDisable		= bTabDisable  ||  ( (nMode & ~UFCom_kSTACKED_BLOCKS )== 0 )
				// printf "\t\t\tShowHideTabControl3\t%s\t%s\tTyp:%d\t%s\tTab:%d/%d  SelTb:%d\tDisable:%d\tblk:%d/%d\tnMode:%d\t->dis:%d\t-> %s\t-> %s\tlstMode:%s \r",  UFCom_pd(sTabCtrlNm,15), UFCom_pd(lstTypes,9), nType, UFCom_pd( sCNm,9), nTab, TabMax, nSelectedTab, bTabDisable, nBlk, BlkMax, nMode,  bDisable, UFCom_pd(lstBlkTi1Ctrl,19), UFCom_pd(lstBlkTi,13), lstMode
...
if ( ! bDisableVis ) 
	// print "ShowHideTabControl3", sFoCNmIdx
...
// tab,blk,row,col are coded in the string: ++ Is easy to read and modify, any order and missing entries are allowed.  -- Needs more space, handling takes longer as complete list is scanned, 
	...
	nInitVal	= numType( nCommonInitVal ) == UFCom_kNUMTYPE_NAN ? 	nInitVal : nCommonInitVal	// only if no common value could be extracted from the list (=the value after the ~)  then use the passed value
...
//		string  	sSpecInitIdx_Val = StringFromList( i, lstInitVal )				//  e.g. '0000_3;0001_2.5;1000_-7;~2.3'	-> '0000_3'
		...
		if ( cmpstr( sCNmIdx, sSpecInitIdx ) == 0 )							//  e.g. '0000'  ?  '0000' : matching		-> extract '3' 
			...
			nInitVal	= numType( nSpecInitVal ) == UFCom_kNUMTYPE_NAN ? nInitVal : nSpecInitVal	// no specific value could be extracted from the list so use the passed value
...
Function		UFCom_ReportFolderVars( sFolder, sPanel, sTxt, sSearch )
// Debugging function
	string  	sTxt, sFolder, sPanel, sSearch

	ReportFolderObjects( sFolder, sPanel, sTxt, sSearch,  "variable", UFCom_kIGOR_VARIABLE )
	ReportFolderObjects( sFolder, sPanel, sTxt, sSearch,  "string    ", UFCom_kIGOR_STRING )
	ReportFolderObjects( sFolder, sPanel, sTxt, sSearch,  "wave    ", UFCom_kIGOR_WAVE )
End

Function		ReportFolderObjects( sFolder, sPanel, sTxt, sSearch,  sObject, nType )
// Debugging function
	string  	sTxt, sFolder, sPanel, sSearch
	string  	sObject
	variable	nType 
	string  	sFoCoNm = "", sObjName, sValue
	variable	index, nObjs, bDisable, nDisabled
	// Step 1: Loop through all control variables/strings/waves in this folder and count disabled. This is an extra step so that 'nDisabled' is already known in the next step which in turn saves output lines 
	nDisabled	= 0
	nObjs	= CountObjects( sFolder + sPanel + ":", nType )
	for ( index	= 0; index < nObjs; index += 1 )
		sObjName = GetIndexedObjName( sFolder + sPanel + ":",  nType, index )
		...
		// This works for tabs: hidden tabs are NOT disabled,  but it FAILS for hidden sections whose variables are DISABLED and so can not be restored
		sFoCoNm	 =  ReplaceString( ":", sFolder + sPanel + ":" +  sObjName, "_" )
		...
		bDisable	 =   V_flag  ?  V_Disable  : UFCom_TRUE			// when there is a variable but no corresponding control  the flag for this non-existant control is set to 'disabled'
		nDisabled 	+= bDisable
	endfor
	// Step 2: Loop through all control variables/strings/waves in this folder and search those matching 'sSearch'
	for ( index	= 0; index < nObjs; index += 1 )
		sObjName = GetIndexedObjName( sFolder + sPanel + ":",  nType, index )
		if ( nType == UFCom_kIGOR_VARIABLE )
			nvar	value	= $sFolder + sPanel + ":" +  sObjName
		endif
		if ( nType == UFCom_kIGOR_STRING )
			svar	str		= $sFolder + sPanel + ":" +  sObjName
		endif
		// Build the control name from the variable name and check that the control exists.  This is necessary as (at the moment) there are variables in the folder without control
		// This works for tabs: hidden tabs are NOT disabled,  but it FAILS for hidden sections whose variables are DISABLED and so can not be restored
		sFoCoNm	 =  ReplaceString( ":", sFolder + sPanel + ":" +  sObjName, "_" )
		ControlInfo	    /W=$sPanel	$sFoCoNm					// Check that the control exists, as there are variables in the folder without control e.g. buttons, separators, the single-radio-var  or  false tabcontrol (no tabs)  
		bDisable	 =   V_flag  ?  V_Disable  : UFCom_TRUE			// when there is a variable but no corresponding control  the flag for this non-existant control is set to 'disabled'
		if ( strsearch( sObjName, sSearch, 0, 2 ) != UFCom_kNOTFOUND)// ignore case
			if ( nType == UFCom_kIGOR_VARIABLE )
				sValue	=  "value: " + num2str( value ) 
			endif
			if ( nType == UFCom_kIGOR_STRING )
				sValue	=  "'" + str + "'"	
			endif
			printf "\t\t\tUFCom_ReportFolderVars(a)\t%s\t%s\t%s\t-> \tsearching for   %s\t%s \t%5d\t/%5d \t[Disabled:%4d]\t%s\t%s\tdisable:\t%d\t%s \r",  UFCom_pd(sTxt,24), UFCom_pd(sFolder,10), UFCom_pd(sPanel,7), sObject, UFCom_pad( "*" + sSearch + "*" ,13), index, nObjs, nDisabled, UFCom_pd(sObjName,15), UFCom_pd( sFoCoNm, 29), bDisable, sValue
		endif
	endfor
End


// 2008-02-27 introduced for minidet but of general use
Function	/S	UFCom_SaveAsPanelVars( sFo, sSavePn, sPath, sFileNm, sFileExt ) 
// Allows to save multiple panel setting for any data file.  The user must input directory and folder for the settings=configuration file.
	string  	sFo, sSavePn, sPath, sFileNm, sFileExt

	string  	sDirPath = "", sFileBase
	variable	nRefNum											// Save As dialog: 
	Open /D /T= "." + sFileExt    nRefNum  as  sPath + sFileNm + "." + sFileExt	// Correctly offers directory and file, if the directory exists,  if not any directory may be offered...  
															//... e.g. 'D:UserIgor:MiniDet_'  or  the Igor Pro Folder  and  the file name field stays empty.
	// printf "\tUFCom_SaveAsPanelVars(a) \tOld ConfigPath: '%s'     returns  \t'%s' \r",   sPath, S_fileName 
	if ( strlen( S_fileName ) )										// user did not cancel
		sFileBase		= UFCom_FileNameOnly( S_fileName )
		sDirPath		= UFCom_FilePathOnly( S_fileName )
		UFCom_PossiblyCreatePath( sDirPath )						// should not be required
		// printf "\tUFCom_SaveAsPanelVars(b) \tFo: %s\t\t\t\t\t'%s'   '%s.%s'  \tsNew ConfigPath: '%s'  \r",   UFCom_pd(sFo,19),  sDirPath,  sFileBase, sFileExt, sDirPath
		// Save the specific panel settings file in a user defined directory using  the user-selected ConfigPath
		UFCom_SaveAllFolderVars( sFo, sSavePn, RemoveEnding( sDirPath, ":" ), sFileBase, sFileExt )	
	endif
	return	sDirPath
End
...
//  'SaveRegionsAndCursors()'  and  'SaveAllFolderVars()'  are companion functions
	...
	variable	index, nVars	= CountObjects( sFolder + sPanel + ":", UFCom_kIGOR_VARIABLE )
	variable	Checksum = 0

	// Loop through all control variables in this folder
	for ( index = 0; index < nVars; index += 1 )
		sObjName = GetIndexedObjName( sFolder + sPanel + ":",  UFCom_kIGOR_VARIABLE, index )
		nvar	value	= $sFolder + sPanel + ":" +  sObjName
		...
		ControlInfo		/W=$sPanel	$sFoCoNm					// Check that the control exists, as there are variables in the folder without control e.g. buttons, separators, the single-radio-var  or  false tabcontrol (no tabs)  
		variable	bDisable	=  V_flag  ?  V_Disable  : UFCom_TRUE		// when there is a variable but no corresponding control  the flag for this non-existant control is set to 'disabled'
		...
		// printf "\t\t\tGetVarsInFolder   \t%s\t%s\t-> \tvariable\t%4d\t/%4d\t%s\tvalue:\t%s\tco:\t%s\tdisable:\t%d\t \r",  UFCom_pd(sFolder,19), UFCom_pd(sPanel,9), index, nVars, UFCom_pd(sObjName,19), UFCom_pad(num2str(value),6), UFCom_pd( sFoCoNm, 29), bDisable	
	endfor
	...
	sFilePath	= sPath + ":" + sFileBase + "." + sFileExt
	 // printf "\t\tGetVarsInFolder  \t%s\t%s\t %2d\tvariables (sum %6.2lf, len:%5d)\t%s\t%s..\r", UFCom_pd(sFolder,13), UFCom_pd(sPanel,5), nVars, Checksum, strlen(lstAllVars), UFCom_pd( sFilePath,104), ReplaceString( "\r", lstAllVars[0,80], "   " ) 
	string		sNBName	= "Nb" 		// = 'NbPn'	

	if (  WinType( sNBName ) != UFCom_WT_NOTEBOOK )					// Only if the Notebook window does not  exist.. 
...
// 2008-02-27 introduced for minidet but of general use.     2009-04-08 (FBrain)   requires  UFCom_Utils.xop ????
Function	/S	UFCom_LoadPanelVars( sFo, sSavePn, sPath, sThisControl, sMyFilter ) 
	string  	sFo, sSavePn, sPath, sThisControl, sMyFilter 
	string		sDirPath = "" , sFileNmExt
	variable	nAnyPos	= 12
	// printf "\t\tfUFCom_LoadPanelVars()\tsPath:\t%s\tsMyFilter: '%s'  \r", UFCom_pd( sPath, 26 ), sMyFilter
	if (  UFCom_DirectoryExists( UFCom_FilePathOnly( sPath ) ) )		// ...???...Invokes FileOpen Dialog if first parameter is not a valid file (e.g. only a directory)
		sPath 		= UFCom_FileDialog( sPath, sMyFilter ) 
		sFileNmExt	= UFCom_RemoveFilePath( sPath )
		if ( strlen( sFileNmExt ) )
			sDirPath	= UFCom_FilePathOnly( sPath ) 
			UFCom_ClearFolderVars( sFo + sSavePn + ":" )							// first kill all variables so that they do not exist in 'PnInitVars()' . If they existed the panel could never shrink again. Different approach: Overwrite variables in 'PnInitVars()' without existance-checking (->other problems..)
			UFCom_RecallAllFolderVars( sFo, sSavePn, sDirPath, sFileNmExt, sThisControl )	// ...and update panel with user settings
			UFCom_Panel3Main(   sSavePn, "Any panel", sFo, nAnyPos, nAnyPos )		 // Compute the location of panel controls and the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
			//ReportPanelsVars_m( "after   button Load" )	
		endif
	endif
	return	sDirPath
End
		...
		do 										
			...
			if ( LineLen > 3 )										//  is a valid line, the only invalid line has len=1 and  must be EOF 	
				...
				ControlInfo		/W=$sWin	$sFoCoNm			// Check that the control exists.  This is necessary as (at the moment) there are variables in the folder without control e.g. buttons, separators, the single-radio-var  or  false tabcontrol (no tabs)  
			 	...
			 	// 2007-0130   BAD CODE /  WORKAROUND :  'UFCom_RecallAllFolderVars()'  debug  printing to check the existance of a control, (which is wrongly displayed if it does not yet exist)...
				// printf "\t\t\tUFCom_RecallAllFolderVars \tV_Flag=Control exists:%2d \t%s\t%s\tDisable:%2d\tValue: %g  \r", V_flag, UFCom_pd( sFoCoNm, 31 ), UFCom_pd( sFoVarNm, 31 ), bDisable, value
				
				switch ( V_flag )
					case UFCom_kCI_BUTTON:					// no setting needed (but we must avoid running into 'default:' )
						Button		$sFoCoNm, win = $sWin,  			disable = bDisable
						break
					case UFCom_kCI_CHECKBOX:
						...
						// Bad code:		TODO
						// After the checkbox has been set we must adjust the single underlying radio buttons variable.		// 2005-1109
						...
						// sType = "CB"
						// printf "\t\t\tUFCom_RecallAllFolderVars \t%s\t%s\t %2d (sum %6.2lf)\t'%s'\t  Ty:%2d  %s\t%s\t%s\t%.2g\t[%d]\t  \r", UFCom_pd(sFolder,19), UFCom_pd(sWin,9), index+1, Checksum, sFilePath, V_Flag, sType, UFCom_pd(sFoVarNm,27), UFCom_pd(sFoCoNm,28),  value, bDisable

						break
					case UFCom_kCI_POPUPMENU:
						...
						if ( cmpstr( sCoNm, sThisControl ) )  	
							Popupmenu	$sFoCoNm, win = $sWin, mode =  max( 1, value ), disable = bDisable		// avoid the mode=0 , as this makes the popmenu unchangeable (should/could be prevented earlier)
							 // printf "\t\t\tUFCom_RecallAllFolderVars()\tSetting: \t%s\t%s\t ->\t%d\t->\t%d\tdisable:%2d\t \r", UFCom_pd( sCoNm,15), UFCom_pd( sFoCoNm,29), value, max( 1, value ), bDisable
//ControlUpdate /W=$sWin $sFoCoNm//
						else
							 // printf "\t\t\tUFCom_RecallAllFolderVars()\tIgnoring:\t%s\t%s\t \r", UFCom_pd( sCoNm,15), UFCom_pd( sThisControl,29) 
						endif
						break
					case UFCom_kCI_SETVARIABLE:						
						SetVariable	$sFoCoNm, win = $sWin, 			disable = bDisable	// no setting needed as the control itself displays the current value
						break
					case UFCom_kCI_TABCONTROL:
						Tabcontrol		$sFoCoNm, win = $sWin, value = value, disable = bDisable
						break
					default:
// 2008-02-06 Removed (in MiniDet) as it reset the values of all hidden controls to 0, which is NOT desired. [ Hidden channels/tabs  and hidden panel sections]
//						value	= 0
				...
				endswitch
				sType	=  StringFromList( V_Flag, UFCom_kCI_lstCONTROLTYPES )			// e.g.  tb,  sv,  cb  
				// Enable the next line to check whether a panel control is stored and recalled correctly.
				// printf "\t\t\tUFCom_RecallAllFolderVars \t%s\t%s\t%3d\t(sum %6.2lf)\t'%s'\t  Ty:%2d  %s\t%s\t%s\tt[Di:%d]\t%g  \r", UFCom_pd(sFolder,10), UFCom_pd(sWin,7), index+1, Checksum, sFilePath, V_Flag, sType, UFCom_pd(sFoVarNm,27), UFCom_pd(sFoCoNm,28), bDisable, value
			endif
		while ( LineLen > 0 )     							//...is not yet end of file EOF
		Close nRefNum								// Close the script file... but reopen as a Notebook  below....
		printf "\t\tUFCom_RecallAllFolderVars() \t\t%s\t%s\t %2d (sum %6.2lf)\trecalled from\t'%s'\t  \r", UFCom_pd(sFolder,19), UFCom_pd(sWin,9), index+1, Checksum, sFilePath
	else
		UFCom_Alert(  UFCom_kERR_FATAL,  " UFCom_RecallAllFolderVars()  could not open '" + sFilePath + "' " )	
	endif
...
// ??? 2009-04-29 $$$$ ???
	...
	//printf "\t\t\tUFCom_RecallAllFolderVars bef. clear\tFo:\t%s.\t %2d\tvariables\r", UFCom_pd(sFolder,19),  CountObjects( sFolder, UFCom_kIGOR_VARIABLE )
	KillVariables  /A  /Z
	//printf "\t\t\tUFCom_RecallAllFolderVars after clear\tFo:\t%s. \t %2d\tvariables\r", UFCom_pd(sFolder,19),  CountObjects( sFolder, UFCom_kIGOR_VARIABLE )
...
static Function		SetTheSingleGlobalRadioVariable( sFoVarNm, sRadioNameStartsWith, value )
// see also: SetTheSingleRadioCBGlobal
	string		sFoVarNm, sRadioNameStartsWith
	variable	value
	// 2005-1109  Bad code  as  	 1. is especially arranged for  and  will  (most probably) work only for 1 dimensional horizontal radio buttons 	2. relies on the control name starting with  'ra'   e.g.  'raStartVal'
...
//===========================================================================================================================================
//  ENABLE  OR  DISABLE  A  BUTTON ,  A  CHECK BOX   A  SETVARIABLE  OR  A  POPUPMENU
...
// Set the underlying variable   AND  change the button's appearance accordingly
	string		sWin, sCtrlName
	variable	bState
	if ( WinType( sWin ) == UFCom_WT_PANEL ) 								// avoid error when panel does not yet exist (e.g. PnPuls when loading script at startup)
		...
		// printf "\t\t\tTurnButton  \t%s\t%s\t%d\t ? \t%d\t%d\t%s\t%s\t  \r", UFCom_pd(sWin,11),  UFCom_pd(sCtrlName,27),  WinType( sWin ), UFCom_WT_PANEL, bState, sTitle, sOneColor
	endif
End


Function		UFCom_EnableButtons( sFo, sWin, lstCtrlBase, bState )
// For multiple buttons of 1 panel 'lstCtrlBase'  enable or disable the button
	string		sFo, sWin, lstCtrlBase		// e.g. 'buEvDetect;buAAnalyse;'
	variable	bState
	variable	n, nItems	= ItemsInList( lstCtrlBase )
	string  	sCtrlName
	for ( n = 0; n < nItems; n += 1 )
		sCtrlName	= ReplaceString( ":" , UFCom_ksROOT_UF_ + sFo + ":" + sWin + ":" + StringFromList( n, lstCtrlBase ) + "0000"  , "_" )	// Flaw: ignores  tabbed or columnised buttons '0001', '0010',,, 
		UFCom_EnableButton( sWin, sCtrlName, bState )
	endfor
End
...
Function		UFCom_EnableButton( sPanelNm, sControlNm, EnableDisable )
	...
	// printf "\t\t  panel   enable/DISABLE button  \t%s\t%s\t%d\t ? \t%d\t%d  \r", UFCom_pd(sPanelNm,11),  UFCom_pd(sControlNm,27),  WinType( sPanelNm ), UFCom_WT_PANEL, EnableDisable
	if ( WinType( sPanelNm ) == UFCom_WT_PANEL ) 			// avoid error when panel does not yet exist (e.g. PnPuls when loading script at startup)
		Button	$sControlNm	win = $sPanelNm,	disable = EnableDisable	// UFCom_kCo_ENABLE = 0, UFCom_kCo_DISABLE = 2
	endif
End
...
Function		UFCom_EnableChkbox( sPanelNm, sControlNm, EnableDisable )
	...
	variable	EnableDisable
	// print "\t\tpanel  enable/UFCom_kCo_DISABLE  chkbox",  UFCom_pd(sPanelNm,11),  UFCom_pd(sControlNm,27),  WinType( sPanelNm ),"=?=", UFCom_WT_PANEL, EnableDisable
	if ( WinType( sPanelNm ) == UFCom_WT_PANEL ) 			// avoid error when panel does not yet exist (e.g. PnPuls when loading script at startup)
...
Function		UFCom_EnableSetVar( sPanelNm, sControlNm, EnableDisable )
	string		sPanelNm, sControlNm
	variable	EnableDisable
	 // print "\t\tpanel  enable/UFCom_kCo_DISABLE  SetVariable",  UFCom_pd(sPanelNm,11),  UFCom_pd(sControlNm,27), WinType( sPanelNm ),"?", UFCom_WT_PANEL, EnableDisable
	if ( WinType( sPanelNm ) == UFCom_WT_PANEL ) 			// avoid error when panel does not yet exist (e.g. PnPuls when loading script at startup)
		if ( EnableDisable == UFCom_kCo_NOEDIT_SV )
			SetVariable  $sControlNm	win = $sPanelNm,	disable = 0,			noedit = 1				// pass UFCom_kCo_NOEDIT_SV=3 : display the control but do not let the user change the value
...
Function		UFCom_EnablePopup( sPanelNm, sControlNm, EnableDisable )
	string		sPanelNm, sControlNm
	variable	EnableDisable
	// print "\t\tpanel   enable/UFCom_kCo_DISABLE popup",  UFCom_pd(psPanelNm,11),  UFCom_pd(sControlNm,27), WinType( sPanelNm ),"?", UFCom_WT_PANEL, EnableDisable
	if ( WinType( sPanelNm ) == UFCom_WT_PANEL ) 			// avoid error when panel does not yet exist (e.g. PnPuls when loading script at startup)
		PopupMenu	$sControlNm	win = $sPanelNm,	disable = EnableDisable	// UFCom_kCo_ENABLE = 0, UFCom_kCo_DISABLE = 2
	endif
End
...
//===========================================================================================================================================
// Maintain  a global list of panels so that we can remove all panels  on Cleanup or Exit
//  If 'sSubFo2' is NOTempty then the string list  'lstPanels'  is located in  root:uf:APP:APPVars:   e.g.  root:uf:acq:acqVars:lstPanels  .   This makes it difficult  to remove the DebugPrint panel.
//  If 'sSubFo2' is 	 empty then the string list  'lstPanels'  is located in  root:uf:APP:   		  e.g.  root:uf:acq:lstPanels  .  		 This makes it easy  to remove the DebugPrint panel.

Function		UFCom_LstPanelsSet( sSubFo1, sSubFo2, lst )
	string  	sSubFo1, sSubFo2, lst
	string  /G			$UFCom_ksROOT_UF_ + sSubFo1 + ":"  + sSubFo2 + SelectString( strlen(sSubFo2)==0, ":" , "" ) + "lstPanels"	= lst
	// printf "\tUFCom__LstPanelsSet \t'%s' \r", lst
End	

Function	/S	UFCom_LstPanels( sSubFo1, sSubFo2 ) 
	string  	sSubFo1, sSubFo2
	string  	sFoLstPanels =	UFCom_ksROOT_UF_ + sSubFo1 + ":"  + sSubFo2 + SelectString( strlen(sSubFo2)==0, ":" , "" ) + "lstPanels"
	svar  /Z	lst		     =	$sFoLstPanels
	if ( ! svar_exists( lst ) )																// OK on first call
		string  /G			$sFoLstPanels	= ""
		svar  /Z	lst 	=	$sFoLstPanels
...
Function		UFCom_LstPanelsAdd( sSubFo1, sSubFo2, sPanel )
	string  	sSubFo1, sSubFo2, sPanel
	string  	lstPanels	= UFCom_LstPanels( sSubFo1, sSubFo2 )
	if ( WhichListItem( sPanel, lstPanels ) == UFCom_kNOTFOUND )
		UFCom_LstPanelsSet( sSubFo1, sSubFo2, AddListItem( sPanel, lstPanels ) )				// add this panel to global list so that we can remove in on Cleanup or Exit
		// printf "\t\tUFCom_LstPanelsAdd( %s  %s ) \tAdding  \t%s  \t-> lstPanels: %s \r",  sSubFo1, sSubFo2, sPanel,  UFCom_LstPanels( sSubFo1, sSubFo2 ) 
	else
		// printf "\t\tUFCom_LstPanelsAdd( %s  %s ) \tSkipping \t%s \t-> lstPanels: %s  [was already in list]\r",  sSubFo1, sSubFo2, sPanel,  UFCom_LstPanels( sSubFo1, sSubFo2 ) 
	endif
End

////===========================================================================================================================================
//// Maintain  a global list of panels so that we can remove all panels  on Cleanup or Exit
////  The string list  'lstPanels'  is located in  root:uf:APP:   		e.g.  root:uf:acq:lstPanels  . 		  This makes it easy  to remove the DebugPrint panel.
//
//Function		UFCom_LstPanelsSet_( sSubFo1, lst )
//	string  	sSubFo1, lst
//	UFCom_LstPanelsSet( sSubFo1, "", lst )
//	// printf "\tUFCom__LstPanelsSet_ \t'%s' \r", lst
//End	
//
//Function	/S	UFCom_LstPanels_( sSubFo1 ) 
//	string  	sSubFo1
//	return	UFCom_LstPanels( sSubFo1, "" ) 
//End
//
//Function		UFCom_LstPanelsAdd_( sSubFo1, sPanel )
//	string  	sSubFo1, sPanel
//	UFCom_LstPanelsAdd( sSubFo1, "", sPanel )
//End
