...
// 2006-0826      was 119260 10.05 10.58
...
// 2006-10-30 strconstant	ksDBMARKER_CREATE	= "_create_"
...
// 2006-0920  rounding errors lead to false high columns...
...
// 2009-12-01  because nTgtRow  is often wrong (=old obsolete value)  when a notebook overlaps the table.  This change is static   UF_Recipes.ipf
//r=rowByMouse
...
Function		UFCom_GetTableRows( sTblNm )
	string  	sTblNm	
	string  	sTblInfo	= TableInfo( sTblNm, -2 )
	variable	nRows	= NumberByKey( "ROWS" , sTblInfo )
	return	nRows
End

Function		UFCom_GetTableRowsVisible( sTblNm )
	string  	sTblNm	
	string  	sTblInfo		= TableInfo( sTblNm, -2 )
	variable	nFirstVisibleRow	= str2num( StringFromList( 0, StringByKey( "FIRSTCELL" , sTblInfo ) , "," ) )	
	variable	nLastVisibleRow= str2num( StringFromList( 0, StringByKey( "LASTCELL" , sTblInfo ) , "," ) )	
	variable	nVisibleRows	= nLastVisibleRow - nFirstVisibleRow
	return	nVisibleRows
End
...
// 2006-1130 also accept a symbolic path for 'sDir' .  It is automatically detected whether 'sDir' is a symbolic path or not.
...
Function		UFCom_Make2DimTxtWave( sFoDBName, lstColNames, nInitialRows )
	string  	sFoDBName, lstColNames
	variable	nInitialRows
	string  	sSubFo	= RemoveEnding( UFCom_RemoveLastListItems( 1, sFoDBName, ":" ), ":" ) // truncate DB name  and  truncate colon
	UFCom_PossiblyCreateFolder( sSubFo ) 
	Make /O /T /N=( nInitialRows, ItemsInList( lstColNames) )  $sFoDBName
End
...
Function	/S	UFCom_EntireLine( wDB, row, sSep )
	wave  /T	wDB
	variable	row
	string  	sSep
...
Function		UFCom_CreatePrimaryKey( sDBFoNm, nColPK )
	string 	sDBFoNm	
	variable	nColPk
	wave  /T	wv	= $sDBFoNm
	variable	n, nRows	= DimSize( wv, 0 )
	for ( n = 0; n < nRows; n += 1 )
		wv[ n ][ nColPK ]	= num2str( n )
	endfor
End

Function		UFCom_HighestPrimaryKey( sDBFoNm, nColPK )
// tothink  todo  make  faster with Extract...
// returns highest positive number found in column 'nColPK'  . If  empty fields or fields containing character strings are encounterd (which may be created if the user executes 'InsertPoints' ) user  interaction is required and requested.
	string 	sDBFoNm	
	variable	nColPk
	wave  /T	wv	= $sDBFoNm
	variable	nHighestUsedPrimaryKey	= UFCom_HighestPrimaryKey_( wv, nColPK )
	return	nHighestUsedPrimaryKey
End

Function		UFCom_HighestPrimaryKey_( wv, nColPK )
// tothink  todo  make  faster with Extract...
// returns highest positive number found in column 'nColPK'  . If  empty fields or fields containing character strings are encounterd (which may be created if the user executes 'InsertPoints' ) user  interaction is required and requested.
	wave  /T	wv
	variable	nColPk
	variable	n, nRows	= DimSize( wv, 0 )
	variable	nHighestUsedPrimaryKey	=  -1
	variable	nInvalidPrimaryKeys		=  0
	string  	sPrimKey	

	variable	uSecs = 0, nTimNr	 = -1
	 nTimNr	= UFCom_StartSimpleTimer() 						// _MicroSecTimer_

	// Step 1: Search highest primary key only of valid primary key cell entries.  Ignore all missing or illegal primary key cell entries.
	for ( n = 0; n < nRows; n += 1 )
		sPrimKey	= wv[ n ][ nColPK ]
		if ( numtype( str2num( sPrimKey ) ) != UFCom_kNUMTYPE_NAN )
			nHighestUsedPrimaryKey	= max( nHighestUsedPrimaryKey, str2num( sPrimKey ) )
		else
			nInvalidPrimaryKeys	+= 1
		endif
	endfor
	// Step 2: Now that the highest valid primary key has been found loop through all missing or illegal primary key cell entries and let the user decide whether to complete and keep  or discard the entry.
	if ( nInvalidPrimaryKeys )
		for ( n = 0; n < nRows; n += 1 )
			sPrimKey	= wv[ n ][ nColPK ]
			if ( numtype( str2num( sPrimKey ) ) == UFCom_kNUMTYPE_NAN )
				string  	sTxt
				string		sDataset	= (wv[ n ][ 1 ] )[0,10] + "   " + (wv[ n ][ 2 ] )[0,10] + "   "  + (wv[ n ][ 3 ] )[0,10] + "   "  + (wv[ n ][ 4 ] )[0,10] + "   "  + (wv[ n ][ 5 ] )[0,10] + "   "  + ( wv[ n ][ 6 ] )[0,10] + "   "  + ( wv[ n ][ 7 ] )[0,10]
				sprintf sTxt, "The data set in  row %d [rows=%d] ,  column %d   in data base   contains an illegal or missing primary key '%s' ", n , nRows, nColPK, sPrimKey
				printf "\t\tWarning:  %s\r", sTxt + "   '" + sDataset[0,120] + "'"
				DoAlert 1, sTxt + "\r'" + sDataset[0, 100] +  "'\rYes: Keep data set  and create primary key.    No: Delete data set"
				if ( V_Flag == 1 )							// Keep.    Yes=1,  No=2
					nHighestUsedPrimaryKey += 1
				  	wv[ n ][ nColPK ]	= num2str( nHighestUsedPrimaryKey )
				endif
				if ( V_Flag == 2 )							// Discard.   Yes=1,  No=2
					DeletePoints n , 1 , wv 
					n		-= 1
					nRows	-= 1
				endif
	...
	uSecs	= UFCom_StopSimpleTimer( nTimNr ) 				// _MicroSecTimer_
	 printf "\t\t\tUFCom_HighestPrimaryKey( %d ) returns %.0lf .  Data base had %d invalid or missing primary keys which have been fixed.  \t[ Timer%2d\tms:%4.0lf\tus/item:%.1lf ] \r",  nColPK, nHighestUsedPrimaryKey, nInvalidPrimaryKeys, nTimNr, uSecs/1000, uSecs/nRows 
	return	nHighestUsedPrimaryKey
End

Function		UFCom_PrimaryKey2Row( sDBFoNm, nColPK, nPK )
// tothink  todo  make  faster with Extract...
	string 	sDBFoNm	
	variable	nColPk, nPK
	...
	variable	n, nRows	= DimSize( wv, 0 )
	for ( n = 0; n < nRows; n += 1 )
		if ( str2num( wv[ n ][ nColPK ] ) == nPK )
			uSecs	= UFCom_StopSimpleTimer( nTimNr ) 		// _MicroSecTimer_
			// printf "\t\tUFCom_PrimaryKey2Row(a %s , nPKCol:%d ,  nPrimaryKey:\t%8d\t )  returns %d \t[ Timer%2d\tms:%4.0lf\tus/item:%.1lf ] \r",  sDBFoNm, nColPK, nPK, n , nTimNr, uSecs/1000, uSecs/n 
			return	n
		endif
	endfor
	uSecs	= UFCom_StopSimpleTimer( nTimNr ) 				// _MicroSecTimer_
	 printf "\t\tUFCom_PrimaryKey2Row(b %s , nPKCol:%d ,  nPrimaryKey:\t%8d\t )  returns %d  (not found)  \t[ Time:%2d\tms:%4.0lf\tus/item:%.1lf ] \r",  sDBFoNm, nColPK, nPK, -1, nTimNr, uSecs/1000, uSecs/nRows 
	return	UFCom_kNOTFOUND
End


Function		UFCom_CopyRowsIncrPrimaryKey( sTbl, sDBFoNm, nPKCol, row )
// Copies the selected/marked row(s) and assigns the next higher valid primary key(s)
	string  	sTbl			
	string 	sDBFoNm		
	variable	row, nPKCol
	wave  /T	wv		= $sDBFoNm
	GetSelection table, $sTbl, 7							// 1 sets V_startRow, V_startCol, V_endRow, V_endCol  based on the selected cells 
	// printf "\t\tUFCom_CopyRowsIncrPrimaryKey()    Clicked row:%d . Getselection returns rows %d ... %d.  (cols: %d ... %d ).   V_Flag:%d    \r",  row,  V_startRow , V_endRow ,  V_startCol , V_endCol , V_Flag
	if ( row == V_startRow )								// unfortunately  GetSelection  always sets V_Flag =1 even if nothing is selected ???
		if ( V_startCol == 0  &&  V_endCol == DimSize( wv, 1 ) -1 )	// an entire row has been selected
			variable	n, nNumRows		= V_endRow - V_startRow + 1
			variable	nHighestPrimaryKey	= UFCom_HighestPrimaryKey( sDBFoNm, nPKCol )
			InsertPoints  /M=0  V_endRow + 1, nNumRows, wv 
			for ( n = V_startRow; n <= V_endRow; n += 1 ) 	
				wv[ n + nNumRows ][] 			= wv[ n ][ q ]  
				wv[ n + nNumRows ][ nPKCol ]	= num2str( nHighestPrimaryKey + n - V_startRow + 1 )
			endfor
		else
			Beep
			 printf "\tWarning: UFCom_CopyRowsIncrPrimaryKey().  Nothing is copied as only a partial row has been selected. \r"
		endif
	else
			Beep; Beep
		 printf "\tWarning: UFCom_CopyRowsIncrPrimaryKey()   Nothing is copied as no  row has been selected. \r"
	endif
End
...
// 2006-1214  UNUSED  if this is reactivated it should be similar to the code  'UFCom_RemoveSimilarDtaSetsAsk() ' below  ( and should include it  by passing an empty dummy wrapper function for 'fBuildDataSetString()' ) !
...
Function	/S	UFCom_FormatLine( wDB, row, sSep )
	wave  /T	wDB
	variable	row
	string  	sSep
...
//Function	/S	UFCom_FindIdenticalCells( wDB, nColumn, bDoDisplay )
//// returns list of rows containing identical entries in 'nColumn' .  This a simplified version of  UFCom_FindIdenticalDatasets()  below.
//// todo TODO   EXTREMELY SLOW
//	wave   /T	wDB	
//	variable	nColumn
//	variable	bDoDisplay
//	variable	r, nRows		= UFCom_NumEntries( wDB )
...
//		sExistingData	= wDB[ r ][ nColumn ]
//		for ( nDoub = r + 1; nDoub < nRows; nDoub += 1 )			// loop only through all following rows
//			sFurtherData	= wDB[ nDoub ][ nColumn ]
////			if ( UFCom_DataSetExists( sExistingData, sFurtherData ) )
//			if ( cmpstr( sExistingData, sFurtherData ) == 0 )
//				// printf "\t\tUFCom_FindIdenticalDatasets(1) \tDBRows:%3d\tRow\t%4d\thas a copy in row\t%4d\t'%s'...   \r", nRows, r, nDoub, sExistingData[0,200]
//				sDoublette	 	= num2str( nDoub )
//				lstDoublettes	= UFCom_PossiblyAddListItem( sDoublette, lstDoublettes )	// add only once 
//				if ( bDoDisplay )
//			 		printf "\t\tUFCom_FindIdenticalCells(1) \tRow\t%4d\t/ %4d\t\thas copy in \trow\t%3d\t'%s'\t ->\tDoublettes:%d    '%s'   \r", r, nRows, nDoub, sExistingData[0,200], ItemsInList( lstDoublettes ), lstDoublettes[0,300]
//				endif
//			endif
//		endfor
//	endfor
//	printf "\t\tUFCom_FindIdenticalCells(2) \tRow\t%4d\t/ %4d\t\thas copy in \trow\t%3d\t'%s'\t ->\tDoublettes:%d    '%s'   \r", r, nRows, nDoub, sExistingData[0,200], ItemsInList( lstDoublettes ), lstDoublettes[0,300]
//	return	lstDoublettes
//End


// NOT  WORKING RELIABLY  (when  in hook  Menu/Edit/Paste ??? ??? 
Function	/S	UFCom_FindIdenticalCells( wDB, nColumn, bDoDisplay )
// returns list of rows containing identical entries in 'nColumn' . 
// TODO: slow.  Not really fast but acceptable for 3000 data sets....todo TODO SLOW
	wave   /T	wDB	
	variable	nColumn
	variable	bDoDisplay
	variable	r, nRows		= UFCom_NumEntries( wDB )
	string  	sDoublette, lstDoublettes	= "", lstAll = ""
	variable	nDoub
	...
	for ( r = 0; r < nRows; r += 1 )											// loop through ALL rows
		if ( WhichListItem( wDB[ r ][ nColumn ], lstAll ) == UFCom_kNOTFOUND )
			lstAll			= AddListItem( wDB[ r ][ nColumn ], lstAll ) 				// add to list of rows of unique entries
		else
			// lstDoublettes	= UFCom_PossiblyAddListItem( num2str( r ), lstDoublettes )	// add the row only once to list of doublettes
			lstDoublettes	= AddListItem( num2str( r ), lstDoublettes )				// add the row as often as found  to list of doublettes
			if ( bDoDisplay )
		 		printf "\t\tUFCom_FindIdenticalCells(1) \tRow\t%4d\t/ %4d\t\thas copy in \trow\t%3d\t'%s'\t ->\tDoublettes:%d    '%s'   \r", r, nRows, r, wDB[ r ][ nColumn ], ItemsInList( lstDoublettes ), lstDoublettes[0,300]
			endif
		endif		
	endfor
	uSecs	= UFCom_StopSimpleTimer( nTimNr ) 				// _MicroSecTimer_
	printf "\t\tUFCom_FindIdenticalCells(2) \tRows\t %4d\t ->\tDoublettes:%d    '%s'  \t[ Timer%2d\tms:%4.0lf\tus/item:%.1lf ]  \r", nRows,  ItemsInList( lstDoublettes ), lstDoublettes[0,200], nTimNr, uSecs/1000, uSecs/nRows 
	return	lstDoublettes
End
...
// Dummy function prototype 
	wave  /T	wDB
	variable	row
	print "in UFCom_ProtoBuildDataSetString() / fBuildDataSetString() with row= ", row
End


static strconstant ksDOUBL_SEP	= "|"	// !!! Assumption , will break if this character is contained in the data set.  ->  Check if containred and select an unused character...
...
// TODO : IS  EXTREMELY SLOW
	...
	for ( r = 0; r < nRows; r += 1 )											// loop through ALL rows
		...
		sData	= fBuildDataSetString( wDB, r )

		if ( WhichListItem( sData, lstAll, ksDOUBL_SEP  )  == UFCom_kNOTFOUND )
			lstAll			= AddListItem( sData, lstAll, ksDOUBL_SEP ) 			// add to list of rows of unique entries
		else
			// lstDoublettes	= UFCom_PossiblyAddListItem( num2str( r ), lstDoublettes )	// add the row only once to list of doublettes
			lstDoublettes	= AddListItem( num2str( r ), lstDoublettes )				// add the row as often as found  to list of doublettes
			if ( bDoDisplay )
		 		printf "\t\tUFCom_FindIdenticalDatasets(1) \tRow\t%4d\t/ %4d\t\tis doublette...\t'%s'\t ->\tDoublettes:%d    '%s'     \r", r, nRows, sData, ItemsInList( lstDoublettes ), lstDoublettes[0,200]
			endif
		endif		

	endfor
	uSecs	= UFCom_StopSimpleTimer( nTimNr ) 								// _MicroSecTimer_
	printf "\t\tUFCom_FindIdenticalDatasets(2) \tRows\t%4d\t\t ->\tDoublettes:%d    '%s'    \t[ Timer%2d\tms:%4.0lf\tus/item:%.1lf ] \r", nRows, ItemsInList( lstDoublettes ), lstDoublettes[0,200], nTimNr, uSecs/1000, uSecs/nRows 

//
//	 nTimNr	= UFCom_StartSimpleTimer() 									// _MicroSecTimer_
//	string  	sFurtherData, sDoublette 
//	variable	nDoub
//	for ( r = 0; r < nRows; r += 1 )											// loop through ALL rows
//
//		sData	= fBuildDataSetString( wDB, r )
...
//			if ( UFCom_DataSetExists( sData, sFurtherData ) )
//				// printf "\t\tUFCom_RemoveIdenticalDatasets(1) \tDBRows:%3d\tRow\t%4d\thas a copy in row\t%4d\t'%s'...   \r", nRows, r, nDoub, sData[0,200]
//				sDoublette	 	= num2str( nDoub )
//				lstDoublettes	= UFCom_PossiblyAddListItem( sDoublette, lstDoublettes )	// add only once 
//				if ( bDoDisplay )
//			 		printf "\t\tUFCom_FindIdenticalDatasets(3) \tRow\t%4d\t/ %4d\t\thas copy in \trow\t%3d\t'%s'\t ->\tDoublettes:%d    '%s'   \r", r, nRows, nDoub, sData[0,200], ItemsInList( lstDoublettes ), lstDoublettes[0,200]
//				endif
//			endif
//		endfor
//	endfor
//	uSecs	= UFCom_StopSimpleTimer( nTimNr ) 				// _MicroSecTimer_
//	printf "\t\tUFCom_FindIdenticalDatasets(4) \tRows\t%4d\t\t ->\tDoublettes:%d    '%s'   \t[ Timer%2d\tms:%4.0lf\tus/item:%.1lf ]  \r", nRows, ItemsInList( lstDoublettes ), lstDoublettes[0,200], nTimNr, uSecs/1000, uSecs/nRows 
//
	return	lstDoublettes
End
...
// 2007-04-03 should be renamed: UFCom_PossiblyDeleteDataset( wDB, fBuildDataSetString, nRow )
...
// Count empty entries in column 'nColumn' .   Useful for checking the integrity of the primary key e.g.  ID  or  BestellNr
	...
	for ( n = 0; n < nRows; n += 1 )
		...
		if ( strlen( sCell ) == 0 )
			nMissing	+= 1
			if ( UFCom_DebugVar( "com", "DbIntegr" ) )
...
// todo: take into account that from Igor6 on some of header lines may be turned off.....
	...
	return	max( 98, TblHeightPts ) // 2010-02-24 empirical value for FBrain: Table must have minimum height  so that the last filled row stays in view after the finishing CR jumps to the next empty row. 
// 98 is value tested for 1 row, perhaps one must return TblHeightPts + 22 
...
Function		UFCom_ModifyTableColumnWidthNum( wDB, sTableNm, lstColWidth, nWidthCol0 ) 
	...
	variable	nWidthCol0 
	variable	c, nCols	= ItemsInList( lstColWidth )
if ( DimSize( wDB, 1 ) < nCols )
	printf "??? UFCom_ModifyTableColumnWidthNum()   '%s'    DimSize( wDB, 1 ) = %d  <  %d = nCols   (080724 for FPulse)\r",  sTableNm, DimSize( wDB, 1 ) , nCols
endif
...
Function		UFCom_ModifyTableColumnWidth( wDB, sTableNm, lstColWidth, nWidthCol0 ) 
	...
	variable	c, nCols	= ItemsInList( lstColWidth )
if ( DimSize( wDB, 1 ) < nCols )
	printf "??? UFCom_ModifyTableColumnWidth()   '%s'   DimSize( wDB, 1 ) = %d  <  %d = nCols   (080724 for FPulse) \r",  sTableNm,  DimSize( wDB, 1 ) , nCols
endif
	nCols	= min( nCols, DimSize( wDB, 1 ) )					// prevent Igor from complaining when there are not as many wave columns as width columns
	ModifyTable /W = $sTableNm  width[ 0 ] = nWidthCol0
	for ( c = 0; c < nCols; c += 1 )
...
End
Function		UFCom_ModifyTableColumnTitlesWv( wDB, sTableNm, wColTitles )
// only for GastSpeisen
	wave /T  wDB, wColTitles
	string  	sTableNm
	variable	c, nCols	=  DimSize( wColTitles, 1 ) 
	nCols	= min( nCols, DimSize( wDB, 1 ) )					// prevent Igor from complaining when there are not as many wave columns as title columns
	for ( c = 0; c < nCols; c += 1 )
		ModifyTable /W = $sTableNm  title[ c + 1 ]		=  ReplaceString( "\"", wColTitles[0][ c ], "" )
		ModifyTable /W = $sTableNm  alignment[ c + 1 ]	= 0 		// 0 starts entry at left border 
	endfor
End
...
Function		UFCom_JumpToBottomRow( wDB, sTableNm )
	wave 	wDB
	string  	sTableNm
	variable	nRowsVisible	= UFCom_GetTableRowsVisible( sTableNm )
	variable	nRows		= DimSize( wDB, 0 )
	ModifyTable	/W = $sTableNm	TopLeftCell = ( max( 0, nRows - nRowsVisible),  0 )  	// scroll table contents so that there are 'nRowsVisible' visible above the last row 
	// printf "\t\tUFCom_JumpToBottomRow((\tTable:\t%s\t)\tSelecting row %3d ( cols: %d ) \r", UFCom_pd( sTableNm, 19),  nRow, nCols
End
...
Function		UFCom_SortDatabaseNumerical( sFolder, sDBNm, nPrimSortCol, nOrder )
// Sort the data base  'sDBNm'   according to 'PrimSortCol' .  If not primarily sorting column 0  then column 0 is used as a secondary sorting index.
//  Order is not implemented.
	string  	sFolder, sDBNm
	variable	nPrimSortCol, nOrder
	string  	sFoDBNm	= UFCom_ksROOT_UF_ + sFolder + sDBNm
	UFCom_SortDatabaseNumerical_( sFoDBNm, nPrimSortCol, nOrder )
End	


Function		UFCom_SortDatabaseNumerical_( sFoDBNm, nPrimSortCol, nOrder )
// Sort the 2-dim text wave data base  'sDBNm'   numerically  according to 'PrimSortCol' .  
// Unfortunately Igor can sort only 1-dim waves.    Order is not implemented.
	string  	sFoDBNm
	variable	nPrimSortCol, nOrder
	wave 	/T  wt  = $sFoDBNm

	variable	rows		= DimSize(wt,0)
	variable	c, cols	= DimSize(wt,1)

	// Extract the column with the numbers (as strings) after which we want to sort
	make /O /N=(rows) ww, wIndex=p
	ww = str2num( wt[ p ][ nPrimSortCol ] )
	Sort ww, wIndex					// construct the index wave which gives the sorting order
	// print ww, wIndex
	Killwaves /Z ww

	// Sort each column by applying the index wave.  Unfortunately Igor can not sort the 2-dim wave in one step.
	string  	sColNm	= ""
	for ( c = 0; c < cols; c += 1 )
		sColNm	= sFoDBNm + num2str( c )
		make /O /T /N=(rows)  $sColNm
		wave /T wv= $sColNm
		wv		= wt[ p ][ c ]
		IndexSort wIndex, wv 
		// print "1", sColNm, wv
	...
	// Rebuild the 2-dim text wave by putting together the sorted columns.
	for ( c = 0; c < cols; c += 1 )
		sColNm	= sFoDBNm + num2str( c )
		wave /T wv= $sColNm
		wt[][ c ]	= wv[ p ] 
	...
	// print  wIndex,  wt
// 2009-09-29 z
	Killwaves /Z wIndex 	// Cave: Must explicitly delete wIndex.  If wIndex is not deleted it will remain forever in any folder which will give erroneous program behaviour  if e.g. all waves of that folder are to be processed.
End	


// sorts wrong  1.e2  <  2.e1 < 3
...
// 2007-0806 wDB numerical    SIMPLIFY???
			wSort	= wTmp
		endif
	endfor
...
// 2009-09-29 z
	Killwaves /Z wSortIndx 	// Cave: Must explicitly delete wIndex.  If wIndex is not deleted it will remain forever in any folder which will give erroneous program behaviour  if e.g. all waves of that folder are to be processed.
End
...
// 2010-01-17  remove trailing colon (colon is allowed if list contains 2 ore more items, but not if list cotains only a single item)
// no.................
//lstWaves	= RemoveEnding( lstWaves, ";" )
	string 	sPathFile = ""
...
static constant			kTBL_MARGINX	= 16// 2010-02-24 for fBrain, was 8// 2007-0825 for recipes, was 20						// bad empirical value to compute the required table window width from the table column widths
...
// If the screen is wide and high enough display table with its full width from 'lstColumnWidths' and height, but 'xLeft' , 'xRight',  'yTop' and 'yBot'   may  narrow this range .
// Depending on  'bXYinPercent_NotPoints'  x and y are interpreted as  percentage of screen  or  as  points.
	wave 	/T  wDB
	...
	variable	bXYinPercent_NotPoints	= 1		// x and y in percent of screen is old-established behavior.   Set to 0  if x and y are in points which is preferable if the window location is stored in an INI file (first used for 'tbAcq') 
	UFCom_DisplayTable_( xLeft, yTop, xRight, yBot, wDB, lstColumnNames, lstColumnWidths, lstColumnShades, sTblNm, sTitle, sfHook, bKillOrResize, bXYinPercent_NotPoints ) 
End

Function		UFCom_DisplayTable_( xLeft, yTop, xRight, yBot, wDB, lstColumnNames, lstColumnWidths, lstColumnShades, sTblNm, sTitle, sfHook, bKillOrResize, bXYinPercent_NotPoints ) 
// Construct table and display it. Depending on  'bKillOrResize'  the table is  killed  and recreated   or  only resized.
// If the screen is wide and high enough display table with its full width from 'lstColumnWidths' and height, but 'xLeft' , 'xRight',  'yTop' and 'yBot'  (ALL IN %)   may  narrow this range 
	...
	string		lstColumnNames, lstColumnWidths, lstColumnShades, sTblNm, sTitle, sfHook
	variable	bKillOrResize
	variable	bXYinPercent_NotPoints
	
	if ( ! waveExists( wDB ) )
		printf "Internal Error in UFCom_DisplayTable_  wDB does not exist\r"
	endif	
	...
	if ( WinType ( sTblNm ) != UFCom_WT_TABLE )
		...
		if ( bXYinPercent_NotPoints == 1 )
			...
			xRight = 	xLeft + xSz
			yBot	  = 	yTop + ySz
			// print "\t\tUFCom_DisplayTable() ", sTitle, "bKill:", bKillOrResize,  "xmin:", rxMinPoints, "xmax:", rxMaxPoints, "ymin:", ryMinPoints, "ymax:", ryMaxPoints, "-> xSz ", xSz, "-> l t r b " , xLeft, yTop, xLeft+xSz, xright, yTop+ySz, yBot	
		endif

		Edit	   /K=1  /N=$sTblNm	/W=( xLeft, yTop, xRight, yBot )   wDB  as  sTitle 				// ...and  kTBL_MARGINX  is only bad empirical value (maybe font and font size matter?...todo...)
		if ( strlen( sfHook ) )
...
Function		UFCom_DBExtractList( sDBFoNm, sDBFoNmExtr, lstColNames, lstRows, fBuildDataSetString )
// Appends all rows 'lstRows' from original data base 'sDBFoNm'  to  extracted  data base 'sDBFoNmExtr'  (only if they are not yet contained in the extracted data base.) .  
// Rows are appended as needed, columns remain unchanged.  To avoid doublettes only not yet existing rows are appended.  'fBuildDataSetString'  decides whether a row is new.
	string 	sDBFoNm, sDBFoNmExtr	
	string  	lstColNames
	string  	lstRows
	Funcref	UFCom_ProtoBuildDataSetString		fBuildDataSetString		

	// Now do something useful with the list of rows found :  Extract into a smaller table...
	variable	nORow, n, nExtractedRows	= ItemsInList( lstRows )
	for ( n = 0; n < nExtractedRows; n += 1 )
		nORow	= str2num( StringFromList( n, lstRows ) )
		UFCom_DBPossiblyCopyToExtracted( sDBFoNm, sDBFoNmExtr, lstColNames, nORow, fBuildDataSetString )
	endfor
	printf "\t\tUFCom_DBExtractList(\t\t%s -> %s ) \tTotal entries: %3d \tAdding\t%4d\t->\t%4d\t \r", sDBFoNm, sDBFoNmExtr, DimSize( $sDBFoNm, 0 ), nExtractedRows, DimSize( $sDBFoNmExtr, 0 )
End


Function		UFCom_DBExtractNonEmpty( sDBFoNm, sDBFoNmExtr,  lstColNames, nColumn, fBuildDataSetString  )
// Appends all rows whose cell in 'nColumn' is not empty from original data base 'sDBFoNm'  to  extracted  data base 'sDBFoNmExtr'  (only if they are not yet contained in the extracted data base.) .  
// Rows are appended as needed, columns remain unchanged.  To avoid doublettes only not yet existing rows are appended.  'fBuildDataSetString'  decides whether a row is new.
	string  	sDBFoNm, sDBFoNmExtr
	string  	 lstColNames
	variable	nColumn
	...
	wave	/T	wDB		= $sDBFoNm
	variable	nORow, nItems	= DimSize( wDB, 0 )
	variable	nFound		= 0

	for ( nORow = 0; nORow < nItems; nORow += 1 )
 		if ( strlen( UFCom_RemoveWhiteSpace( wDB[ nORow ][ nColumn ] ) ) )			// if there is an entry in this row/column (any character will do) 
			UFCom_DBPossiblyCopyToExtracted( sDBFoNm, sDBFoNmExtr, lstColNames, nORow, fBuildDataSetString )
			nFound	+= 1
		endif
	endfor
	printf "\t\tUFCom_DBExtractNonEmpty(\t%s -> %s ) \tTotal entries: %3d \tAdding\t%4d\t->\t%4d\t (with  an entry  or  flag set  in column: %d   \r", sDBFoNm, sDBFoNmExtr, nItems, nFound , DimSize( $sDBFoNmExtr, 0 ), nColumn
End

Function		UFCom_DBPossiblyCopyToExtracted( sDBFoNm, sDBFoNmExtr, lstColNames, nORow, fBuildDataSetString )
// possible todo: additional parameter bBegOrEnd  determines where copied data rows are inserted
	string  	sDBFoNm, sDBFoNmExtr
	string  	lstColNames
	variable	nORow
	Funcref	UFCom_ProtoBuildDataSetString		fBuildDataSetString		
	...
	wave	/T	wDB		= $sDBFoNm
	wave	/Z/T	wDBExtr	= $sDBFoNmExtr
	string  	sFurtherData	= fBuildDataSetString( wDB, nOrow )

	// If  the extracted wave does not exist yet then the first data set must certainly be added
	if (  ! waveExists( wDBExtr )  ||  UFCom_DatasetMustBeAdded( wDBExtr, fBuildDataSetString, sFurtherData ) )

		variable	nRow, nItems	= ItemsInList( lstColNames )
		if ( ! waveExists( wDBExtr ) )
			make /O/T /N=( 1, nItems )   $sDBFoNmExtr		// wave does not exist so we create it with just 1 row... 
			wave   /T	wDBExtr		= $sDBFoNmExtr
			nRow	= 0								//...and we will fill row 0
		else
			nRow	= DimSize( wDBExtr, 0 ) 				// an existing wave has at least 1 row,,,
			Redimension /N=( nRow+1, -1 )	wDBExtr			//...we will fill the last row after we appended it
		endif

		wDBExtr[ nRow ][]	= wDB[ nORow ][ q ] 				// Copy all columns of row 'nORow' in wDB  into  newly appended row 'nRow' of extracted data base  
	endif
End


// 2009-10-29 c
//Function		UFCom_MyTable( 			sWin, sTitle, bDisplay, RowOfs, sTxt, sKeySep, sListSep )
Function		UFCom_MyTable( sFo, sWvNm, sWin, sTitle, bDisplay, RowOfs, sTxt, sKeySep, sListSep )
	string  	sFo, sWvNm
	string  	sWin	
	string  	sTitle	
	variable	bDisplay	
	...
	string  	sSubFoIni = "",  sKey = ""
	variable	bXYinPercent_NotPoints	= 1			//  0  if x and y are in points which is preferable if the window location is stored in an INI file (first used for 'tbAcq')  ,  1 if x and y are in percents of screen (old-esatblished behaviour)

	UFCom_MyTable_( sFo, sWvNm, sWin, sTitle, bDisplay, RowOfs, sTxt, sKeySep, sListSep, sfHook, sSubFoIni, sKey, bXYinPercent_NotPoints)
	return	0
End	

// 2009-10-29 c
Function		UFCom_MyTable_( sFo, sWvNm, sWin, sTitle, bDisplay, RowOfs, sTxt, sKeySep, sListSep, sfHook, sSubFoIni,  sKey, bXYinPercent_NotPoints)
	string  	sFo, sWvNm
	string  	sWin	
	string  	sTitle	
	variable	bDisplay	
	...
	string  	sfHook							// function name	
	string  	sSubFoIni,  sKey
	variable	bXYinPercent_NotPoints				//  0  if x and y are in points which is preferable if the window location is stored in an INI file (first used for 'tbAcq')  ,  1 if x and y are in percents of screen (old-esatblished behaviour)
	...
	if ( ! waveExists( wv ) )
		nRows	= 1
		make /T /N=( nRows, nItems )  	$sWvNm		// wave does not exist...
		wave  /T	wv			= 	$sWvNm		// ...so we create it with just 1 row 
	else														
	...
	variable 	left = 0,  top = 10,  right = 70,  bot = 70

	// 2008-09-03  restore saved position  
	if ( strlen( sSubFoIni )   &&   strlen( sKey )  )
		string  	sWndInfo	= UFCom_Ini_Section( sFo, sSubFoIni, sWin, sKey )
			sWndInfo	= UFCom_WndPosition_( sFo, sWndInfo, left, top, right, bot )
		// printf "\t\t  UFCom_MyTable_()  restoring   %s %s %ss %s : %s \r", sFo,  sSubFoIni,  sWin, sKey, sWndInfo
	endif

	UFCom_DisplayTable_( left, top, right, bot, wv, lstColumnNames, lstColumnWidths, lstColumnShades, sWin, sTitle, sfHook, UFCom_kRESIZE_TBL, bXYinPercent_NotPoints ) 
	ModifyTable   /W=$sWin	 font = kTBL_FONT, size=kTBL_FONTSZ,  alignment[1]=0, alignment[2,inf]=2,  autosize={ 0,0,8,1,2}
//	ModifyTable   /W=$sWin alignment[1]=0
	DoUpdate
...
// 2009-10-29 c
Function		UFCom_MyTableSetCell( sFo, sWvNm, sTbl, sValue, sKey, lstTitles )
	string  	sFo, sWvNm, sTbl, sValue, sKey, lstTitles	

	variable	nCol	= WhichListItem( sKey, lstTitles, ";", 0, 0 )	// startIndex, matchCase )
	if ( nCol == UFCom_kNOTFOUND )
		printf "****Internal error [UFCom_MyTableSetCell] : Key '%s' not found in '%s' .\r",  sKey, lstTitles
	endif
	wave  /Z/T	wv	= $sWvNm
	if ( waveExists( wv ) )
		variable 	nRows	= DimSize( wv, 0 ) 
		wv[ nRows ][ nCol ]	= sValue
	endif
End		
...
//=============================================================================================================================
//  GENERALLY  USEFUL  DATA BASE  FUNCTIONS  INITIALLY USED  IN  AND  ADDED  FROM  THE  GASTSPEISEN  DATA BASE   2010-JanuaryFunction		UFCom_DBCountPerID( wDB, IdDat, IdCol )

Function		UFCom_LoadMSAccessTxtTable( sPathFile, wDBNm, wTblNm, sTblTitle )
// Loads Access table into Igor.  Loads implicitly into the current data folder, which is simple but might be a bit dangerous.... 
// The Access must have beeb exported as text format. It is converted into a 2-dim text wave. 
// The table column titles are kept.  The wave column names are NOT set (not yet..?)
	string  	sPathFile, wDBNm, wTblNm, sTblTitle
	LoadWave /Q /N=$wDBNm+"ColNames"	/M /J /K=2  /L={0,0,1,0,0}	 sPathFile	// mit spalte row, Id, vor, nach
	LoadWave /Q /N=$wDBNm 			/M /J /K=2  /L={0,1,0,0,0}	 sPathFile	// mit spalte row, Id, vor, nach
	Rename	$wDBNm +"0"  $wDBNm									// remove the 0 which Igor automatically appends to the wave name
	UFCom_PossiblyKillTable( wTblNm )

	Edit /K=1 /N=$wTblNm   	$wDBNm  as sTblTitle
	UFCom_ModifyTableColumnTitlesWv( $wDBNm, wTblNm, $wDBNm+"ColNames0" ) 

	KillWaves	$wDBNm+"ColNames0"
End

Function		UFCom_RemoveApostrophesFromDB( sFoWvNm )
	string  	sFoWvNm
	wave  /T	wDB		= $sFoWvNm
	variable	r, nRows	= DimSize( wDB, 0 )
	variable	c, nCols	= DimSize( wDB, 1 )
	for ( r = 0; r < nRows; r += 1 )
		for ( c= 0; c < nCols; c += 1 )
			wDB[ r ][ c ]	= ReplaceString( "\"", wDB[ r ][ c ], "" )
		endfor
	endfor
End

Function		UFCom_SwapColumnsFromDB( sFoWvNm, colA, colB )
	string  	sFoWvNm
	variable	colA, colB
	wave  /T	wDB		= $sFoWvNm
	variable	r, nRows	= DimSize( wDB, 0 )
	...
	for ( r = 0; r < nRows; r += 1 )
		sTmp		= wDB[ r ][ colA ]
		wDB[ r ][ colA ]	= wDB[ r ][ colB ]
		wDB[ r ][ colB ]	= sTmp
	endfor
End


Function		UFCom_RemoveColumnFromDB( sFoWvNm, col )
	string  	sFoWvNm
	variable	col
	wave  /T	wDB		= $sFoWvNm
	variable	r, nRows	= DimSize( wDB, 0 )
	...
	Redimension /N=(-1,nCols-1) $sFoWvNm
End

//--------------------------------------------------------------------------------------------------------

Function		UFCom_DBCountPerID( wDB, IdDat, IdCol )
// returns number of entries in 2-dim text wave wDB which match  'IdDat'  in the ID column IdCol
	wave  /T	wDB
	variable	IdDat, IdCol
	variable	r, rows	= DimSize( wDB, 0 )
	variable	cnt = 0
	for ( r = 0; r < rows; r += 1 )
		if ( cmpstr( wDB[ r ][ IdCol ] , num2str( IdDat)  ) == 0 )
			cnt += 1
		endif
	endfor
	return	cnt
...
Function	/S	UFCom_DBListPerID_( wDB, IdDat, col , IdCol )
// returns list of entries in 2-dim text wave wDB in column 'col'  which match  'IdDat'  in the ID column IdCol.  Fix separator ;
	wave  /T	wDB
	variable	IdDat, col, IdCol
	return	UFCom_DBListPerID( wDB, IdDat, col , IdCol, ";" )
End

Function	/S	UFCom_DBListPerID( wDB, IdDat, col , IdCol, sSep )
// returns list of entries in 2-dim text wave wDB in column 'col'  which match  'IdDat'  in the ID column IdCol
	wave  /T	wDB
	variable	IdDat, col, IdCol
	string  	sSep
	variable	r, rows	= DimSize( wDB, 0 )
...
Function	/S	UFCom_DBAppendListAsId( wDB, col, lst )
// append each entry of  'lst'  in a new row  in 'wDB'   in column  'col' ,  which should be an ID column..    
// ....could be split to append just 1 item.....
	wave  /T	wDB
	variable	col
	string  	lst
	variable	rows		= DimSize( wDB, 0 )
	variable	n, nItems	= ItemsInList( lst )
	for ( n = 0; n < nItems; n += 1 )
		rows 		+= 1
		Redimension  /N=((rows), -1 )	wDB
		wDB[ rows-1 ][ col ] 	= StringFromList( n, lst )
	endfor
End

Function	/S	UFCom_DBAppendList( wDB, IdVal, IdCol, col , lst )
// append each entry of  'lst'  in a new row  in 'wDB'   in column  'col'  which should NOT be the ID column.  Fill the 'IdCol' with  'IdVal' .    
// ....could be split to append just 1 item.....
	wave  /T	wDB
	variable	IdVal, IdCol, col
	string  	lst
	variable	rows		= DimSize( wDB, 0 )
	variable	n, nItems	= ItemsInList( lst )
	for ( n = 0; n < nItems; n += 1 )
		...
		Redimension  /N=((rows), -1 )	wDB
		wDB[ rows-1 ][ col ] 	= StringFromList( n, lst )
		wDB[ rows-1 ][ IdCol] 	= num2str( IdVal )
	endfor
End

//Function	/S	UFCom_DBUniqueListOf1Col( wDB, col )
//// = UFCom_ListOfUnique
//// returns list of entries in 2-dim text wave wDB in column 'col' . The list contains every entry is only once.
//	wave  /T	wDB
//	variable	col
//	variable	r, rows	= DimSize( wDB, 0 )
//	string  	sItem 	= "",  lst 	= ""
//	for ( r = 0; r < rows; r += 1 )
//		sItem	= wDB[ r ][ col ] 
//		if ( WhichListItem( sItem, lst ) == UFCom_kNOTFOUND )
//			lst += sItem + ";"
//		endif
//	endfor
//	return	lst
...
Function	/S	UFCom_DBCheckUniqueCol( wDB, col, sDBNm, sColNm )
// returns error text string if  'col'  in 2-dim text wave wDB  contains doublette entries.  Returns empty string  if  all enties are unique.
	wave  /T	wDB
	variable	col
	string  	sDBNm, sColNm 
	variable	r, rows	= DimSize( wDB, 0 )
	string  	sItem 	= "",  lst 	= "",   sTxt = ""
	for ( r = 0; r < rows; r += 1 )
		sItem	= wDB[ r ][ col ] 
		if ( WhichListItem( sItem, lst ) > UFCom_kNOTFOUND )
			sTxt	  = "\t\tDB integrity error:\t" + sItem + " \t(" + sColNm + ") \tis not unique in '" + sDBNm + "' \r"
			break
		endif
		lst += sItem + ";"
	endfor
...
Function	UFCom_FirstMatchingRow( wDB, nColumn, sItem ) 
// returns first row in  wDB  where  sItem  has been found in  nColumn,  or UFCom_kNOTFOUND (=-1)  if no match could be found
	wave /T	wDB
	variable	nColumn					// to be searched
	string  	sItem						// to be searched
	variable	r,  nRows	= DimSize( wDB, 0 )
	variable	nFound	= 0
	for ( r = 0; r < nRows; r += 1 )
		if ( cmpstr( sItem, wDB[ r ][ nColumn ] ) == 0 )
			return  r
			break
		endif
	endfor
...
Function	UFCom_CountMatchingRows( wDB, nColumn, sItem ) 
// returns number of rows in  wDB  where  sItem  has been found in  nColumn
	wave /T	wDB
	variable	nColumn					// to be searched
	string  	sItem						// to be searched
	variable	r, nRows	= DimSize( wDB, 0 )
...
Function	UFCom_DeleteMatchingRows( wDB, nColumn, sItem ) 
// deletes rows and  returns number of rows in  wDB  where  sItem  has been found in  nColumn
	wave /T	wDB
	variable	nColumn					// to be searched
	string  	sItem						// to be searched
	variable	r, nRows	= DimSize( wDB, 0 )
	...
	for ( r = nRows - 1;  r >= 0; r -= 1 )
		if ( cmpstr( sItem, wDB[ r ][ nColumn ] ) == 0 )
			UFCom_DeleteDatabaseEntry( wDB, r )
			nFound += 1
		endif
	endfor
	return	nFound
