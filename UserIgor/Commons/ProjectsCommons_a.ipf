...
//
//	RevHist
// 	070822	removed Trial time (as it can easily be overcome 1.using a virtual machine and 2.simply loading an experiment rather than starting FPulse by selecting a menu item)...
...
static strconstant	ksMY_DRIVE				= "C:"								// where my sources are , ................	must be the  SAME  as UFPE_ksSCRIPTS_DRIVE
static strconstant	ksLINK2DIR				= "Link2Dir_"							// Base name (FPulse or FEval will be appended!) of link of working directory (the specific IPFs) in 'User procedures' .  
...
// 070822 remove TT
//Function		UFProj_CreateRelease( sAppNm, sMarker, lstPrefixXops, lstPrefixIpfs, sSomeComDir, lstDLLFiles, lstUserFiles, lstMoreBackups, days, sCryptBsNm, sApplSubDir, sAllUsersSubDir )
...
//	UFProj_ProjectRelease( sAppNm, sVersion, sArchiveDir, lstPrefixXops, lstPrefixIpfs, sDllDrvDir, sSourcesDrvDir, sSomeComDir, sExeDir, sReleaseOutDir, sInstallDir, lstDLLFiles, lstUserFiles, days, sCryptBsNm, sApplSubDir, sAllUsersSubDir ) 
//	UFProj_BackupAllTheRest( sAppNm, sVersion, sArchiveDir, sSourcesDrvDir, lstMoreBackups )	// Backup files (e.g. bmp, iss, ttf ) into the which are  NOT to be distributed to the user.
End
...
// 070822 remove TT
//Function		UFProj_ProjectRelease( sAppNm, sVersion, sArchiveDir, lstPrefixXops, lstPrefixIpfs, sDllDrvDir, sSourcesDrvDir, sSomeComDir, sExeDir, sReleaseOutDir, sInstallDir, lstDllFiles, lstUserFiles, days, sCryptBsNm, sApplSubDir, sAllUsersSubDir )
...
// ProjectRelease() must be executed by the programmer when a new  FPulse  version  is to be released
	...
	if ( nXops )
		DoAlert 0, "Make sure that the following " + num2str( nXops ) + " compilations give no errors. \rCheck the DOS boxes which will be created. " 
	endif
...
if ( strlen( lstDllFiles ) )
	...
	if ( V_Flag )
		printf "++++Error: Could not DeleteFolder\tTo enable DeleteFolder, open the Miscellaneous Settings Dialog's Misc category \r\tand check the 'Enable DeleteFolder, CopyFolder/O and MoveFolder/O  commands'  checkbox.\r"
	endif
...
// 070822 remove TT  but  keep  an empty string  'sBirthFile->sBirthFile_Unused'  as a dummy parameter to  'CallInno -> Execute ISCC.exe'...   which can be used should need arise to introduce another optional InnoSetup parameter
	string  	sBirthFile_Unused = ""
// 070822 remove TT
//	if ( days > 0  )
//		// Create the trial time file and copy it to the release HD directory
//		sBirthFile	= UFBd_ProjectsMakeFile( days, sCryptBsNm, sAppNm, sVersion, sOutDir )	// creates bd file e.g. 'cryptbdiml.dll'  which is required by InnoSetup if /dBirth is specified (as it is in FPulse, not in FEval)
//		sBirthFile	= UFCom_RemoveFilePath( UFCom_Path2Mac( sBirthFile ) )				// e.g 'D:\WINNT\system32\cryptbemml.dll'  -> 'cryptbemml.dll' 	
//	endif
...
// 070822 remove TT
//Function	UFProj_Prolong( sAppNm, sVersionMarker, sCryptBsNm, days )
//// Prolongs trial time of application 'sAppNm' by 'days'.  Call e.g. xUtilProlong( "FPulse", 12 ) .  Must quit and restart Igor to take effect.
//	string  	sAppNm, sVersionMarker, sCryptBsNm
//	variable	days
//	string  	sSourcesDrvDir	= SourcesDrvDir( sAppNm )
//	string  	sVersion		= RetrieveVersion( sSourcesDrvDir + ":" + sAppNm + ".ipf", sVersionMarker )	// e.g.  'ksFP_VERSION'
//	string  	sOutDir		= UFBd_GetSystemDirectory()								// e.g.  ''D:\WINNT\system32'
//
//	// Create or overwrite bd file e.g. 'cryptbdiml.dll'  which may be required on Igor start if a Trial time limit has been specified = there is a link to an Xop which executes  'UtillBd()'  in  'Main()'  in  'XopMain.c'
//	string  	sBirthFilePath	= UFBd_ProjectsMakeFile( days, sCryptBsNm, sAppNm, sVersion, sOutDir )	
//	string  	sBirthFile		= UFCom_RemoveFilePath( UFCom_Path2Mac( sBirthFilePath ) )		// e.g 'D:\WINNT\system32\cryptbemml.dll'  -> 'cryptbemml.dll' 	
//End
		...
		for ( n = 0; n < ItemsInList( lstDirs ); n +=1 )	
			...
			switch( nAction )
				...
				case	kRLXP_DELETE: 
					...
					if ( V_Flag )
						printf "++++Error: Could not DeleteFolder '%s' \tTo enable DeleteFolder, open the Miscellaneous Settings Dialog's Misc category \r\tand check the 'Enable DeleteFolder, CopyFolder/O and MoveFolder/O  commands'  checkbox.\r", sXopRenmDir
					endif
   					break
				default:
...
// 070224 do not delete LIB files.  We need  UFBd_LibBd.lib ! )	
...
// 070822 remove TT
//Function	CheckEditReleaseSetting( sAppNm, TTd, sMarkerFlag )
//	string  	sAppNm
...
//	CheckReleaseSetting( sAppNm, sVersion, TTd )
...
//End
//
//Function	CheckReleaseSetting( sAppNm, sVersion, TTd )
//	string  	sAppNm, sVersion
//	variable	TTd
...
//	printf  "\r\tCurrent settings for '%s' : \r\t\tVersion :  \t%s\r\t\tTrial time :\t%s \r", sAppNm, sVersion, sLimit
//End
...
// 070212a	 innosetup5 mutters about fpulse.iss line 359 Instexec not known
