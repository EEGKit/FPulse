...
#include "UFCom_ReleaseComments"			// dor all UFCom_Xxx

// todo either all functions UFCom_xxx , then no more static   or  all functions static and no #include  UFCom_ReleaseComments
...
//static strconstant	ksMYBAK_DIR_BS 			= "UserIgor:Archive"						// where the backups of the current version (before switching)   are saved
static strconstant	ksUSERIGOR_DIR			= "UserIgor"							// base directoty for all  (e.g. FPulse or FEval)
...
Function		DeInstallFPFE( sAppNm, sVersion, sArchiveDir, lstlstPrefixesDelLink, sSourcesDrvDir, lstlstPrefixesCopy )
	...
	Local_DeleteLinks( ksIGORPROC_LNK, 	sAppNm + ".ipf"			+ ".lnk" )	// 'FPulse.ipf'  or  'FEval.ipf'
	Local_DeleteLinksFromList(   ksHELP_LNK, 	lstHelpFiles			, ".lnk" )	// '.lnk' is a separate parameter as an extension can only be appended to links, not to a list 	

	ProcessXOPs( kRLXP_REMOVELINK, ksXOP_DIR, ksXOP_SUBDIR_, lstlstPrefixesDelLink, ksXOPSEP, "" , sReleaseOutDir) 

	Local_DeleteLinks( ksUSERPROC_LNK,	ksUSERIGOR_CED+sAppNm+"_.lnk")	// link to directory  UserIgor:Eval_  or  UserIgor:FPulse_ 
...
//must be unique to FPulse/Feval	string  	sIgorProc_Projects_Link	= ksIGORPROC_LNK + ":" + "Projects.ipf" 	+ ".lnk"
	...
	variable	bLinkExists	= Local_FileExistsIgorPath( sIgorProc_AppName_Link )  ||  Local_FileExistsIgorPath( sUserProc_AppDir_Link ) 
	if ( bLinkExists ) 
	...
	string  	lstDirs=  Local_ListOfMatchingDirs( ArchiveBaseDrvDir( sAppNm ), "*", FALSE )	// *.* : list all dirs ,  False: supply only partial path
...
//	endif
	...
	string  	sWindowsPath = ""
	if ( 	Local_SearchDir(   	  "C:WinNT:System32" ) )
		sWindowsPath = "C:WinNT" 
	elseif ( Local_SearchDir( 	  "D:WinNT:System32" ) )	
		sWindowsPath = "D:WinNT" 
	elseif ( Local_SearchDir(    	  "C:Windows:System32" ) )
		sWindowsPath = "C:Windows" 
	elseif ( Local_SearchDir( 	  "D:Windows:System32" ) )	
		sWindowsPath = "D:Windows" 
	endif


	if ( ! Local_FileExists( sWindowsPath + ":" + StringFromList( 0, lstDllFiles ) ) )
		Local_CopyFilesFromList( ArchiveBaseDrvDir( sAppNm ) + ":" + sRestoreDir, lstDllFiles, sWindowsPath )	// e.g. 'C:UserIgor:Ced:Cfs32.dll, 1432ui.dll, Use1432.dll' -> D:WinNT 
	endif


	// Delete all files in development directory
	Local_DeleteFiles( sSourcesDrvDir, "*.*" )			

	// Copy  files from selected archive into development directory
	Local_CopyFilesFromList( ArchiveBaseDrvDir( sAppNm ) + ":" + sRestoreDir , 	 "*.*", sSourcesDrvDir )	// Copy the current User files (e.g. ipf, ihf, xop..) into it

	// Recreate the links
	Local_CreateLinks( "C:UserIgor:Commons", 	   "Projects.ipf", 		ksIGORPROC_LNK )
	Local_CreateLinks( sSourcesDrvDir,		   sAppNm + ".ipf", 	ksIGORPROC_LNK )
	Local_CreateLinksFromList( sSourcesDrvDir,  lstHelpFiles, 		ksHELP_LNK )					// 

	string  	sReleaseOutDir	= ReleaseOutDir( sAppNm )					// e.g. 'UserIgor:FPulseOut'
	ProcessXOPs( kRLXP_CREATELINK, ksXOP_DIR, ksXOP_SUBDIR_, lstlstPrefixes_OrgRen, ksXOPSEP, "", sReleaseOutDir ) 

	Local_CreateAlias( sSourcesDrvDir, ksUSERPROC_LNK + ":" + ksUSERIGOR_CED+sAppNm + "_.lnk" )	// is link to directory  , but could perhaps also use  CreateLinks()...
...
// ProjectRelease() must be executed by the programmer when a new  FPulse  version  is to be released
	...
	// Process specific application files ( e.g.  ipf, ihf, xop, txt  from   UserIgor:Ced )
...
//	UFP_CedUtilBdMake( days, sVersion, sOutDir )				// creates bd file e.g. 'cryptbdiml.dll'  which is required by InnoSetup if /dBirth is specified (as it is in FPulse, not in FEval)
	...
	string  	sBirthFile			= BuildTTFileNm( sVersion ) 
	...
	//  As these files change seldomly they are doubles in most archives.  For this reason  'UserIgor:Ced' should contain no junk but only a few important files.	
...
// Retrieve the current FPulse version not from the string constant (which may be unaccessible when FPulse is closed) but from the file 'sSrcPath' e.g. 'C:UserIgor:Ced:FPulse.ipf'
...
static  Function		CallInno( sVers, sDrive, sPrgDir, sAppNm, sReleaseSrcDir, sSetupExeDir, sInstallationDir, sBirthFile )
		string  	sVers, sDrive, sPrgDir, sAppNm, sReleaseSrcDir, sSetupExeDir, sInstallationDir, sBirthFile
...
//	string  	sIssScript 	= sDrive + "\\" + sMyPrgDir + "\\FPulse.iss"				// the path to the InnoSetup script , e.g. 'C:\UserIgor\Ced\FPulse.iss'
	...
	string  	sStr 		= sCmd + " " + sIssScript + " " + sAppNam + " " + sVersion + " " + sBirth + " " + sSource + " " + sMask + " " + sOutputDir + " " + sDefaultDir 
	printf "\t%s   \r", sStr
...
// Possible improvement: defining 'nAction' as a bit pattern would allow to execute multiple actions with 1 call...
	...
	string  	sText	= pd( StringFromList( nAction, ksRLXP_NAMES ), 10 )		// e.g. 'Copy',   'Rename'  ...
	variable	d, nDirs	= ItemsInList( lstlstPrefixes_OrgRen, sSep )
	for ( d = 0; d < nDirs; d += 1 )
		string  	lstDirPreFixes	= StringFromList( d,  lstlstPrefixes_OrgRen, sSep )		// e.g. 'UFPE_;UFp2_'
		string  	sDirPreFixOrg	= StringFromList( 0,  lstDirPrefixes )					// e.g. 'UFPE_'
		...
		string 	lstDirs		= Local_ListOfMatchingDirs( sDriveSrcDir, sDirPreFixOrg + "*", 0 )		// e.g. 'UFP_Ced;UFP_Mc700'
		variable	n
		for ( n = 0; n < ItemsInList( lstDirs ); n +=1 )	
			...
			string  	sInfo
			sprintf sInfo, "\tProcessXOPs(\t%s )\tDir:%d/%d\tn:%d\tDir: %s\tOrg:\t%s\tRen:\t%s\tSrcD:\t%s\tXop:\t%s\tRnD:\t%s\tArD:\t'%s' ", sText, d, nDirs, n, pd(sDir,13),  pd(sDirPreFixOrg,7),  pd(sDirPreFixRenm,7),  pd( sXopSrcDir,24),  pd(sXopExePath, 36),  pd(sXopRenmDir, 24), sArchiveDir
			printf "%s\r", sInfo[0,396]
			switch( nAction )
				case	kRLXP_COPY:
					string  sArchiveSubDir = sArchiveDir + ":" + sDir
					Local_PossiblyCreatePath( sArchiveSubDir )						// Create  subdirectory within archive	e.g.	'C:UseIgor:ArchiveFPulse:FPulse040315V235:UFCom_Utils'
					Local_CopyFilesFromList( sXopSrcDir, ksXOP_SOURCE_FILES + ksXOP_LIB_FILES, sArchiveSubDir )	// Copy the RELEASE XOP sources into the archive subdirectory 
					break
				case	kRLXP_RENAME:
					if ( strlen( sDirPreFixRenm ) )							// only common funstions must be renamed, specific application functions keep their name
						Local_ReplaceModuleNamesFromLst(  sDirPreFixOrg, sDirPreFixRenm, sXopSrcDir, sXopRenmDir, ksXOP_SOURCE_FILES ) 
						Local_CopyFilesFromList( sXopSrcDir, ksXOP_LIB_FILES, sXopRenmDir )	// Copy the library files (e.g. 'Cfs32.lib'  the renamed Xop sources subdirectory, as the compiler expects them there. We can NOT 'Rename' the library binary file! 
					endif
					break
				case	kRLXP_COMPILE:
					string  	sCompileDir	= SelectString( strlen( sDirPreFixRenm ) , sXopSrcDir, sXopRenmDir )
					CallXOPCompiler_( sCompileDir )
					Local_CopyFilesFromList( sCompileDir + ":Release", "*.xop", "C:" + sReleaseOutDir )// Copy the RELEASE XOP into the release Dir 
					break
				case kRLXP_CREATELINK:
					Local_CreateAlias( sXopExePath, 	 ksXOP_LNK + ":" + sDir + ".xop" + ".lnk" )		// We must keep the  '.xop'  in the link file name to stay compatible with the InnoSetup naming
					break
				case	kRLXP_REMOVELINK:
					Local_DeleteLinks( ksXOP_LNK, 	sDir + ".xop" + ".lnk" )	
					break
				case	kRLXP_DELETE: 
...
// Will compile a 'Release' version no matter which version is defined in Igor  so that the trial time code is never distributed to the user
	...
	printf  "\t\tCompiling XOP \t%s\tin\t'%s' \t-> \t'%s' \r",  pd( sXop,14), sXopRenmDir, sStr
	...
	Local_DeleteFilesFromList( sXopRenmDir, 			ksXOP_DELETE_FILES )					// Delete  OBJ,  PCH,  PDB...
	Local_DeleteFilesFromList( sXopRenmDir + ":Debug",	ksXOP_DELETE_FILES + ksXOP_LIB_FILES )	// Delete  OBJ,  PCH,  PDB.... + LIB.
	Local_DeleteFilesFromList( sXopRenmDir + ":Release",	ksXOP_DELETE_FILES + ksXOP_LIB_FILES )	// Delete  OBJ,  PCH,  PDB...  + LIB
End
...
//=====================================================================================================================================
static Function		Local_SearchDir( sPath ) 
...
static Function  		Local_FileExistsIgorPath( sPathFile )
	string 	sPathFile
	variable	nRefNum
...
static Function  		Local_FileExists( sPathFile )
	string 	sPathFile
	variable	nRefNum
...
static Function	/S	Local_ListOfMatchingDirs( sSrcDir, sMatch, bFullPath )
...
static Function	/S	Local_ListOfMatchingFiles( sSrcDir, sMatch, bUseIgorPath )
...
static Function		Local_CopyFilesFromList( sSrcDir, lstFileGroups, sTargetDir )
// old version , Install.ipf ( was FPulse_Install.pxp)  has  a newer, more versatile one
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		Local_CopyFiles( sSrcDir, sFileGroup, sTargetDir ) 						// Copy the current User files (e.g. ipf, xop..) into it
	endfor
End

static Function		Local_CopyFiles( sSrcDir, sMatch, sTgtDir )
// old version , Install.ipf ( was FPulse_Install.pxp)  has  a newer, more versatile one
// e.g. 		CopyFiles(  "D:UserIgor:Ced"  ,  "FP*.ipf" , "C:UserIgor:CedV235"  ) .  Wildcards  *  are allowed .
	string  	sSrcDir, sMatch, sTgtDir
	string  	lstMatched	= Local_ListOfMatchingFiles( sSrcDir, sMatch, FALSE )
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		Local_Copy1File( sSrc, sTgt )
	endfor
End

static Function		Local_Copy1File( sSrc, sTgt )		
	string  	sSrc, sTgt 		
// 070217
...
static Function		Local_DeleteFilesFromList( sSrcDir, lstFileGroups )
// old version , Install.ipf ( was FPulse_Install.pxp)  has  a newer, more versatile one ???
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		Local_DeleteFiles( sSrcDir, sFileGroup ) 
	endfor
End

static Function		Local_DeleteFiles( sSrcDir, sMatch )
...
static Function		Local_CreateLinksFromList( sSrcDir, lstFileGroups, sTgtDir )
// old version , FPulse_Install.pxp has  a newer one
	...
	for ( n = 0; n < nCnt; n += 1 )
	  	sFileGroup		= StringFromList( n, lstFileGroups )
		Local_CreateLinks( sSrcDir, sFileGroup, sTgtDir ) 					
	endfor
End

static Function		Local_CreateLinks( sSrcDir, sMatch, sTgtDir )
// e.g. 		CreateLinks(  "D:UserIgor:Ced"  ,  "FP*.ipf" , "D:....:Igor Pro Folder:User Procedures"  ) . Wildcard *   is only allowed in filebase,   * is NOT allowed in the file extension.
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		Local_CreateAlias( sSrc, sTgt )
	endfor  
End

static Function		Local_CreateAlias( sFromPathFile, sToLinkFile )
...
static Function		Local_DeleteLinksFromList( sSrcDir, lstFileGroups, sExt )
	string  	sSrcDir, lstFileGroups, sExt
	variable	n, nCnt	= ItemsInList( lstFileGroups )
	string  	sFileGroup	
	for ( n = 0; n < nCnt; n += 1 )
	  	sFileGroup		= StringFromList( n, lstFileGroups ) + sExt
		Local_DeleteLinks( sSrcDir, sFileGroup ) 
	endfor
End


static Function		Local_DeleteLinks( sSrcDir, sMatch )
//static Function		DeleteLinks( sSrcDir, sMatch )
// e.g. 		DeleteLinks(  "Igor help Files:"  ,  "FP*.ihf"  ) . Wildcards  *  are allowed .
	string  	sSrcDir, sMatch
	string  	lstMatched	= Local_ListOfMatchingFiles( sSrcDir, sMatch, 0 )
	variable	n, nCnt		= ItemsInList( lstMatched )
...
// 070213........................
static Function		Local_PossiblyCreatePath( sPath )
	string 	sPath
	...
	for ( nRemove = nDirLevel - 1; nRemove >= 0; nRemove -= 1 )				// start at the drive and then add 1 dir at a time, ...
		...
		sPathCopy		= Local_RemoveLastListItems( nRemove, sPathCopy, ":" )	// ..This  CREATES the directory on the disk
...
static Function	/S	Local_RemoveLastListItems( cnt, sList, sSep ) 
...
static Function		Local_ReplaceModuleNamesFromLst( sSrcTxt, sReplaceTxt, sSrcDir, sTgtDir, lstMatch ) 
...
// If the file name starts with 'sSrcTxt'  (e.g. 'UFCom_')  then also rename the file by replacing 'sSrcTxt'  by  'sReplaceTxt'  (e.g. 'UFSec_'  or  'UFFPu_') .  This is for  COMMON  files.
	...
	for ( n = 0; n < nitems; n += 1 )
		...
		Local_ReplaceModuleNames( sSrcTxt, sReplaceTxt, sSrcDir, sTgtDir, sMatch ) 
	endfor
End

static Function		Local_ReplaceModuleNames( sSrcTxt, sReplaceTxt, sSrcDir, sTgtDir, sMatch ) 
// Copy all files which match  'sMatch' (e.g. '*.ipf')  from  'sSrcDir'  into  'sTgtDir'   after having  replaced  'sSrcTxt'  (e.g. 'UFCom_')  by  'sReplaceTxt'  (e.g. 'UFSec_'  or  'UFFPu_') .  This is for COMMON and specific files.
// If the file name starts with 'sSrcTxt'  (e.g. 'UFCom_')  then also rename the file by replacing 'sSrcTxt'  by  'sReplaceTxt'  (e.g. 'UFSec_'  or  'UFFPu_') .  This is for  COMMON  files.
	...
	variable	n, nCnt		= ItemsInList( lstMatched )
	if ( nCnt )
		Local_PossiblyCreatePath( sTgtDir )					// Design issue: do not create directory if there are no replacements to make  
	...
	for ( n = 0; n < nCnt; n += 1 )
		...
		Local_ReplaceStringInFile( sSrcTxt, sReplaceTxt, sSrc, sTgt ) 
	endfor  
End

static Function		Local_ReplaceStringInFile( sSrcTxt, sReplaceTxt, sSrcPath, sTgtPath ) 
// Read  file  'sSrcPath' ,  replace  'sSrcTxt'  by  'sReplaceTxt'   and  store in file  'sTgtPath' .   'sSrcPath'  and  'sTgtPath'  may be the same.   
	...
	if ( nRefNum != 0 )									// file could be missing  or  user cancelled file open dialog
		do 											// ..if  ReadPath was not an empty string
			...
			if ( ! bIsInPicture )
				if ( ( cmpstr( Local_FirstWord( sLine ), "PICTURE" ) == 0  )  ||  ( cmpstr( Local_FirstWord( sLine ), "STATIC" ) == 0  &&  cmpstr( Local_SecondWord( sLine ), "PICTURE" ) == 0   ) )				
					bIsInPicture = TRUE
				endif
			endif
			if ( bIsInPicture )
				if ( ( cmpstr( Local_FirstWord( sLine ), "END" ) == 0  ) )
					bIsInPicture	= FALSE
				endif
			endif
...
static Function	/S	Local_FirstWord( sLine )
	string  	sLine
	string  	sWord
...
static Function	/S	Local_SecondWord( sLine )
	string  	sLine
