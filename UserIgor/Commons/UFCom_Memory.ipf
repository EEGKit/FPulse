...
static constant	MBYTE = 0x100000
...
constant			UFCom_kBYTE8 = 0,  UFCom_kUBYTE8 = 1,  UFCom_kINT16 = 2,  UFCom_kUINT16 = 3,  UFCom_kINT32 = 4,  UFCom_kUINT32 = 5,  UFCom_kREAL32 = 6,  UFCom_kDOUBLE64 = 7  
static strconstant	lstNUMTYP			= "Byte8;UByte8;Int16;UInt16;Int32;UInt32;Real32;Double64;"
static strconstant	lstNUMTYP_MEM		= "1;1;2;2;4;4;4;8;"
static constant		bPRINT_ALL_MAKES 	= 0//1			// for debugging : will print all memory allocations for waves ( only those called over 'MakeN()'  )
...
// Ensures that a sufficiently large 1-dimensional  number wave exists. The wave will be created or will grow, but will never shrink.  Returns 0 when successful  or -1 when failing.
	variable	n0, InitVal
	variable	nErrorLevel			// determines action in case of error: UFCom_kERR_FATAL will stop program by opening dialog box.  ONLY FOR TESTING : UFCom_kERR_IMPORTANT (and less) allows proceeding 
...
constant		UFCom_kMEM_GRANULARITY	= 50000		// No attempt is made to measure the memory more precise than this. Do not make this smaller than 200K because each computation step may take as long as 1 minute (if virtual memory is accessed) 
...
// 								 After a reboot virtual memory seems to be accessed				 :  960 MB but computation time ~ 5 minutes (and the disk heavily working). 
	...
	variable	nMemOK 		= 0
	variable	nVirtualMem	= str2num( StringByKey( "FREEMEM", IgorInfo( 0 ) ) )			// or   = UFCom_UtilAvailVirtual()
	...
	nAvailVirtual	= UFCom_UtilAvailVirtual()	//
	nTotalPhys	= UFCom_UtilTotalPhys()	//
	nAvailPhys	= UFCom_UtilAvailPhys()	//
	...
	do
		...
		bMemFail	= UFCom_UtilContiguousMemory( nMem )				// returns 0 on success
		if ( bMemFail )
			nMemFail = nMem
		else
			nMemOK = nMem
	...
	while ( nMemFail - nMemOK > UFCom_kMEM_GRANULARITY  &&  !bIsVirtualMemory   &&  cnt < 30 )					// emergeny bail out 

	if ( strlen( sTxt ) )
		string  	sText			= SelectString( bIsVirtualMemory, "- Probably in physical memory: OK" , "- In virtual memory, not useful ! " ) 
		printf "\t\tBiggestContiguousMemory(\t%s\t)  Gran: %d\tIters:%3d\tAlloc:\t%7.2lf\tMB\tPhysAv: %7.2lf\t/%7.2lf\tVirtAv: %7.2lf \t%s\t \r",  UFCom_pd( sTxt, 19), UFCom_kMEM_GRANULARITY, cnt, nMemOK/MBYTE, nAvailPhys/MBYTE, nTotalPhys/MBYTE, nAvailVirtual/MBYTE, sText 
	endif
