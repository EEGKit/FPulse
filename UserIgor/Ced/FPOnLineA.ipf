...
Static  Function		RemoveAnalysisWnd()
//// Construct and display  1 additional  Analysis windows with the next default name
	variable	wCnt		= ItemsInList( AnalysisWindows() ) 
	if ( wCnt )
		string 	sWNm	= AnalWndNm( wCnt-1 )
		KillWindow $sWNm
		string  	sFolder		= ksACOld
		string  	sPnOptions	= ":dlg:tPnOLADisp" 
		InitPanelOLADisp( sFolder, sPnOptions )								// necessary to display the changed panel state immediately
...
Function		OnlineAnalysis_( sFolder, wG, wIO, pr, bl, fr )
	...
	if (  AnalysisIsOn() )							// If no cUSER region is defined we skip the whole analysis (this must be refined if there are analysis types requiring no cUSER region)
		...
		if ( pr == 0  &&  bl == 0  &&  fr == 0 )			// Insert a pause for every new protocol by  inserting a Nan rather than data in the display trace.
			gOLAFrmCnt	+= 1	
			InsertNanInDisplayWave( gOLAFrmCnt )	// Inserting a Nan rather than data in the display trace marks those points as separators and allows blanking out intervals between protocols..
		endif													
		gOLAFrmCnt	+= 1	
		OnlineAnalysis1Point( sFolder, wG, wIO, pr, bl, fr, gOLAFrmCnt )								
	endif
End												


static Function		InsertNanInDisplayWave( index )
// Fill  display  wave with 1 Nan point to blank out the data point.
	variable	index
	variable	nP, rnType, n, nItems = RegionCnt()
	...
	for ( n = 0; n <  nItems; n += 1 )							// For all  channel/region combinations  which the user has defined 
		RegionSrcType( n, rsSrc, rnType )					// passed parameters are changed

		lstOLANm	= OlaNmLst( rsSrc, rnType, lstRESreg )			//  e.g. 'Adc1Base' , 'Adc0PkBg' 
		for ( nP = 0; nP < ItemsInList( lstOlaNm ); nP += 1 )
			...
			NewElementOlaDisp( sOLANm, index ) 				// Returns TRUE if a new wave has just been constructed.
		 	SetElementOlaDisp( sOlaNm, index, Nan )				// Nan allows blanking out intervals between protocols.
		endfor		
	endfor		
End
...
static Function	OnlineAnalysis1Point( sFolder, wG, wIO, pr, bl, fr, index )
...
variable	BegPt		= FrameBegSave( sFolder, pr, bl, fr )		// could this simplify BResultXShift  ???? (see 070424)
	...
	for ( n = 0; n <  nRegions; n += 1 )								// For all  channel/region combinations  which the user has defined 
		...
		RegionSrcType( n, sSrc, nType )									// passed parameters are changed
	  	sResultNm		= ResultNm( sSrc )								// e.g. 'ResAdc0' , 'ResPoN1'
		...
		lstOlaNm	= OlaNmLst( sSrc, nType, lstRESreg )						//  e.g. 'Adc1Base' , 'Adc0PkBg' . Used for the keywords in the file and for the result wave names
		for ( nP = 0; nP < ItemsInList( lstOlaNm ); nP += 1 )
			sOlaNm	= StringFromList( nP, lstOlaNm )
			NewElementOlaDisp( sOLANm, index ) 						//  Returns TRUE if a new wave has just been constructed.
		endfor		
	...
	for ( n = 0; n <  nRegions; n += 1 )									// For all defined regions
		RegionSrcType( n, sSrc, nType )
	
		variable	Gain		= GainByNmForDisplay_( wIO, sSrc )
	  	sResultNm			= ResultNm( sSrc )						// e.g. 'ResBase' , 'ResPeakUp'
...
// 060511b
		elseif ( nType == rgBAS3 )
			wResult[ nType ][ 0 ]	= BaseVal( wIO, sSrc, nType,  BegPt, nSmpInt / kXSCALE, Gain )
		elseif ( nType == rgPEA3 )
			wResult[ nType ][ 0 ]	= PeakVal( wIO, sSrc, nType, BegPt, nSmpInt / kXSCALE, Gain, wResult[ rgBAS3 ][ 0 ] ) - wResult[ rgBAS3 ][ 0 ] 
		...
		lstOlaNm	= OlaNmLst( sSrc, nType, lstRESreg ) 												// e.g. Adc0Base  or  PoN1DcA0; PoN1DcT0;PoN1DcCo;			
		string  	sOnePair
		variable	nPars		= ItemsInList( lstOlaNm )	
		for ( nP = 0; nP < nPars; nP += 1 )
			sOlaNm	= StringFromList( nP, lstOlaNm )
		 	SetElementOlaDisp( sOlaNm, index, wResult[ nType ][ nP ] )									// The  RESULT value is  always  added to the display wave. 
			if ( nPars == 1 )	// i.e. nP == 0
				sprintf  sOnePair, "%s%s%12.4lf" , sOLANm, ksSEP_EQ,  wResult[ nType ][ nP ]				// Base, Peak, Quot... all add just 1 value 
			else
				sprintf sOnePair, "%s%s%s%12.4lf", sOlaNm[ 0,5 ], ParName( nFitFunc, nP), ksSEP_EQ, wResult[ nType ][ nP ]	// xxxxDcay -> xxxxDc  so that  8 letters are generated e.g. Adc0DcT0  or PoN2DcA1 
			endif
			sLine		= AddListItem( sOnePair, sLine, ";" , Inf )		//  add the result to the value string which is written to the data file (was AddToFile()..)
		endfor

		sLine += AddToFileBegOrEnd( sSrc, nType, GetReg( sSrc, nType, cUSER, RLFT), lstRGBEG )	// But only if there is a name entry in the region  Beginning / End list this is added to the file. 
		sLine += AddToFileBegOrEnd( sSrc, nType, GetReg( sSrc, nType, cUSER, RRIG ), lstRGEND)	// This avoids storing meaningless region beginnings and ends e.g. for  'PQuot'					
	endfor
...
Static  Function		RectangularIndex( wIO, n )
...
Static  Function	/S	DefinedRegions()
// Returns a list containing all those regions which the user has specified . Needed to supply the appropriate list for the OLA analysis panel to be built .
	...
	string  	sSrc
	variable	nType
	variable	n, nItems	= RegionCnt()
	for ( n = 0; n < nItems; n += 1 )
		RegionSrcType( n, sSrc, nType )
		sList	= PossiblyAddListItem( LongSrcRegNm( sSrc, nType ), sList )
	endfor	
...
Static  Function	/S	DefinedPkRegions()
// Returns a list containing all those regions which the user has specified . Needed to supply the appropriate list for the OLA analysis panel to be built .
	string  	sList	= ""
	string  	sSrc
	variable	nType
...
Static  Function	/S	DefinedRsRegions()
// Returns a list containing all those regions which the user has specified . Needed to supply the appropriate list for the OLA analysis panel to be built .
	string  	sList	= ""
	string  	sSrc
	variable	nType
...
Static  Function	/S	DefinedDcRegions()
// Returns a list containing all those regions which the user has specified . Needed to supply the appropriate list for the OLA analysis panel to be built .
	string  	sList	= ""
	string  	sSrc
	variable	nType
...
Static  Function	/S	ActiveResults()
...
// Needed to supply the appropriate list of results for the OLA analysis panel to be built not with all but only with possible result items.
	...
	for ( n = 0; n < nItems; n += 1 )
		RegionSrcType( n, sSrc, nType )
		sList	= PossiblyAddListItem( Ola_NmLst( sSrc, nType, lstRESreg ), sList )	// Displays names with spaces in the panel. The spaces are automatically removed when referencing variables, waves or controls
	endfor	
...
Static  Function  /S	AddToFileBegOrEnd( sSrc, nType, value, sList )
// Converts 'value'  into a key-value-pair string entry to be added to the result file  and returns this string.  By ignoring this  return string  we do not add  to file .
	...
	if ( strlen( RemoveWhiteSpace( StringFromList( nType, sList ) ) ) )						// Only if there is a name entry in the region  beginning/end  list...
	  	string  	sOLANm	= Ola1Nm( sSrc, nType, sList ) 							// 
...
Static  Function	/S	Ola1Nm( sSrc, nType, sList )
...
Static  Function	/S	OlaNmLst( sSrc, nType, sList )
...
// Most regions (e.g. Base, Peak)  have 1 entry = 1 line,  but some regions ( e.g. Fits ) have multiple entries = multiple lines.
	...
	variable	nType
// 040730
	return	OLAList( sSrc, nType, sList, "" )						// include no space in the name	   'Adc1' + 'Base'   ->   'Adc1Base' 
End


Static  Function	/S	Ola_NmLst( sSrc, nType, sList )
...
// Most regions (e.g. Base, Peak)  have 1 entry = 1 line,  but some regions ( e.g. Fits ) have multiple entries = multiple lines.
	string		sSrc, sList 
	variable	nType
	return	OLAList( sSrc, nType, sList, " " )						// include a space in the name for improved readability
End

Static  Function	/S	OLAList( sSrc, nType, sList, sSep )
...
// Most regions (e.g. Base, Peak)  have 1 entry = 1 line,  but some regions ( e.g. Fits ) have multiple entries = multiple lines.
	...
	if ( nType == rgRISE  ||  nType == rgDECAY )
		...
		sParamNames		= ParamNames( nFitFncIndex )
...
Static  Function	/S	LongSrcRegNm( sSrc, nReg )
...
Static  Function	/S	ResultNm( sSrc )
	string		sSrc
	return	"Res" + sSrc						// returning  'sSrc'  alone crashes Igor ! ( Name conflict with acq wave names?)
End

Static  Function	/S	CrossXNm( sSrc, nReg, nStg )
	string		sSrc
	variable	nReg, nStg
...
Static  Function	/S	CrossYNm( sSrc, nReg, nStg )
	string		sSrc
	variable	nReg, nStg
...
Static  Function	/S	FittedNm( sSrc, nReg, nStg )
	string		sSrc
	variable	nReg, nStg
...
static Function		BaseVal( wIO, sSrc, nType, nBegPt, SIFact, Gain )
...
// cFINAL result  box :	X = user defined box,	Y = the evaluated base value
	...
	variable	PtLeft	= nBegPt + GetReg( sSrc,  nType, cUSER,  RLFT ) / SIFact
	variable	PtRight	= nBegPt + GetReg( sSrc,  nType, cUSER,  RRIG ) / SIFact
//	wave   	/Z	w = $sSrc
	wave   	/Z	w = $ksROOTUF_ + ksACOld_ + ksF_IO_ + sSrc
// printf "\t\tBaseVal( %s\t) waveexists:%d   w1:%d \r", sSrc, waveExists(w), waveExists(w1)
	...
	// Draw cross
	SetRegLoc( wIO, sSrc, nType, cFINAL, ( PtLeft   - nBegPt ) * SIFact ,  value , ( PtRight - nBegPt ) * SIFact,  	value )	// BroadenY makes a (usually tiny) vertical cross line which is an indicator of the average noise
...
static Function		PeakVal( wIO, sSrc, nRTyp, nBegPt, SIFact, Gain, BaseValue )
// Return the minimum or maximum   (or the larger of both)  Y value of the wave in the given interval. Average over some points to the right and to the left of the peak to reduce noise influence.
	...
	variable	PtLeft		= nBegPt + GetReg( sSrc,  nRTyp, cUSER,  RLFT ) / SIFact
	variable	PtRight		= nBegPt + GetReg( sSrc,  nRTyp, cUSER,  RRIG )  / SIFact
	...
	variable	nPkDir	= GetReg( sSrc, nRTyp, cUSER, RMODE )
	nPkDir	= numType( nPkDir ) == kNUMTYPE_NAN ?  cPKDIRUP : nPkDir	// if the user defines a peak region and starts an acq without first having opened the OLA panel and without...
	...
	// Draw cross
	SetRegLoc( wIO, sSrc, nRTyp, cFINAL, PkTimeFound,   PkValue, PkTimeFound,  PkValue )
...
static Function		RTimVal( wIO, sSrc, nType, nBegPt, SIFact, Gain, lowPercent, highPercent , base, peakFromExtraPeak )
...
// Flaw: if region does not include 80 (or20%) point, NO peak is found. If regions includes both 80% points on both sides of peak the false 80% point may be taken. Solution: start exactly at peak............
	...
	variable		PtLeft	= nBegPt + trunc( GetReg( sSrc,  nType, cUSER,  RLFT ) / SIFact ) 	// truncation is necessary because the following rise time detection expects integers
	variable		PtRight	= nBegPt + trunc( GetReg( sSrc,  nType, cUSER,  RRIG )  / SIFact )
	variable 		n
	wave   	/Z	w = $ksROOTUF_ + ksACOld_ + ksF_IO_ + sSrc
	...
	SetRegLoc( wIO, sSrc, nType, cFINAL, ( Pt20   - nBegPt ) * SIFact ,  y20, ( Pt80 - nBegPt ) * SIFact,  y80 )
...
static Function		PQuotVal( sSrc, nRTyp, Peak1Value, Peak2Value )
	string 	sSrc
...
static Function		RiseOrDecayFit( wResult, sSrc, nRegTyp, nBegPt, SIFact, Gain, lstActualFit )
...
// Display the fitted function
	...
	string  	sMsg
	variable	Left		= GetReg( sSrc,  nRegTyp, cUSER,  RLFT ) 
	variable	Right		= GetReg( sSrc,  nRegTyp, cUSER,  RRIG ) 
	variable	PtLeft	= nBegPt + Left 	  / SIFact
	variable	PtRight	= nBegPt + Right / SIFact
//	wave   	/Z	w = $sSrc
	wave   	/Z	w = $ksROOTUF_ + ksACOld_ + ksF_IO_ + sSrc
	
	variable	nRadFitFunc		 = GetReg( sSrc, nRegTyp, cUSER, RMODE )			// indexing of the radio buttons in the panel  is 0,1,2.. . This conversion could also be done earlier in 
	...
	if ( nFitFunc != FT_NONE )
		string  	sFittedNm	= FittedNm( sSrc, nRegTyp, cFINAL )
		string  	sFoFittNm	= FolderFittedNm( FittedNm( sSrc, nRegTyp, cFINAL ) )
		...
		SetStartParams( wPar, w, PtLeft, PtRight, SIFact, nFitFunc )						// stores results in  wPar
		...
		PrintFitResults( wPar, wStartPar, nRegTyp, nFitFunc, V_FitError, V_FitNumIters, V_FitMaxIters, V_chisq )
...
static Function		SetStartParams( wPar, w, PtLeft, PtRight, SIFact, nFitFunc )
	wave  	/D wPar
	wave	w
...
static Function		PrintFitResults( wPar, wStartPar, nRegTyp, nFitFunc, FitError, nIter, maxIter, chisq )
	...
	if ( TRUE)//gPrintMask &  RP_FIT )
		...
		for ( n = 0; n < ItemsInList( ParamNames( nFitFunc ) ); n += 1 )
			sFitResults += ParName( nFitFunc, n ) + ": " + num2str( wPar[ n ] ) + "   "
			sStartPars  += ParName( nFitFunc, n ) + ": " + num2str( wStartPar[ n ] ) + "   "
		endfor
...
// extracts region coordinates from mouse position,  does NOT use IGORs Marquee values
	...
	if ( isMouseUp  &&   ( It_Is( modifier, STOREBASE )  ||  It_is( modifier, STOREPEAK ) ) )
	  	gR2x	= CursX
	  	gR2y	= CursY
		nType	= RegionTypeFromModifier( modifier )
...
static Function		SetRegLoc( wIO, sSrc, nReg, nStg, left, top, right, bot )
// HERE the region string is built
	...
	variable	nIndex	= Index( sSrc, nReg )					// the line in the text wave, e.g.  Adc0Peak    or   Adc1Base
	...
	if ( nIndex == kNOTFOUND )
		...
		wtAnRg[ nIndex ]	= Identifier( sSrc, nReg ) + ";"
		for ( nst = cUSER; nst < MAXREG_STAGE; nst+= 1 )
			wtAnRg[ nIndex ]	+= StringFromList( nst, lstRGSTAGE ) + sSeps 
		endfor

		for ( nst = cUSER; nst < MAXREG_STAGE; nst+= 1 )
			nPos	 = 1 + nst * ( MAXREG_ITEMS ) + RRED ;	wtAnRg[ nIndex ] 	= ReplaceListItem( DefaultRegColor( nReg, nst, RRED ),	wtAnRg[ nIndex ] , ";",  nPos ) 
			nPos	 = 1 + nst * ( MAXREG_ITEMS ) + RGRN ;	wtAnRg[ nIndex ] 	= ReplaceListItem( DefaultRegColor( nReg, nst, RGRN ),	wtAnRg[ nIndex ] , ";",  nPos ) 
			nPos	 = 1 + nst * ( MAXREG_ITEMS ) + RBLU ;	wtAnRg[ nIndex ] 	= ReplaceListItem( DefaultRegColor( nReg, nst, RBLU ),	wtAnRg[ nIndex ] , ";",  nPos ) 
			nPos	 = 1 + nst * ( MAXREG_ITEMS ) + RSHP ;	wtAnRg[ nIndex ] 	= ReplaceListItem( DefaultRegShape( nReg, nst ), 		wtAnRg[ nIndex ] , ";",  nPos )	// default setting for the region shape
...
//			nvar		tmp	= $sRadButVarNm 
	...
	nShape	= str2num( DefaultRegShape( nReg, nStg ) )
	...
	if ( nShape != kHIDDEN )
		string  	sCrossXNm	= CrossXNm( sSrc, nReg, nStg )
		string  	sCrossYNm	= CrossYNm( sSrc, nReg, nStg )
...
static  Function		SortRegions( wIO )
	wave  /T	wIO
	wave  /T	wtAnRg	= root:uf:aco:ola:wtAnRg
	...
	for	( n = 0; n < nItems ; n += 1 )
		wSortkey[ n ]	= RectangularIndex( wIO, n )			// Fill the sortkey wave (according to which we will sort)  with the indices computed from  PossibleIOChannels x MAXREG (containing gaps)
...
static Function   /S	DefaultRegColor( nType, nStage, nColor )
...
static Function    /S	DefaultRegShape( nType, nStage )
...
static  Function		SetRegion( sSrc, nReg, nStg, nRgItem, value )
	string  	sSrc	
...
static Function		UnDefineReg( sSrc,  nType )
	string  	sSrc
	variable	nType
	wave  /T	wtAnRg	= root:uf:aco:ola:wtAnRg
	variable	n, nItems	= numPnts( wtAnRg )
	variable	nIndex	= Index( sSrc, nType )
	if ( nIndex == kNOTFOUND )
...
static  Function	bBothRegAreDefined( sSrc, nType )
	string  	sSrc
	variable	nType
	return	Index( sSrc, nType ) != kNOTFOUND
End


static  Function		GetReg( sSrc, nType, nStg, nItem )
	string  	sSrc
	variable	nType, nStg, nItem
	wave  /T  	wtAnRg = root:uf:aco:ola:wtAnRg
	variable	nItems	= numPnts( wtAnRg )
	variable	nIndex	= Index( sSrc, nType )	// the line in the text wave, e.g.  Adc0Peak    or   Adc1Base
...
static Function		GetRgColor( sSrc, nType, nStage, rnRed, rnGreen, rnBlue )
	string  	sSrc 
	variable	nType, nStage, &rnRed, &rnGreen, &rnBlue
	rnRed	= GetReg( sSrc, nType, nStage, RRED )
	rnGreen	= GetReg( sSrc, nType, nStage, RGRN )
	rnBlue	= GetReg( sSrc, nType, nStage, RBLU )
End


static Function		GetRgLoc( sSrc, nType, nStage, rnLeft, rnTop, rnRight, rnBot )
	string  	sSrc 
	variable	nType, nStage
	variable	&rnLeft, &rnTop, &rnRight, &rnBot			
	rnLeft	= GetReg( sSrc, nType, nStage, RLFT )
	rnTop	= GetReg( sSrc, nType, nStage, RTOP )
	rnRight	= GetReg( sSrc, nType, nStage, RRIG )
	rnBot  	= GetReg( sSrc, nType, nStage, RBOT )
End


static Function		GetRegion( rnLeft, rnTop, rnRight, rnBot )
	variable	&rnLeft, &rnTop, &rnRight, &rnBot
	GetMarquee left, bottom
	if ( V_Flag )											// if a marquee is active...
		rnLeft	= V_left
...
static Function		Index( sSrc, nType )
	string  	sSrc	
	variable	nType
	wave  /T	wtAnRg	= root:uf:aco:ola:wtAnRg
	variable	n, nItems	= numPnts( wtAnRg )
	for ( n = 0; n < nItems; n += 1 )
		if ( cmpstr( StringFromList( 0, wtAnRg[ n ] ), Identifier( sSrc, nType ) )  == 0 )	// this trace-type-combination exists already
...
static Function  /S	Identifier( sSrc, nType )
	string  	sSrc	
	variable	nType
...
static Function		DispClearOneTrcOneTyp( sSrc, nType, nStage )
	...
	for ( w = 0; w <  wCnt; w += 1 )
		...
		for ( t = 0; t < tCnt; t += 1 )						// all traces contained in this window sWnd
			...
			if ( cmpstr( sTNm, sSrc ) == 0 )
				...
				if ( nStage != ALLSTAGES )
					//DisplayRegion( sWNm, sSrc, nType, nStage, nMode, nRange ) 
					DisplayRegion( sWNm, sSrc, nType, nStage, nRange ) 
				else
					variable	stg
					for (  stg = 0;  stg <  MAXREG_STAGE;  stg += 1 )
						//DisplayRegion( sWNm, sSrc, nType, stg, nMode, nRange ) 
						DisplayRegion( sWNm, sSrc, nType, stg, nRange ) 
					endfor
				endif
...
Static  Function		DisplayRegion( sWnd, sSrc, nReg, nStg, nRange )
...
//	variable	nMode								// clear it  or  draw it.	
	...
	GetRgColor( sSrc, nReg, nStg, rRed, rGreen, rBlue )
	GetRgLoc( sSrc, nReg, nStg, rLeft, rTop, rRight, rBot )

	nShape = GetReg( sSrc, nReg, nStg, RSHP )
	...
	if ( nShape == kFITTED ) 
		sTNL				= TraceNameList( sWnd, ";", 1 )
		string  	sFittedNm		= FittedNm( sSrc, nReg, nStg )
		...
		if (  waveExists( wFittedSegment ) )
			...
			printf "DisplayRegion() : fitted waves  '%s'    do not exist  \r ",  FolderFittedNm( sFittedNm )	// e.g. Base, Peak, Quot
		endif
	endif
End
...
Function		DurAcqDrawRegion( sWNm, sSrc, nRange ) 
	...
	for ( nType = 0; nType <  MAXREG_TYPE; nType += 1 )
		if ( bBothRegAreDefined( sSrc, nType ) )
			for ( nStage = 0; nStage <  MAXREG_STAGE; nStage += 1 )
...
Function		InitPanelOLADisp( sFo, sPnOptions )
	...
	n = PnControl(	tPn, n, 1, ON, 	"PN_CHKBOX", kVERT, 				"Regions",			"root:uf:aco:ola:cbReg",	DefinedRegions(),	"Regions", 	"" , kWIDTH_NORMAL, sFo ) 
	//n = PnControl(	tPn, n, 2, ON, 	"PN_RADIO",	 kVERT,   				"Peak directions",		"root:uf:aco:ola:PkDir",	lstRGPKDIR,		"PeakRegions",   DefinedPkRegions() , kWIDTH_NORMAL, sFo )	 // VERTICAL arrangement
	n = PnControl(	tPn, n, 2, ON, 	"PN_RADIO",  ItemsInList(lstRGPKDIR),	"Peak directions",		"root:uf:aco:ola:PkDir",	lstRGPKDIR,		"PeakRegions",   DefinedPkRegions() , kWIDTH_NORMAL, sFo ) // HORIZONTAL arrangement
//	n = PnControl(	tPn, n, 2, ON, 	"PN_RADIO",  ItemsInList(lstRGPKDIR),	"Average peak over ms",	"root:uf:aco:ola:PkAvMs",	lstRGPKDIR,		"PeakRegions",   DefinedPkRegions() , kWIDTH_NORMAL, sFo ) // HORIZONTAL arrangement

	// better realisation (more than 3..4 choices)  requires popup
	n = PnControl(	tPn, n, 2, ON, 	"PN_RADIO",  ItemsInList(lstRGRISEFIT),	"Rise fit",				"root:uf:aco:ola:RsFit",	lstRGRISEFIT,		"RiseRegions",    DefinedRsRegions() , kWIDTH_NORMAL, sFo ) // HORIZONTAL arrangement
	n = PnControl(	tPn, n, 2, ON, 	"PN_RADIO",  ItemsInList(lstRGDECAYFIT),"Decay fit",			"root:uf:aco:ola:DcFit",	lstRGDECAYFIT,	"DecayRegions", DefinedDcRegions() , kWIDTH_NORMAL, sFo ) // HORIZONTAL arrangement
	...
	//n = PnControl(	tPn, n, 2, OFF, 	"PN_CHKBOX", kVERT,				"Display Results in window","root:uf:aco:ola:wa",		AnalysisWindows(),	"Results", 		  ActiveResults() , kWIDTH_NORMAL, sFo ) 	// VERTICAL arrangement
	n = PnControl(	tPn, n, 2, OFF, 	"PN_CHKBOX", ItemsInList(AnalysisWindows()),	"Display Results in window","root:uf:aco:ola:wa",  	AnalysisWindows(),	"Results", 		  ActiveResults() , kWIDTH_NORMAL, sFo )	// HORIZONTAL arrangement
...
// Sample  action procedure  for 2dimensional   PnControl()  with Radio buttons 
	...
	SetRegion( rsSrc, rRTyp, cUSER, RMODE, nDirValue )
End
...
// Sample  action procedure  for 2dimensional   PnControl()  with Radio buttons 
	...
	SetRegion( rsSrc, rRTyp, cUSER, RMODE, nRadioIndex )	// The index of the radio button in the panel 0,1,2..  is stored.  One could also convert right here to the actual fit functions (see lstACTUALDECAYFIT)
	string  	sFolder		= ksACOld
	string  	sPnOptions	= ":dlg:tPnOLADisp" 
	InitPanelOLADisp( sFolder, sPnOptions )						// necessary to display the changed panel state immediately
...
//	 printf "\tProc (CHECKBOX)\tcontrol name (  '%s'  \tbValue: %d )    \t'%s' \texists: %d \tTrace '%s' \t in Wnd '%s'   is %s \r", sControlNm, bValue, sTrcOnNm, nvar_Exists( bTrcOn ), rsOLANm, rsWNm, SelectString( bTrcOn, "OFF" , "ON" )
	...
	DrawAnalysisWndTrace( rsWNm, rsOLANm )
	DrawAnalysisWndXUnits( rsWNm )	
End
...
static Function		RedrawAnalysisWndAllTraces()
	variable	w, wCnt		= ItemsInList( AnalysisWindows() )
	for ( w = 0; w < wCnt;  w += 1 )
		string  	sWNm	= StringFromList( w, AnalysisWindows() )
		if ( WinType( sWNm ) == kGRAPH )
...
// MUST HAVE SAME NAME AS VARIABLE
	...
	variable	nP, t , tCnt	= RegionCnt()	

	if ( ! bValue )														// Fill the pauses = connect data points even during pauses....
		for ( t = 0;  t < tCnt; t += 1 )	
			RegionSrcType( t, sSrc, nType )									// passed parameters are changed
...
Static  Function		DrawAnalysisWndTrace( sWNm, sOLANm )
	string  	sWNm, sOLANm
	variable	rnType
	...
	if ( WhichListItem( sOLANm, TraceNameList( sWNm, ";", 1 ) )  !=  kNOTFOUND )	
		...
		OLANmR( sOLANm, rsSrc, rnType, lstRESreg )						// The trace does not yet exist : use program supplied default colors and mode (called when first building the trace)
...
// Flaw: If the user checks this checkbox before the 1. acq then the analysis results will not automatically be displayed. For this to happen the user must again uncheck/check the button after acq start.
...
//	if ( bTrcOn )
//		string  	sEvntSecMin	= StringFromList( nXAxis, lstRESfix_ )
//		wave   	wX			= $FolderOlaDispNm( sEvntSecMin )			//  X  AXIS  is  frames, seconds or minutes
//		AppendToGraph /W=$sWNm	wOlaDisp vs  wX
//		ModifyGraph	/W=$sWNm	rgb( $sOLANm )		= ( Red, Green, Blue ) 
//		ModifyGraph	/W=$sWNm	mode( $sOLANm )	= nMode 			// 4: connect and mark points with +, 3 : only markers +
//	
...
static Function		DrawAnalysisWndXUnits( sWNm )
	string  	sWNm
	nvar		nXAxis	= root:uf:aco:ola:radXAxis
