...
Function		Grow1( n0, sWvNm, InitVal, nErrorLevel )
...
// Ensures that a sufficiently large 1-dimensional  number wave exists. The wave will be created or will grow, but will never shrink.  Returns 0 when successful  or -1 when failing.
	...
	if ( numPts < n0 )
		...
		Alert( nErrorLevel, sMessage )
		return	kNOTFOUND
	endif
		printf "Grow1() : \t%s\t ( should have at least \t   %12d \tpoints, has now \t   %12d \t ) is OK.\r", pd( sWvNm,33), n0, numPts
	return	0
End 

Function		Grow4( n0, n1, n2, n3, sWvNm, InitVal, nErrorLevel )
// Attempt to avoid memory fragmentation
// Ensures that a sufficiently large 1-dimensional  number wave exists. The wave will be created or will grow, but will never shrink.  Returns 0 when successful  or -1 when failing.
	...
	if ( numPts0 < n0 )
		...
		printf "Grow4() : \t%s\t ( should have at least \t%8d\t%8d\t%8d\t%8d\tpoints, has now \t%8d\t%8d\t%8d\t%8d\t\t ) is OK.\r", pd( sWvNm,33), n0, n1, n2, n3, numPts0, numPts1, numPts2, numPts3
	return	0
End 


Function		Make1( sWvNm, n0, nType, InitVal, bOverwrite, nErrorLevel )
// Constructs  1-dimensional  number wave.  Returns 0 when successful  or -1 when failing.
	...
	if ( bPRINT_ALL_MAKES )
		printf "\t\t\tMake1 has constructed\t%s\t( %s,\tpts:\t%10d\t = %.3lf MB ) \r", pd(sWvNm,25),  StringFromList( nType, lstNUMTYP), n0, n0 * str2num( StringFromList( nType, lstNUMTYP_MEM) ) / 1024 /1024
	endif
	return	0
End 
...
Function		Make4( sWvNm, n0, n1, n2, n3, nType, InitVal, bOverwrite, nErrorLevel )
...
Function		BiggestContiguousMemory( sTxt )
...
//		bMemFail	= xUtilContiguousMemory( nMem )				// returns 0 on success
	...
	if ( strlen( sTxt ) )
		...
		printf "\t\tBiggestContiguousMemory(\t%s\t)  Gran: %d\tIters:%3d\tAlloc:\t%7.2lf\tMB\tPhysAv: %7.2lf\t/%7.2lf\tVirtAv: %7.2lf \t%s\t \r", pd( sTxt, 19), kMEM_GRANULARITY, cnt, nMemOK/MBYTE, nAvailPhys/MBYTE, nTotalPhys/MBYTE, nAvailVirtual/MBYTE, sText 
	endif
