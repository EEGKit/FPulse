...
//	variable	/G	gbDisplayAllPts	= 1		// displaying every point AFTER ACQ without data decimation can be slow with MB waves 
...
Function	  	DisplayRawAftAcq( sFolder, wG, wIO, wFix )
...
// too slow when every point is checked (whether it lies within or outside a SAVE period) and drawn: decimating to 'nDrawPts'
	...
	wave  /T	wIO
	wave	wG, wFix
	nvar		gbDisplayAllPtsAA	= $"root:uf:" + sFolder + ":dlg:gbDisplayAllPtsAA"
...
Function	DisplayOffLine( sFolder, wG, wIO, wFix, nWndBeg, nWndEnd )
	...
	for ( b  = 0; b < eBlocks( wG ); b += 1 )
		...
		for ( f = 0; f < fCnt; f += 1 )
			...
			for ( s = 0; s < sCnt; s += 1 )
				DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, s, 0,   kWNDDIVIDER - 1, nWndBeg, nWndEnd )		// 031008 p=0 : not really protocol aware
			endfor
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_FRAME,  0,	kWNDDIVIDER - 1, nWndBeg, nWndEnd )	
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_PRIM,	    0,	kWNDDIVIDER - 1, nWndBeg, nWndEnd )		
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_RESULT, 0, kWNDDIVIDER - 1, nWndBeg, nWndEnd )	
		endfor
	endfor
End
...
Function		LoadDisplayCfg( sFolder,  sFileName ) 
	...
	for ( nIO = 0; nIO < kIO_MAX; nIO += 1 )
		...
		for ( c = 0; c < cCnt; c += 1 )
			...
			for ( row = 0; row < nRows; row += 1 )
				...
				GetAutoWindowCorners( row, nRows, col, nCols, rnLeft, rnTop, rnRight, rnBot, 0, kWNDDIVIDER )
...
Function		DispDuringAcqCheckLag( sFolder, wG, wIO, wFix, pr, bl, fr, sw )		// partially  PROTOCOL  AWARE
	string  	sFolder
	wave  /T	wIO
	wave  	wG, wFix
	...
	if ( gbDispAllDataLagging  ||  nLag < cLAG  ||  fr + sw == 0 )
		DispDuringAcq( sFolder, wG, wIO, wFix, pr, bl, fr, sw, 0,   kWNDDIVIDER - 1, nWndBeg, nWndEnd )
...
Function		DispDuringAcq( sFolder, wG, wIO, wFix, pr, bl, fr, sw, BorderLeft, BorderRight, nWndBeg, nWndEnd )			// partially  PROTOCOL  AWARE
//  Display superimposed and current sweeps and frames
	...
	for ( w = nWndBeg; w <= nWndEnd; w += 1 )							// loop thru windows : loop thru rectangular array defined by Acquis wnd options...
		...
		for ( nCurve = 0; nCurve < CurveCnt; nCurve += 1 )			
			...
			string  	sFolderTNm	= "root:uf:" + sFolder + ":" + ksF_IO + ":" + rsTNm
...
Function		GainByNmForDisplay( wIO, sTNm )
...
Function		ConstructYAxis()
	...
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt() )			root:uf:disp:wYAxUsedMax	  = 0		// maximum number of traces / curves used in a specific window
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:disp:wYAxisExists		  = 0		// flag telling if the specified trace / curve is already displayed in the window
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:disp:wYAxisNegLim	  = -5000	// minimum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:disp:wYAxisLastNegLim = -5000	// minimum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:disp:wYAxisPoslim	  =  5000	// maximum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:disp:wYAxisLastPosLim  =  5000	// maximum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt() )			 root:uf:disp:wYABotAxEndMax = -Inf	// bottom axis end point in a specific window
...
// 060511c
	string  	sTNmPostFix= BuildMoRaName( rnRange, rnMode ) + sMORAPOINTSEP		// e.g.Adc0 + SM + _
	AutoscaleZoomAndOfs( sWNm,  rsTNm+sTNmPostFix, bAutoscl, rYOfs, rYZoom, Gain )
...
static  strconstant	sMODETEXT		= "Current;SuperImp;"
static  strconstant	sMODENM		= "C;M"
static  strconstant	sMODEFOLDER	= "root:uf:disp:Mod"
...
Function  		DisplayOptionsAcqWindows()
	...
	ConstructOrDisplayPanel(  "PnAcqWin" , "Disp Acquisition" , sFolder, sPnOptions,  98, 80 )	// same params as in  UpdatePanel()
End
...
Function		root_uf_disp_Mod( sControlNm, bValue )					// name is derived from  sRANGEFOLDER 
// Sample  action procedure  for 2dimensional   PnControl()  with  CheckBoxes 
// MUST HAVE SAME NAME AS VARIABLE. NAME MUST NOT BE MUCH LONGER !
// Sample : sControlNm  'root_uf_disp_Mod_Adc1_Frames'  ,    boolean variable name : 'root:uf:disp:Mod:Adc1_Current'  , 	sOLANm: 'Adc1'  ,  sMode: 'Current'
	string  	sControlNm
	variable	bValue
	...
	ChkBoxVarNmExtractDim2( sControlNm, rsFolderVarNmBase, rsSource, rsMode )
	 variable   val	=  ChkBoxValDim2( sControlNm )
	printf "\tProc CHKBOX root_uf_disp_Mod(\tctrlNm: \t%s\tbVal:%d )    \t\t\t\tsSource:\t%s\tMode:\t'%s'   val:%d \r", pd(sControlNm,27), bValue, pd(rsSource,12), rsMode, val 
	// ...this is just sample code to be commented  out .
...
Function		buDispWARectUsSpec( ctrlName ) : ButtonControl
	...
	svar		gsScriptPath= root:uf:script:gsScriptPath
	string  	sFolder	= ksACQ
	LoadDisplayCfg( sFolder,  gsScriptPath )
