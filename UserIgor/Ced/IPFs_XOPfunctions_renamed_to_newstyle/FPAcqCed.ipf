...
//	2. wRaw must be redimensioned to different sizes when going from Script to Script  (=normal case which cannot be avoided, except for letting wRaw keep a fixed size which is very bad)     
// 	3. Folders with redimensioned waves are killed (e.g.  ...acq:dig   and  ... acq:store) when going from Script to Script 
// 	4. Waves are killed and created and redimensioned  (e.g.  ...acq:dig:wDgoCh etc.   and  ... acq:store:wStoreTimes ) when going from Script to Script 
...
//	string  	sFolder
//	NewDataFolder  /O  /S  $"root:uf:" + sFolder							// acquisition: create a new data folder and use as CDF,  clear everything
...
// creates all folder-specific variables. To be used once from CreateGlobals() as the current data folder is changed and not restored
	...
	NewDataFolder  /O  /S  $"root:uf:" + sFolder + ":co"							// acquisition: create a new data folder and use as CDF,  clear everything
...
Function		StartStimulusAndAcquisition() 
	...
	nvar		bAppendData	= $"root:uf:"+sFolder+":dlg:gbAppendData"
	//nvar	gbIncremFile	= root:uf:acq:co:gbIncremFile
	...
	if ( wG[ kPNTS ]  )
		sprintf bf, "\tSTARTING ACQUISITION %s %s... \r", SelectString( raTrigMode, " ( after 'Start' , " , " ( waiting for trigger on E3E4, "  ),  SelectString( bAppendData, "writing separate files )", "appending to same file )" )
		Out( bf ) 
		NewStatusBar( "SB_ACQUISITION" , "Status Bar Acquisition" )	//  change message line on bottom depending on prg part
		KillAllTimers()

//		InitializeCFSDescriptors( sFolder )						// 050207 Should NOT be called HERE. Should be called only once during initialisation, but is actually called with every new data file as..
//														// 1. flaw in Pascal Pulse (file name is inDesc)   2. gsScript must be known as they are counted to store script. Could be improved?
...
Function		CEDInitialize( sFolder) 
...
//	nvar		gbRunning		= root:uf:acq:co:gbRunning 
	nvar		gbRunning		= root:uf:acq:keep:gbRunning 
	nvar		gCedMemSize		= root:uf:acq:co:gCedMemSize
	nvar		gnCompressTG		= root:uf:acq:co:gnCompressTg
	nvar		gbSearchStimTiming	= $"root:uf:"+sFolder+":mis:ImprStimTim0000"
	nvar		gbRequireCed1401	= $"root:uf:"+sFolder+":mis:RequireCed0000"
...
//					xCEDClose( MSGLINE_C )
				...
				if ( kbIS_RELEASE  &&  gbRequireCed1401 )	// 
					Alert( kERR_FATAL,  "Ced  is not responding. The test mode does not require a Ced1401.  Uncheck  'Require Ced..'  in the  'Misc'  panel.  Aborting..." )
// 2009-10-22 modify for Igor6
//					xCEDClose( MSGLINE_C )
					UFP_CEDClose( MSGLINE_C )
					return	kERROR
...
//		endif
		...
		if ( nTrfAreaPts <= 0 )
			...
			string		lstPrimes	= FPPrimeFactors( nPnts )			// list containing the prime numbers which give 'nPnts'
		...
		wave  /Z	wRaw	= $"root:uf:" + sFolder + ":keep:wRaw"	// 041203 must be PERMANENT data folder, will give sporadic heap scramble problems if a regularly cleared folder (e.g.ar) is used	
														// 050128..050205 ????? why ??? was is this error....?
		// printf "\t\tCEDInitialize(1 '%s' ) \texists wRawDDA: %d   points: \t%8d\t  TAPts: \t%8d \r", sFolder, waveExists( wRaw ),  waveExists( wRaw ) ? numPnts(wRaw) : -1 , nTrfAreaPts // 050128

		if ( waveExists( wRaw ) )
			if ( CEDHandleIsOpen() )
// 2009-10-22 modify for Igor6
//				code	= xCEDUnsetTransferArea( 0, wRaw, ShowMode ) 	// the attempt to unset a non-existing transfer area  (=error -528)  will do no harm as it is caught in the XOP
...
//					xCEDClose( MSGLINE_C )
		...
		make  	/W /N=( nTrfAreaPts )  $"root:uf:" + sFolder + ":keep:wRaw" 		// allowed area 0, allowed size up to 512KWords=1MB , 16 bit integer
		// print "050128 make  /W \twRaw",  nTrfAreaPts
		wave	wRaw		= $"root:uf:" + sFolder + ":keep:wRaw"			// 041203 must be PERMANENT data folder, will give sporadic heap scramble problems if a regularly cleared folder (e.g.ar) is used	
		...
		// printf "\t\tCEDInitialize(2 '%s' ) \texists wRawDDA: %d   points: \t%8d\t  TAPts: \t%8d \r", sFolder, waveExists( wRaw ),  waveExists( wRaw ) ? numPnts(wRaw) : -1 , nTrfAreaPts
// 2009-10-22 modify for Igor6
//		if ( ! CEDHandleIsOpen()  ||  xCEDSetTransferArea( 0, nTrfAreaPts, wRaw , ShowMode ) == 0 ) 
		if ( ! CEDHandleIsOpen()  ||  UFP_CedSetTransferArea( 0, nTrfAreaPts, wRaw , ShowMode ) == 0 ) 
			// printf "\t\tCEDInitialize(3 '%s' ) \texists wRawDDA: %d   points: \t%8d\t  TAPts: \t%8d \r", sFolder, waveExists( wRaw ),  waveExists( wRaw ) ? numPnts(wRaw) : -1 , nTrfAreaPts
			if ( CEDInitializeDacBuffers( sFolder, wRaw ) ) 						// ignore 'Transfer' and 'Convolve' times here during initialization as they have no effect on acquisition (only on load time)
				return	kERROR
			endif
		else
		...
		TelegraphGainPreliminary( wG, wIO )		
...
Function		CEDInitializeDacBuffers( sFolder, wRaw ) 
...
if ( cPROT_AWARE )
		ConvolveBuffersDA( sFolder, wIO, nChunk, 0, gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, 1 )			// ~ 3ms
else
		ConvolveBuffersDA( sFolder, wIO, nChunk, 0, gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, nPnts )		// ~ 3ms
endif
...
Function		CedStartAcq() 
	...
	nvar		gbAcquiring	= root:uf:acq:co:gbAcquiring	// 031024 
	nvar		gbRunning	= root:uf:acq:keep:gbRunning
	nvar		gnTicksStart	= root:uf:acq:keep:gnTicksStart
	...
	if ( ArmDAC( sFolder, gSmpArOfsDA, gPntPerChnk * nCntDA * gChnkPerRep, gnReps ) == kERROR )
	...
	if ( ArmADC( sFolder, gSmpArOfsAD, gPntPerChnk * (nCntAD+nCntTG) * gChnkPerRep, gnReps ) == kERROR ) 
		return	kERROR						// 031016 Avoid  the user ignoring this error and continuing . This error happens very rarely and is not understood...
	endif
	if ( ArmDig( sFolder, gnOfsDO ) == kERROR )
		return	kERROR						// 031016 Avoid  the user ignoring this error and continuing . This error happens very rarely and is not understood...
	endif	
...
//	SetFormula	$"root:uf:"+sFolder+":dlg:gnAcqStatus", "root:uf:acq:co:gbRunning + 2 * root:uf:acq:co:gbAcquiring"	// 041022
	SetFormula	$"root:uf:"+sFolder+":dlg:gnAcqStatus", "root:uf:acq:keep:gbRunning + 2 * root:uf:acq:co:gbAcquiring"	// 041022
	
	gbRunning	= TRUE 								// 031030
	StartStopFinishButtonTitles( sFolder )						// 031030 reflect change of 'gbRunning'  (ugly here...button text should change automatically) 
	...
	EnableSetVar( "PnPuls", "root_uf_acq_keep_gnProts",kNOEDIT)// ..we cannot change the number of protocols as this would trigger 'ApplyScript()'
	EnableButton( "PnPuls", "buDelete",		kDISABLE )		//  050530 Never allow deletion of the file which is currently written
	//StartStopFinishButtonTitles( sFolder )					// ugly here...button text should change automatically 050207weg
	...
	Process( sFolder, -1) 			//  PULSE  specific: for initialization of CFS writing
...
static Function	ADDASwing() 
	...
	wave  	wG		= $"root:uf:" + sFolder + ":keep:wG"  	// This  'wG'  	is valid in FPulse ( Acquisition )
	wave  /T	wIO		= $"root:uf:" + sFolder + ":ar:wIO"  			// This  'wIO'   	is valid in FPulse ( Acquisition )
	wave 	wRaw	= $"root:uf:" + sFolder + ":keep:wRaw"// This  'wRaw' 	is valid in FPulse ( Acquisition )
	...
	if ( !CEDHandleIsOpen() )						// MODE  WITHOUT  CED works well for testing
		...
		if ( cPROT_AWARE )
			ConvolveBuffersDA( sFolder, wIO, gnChunk, gnRep-1, gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, 1 ) 
		else
			ConvolveBuffersDA( sFolder, wIO, gnChunk, gnRep-1, gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, nPnts ) 
		endif
		StopTimer( "Convolve" )		
		StartTimer( "Transfer" )		
		...
		if ( nDacReady == cADDATRANSFER ) 
			...
			if (  		  gnRep < gnReps ) // DAC: don't transfer the last 'ChunkspRep' (appr. 250..500kByte, same as in  ADDAStart) as they are already transfered..
				...
				if ( cPROT_AWARE )
					ConvolveBuffersDA( sFolder, wIO, gnChunk, gnRep,	gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, 1 )	
				else
					ConvolveBuffersDA( sFolder, wIO, gnChunk, gnRep,	gChnkPerRep, nCntDA, gPntPerChnk, wRaw, gnOfsDA/2, nPnts )	
				endif
				StopTimer( "Convolve" )		
				StartTimer( "Transfer" )		
				nPts				= gPntPerChnk * nCntDA 
...
// print nDacReady
	if (  !CEDHandleIsOpen()   ||  nDacReady  == cADDATRANSFER )	// WITH  or  WITHOUT  CED
		...
		variable	ptAD
if (  cPROT_AWARE )
		ptAD = DeconvolveBuffsAD( sFolder, wG, wIO, gnChunk, gnRep, gChnkPerRep, gPntPerChnk, wRaw, gnOfsAD/2, nCntAD , nCntTG, gnCompress, 1  )	//? 221002 gnRep=nReps ->0???
else
		ptAD = DeconvolveBuffsAD( sFolder, wG, wIO, gnChunk, gnRep, gChnkPerRep, gPntPerChnk, wRaw, gnOfsAD/2, nCntAD , nCntTG, gnCompress, nPnts )	//? 221002 gnRep=nReps ->0???
endif
		...
		Process( sFolder, ptAD) 								// different approach: call CFSStore() and TDisplaySuperImposedSweeps() from WITHIN this function
...
Static Function  CheckReadyDacPosition( sFolder, wG, command, nRep, nChunk )
	...
	nvar		gReserve			= root:uf:acq:co:gReserve
	nvar		gMinReserve		= root:uf:acq:co:gMinReserve
	nvar		gPrediction		= root:uf:acq:keep:gPrediction
	nvar		gMaxSmpPtspChan	= root:uf:acq:co:gMaxSmpPtspChan
	nvar		gErrCnt			= root:uf:acq:co:gErrCnt
	nvar		gRadDebgSel		= root:uf:dlg:gRadDebgSel
	nvar		pnDebgAcqDA 		= root:uf:dlg:Debg:AcqDA
...
//	variable	nADPtr	= xCEDGetResponse( "ADCBST,P;", "ADCBSTpointer ",  0 )	// last param is 'ErrMode' : display messages or errors
	...
	// printf "\t++++kERROR in Test mode: abort prematurely in StopAdda() because of 'Loosing data error' , will also  stop  Background task. \r" 
...
Function		StopADDA( strExplain , bDoApplyScript )
	...
	string  	sFolder			= ksACQ
	wave  	wRaw			= $"root:uf:" + sFolder + ":keep:wRaw"		

	nvar		gBkPeriodTimer		= root:uf:acq:co:gBkPeriodTimer
	nvar		gbRunning		= root:uf:acq:keep:gbRunning
	nvar		gnTicksStop		= root:uf:acq:keep:gnTicksStop

	nvar		raTrigMode		= $"root:uf:"+sFolder+":dlg:raTrigMode"
	nvar		gbQuickAcqCheck	= $"root:uf:"+sFolder+":mis:AcqCheck0000"	
	nvar		gRadDebgGen		= root:uf:dlg:gRadDebgGen
	nvar		gRadDebgSel		= root:uf:dlg:gRadDebgSel
	nvar		pnDebgAcqDA		= root:uf:dlg:Debg:AcqDA
	nvar		pnDebgAcqAD		= root:uf:dlg:Debg:AcqAD
	...
	if (  CEDHandleIsOpen() )
		...
		if ( gnReps > 1 )		
			...
			CEDInitializeDacBuffers( sFolder, wRaw ) 		// Ignore (by NOT measuring)  'Transfer' and 'Convolve' times here after finishing as they have no effect on acquisition (only on load time)
		endif
...
//	sprintf bf, "%s (hnd=%d) \r", strExplain,  xCEDGetHandle() ; Out( bf )
	...
	if ( gbQuickAcqCheck )								//  for quick testing the integrity of acquired data including telegraph channels
		QuickAcqCheck( sFolder, wG, wIO, wRaw )
	endif
	...
	EnableSetVar( "PnPuls", "root_uf_acq_keep_gnProts",TRUE )	// ..allow also changing the number of protocols ( which triggers 'ApplyScript()'  )
	...
	// 060206 ShowTimingStatistics( sFolder, wG, wFix )

	if ( bDoApplyScript )
// 060515a	constant	kbKEEP_ACQ_DISP = 0,  kbNEW_ACQ_DISP = 1

		ApplyScript( kbKEEP_ACQ_DISP )
	endif
...
Function		QuickAcqCheck( sFolder, wG, wIO, wRaw )
...
Function		ShowTimingStatistics( sFolder, wG, wFix )
	...
	nvar		gMinReserve		= root:uf:acq:co:gMinReserve
	nvar		gMaxSmpPtspChan	= root:uf:acq:co:gMaxSmpPtspChan
	nvar		gbShowTimingStats	= $"root:uf:"+sFolder+":mis:TimeStats0000"
...
// 2009-10-22 modify for Igor6
//		variable	code	= xUtilConvolve( wDacReal, wRaw, DADIREC, nChs, 0, begPt, endPt, RepOs, PtpChk, ofs, yscl, 0, 0, 0, nPnts, 0 )	// ~ 40ns / data point
...
// 2009-10-22 modify for Igor6
//		variable	code	= xUtilConvolve( wReal, wRaw, ADDIREC, nCntAD, nCntTG, BegPt, EndPt, RepOs, PtpChk, nHostOfs, yscl, nCompress, nChunk, c, nPnts, bStoreIt )	// ~ 40ns / data point
...
// Returns wave name (including folder) for Adc or telegraph wave  when  index  c = 0,1,2... is given. 	ASSUMPTION: order of channels is first ALL AD, afterwards ALL TG
	...
	wave  /T	wIO
	variable	c
	string  	sNm	= "root:uf:" + sFolder + ":" + ksF_IO + ":" + ADTGNm( wG, wIO, c ) 
...
// Returns wave name  (including folder)  for  telegraph wave when true channel number from script is given. This name must be different from the name for the true Adc wave
	string  	sFolder
	variable	ch
	return	"root:uf:" + sFolder + ":" + ksF_IO + ":" + TGNm( ch ) 
End
...
//	sprintf  bf, "\t\t\t\t\tCed SetPoints( ProtAw:%d)\tMxPts\t\tnPts\tSlc\t DA AD TG Cmp\tSum  MxPtpChan\t MinChk\tChunks\tMinRep\tREPS CHKpREP PTpCHK\t ChkTim\tTA%% M%% FoM \r", cPROT_AWARE
	...
	if ( gnReps * gChnkPerRep > gPntPerChnk   &&  gnReps > 1 ) 	// in the special case of few data points (=they fit with 1 repetition in the Ced memory) allow even few PtsPerChk in combination with many 'nChunk'
		...
		lstPrimes	= FPPrimeFactors( nPnts )					// list containing the prime numbers which give 'nPnts'
...
// In contrast to 'swSetTimes()'  this is not ordered by prot/block/frame/sweep . Here 1 period is stored in 1 index.
	...
	if (  ! waveExists( wStoreTimes ) )
		...
		// printf "\t\t\tStoreTimesSet()  make  /I \twStoreTimes  [ gnStoreCnt:%d , 2 ]  \r", gnStoreCnt	// 050128
	endif
	nvar		gnStoreCnt	= $"root:uf:" + sFolder + ":store:gnStoreCnt"
	...
	if ( n == gnStoreCnt )													// there are more times to store than the wave can currently hold: increase wave size
		...
		// printf "\t\t\tStoreTimesSet() redim    \twStoreTimes[ gnStoreCnt:%d , -1 ]  \r", gnStoreCnt	// 050128
	endif
	wStoreTimes[ n ][ 0 ]	= nSwpBegPt
	wStoreTimes[ n ][ 1 ]	= nSwpEndPt
	// printf "\t\t\tStoreTimesSet( \tn:%3d\tnSwpBegPt:\t %11d \tnSwpEndPt:\t %11d \t )   Current dimension=gnStoreCnt: %d \r", n, nSwpBegPt, nSwpEndPt , gnStoreCnt
End
...
static Function  ArmDAC( sFolder, BufStart, BufPts, nrep )
//	wave  	wG
//	wave  /T	wIO
	string  	sFolder
...
static Function  ArmADC(  sFolder, BufStart, BufPts, nrep )
//	wave	wG
//	wave  /T	wIO
	string  	sFolder
	variable	BufStart, BufPts, nrep
...
Function  		ArmDig( sFolder, OffsDO )
	string  	sFolder
//	wave	wG
	variable	OffsDO
	wave 	wG			= $"root:uf:" + sFolder + ":keep:wG"	  		// This  'wG'  	is valid in FPulse ( Acquisition )
...
Function		ProcessDigitalOutputs( sFolder, wG, wVal, wFix, wEinCB, wELine, wE, wBFS, nEle )
	...
	nCode += DigoutMakeCEDString( 	sFolder, wG, wFix, wEinCB, wE, wBFS, nPnts )				
	StopTimer( "Digout" )
	return	nCode
End
...
if (  bCRASH_ON_REDIM_TEST14 )				// 051105   
	make  	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoCh"   ;	wave  wDgoCh	= $"root:uf:" + sFolder + ":dig:wDgoCh"	// 32bit integer for Dig wave
	make  	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoBeg" ;	wave  wDgoBeg= $"root:uf:" + sFolder + ":dig:wDgoBeg"	// 32bit integer for Dig wave
	make  	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoDur"  ;	wave  wDgoDur	= $"root:uf:" + sFolder + ":dig:wDgoDur"	// 32bit integer for Dig wave
else
	make  /O	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoCh"   =0;wave  wDgoCh	= $"root:uf:" + sFolder + ":dig:wDgoCh"	// 32bit integer for Dig wave
	make  /O	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoBeg" =0;wave  wDgoBeg= $"root:uf:" + sFolder + ":dig:wDgoBeg"	// 32bit integer for Dig wave
	make  /O	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoDur" =0 ;wave  wDgoDur= $"root:uf:" + sFolder + ":dig:wDgoDur"	// 32bit integer for Dig wave
					...
					if ( numtype( BegPt ) != kNUMTYPE_NAN )		//  ..(if this is not checked, empty lines containing no digout information are processed...)	 030513
						for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
							...
							if ( numtype(  nChan )  !=  kNUMTYPE_NAN )									// true channel number 
								variable	nDur	= round( wDGO[ ciDgo ][ bfsPtr ][ e ][ DODUR ] * kMILLITOMICRO / nSmpInt )  
								variable	nDel	= round( wDGO[ ciDgo ][ bfsPtr ][ e ][ DODEL  ] * kMILLITOMICRO / nSmpInt )  
								...
								if ( ( nChan == 3  ||  nChan == 4 )   &&   ( b == 0  &&  f == 0  &&  s == 0  &&  e == 0 )  )
									nElemCnt = FillDigDispArrays( "AutoTrig", nSmpInt, nElemCnt, nChan, nBeg-cTIME_DAC_TO_DIGOUT, nDur,  f, s, bfsPtr, e, BegPt, nDel, wDgoCh, wDgoBeg, wDgoDur )  
								...
								elseif ( ( nChan == 3  ||  nChan == 4 )   &&     e > 0   )	
									nElemCnt = FillDigDispArrays( "UserTrig", nSmpInt, nElemCnt, nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel,  wDgoCh, wDgoBeg, wDgoDur  )  
								// Pass all other channels in every block, frame, sweep
								elseif ( ! ( nChan == 3  ||  nChan == 4 ) )									
									nElemCnt  = FillDigDispArrays( "normal  ", nSmpInt, nElemCnt,  nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel,  wDgoCh, wDgoBeg, wDgoDur  )  
								endif			
							endif			
						endfor			
					endif
...
//	redimension  /I	/N=( nElemCnt ) wDgoCh 					// for  Digout wave for display
	...
	for ( ciDgo = 0; ciDgo< nDgoChs; ciDgo += 1)
		...
		make /O /B /N=(  nPnts )  $"root:uf:" + sFolder + ":stim:" + DispWaveDgoFull( gsDigOutChans, ciDgo ) =  0// ..for the display of the digital output (BYTE wave)
		// print "050128 make  /B  \t'",  DispWaveDgoFull( gsDigOutChans, ciDgo ) , nPnts
	endfor
	for ( n = 0; n < nElemCnt; n += 1)
		for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
			wave	wDOFull	= $( "root:uf:" + sFolder + ":stim:" + DispWaveDgoFull( gsDigOutChans, ciDgo ) )
...
if (  bCRASH_ON_REDIM_TEST14 )				// 051105   
	make   	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoTime"	;	wave  wDgoTime	= $"root:uf:" + sFolder + ":dig:wDgoTime"	// 32bit integer for Dig wave
	make   	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoChan"	;	wave  wDgoChan	= $"root:uf:" + sFolder + ":dig:wDgoChan"	// 32bit integer for Dig wave
	make   	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoChange" ;	wave  wDgoChange	= $"root:uf:" + sFolder + ":dig:wDgoChange"// 32bit integer for Dig wave
else
	make  /O  	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoTime"	=0;	wave  wDgoTime	= $"root:uf:" + sFolder + ":dig:wDgoTime"	// 32bit integer for Dig wave
	make  /O 	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoChan"	=0;	wave  wDgoChan	= $"root:uf:" + sFolder + ":dig:wDgoChan"	// 32bit integer for Dig wave
	make  /O	/I /N=( MAXDIGOUT ) $"root:uf:" + sFolder + ":dig:wDgoChange" =0;	wave  wDgoChange	= $"root:uf:" + sFolder + ":dig:wDgoChange"// 32bit integer for Dig wave
				...
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )		// ..through all elements (=Segments, Ramp..)
					...
					if ( numtype(BegPt)  != kNUMTYPE_NAN )		//  ..(if this is not checked, empty lines containing no digout information are processed...)  030513
						for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
							variable	nChan	= wDGO[ ciDgo ][ bfsPtr ][ e ][ DOCHA ]
							if ( numtype(  nChan )  !=  kNUMTYPE_NAN )										// true channel number 
								variable	nDur	= wDGO[ ciDgo ][ bfsPtr ][ e ][ DODUR ] * kMILLITOMICRO / nSmpInt  
								variable	nDel	= wDGO[ ciDgo ][ bfsPtr ][ e ][ DODEL  ] * kMILLITOMICRO / nSmpInt   
								variable	nBeg	= BegPt  +  nDel ;  wDGO[ ciDgo ][ bfsPtr ][ e ][ DOBEG ] =  nBeg 	// BegPoint of Digout  is BegPt of  Dac of current line + Digout Delay
								...
								// Event 3 and 4 trigger the Adc / Dac : Pass the automatic (=InsertAdcDacDigoutTrigger)  pulse only once in 1.bl, 1.fr, 1.sweep (e==0, must be in 1.line) . 
								if ( ( nChan == 3  ||  nChan == 4 )   &&   ( b == 0  &&  f == 0  &&  s == 0  &&  e == 0 )  )
									nSlices  = FillDigTimArrays( "AutoTrig", nSmpInt, nSlices, nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel, nPnts,wDgoTime, wDgoChan, wDgoChange )  
								// Event 3 and 4 trigger the Adc / Dac : Pass user pulses in chan 3 and 4 ((e>0, not in 1. line) in every block, frame, sweep
								elseif ( ( nChan == 3  ||  nChan == 4 )   &&     e > 0   )	
									nSlices  = FillDigTimArrays( "UserTrig", nSmpInt, nSlices, nChan, nBeg + cTIME_DAC_TO_DIGOUT, nDur, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
								elseif ( ! ( nChan == 3  ||  nChan == 4 ) )									// Pass all other channels in every block, frame, sweep
									nSlices  = FillDigTimArrays( "normal   ", nSmpInt, nSlices, nChan, nBeg + cTIME_DAC_TO_DIGOUT, nDur, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
								endif			
							endif			
						endfor			
					endif
...
static Function		FillDigDispArrays( sText, nSmpInt, nElemCnt, nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel, wDgoCh, wDgoBeg, wDgoDur  )  
	string		sText
	variable	nSmpInt, nElemCnt, nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel
	...
	if ( nBeg < -1  )	
		Alert( kERR_SEVERE, "Digital Out pulse (ch:" + num2str( wDgoCh[ nElemCnt ] ) + ") cannot start at times more negative than 1 time slice (= sample interval) . Must start at  -" + num2str( nSmpInt ) + " us or later. (Starts at " + num2str( nBeg ) + " time slices)" )
	endif
...
//Function		FillDigDispArrays( sText, wG, nElemCnt, nChan, nBeg, nDur, b, f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoChBegDur  )  
//	string		sText
//	wave	wG
...
//	nvar		PnDebgDigout	= root:uf:dlg:Debg:Digout
//	
//	wDgoChBegDur[ nElemCnt ][1]	= round ( nBeg )
//	wDgoChBegDur[ nElemCnt ][2]	= nDur   
//	wDgoChBegDur[ nElemCnt ][0]	= nChan
//	if ( gRadDebgSel > 2  &&  PnDebgDigout )
//		printf "\t\t\t\tDigoutMakeDisplayWaves(21.)  %s \tel:%2d  \tf:%d\ts:%d\tff:%d\te:%d\tch:%d\tBegPt:\t%8d\tDgoDel:%5d\t -> DgoBegPt:\t%8d \tDgoDur:%5d \r", sText, nElemCnt, f, s, bfsPtr, e, nChan, BegPt , nDel, nBeg, nDur 
//	endif
//	if ( nBeg < -1  )	
//		Alert( kERR_SEVERE, "Digital Out pulse (ch:" + num2str( wDgoChBegDur[ nElemCnt ][0] ) + ") cannot start at times more negative than 1 time slice (= sample interval) . Must start at  -" + num2str( nSmpInt ) + " us or later. (Starts at " + num2str( nBeg ) + " time slices)" )
//	endif
//	if ( nBeg == -1 &&  !( nChan == 3  ||  nChan == 4 ) )	// do not complain about Events 3 and 4 which always start at time zero - 1 sample interval
//		Alert( kERR_IMPORTANT, "Digital Out pulse (ch:" + num2str( wDgoChBegDur[ nElemCnt ][0] ) + ") starting at -" + num2str( nSmpInt ) + " us will only be output in the 1. protocol") 
//	endif
//	nElemCnt += 1
//	return	nElemCnt
//End	
...
static Function		FillDigTimArrays( sText, nSmpInt, nSlices, nChan, nBeg, nDur,  f, s, bfsPtr, e, BegPt, nDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
	string		sText
	variable	nSmpInt, nSlices, nChan, nBeg, nDur, f, s, bfsPtr, e, BegPt, nDel, nPnts
...
// 080503 : ???? the following line  with  YControlBar  is not needed??? ( statusbar in readcfs worked without??? )
		...
		TextBox /A=LC /X=0 /F=0 "  Stim: \\{root:uf:script:gsScriptPath}     Data: \\{root:uf:cfsw:gsDataPath}\\{root:uf:cfsw:gsDataFileW}     Pred:\\{\"%4.2lf\",root:uf:acq:keep:gPrediction}    Lag:\\{\"%2ds\", LaggingTime()}    Blk:\\{root:uf:disp:gPrevBlk} / \\{root:uf:acq:keep:wG[kBLOCKS]}    Swp:\\{root:uf:acq:keep:wG[kSWPS_WRITTEN]} / \\{ root:uf:acq:keep:wG[kTOTAL_SWPS] } "
		...
		ValDisplay vd1,  pos={0,2},  size={120,15},  title="Pred",  format="%4.2lf",  limits={ 0,2,1},  barmisc={0,32},  value= #"root:uf:acq:keep:gPrediction", lowColor=(65535,0,0), highColor=(0,50000,0)
