...
static Function		MKMustOccurOnce( nMK ) 
...
// Returns  IO string   (including folder)    when  IO type , index into ioType  and  data type to be retrieved is given
	...
	wave  /T	wIO
	variable	nIO, c, nData
	return	"root:uf:" + sFolder + ":" + ksF_IO + ":" + ios( wIO, nIO, c, nData )
End
...
Function	/S	FldAcqioPoNio( sFolder, wIO, nIO, c, nData )	 
	...
	wave  /T	wIO
	variable	nIO, c, nData
	return	"root:uf:" + sFolder + ":" + ksF_IO + ":" +  "PoN" + sIOCHSEP + ios( wIO, nIO, c, nData )
End
...
// this routine is very slow when the script loaded contains MANY FRAMES and few sweeps/blocks (e.g 100 and  1/1)  compared to 10/10/1  or 10/1/10.  
	...
	NewDataFolder  /O	$"root:uf:" + sFolder + ":" + ksF_IO	// for the large basic IO waves  e.g. 'Dac0'  ,  'Adc1' , 'PoN1'  and  for  'sDacUnits..'
// test 041028
if ( bDoAcq )
	CreateGlobalsInFolder_Co( sFolder )						// creates  'root:uf:aco:co'
	NewDataFolder	/O	$"root:uf:" + sFolder + ":dispFS"			// root:uf:aco:dispFS  (acquisition) : for the whole bunch of similar display waves Adc..., Dac... with different FrmSwp-suffices
endif

	ResetStartTimer( sFolder, "OverAll" )
...
//keep:wG
	...
	wave  	wG	     = 	 $"root:uf:" + sFolder + ":keep:wG"  	// There are 2  instances of  'wG :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 

	if ( ExtractLinesIntoIO( sFolder, wG ) )					// Construct wIO
		return kERROR
	endif
	wave  /T	wIO	    = 	 $"root:uf:" + sFolder + ":ar:wIO"  	// There are 2  instances of  'wIO' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
...
if ( bDoAcq )
	...
	wave  	wFix	 = $"root:uf:" + sFolder + ":ar:wFix"  		// There are 2  instances of  'wFix' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
	...
	wave	wEinCB	= $"root:uf:" + sFolder + ":ar:wEinCB"	// There are 2  instances of  'wEinCB' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
	wave  	wELine	= $"root:uf:" + sFolder + ":ar:wELine"  	// There are 2  instances of  'wELine' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
	if ( maxEle == kERROR )
		return kERROR
	endif
	...
	wave  /T	wVal	= $"root:uf:" + sFolder + ":ar:wVal"  		// There are 2  instances of  'wVal' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
	...
	wave  /T	wBFS 	= $"root:uf:" + sFolder + ":ar:wBFS"  	// There are 2  instances of  'wBFS' :	in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
	wave  	wE	 	= $"root:uf:" + sFolder + ":ar:wE"  		// There are 2  instances of  'wE' :  	in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
...
static Function		CheckMandatorySubkeys( wIO )
	...
	cCnt	= ioUse( wIO, nIO )
	for ( c = 0; c < cCnt; c += 1 )
		nError	+=	CheckMissing( wIO, nIO, c, cIOCHAN, kERR_FATAL, "After  'Dac:'  the specification   'Chan = n'   is required. " )
		nError	+=	CheckMissing( wIO, nIO, c, cIOGAIN,  kERR_FATAL, "After  'Dac:'  the specification   'Gain = n'   is required. " )
	endfor

	nIO	= kIO_ADC
	cCnt	= ioUse( wIO, nIO )
	for ( c = 0; c < cCnt; c += 1 )
		nError	+=	CheckMissing(   wIO, nIO, c, cIOCHAN, kERR_FATAL, "After  'Adc:'  the specification   'Chan = n'   is required. " )
...
// assumes and relies on a syntactically correct Frames/Sweeps structure in the script (must be expanded and checked first...)
	...
	wave  	wFix					 = 	$"root:uf:" + sFolder + ":ar:wFix"  			// There are 2  instances of  'wELine' :  in FPulse  in folder  'Acq'   and in FEval  in folder  'evo' 
...
// Get number of  entries
	string  	sFolder
	svar		gsCoScript	= $"root:uf:" + sFolder + ":gsCoScript"
...
// inserts 'cnt'  lines containing 'sInsertLine'  after line 'pos'  in 'wLine' , increases 'wLine' 
	...
	duplicate /O 	twave	$"root:uf:" + sFolder + ":keep:wLines"
...
//	variable	nLine
//	wave  /T	wLines	= $"root:uf:" + sFolder + ":keep:wLines"
//	// print "AppendScrLine1  n:", nLine, numpnts( $"root:uf:" + sFolder + ":keep:wLines" )
//	wLines[ nLine ]  = { "" }				// this indeed make the text wave 1 bigger...(without redimensioning?)
//	// print "AppendScrLine2  n:", nLine, numpnts( $"root:uf:" + sFolder + ":keep:wLines" )
//End
