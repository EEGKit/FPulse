...
Function		fPrevFile( s )
	...
	if (  s.eventCode == kCCE_mouseup ) 
		ReadCfsFile( ksEVO, cPREVFILE, cFIRSTDATA )
	endif
End
...
Function		fNextFile( s )
	struct	WMButtonAction	&s
	if (  s.eventCode == kCCE_mouseup ) 
		ReadCfsFile( ksEVO, cNEXTFILE, cFIRSTDATA )
	endif
End
...
Function		fCurAcqFile( s ) 
	struct	WMButtonAction	&s
		if (  s.eventCode == kCCE_mouseup ) 
		ReadCfsFile( ksEVO, cCURACQFILE, cFIRSTDATA )
	endif
End
...
Function		fSelFile( s ) 
	...
	if (  s.eventCode == kCCE_mouseup ) 
		...
		ReadCfsFile( ksEVO, cSELFILE, cFIRSTDATA )
	endif
End
...
Function	ReadCfsFile( sFolder, nFile, nMovetoData )
	...
	elseif ( nFile == cCURACQFILE )
		...
		gsReadDPath	= SearchCurAcqFile( gsReadDPath, ksCFS_EXT )			// 040831	Search for the youngest file. Does not use globals like 'SearchAndSetLastUsedFile()' .
	...
	try
		...
		if ( CFSHndIn > 0 )									// valid handles are positive, negative or zero means error
			if ( cmpstr( sPathAndFile, gsLastPathAndFile ) )			// Do the time consuming reading only once, do not reread the file if we are still moving within the same file
				...
				CFSDisplayAllChanInit( ksEVO, gChannels )
...
static Function		CFSHeaderOverview(  sFolder, CFSHndIn )
	...
	// Process the script extracted from the acquired data (in EVAL) the same way as script in ACQ: fill a notebook with this string, display it, and convert to global script string
	if ( gbIsIgor )	
