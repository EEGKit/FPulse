...
//	NewDataFolder  /O   root:uf:com:dlg
	...
	NewDataFolder	/O	$"root:uf:" + sFo				// creates  'root:uf:aco' .  Holds subfolders  'ar' , 'stim' , 'dig' , 'store' , 'io' , 'co' , 'dispFS'  which are cleared whenever a new script is loaded.

	NewDataFolder  /O	$"root:uf:" + sFo + ":" + ksfACOVARS		// required early e.g. for the panel list 'lstPanels'
	...
	MiscellaneousDlg1( kPN_INIT)						// 050815 create a hidden panel just to construct globals like  ...

	CreateGlobalsInFolder_Util_aco()
	DisplayOptionsStimulus_aco( 50, 10, kPN_INIT )		// 050815 create a hidden panel just to construct globals like  'gbDisplay'  
...
// 2009-10-28 alles weg
//static Function		Cleanup_Fp( sFo )
//	string  	sFo
...
//	PossiblyKillPanel( "PanelPref" ) 
//	PossiblyKillPanel( "PnMiscellaneous" )
//	PossiblyKillPanel( SDNamee( sFo ) ) //"PnDispStim" + sFolder )	// the StimDisp panel in Acq
//	PossiblyKillNotebook( ksSCRIPT_NB_WNDNAME )
//	KillDataFolderUnconditionally( "root:uf:aco:cfsw" )	
//	KillDataFolderUnconditionally( "root:uf:aco:ola" )	
//	KillDataFolderUnconditionally( "root:uf:aco:disp" )	
//	KillDataFolderUnconditionally( "root:uf:" + sFo )	// kills all subfolders
////if ( cmpstr( sFolder, ksACQ ) == 0 )
...
// creates all folder-specific variables. To be used once from CreateGlobals() as the current data folder is changed and not restored
	...
	NewDataFolder  /O  /S  $"root:uf:" + sFolder +":dlg"				// acquisition: make a new data folder and use as CDF

	// The FPulse Panel
	variable	/G	gbShowScript		= 1		
	variable	/G	gnAcqStatus		= 0						// 031023  0 : waiting for 'Start',   1: 'Start' has been pressed (but Acq not yet started, waiting for E3E4 trigger),  2 : Acq is running
	variable	/G	raTrigMode		= 0						// 031027  0 : normal SW trigger ('Start' button) ,    1 : HW trigger by E3E4 mode: Acq starts with HI pulse on CED Event4  ,    2 : timer triggered
...
//  	 endif
	...
	// MC700Dlg_() 				 						// show Preferences panel always at startup : only for testing the MCC700B 
End
...
// 040901
//Function		IgorQuitHook( sIgorApplicationNameStr )
//	string 	sIgorApplicationNameStr 
//	printf "..quitting %s  \r", sIgorApplicationNameStr
//
//	KillAllGraphs()								// clean screen 
//	KillWaves  /A  /Z							// delete waves in root
//
//	string 	sDFSave	= GetDataFolder( 1 )
//	SetDataFolder	root:uf:stim
//	KillWaves  /A					
//	SetDataFolder	$ksFOLDER_ACQTMP
//	KillWaves  /A					
//	SetDataFolder( sDFSave )
//
//	delay( .2 )	// seconds time to read the above message
//End
...
//	sprintf	sPnTitle, "%s %s", RemoveEnding( ksPRG_NAME, ".ipf" ) ,  FormatVersion()
	sprintf	sPnTitle, "%s %s", ksAPP_NAME,  FormatVersion()
...
// 2009-10-28 alles weg
//	SetWindow	 		$sWin	, hook = fHookPnPuls 				// prevent the user to accidentally minimize, maximize or close the panel
...
// 2009-10-28 alles weg
//Function 		fHookPnPuls( sInfo )
//// Catch window kill message and ignore it. Disables Window Close button to keep the user from accidentally closing the main PULSE Panel...
...
//	// printf  "\t\t\t\t\t fHookPnPuls( '%s' )  '%s'\r", sInfo, sWNm
...
//		DoAlert 1, "All graphs will be deleted. \rDo you really want to quit '" + ksAPP_NAME + "' ? "	// Param = 1 : 'Yes'  and  'No'  buttons	
...
// 2009-10-25 disable
//	n += 1;	tPn[ n ] =	"PN_BUTTON;	buPreferencesDlg	;Preferences;	| PN_CHKBOX;	root:uf:"+sFolder+":dlg:cbPnMisc	;Pn Miscellan;	" 
...
// 060511e
strconstant		ksACOld_HELP_WND	= "FPulse Acq Help"   
strconstant		ksACOld_HELP_PATH	= "FPulseHelp.txt"

Function		buAcqHelp( ctrlName ) : ButtonControl
	string		ctrlName		
	string  	sPath = FunctionPath( "" )						// Path to file containing this function.
	if ( cmpstr( sPath[0], ":" ) == 0 )
		InternalError( "Could not locate file '" + ksACOld_HELP_PATH + "' ." )
	...
	sPath = ParseFilePath( 1, sPath, ":", 1,0 ) + ksACOld_HELP_PATH// Create path to the help file.
	 printf "\t\tfAcqHelp     \thelp path: '%s' \r", sPath

	OpenNotebook	/K=1	/V=1	/N=$ksACOld_HELP_WND   sPath	// visible, could also use /P=symbpath...
	MoveWindow /W=$ksACOld_HELP_WND	1, 1, 1, 1			// restore from minimised to old size
End
...
Function		buDisplayStimDlg( ctrlName ) : ButtonControl
	string		ctrlName	
	DisplayOptionsStimulus_aco( 80, 0, kPN_DRAW )
End
...
Function		buMiscellaneousDlg( ctrlName ) : ButtonControl
	string		ctrlName		
	MiscellaneousDlg1( kPN_DRAW )
End

// 2009-10-25 disable
//Function		root_uf_aco_dlg_cbPnMisc( sControlNm, bValue )
//	string		sControlNm
//	variable	bValue
//	string  	sFolder	= ksACOld
//	if ( bValue )
//		MiscellaneousDlg1( kPN_DRAW )
//	else
//		Panel3SubHide( "misc" )	// = 	MoveWindow /W = $"misc"  0, 0, 0, 0 	// minimise the panel to an icon
//
//	endif
//End
...
Function		PreferencesDlg()	
	...
	InitPanelPreferences( sFolder, ":dlg:tPnPref" )	// constructs the text wave  'root:uf:aco:dlg:tPnPref'  defining the panel controls
	ConstructOrDisplayPanel(  sWin, "Preferences", sFolder, ":dlg:tPnPref",  100,100 )
	LstPanels_Fp3Set( AddListItem( sWin, LstPanels_Fp3() ) )	// ??? todo_c could prevent adding more than once....
End

Function		InitPanelPreferences( sFolder, sPnOptions )
	string  	sFolder, sPnOptions
	string		sPanelWvNm = "root:uf:" + sFolder + sPnOptions
...
//Function		gShrinkCedMemMB( ctrlName, varNum, varStr, varName ) : SetVariableControl
...
//Function		gMaxReactnTime( ctrlName, varNum, varStr, varName ) : SetVariableControl
...
Function		MiscellaneousDlg1( nMode )
	variable	nMode
	string  	sFBase		= "root:uf:"
	string  	sFSub		= ksACOld_	
// 2009-12-12 old-style      renamed to avoid confusion with new-style
//	string  	sWin			= "mis" 
	string  	sWin			= "misc" 
	...
	SetDataFolder sDFSave									// Restore CDF from the string  value
	InitPanelMiscellaneous1( sFBase + sFSub, sWin )					// fills big text wave  'sPnOptions' (=wPn)  with all information about the controls necessary to build the panel
	Panel3Sub(   sWin, 	sPnTitle, 	sFBase + sFSub,  100, 95,  nMode ) 	// Compute the location of panel controls and the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
	LstPanels_Fp3Set( AddListItem( sWin, LstPanels_Fp3() ) )	// ??? todo_c could prevent adding more than once....
End

Function		InitPanelMiscellaneous1( sF, sPnOptions )
...
//	if (  s.eventCode != kCCE_idle  &&  s.eventCode != kCCE_mousemoved  &&  s.eventCode != kCCE_leave  &&  s.eventCode != kCCE_enter  &&  s.eventCode != kCCE_mouseup ) 
	if (  s.eventCode == kCCE_mouseup ) 
		...
		string  	sFolder	= ReplaceString( "root:uf:", RemoveEnding( sFo ), "" )		// e.g.  'root:uf:aco:'  ->  'aco'
