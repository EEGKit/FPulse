...
Function		FiltApRm( s, sFo, sWin )
	...
	if ( s.eventCode == kCCE_mouseup )
		nvar		gFilterFreq	=$"root:uf:" + sFo + ":util:gFilterFreq"
		variable	CutoffFreq
		...
		for ( t = 0; t < tCnt; t += 1 )
			...
			if ( WhichListItem( sWvNm, lstWaves ) == kNOTFOUND )				// One  wave can be displayed as multiple traces xxx#1, xxx#2 , but we need ...
				...
				variable	lenDO	= strlen( "root:uf:" + ksACOld + ":stim:DOFull" )	// ...AND we never filter the Digout and the Save/NoSave wave..
				variable	lenSV	= strlen( "root:uf:" + ksACOld + ":stim:SV" )		// This is cosmetics / design issue, we might just as well filter those too
				if ( cmpstr( sWvNm[ 0 , lenDO - 1 ] ,"root:uf:" + ksACOld + ":stim:DOFull" )  && cmpstr( sWvNm[ 0, lenSV - 1 ] ,"root:uf:" + ksACOld + ":stim:SV" ) )	// ...AND we never filter the Digout and the Save/NoSave wave..
	...
	if ( strlen( sTopGraphWndNm ) )										// there is a top graph
		if ( s.nVal == 0 ) 												// we are in State 0 = starting : no cursors have been set, so we display them for the user to adjust
			...
			if ( strlen( sTrace ) ) 
				...
				svar		gsCutPath		= $"root:uf:" + sFo + ":util:gsCutPath"	// Name of the cut out wave
