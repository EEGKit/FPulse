...
static Function	InitAllLoadScript( sFolder )
	string  	sFolder
...
//		We kill selected waves  by  killing  waves  located  special  data folders. Another approach: pass wave names to be killed  in a list.
	...
	KillTracesInMatchingGraphs_( ksW_WNM + "*" )		// Any window (not only Acq windows) will be erased when a script is loaded or applied if its name starts with 'W'
...
// todo KillGraphs( ksAFTERACOldWNM  ) without sorting
	...
	KillGraphs( ksEVO_WNM		 + "*", ksW_WNM )	// the windows built by 'ReadCfs/Eval new '  041215
	//KillGraphs( "MultiChannel", ksW_WNM )		// 040901 ??????????
...
// 051103  sDFSave  might be one of the folders deleted above  (e.g.  root:uf:aco:ar)  so we may not be able to restore it
	...
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":ar" )
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":stim" )
if (  bCRASH_ON_REDIM_TEST14 )				// 051105   
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":dig" )
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":store" )
endif
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":" + ksF_IO )	// root:uf:aco:io     (acquisition) : for the large basic IO waves  e.g. 'Dac0'  ,  'Adc1' , 'PoN1'
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":co" )
	KillDataFolderUnconditionally( "root:uf:" + sFolder + ":dispFS" )			// root:uf:aco:tmp  (acquisition) : for the whole bunch of similar display waves Adc..., Dac... with different FrmSwp-suffices
...
// Returns  empty string if user clicked  Cancel .  Even if script file was not valid  the script name is returned so that  the user can open and edit the script 
	...
	if ( strlen( sFileName ) )
		...
		InitAllLoadScript( sFolder )									// kills ar:wLine, clears graphs, kills all waves in 'root:uf:aco:'  
		...
		string    sRawScript	= ReadScript( sFolder, sFileName )			
		...
		InitializeCFSDescriptors_( ksACOld, sRawScript )					// 050207 Should be called  only once during initialisation, but is actually called with every new script file as..
		...
		if ( InterpretScript_( ksACOld, ksPN_NAME_SDAO, kDOACQ ) != kERROR )				// Sets  wG , wIO ,  wFix , wE...... in  root:uf:aco:	, needs wLine(aco)	
			...
			DoWindow /K $sWin									// the StimDisp panel in Acq 
			DoWindow /K PnAcqWin
...
static Function	/S	ReadScript( sFolder, sFilePath )
...
// if ksDIRSEP == "\\" windows style the Macintosh style ":" returned by IGOR must here be converted. Cave: C:MacFolder-> C:\Winfolder
	...
	if ( nRefNum != 0 )								//  2 failure modes: script file missing  or  user cancelled file open dialog
		...
		nvar		bShowScript	= $"root:uf:"+sFolder+":dlg:gbShowScript"	
...
// Discard    empty lines  and  comments  from script string  for the stimulus extraction.  Fills  globals  gsScript  and  gsCoScript
	...
	string	  /G	$"root:uf:" + sFolder + ":gsScript"      = sRawScript	// Keep the original wave including comments and empty lines...
													// ...for the script notebook and for storing it in the acquisition data file   
	string	  /G	$"root:uf:" + sFolder + ":gsCoScript" = sCompactScript	// Keep compacted  wave without comments and empty lines...
													// ...for the stimulus generation

//if (  !  bNEWSCRIPT )
	variable	nCoLines		= ItemsInList( sCompactScript, "\r" ) 		// or  nLines()  (valid after wLine has been set)
	make /O /T /N=(nCoLines)  $"root:uf:" + sFolder + ":keep:wLines"	
...
// Compute a largely arbitrary window location and return the coordinates as parameters. nOfsX, nOfsY can be used to specify  TopRight  or  MidLeft etc...
	...
	if ( cmpstr( sFolder, ksEVO ) == 0 )								// the script windows in  Acq  and in  Eval  are offset a bit... 
