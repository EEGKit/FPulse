...
#include "\program files\wavemetrics\IgorXOPs\XOPSupport\XOPStandardHeaders.h" 
...
//   Handle CFileName  = IHC2( p->FileName, &hState1 );
   ...
   char buf[200];
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (xCfsCreateFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
 ...
 // WORKAROUND for the above error: check accessibility of desired path before erroneous CreateCFSFile() is called 
   ...
   if ( code )
   {
      sprintf( buf, "++++Error: Cannot open '%s' , (xCfsCreateFile / XOPCreateFile return code=%d/0x%x) \r", nativePath, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
      DisposeHandleIHC( CComment );		// 050204
      DisposeHandleIHC( CFileName );	// 050204
      p->res = code < 0 ? code : -code; // ...as a positive code would be interpreted as a valid CFS file handle
   ...
   {                                    // the file has been created: delete it again...
      ...
      {
         sprintf( buf, "\t\txCfsCreateCFS...receives '%s' '%s' %d %d ptr ptr %d %d,  returns code=%d \r",
...
//   Handle CFileName  = IHC2( p->FileName, &hState );
   ...
   short  code;
	...
	if ( code )
	{
      sprintf( buf, "++++Error: Cannot open '%s' , (xCfsOpensFile / GetNativePath() return code=%d/0x%x) \r", *CFileName, code, code );
      XOPNotice( buf );                 // return ALWAYS A NEGATIVE error code to calling user function..
   ...
   { 
      ...
      if ( (int)p->ErrMode & MSGLINE )
      {
         sprintf( buf, "\t\txCfsOpenCFS...receives '%s' %d %d ,  returns code=%d \r",
...
// Flaw / Limitation: Neither 'time' nor 'date' nor 'comment' may contain the character 'CFSSEP' 
   ...
   {
      sprintf( errbuf, "++++Error: xCfsGetGenInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
...
// Difference to CFS: as IGOR doesn't know pointers the 4 CFS variable pointers are returned as string list   
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: xCfsGetFileInfo() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
...
// Flaw / Limitation: Neither 'Units' nor 'Description' may contain the character 'CFSSEP' 
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: xCfsGetFileChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
...
// Difference to CFS: the 6 CFS variabls are returned as string list   
{
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle, do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: xCfsGetDSChan() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				             // out of memory
   }
   else                                    // string length is OK so return string 
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   {
      sprintf( errBuf, "++++Error: xCfsGetChanData() received nonvalid wave \r" );
		XOPNotice( errBuf );
   ...
   {
    	sprintf( errBuf, "++++Error: xCfsGetChanData() received non integer wave \r" );
		XOPNotice( errBuf );
...
// Difference to CFS: as IGOR doesn't know pointers the CFS buffer is passed as a wave   
{
   ...
   void *Raw;
	...
	if ( p->wDataADS == NIL )                // check if wave handle is valid
   {
      sprintf( errBuf, "++++Error: xCfsWriteData() received nonvalid wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );			           // return NaN if wave is not valid   
	   return( NON_EXISTENT_WAVE );
   }
   if (   WaveType(p->wDataADS) != NT_I16 ) // check wave's numeric type  
   {
    	sprintf( errBuf, "++++Error: xCfsWriteData() received non integer wave \r" );
		XOPNotice( errBuf );
	   SetNaN64( &p->res );				        // return NaN if wave is not 2Byte int
      return( IS_NOT_2BYTE_INT_WAVE );
   }
   ...
   if ( ErrMode & MSGLINE )
	{
	   sprintf( errBuf, "\t\t\txCfsWriteData() has written %d bytes \r", Bytes );
		XOPNotice( errBuf );
	}
...
*/
   ...
   char      errbuf[10000];
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\txCfsSetVarVal(string) \t hnd:%d  varno:%2d \r", hnd, VarNo );   
         XOPNotice( errbuf );
      }
   ...
   if ( VarType == LSTR )                          // String
   {
		if ( PassedSize >= VarSize )
		{
	      ...
	      sprintf ( errbuf, "++++Error: xCfsSetVarVal() String too long, truncated from %d to %d chars: '%s'\r", PassedSize, VarSize-1, *CVarAsString );   
			XOPNotice( errbuf );
      } 
      ...
      if ( ErrMode & MSGLINE )
   	{
         sprintf ( errbuf, "\t\t\txCfsSetVarVal(string) \t type:%d  len:%2d (max:%d)  string:'%s' \r", VarType, PassedSize, VarSize, *CVarAsString );   
         XOPNotice( errbuf );
      }
   }
// 050203
...
//? two sorts of errors can occur: memory and CFS ...not all are handled correctly...
{
   ...
   {
      sprintf( errbuf, "++++Error: xCfsGetVarVal() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;											// out of memory
   }
   else
   ...
   if ( (int)p->ErrMode & MSGLINE ) {
      sprintf ( errbuf, "\t\t\t\t\txCfsGetVarVal(  any  )\t type:%d  len:%2d  varsize:%d  string:'%s'\r", VarType, strlen(sValOrTxt), VarSize, sValOrTxt ); 
      XOPNotice( errbuf );
	}
	err = 0;
...
//sprintf( tmp, "xCfsGetVarDesc() hnd:%d VarSize:%d %s VarType:%d %s Units:%s %s Description:%s \r", hnd, VarSize, CFSSEP, VarType, CFSSEP, Units, CFSSEP, Description ); XOPNotice( tmp );
   ...
   if (( str1 = NewHandle( len )) == NIL ) // get output handle , do not provide space for '\0' 
   {
      sprintf( errbuf, "++++Error: xCfsGetVarDesc() Not enough memory. Aborted...\r");
      XOPNotice( errbuf );
      err = NOMEM;				               // out of memory
   }
   else                                      // string length is OK so return string 
