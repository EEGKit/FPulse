...
// 041111 Trial time expired : Globals
...
// 2009-10-22  remove birthday 
/*
// 041111 Trial time expired : only the first 4 XOP functions will work..
	if ( funcIndex >= 4 && TrialTimeExpired < 0 ) 
	   err = TRIAL_TIME_EXPIRED;
*/
  return err;
}
...
// 2009-10-22  remove birthday 
/*
	// 041111 Trial time expired : This computes the remaining trial time. 
	// As a disk access is used it seems unsafe (though theoretically possible) to incorporate this call in various XOP functions.
	// Rather the remaining time is computed just once at program start..
	// ..and the stored in the static global  'TrialTimeExpired'.
	// It is accepted that bypassing the trial version is made easier this way.
	UtilBD( 2 );	// Param is print control. 0 : nothing , 2 : remaining time , 4++ : debug info
*/
}
