...
// Drawback 2: The  'TableInfo' string  for any row or column -1  when the cursor keys are used  -> This case must be trapped and  'GetTableRowColFromArrowKeys()'  must be used .
...
Function		GetTableRowColFromArrowKeys( s, r, c )
...
Function		LoadDatabaseMulti1Dim_( sDir, sFileNm, sBaseF, sF, lstWantWavesNm, lstWantWavesTyp )
// Read  multiple 1dim text or number waves = 1 data base  from disk.  Strategy is to keep existing waves and to automatically integrate waves which may be added during development 
	...
	if ( nMissing )															// only if waves have been added then...
		SaveDataBase( sDir, sFileNm, sBaseF, sF, lstWantWavesNm )								// ...save the old and the newly created missing wave(s) which are still empty
	endif
End


Function		LoadDatabaseOne2Dim_EMPTY( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols )
	...
	LoadDatabaseOne2Dim_NRows( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols, nInitialRows )
End

Function		LoadDatabaseOne2Dim_( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols  )
	string  	sDir, sFileNm, sBaseF, sF, sWantWave, lstWaveCols
	variable	nInitialRows	= 1							// 'nInitialRows' = 1 : creates an empty row (as in Recipes)
	LoadDatabaseOne2Dim_NRows( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols, nInitialRows )
End

Function		LoadDatabaseOne2Dim_NRows( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols, nInitialRows )
...
// Cave: The current data folder must not be  'sBaseF + sF'  (e.g. must not be 'ksROOTUF_ + ksF_REC_ ')   because  'duplicate'  below will fail
	string  	sDir, sFileNm, sBaseF, sF, sWantWave, lstWaveCols
...
// 060828
	...
	PossiblyCreatePath( sDir )
	
	if ( ! FileExists( sFile ) )												// The data base wave is missing : create it
		...
		SaveDataBase( sDir, sFileNm, sBaseF, sF, sWantWave )							//...and save the newly created missing wave (still empty)
		nCode	= 0
		...
		if ( nWantWvCols >  nColumns )
			sprintf sTxt, "Expecting 2 dimensional text wave '%s'  having  %d columns. Found %d columns. Redimensioning...", sWantWave, nWantWvCols, nColumns
			Alert1( kERR_IMPORTANT, sTxt )		
...
//// Cave: The current data folder must not be  'sBaseF + sF'  (e.g. must not be 'ksROOTUF_ + ksF_REC_ ')   because  'duplicate'  below will fail
//	string  	sDir, sFileNm, sBaseF, sF, sWantWave, lstWaveCols
//	variable	c, nWantWvCols	= ItemsInList( lstWaveCols )
//	string 	sFile				= sDir + sFileNm		
//	string 	sTxt
//	
//// 060828
...
//		Alert1( kERR_IMPORTANT, sTxt )		
//		print "\t\tLoadDatabaseOne2Dim", "\t", sWantWave, "\t", "\tSubWaves:", lstWaveCols, nWantWvCols 
////make  /O  /T  /N= ( 1, nWantWvCols )  	$sFolderWave = ""			// the 1. dimension must NOT be 0 (even though still empty!)
//make  /O  /T  /N= ( 0, nWantWvCols )  	$sFolderWave = ""			// CREATE ZERO ROWS
//		wave	/T	WantWave	= 		$sFolderWave				// create the missing wave in the folder...
//		SaveDataBase( sDir, sFileNm, sBaseF, sF, sWantWave )							//...and save the newly created missing wave (still empty)
//		nCode	= 0
//	else															// the wave has been loaded  so now we must check if no columns are missing... 
...
// not yet used 060904
//Function		InitDatabaseOne2Dim_( sDir, sFileNm, sBaseF, sF, sWantWave,  lstInitEntries )
...
Function	/S	NewDatabaseOne2Dim_NRows( sDir, sFileNm, sBaseF, sF, sWantWave,  lstWaveCols, nInitialRows )
...
Function		GetInitialTableWidth( sTableNm, nWidthCol0, lstColWidth ) 
	string  	sTableNm, lstColWidth
	variable	nWidthCol0 
	variable	c, nCols	= ItemsInList( lstColWidth )
	variable	nColWidth	= nWidthCol0
	for ( c = 0; c < nCols; c += 1 )
		nColWidth	+= str2num( RemoveWhiteSpace( StringFromList( c, lstColWidth ) ) ) 
	endfor
...
Function		ModifyTableColumnWidth( wDB, sTableNm, lstColWidth, nWidthCol0 ) 
	wave 	/T  wDB
	string  	sTableNm, lstColWidth
	variable	nWidthCol0 
	variable	c, nCols	= ItemsInList( lstColWidth )
	...
	for ( c = 0; c < nCols; c += 1 )
		variable	nColWidth	= str2num( RemoveWhiteSpace( StringFromList( c, lstColWidth ) ) ) 
		ModifyTable /W = $sTableNm  width[ c + 1 ] = nColWidth		
	endfor
End


Function		ModifyTableColumnTitles( wDB, sTableNm, lstColTitles ) 
	wave 	/T  wDB
...
Function		SortDatabase( sFolder, sDBNm, PrimSortCol, order )
// Sort the data base  'sDBNm'   according to 'PrimSortCol' .  If not primarily sorting column 0  then column 0 is used as a secondary sorting index.
	...
	SortByColumn( wt, PrimSortCol, order )
End	


constant		kSORTACTION_TOGGLE = 0,   kSORTACTION_UP = 1,	kSORTACTION_DOWN = -1

Function		SortByColumn_AlphaNum_CaseIns( wt, PrimSortCol, SecSortCol, TertSortCol, nOrder )
...
// Todo:  Umlaute ?   Pass direction or toggle?
	...
	SortByColumn_( wt, PrimSortCol, SecSortCol, TertSortCol, nOrder, kSORT_ALPHA_NUM_CASE_I )	//  Parameter is actually for 'SortList' , not for 'MakeIndex' .
End

Function		SortByColumn( wt, PrimSortCol, nOrder )
...
// Todo:  Umlaute ?   Pass direction or toggle?
	...
	SortByColumn_( wt, PrimSortCol, SecSortCol, TertSortCol, nOrder, 0 )		// 0 means default sort. Parameter is actually for 'SortList' , not for 'MakeIndex' .
End

Function		SortByColumn_( wt, PrimSortCol, SecSortCol, TertSortCol, nOrder, nMode )
...
// 060826
wtSortCol[ n ]	= ConvertUmlaute(  wt[ n ][ PrimSortCol ] )					// ~110 us
wtSortCol2[ n ]	= ConvertUmlaute(  wt[ n ][ SecSortCol ] )
wtSortCol3[ n ]	= ConvertUmlaute(  wt[ n ][ TertSortCol ] )
	endfor
	...
	if ( nOrder == kSORTACTION_TOGGLE )								// toggle = invert  the current sorting order
		...
		//printf "\t\tSortByColumn_()\tPrimSC:%d\tRows:%d\tw[0]:\t%s\tw[last]:\t%s\t-> \tSimple non-AN cmpstr gives Order: %d\t  OrderByLst:%d\t->AN-Order: %d\tDuOrg: %s\t ->\tDuAN: %s \r", PrimSortCol, nRows, pd(wtSortCol[ 0 ],11) , pd(wtSortCol[ nRows - 1],11) , cmpstr( wtSortCol[ 0 ] , wtSortCol[ nRows - 1] ) , nOrderByLst, nOrder, pd(lstDummy,13),  lstDummyAlNum
	endif
...
Function		SaveDataBase( sPath, sFileNm, sBaseF, sFolder, lstWantWavesNm )
...
// !!! Cave  If this proves to be too fragile  instead of  'IsSymbolicPath()'  an additional boolean 'bIsSymb' could be passed to 'SaveDataBase()' .
	...
	string 	sPathFile = ""
	variable	bIsSymbolicPath	= IsSymbolicPath( sPath )
	if ( bIsSymbolicPath )
	 ...
	 printf "\t\tSaveDatabase(  [%s path]\t%s\t%s\t%s\t%s\t%s\t )  save waves\t%s\tinto file '%s' \r" , SelectString( bIsSymbolicPath, "normal", "symbolic" ), pd( sPath,13), pd( sFileNm,25), pd( sBaseF,7), pd(  sFolder,7), pd( lstWantWavesNm, 19), pd(lstWaves,23), sPathFile
End

Function	IsSymbolicPath( sPath )
// sPath  can be a normal path (e.g. 'C:dir1:dir2:'  or the string name of a symbolic path (e.g.  'sSymbPath'  or  'ksMYDATAPATH' ) .  The distinction between the 2 is the colon. 
// !!!  Cave   If this proves to be too fragile  instead of  'IsSymbolicPath()'  an additional boolean 'bIsSymb' could be passed to 'SaveDataBase()' .
	string   	sPath
...
End	
// 060913 obsolete
//Function		SaveDatabase( sPath, sFileNm, sFolder, lstWantWavesNm )
//// Store  list of arbitrary  waves . It is possible to store a mix of text, number, 1dim, 2dim waves in 1 file. 
//// However, it is recommended to store 1 wave per file as the loading of a mix of waves may pose problems or not work at all. 
//	string  	sPath, sFileNm, sFolder, lstWantWavesNm
//	variable	n, nWantWaves= ItemsInList( lstWantWavesNm )
//	string 	sFile			= sPath + sFileNm		
//	string 	sWantWave
//	string  	lstWaves = ""
//	for ( n = 0; n < nWantWaves; n += 1 )
//		lstWaves	= AddListItem( ksROOTUF_ + sFolder + StringFromList( n, lstWantWavesNm ) , lstWaves, ";", inf ) 
//	endfor
//	// printf "\t\tSaveDatabase( %s, %s,  %s,  %s )  save waves '%s'  into file '%s' \r" , sPath, sFileNm, sFolder, lstWantWavesNm, lstWaves, sFile		
//	save /O /T /B  	lstWaves  as sFile
//End


Function		CompactDatabase_( sBaseF, sFolder, sDBNm, nCompactByColumn )
// Deletes any data base entries whose 'nCategoryColumn' entry is empty
	...
	for ( n = 0; n < nItems; n += 1 )
 		if ( strlen( wDB[ n ][ nCompactByColumn ] ) == 0 )
			// printf "\t\t\t\t%3d /%4d\t%s\t%s\t%s \r", n+1, nItems, pad( wDB[ n ][ 0 ] , 15 ) , wDB[ n ][ 1 ]  , wDB[ n ][ 2 ]  
...
Function		DeleteDatabaseEntry_( sBaseF, sFolder, sDBNm, nRow )
...
Function		DisplayTable_( xLeft, yTop, xRight, yBot, wDB, lstColumnNames, lstColumnWidths, sTblNm, sTitle, sfHook ) 
// Construct table and display it. If the table existed it is killed first.
	wave 	/T  wDB
	variable	xLeft, yTop, xRight, yBot
	string		lstColumnNames, lstColumnWidths, sTblNm, sTitle, sfHook

	
	DoWindow /K $sTblNm
	variable	nColWidth		= GetInitialTableWidth( sTblNm, kWIDTHCOL0, lstColumnWidths ) 				// Computing  the required table window width from the table column widths is only approximate...
	variable	yTblHeightPts	= GetTableHeight( wDB )								 				// Computing  the required table window height

	variable	rxMinPoints, rxMaxPoints, ryMinPoints, ryMaxPoints, xSz, ySz
	GetIgorAppPoints( rxMinPoints, rxMaxPoints, ryMinPoints, ryMaxPoints )
	...
	yTop	  = 	  ryMinPoints + yTop	 * ( ryMaxPoints - ryMinPoints - kTBL_BOTTOMYX ) / 100
	 print "DisplayTable_()", sTitle, rxMinPoints, rxMaxPoints, ryMinPoints, ryMaxPoints, "-> xSz ", xSz, "-> l t r b " , xLeft, yTop, xLeft+xSz, yTop+ySz	
...
//	variable	c, nWaveCols	= DimSize( wDB, 1 )
	ModifyTableColumnWidth( wDB, sTblNm, lstColumnWidths, kWIDTHCOL0 ) 
	ModifyTableColumnTitles( wDB, sTblNm, lstColumnNames ) 
//	for ( c = 0; c < min( nColsFromTitles, nWaveCols); c += 1 )
//		ModifyTable title[ c + 1 ]		= ColTitle( c, lstColumnNames ) 	
//		ModifyTable alignment[ c + 1 ]	= 0					// 0 starts entry at left border 
//	endfor
	printf "\t\tDisplayTable_( \t%s\tnColTitles:%3d\tnColWidths:%3d\tnWaveCols:%3d\t   \r", pd( sTblNm, 13), nColsFromTitles, ItemsInList( lstColumnWidths ), DimSize( wDB, 1 )
End

//Function		KillTable( ctrlName, sTblNm ) 
//	string		ctrlName, sTblNm 
Function		KillTable( sTblNm ) 
	string		sTblNm 
	DoWindow /K $sTblNm
End

Function	/S	ColTitle( col, lstColumnNames ) 	
	variable	col
...
Function		FindOriginalColumnIndex( nColFew, lstFewColumnNames, lstColumnNames )
	variable 	nColFew
	...
	string  	sTitle		= ColTitle( nColFew, lstFewColumnNames ) 		
...
Function   /S	ConvertUmlaute( sString )
	string  	sString
