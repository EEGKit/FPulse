...
Function		StartStimulusAndAcquisition_() 
	...
	if ( wG[ kPNTS ]  )
		...
		Out( bf ) 
		NewStatusBar( ksSB_WNDNAME, "Status Bar Acquisition" )	//  change message line on bottom depending on prg part			
		KillAllTimers( sFolder )
...
//	nvar		gbRunning		= root:uf:aco:co:gbRunning 
	...
	nvar		gbSearchStimTiming	= $"root:uf:"+sFolder+":misc:ImprStimTi0000"
	nvar		gbRequireCed1401	= $"root:uf:"+sFolder+":misc:RequireCed0000"
...
// 2010-01-05
		...
		if ( CEDHandleIsOpen() ) 
			...
			code	  	= CEDInit1401DACADC( hnd, bMode )			// 2010-02-09  in the E3E4 mode this is required for  EVERY  Ced type.

			code		= CedSetEvent( sFolder, hnd, bMode )
			if ( code )
				hnd 	= xCEDClose( hnd )
				CedHandleSet( CED_NOT_OPEN )
				return	code
		...
		// KillWaves /Z $"root:uf:" + sFolder + ":keep:wRaw" 							// without this Igor will complain (..can not change wave in the middle of assignment...)
		// make  	/O /W /N=(nTrfAreaPts)  		$"root:uf:" + sFolder + ":keep:wRaw" 		// allowed area 0, allowed size up to 512KWords=1MB , 16 bit integer
		...
		make  	/O /W /N=(cMAX_TAREA_PTS)  $"root:uf:" + sFolder + ":keep:wRaw" 	// allowed area 0, allowed size up to 512KWords=1MB , 16 bit integer
...
Function		CedStartAcq_() 
	...
	if ( ArmDAC( sFolder, gSmpArOfsDA, gPntPerChnk * nCntDA * gChnkPerRep, gnReps, hnd ) == kERROR )
	...
	if ( ArmADC( sFolder, gSmpArOfsAD, gPntPerChnk * (nCntAD+nCntTG) * gChnkPerRep, gnReps, hnd ) == kERROR ) 
		return	kERROR						// 031016 Avoid  the user ignoring this error and continuing . This error happens very rarely and is not understood...
	endif
	if ( ArmDig( sFolder, gnOfsDO, hnd ) == kERROR )
		return	kERROR						// 031016 Avoid  the user ignoring this error and continuing . This error happens very rarely and is not understood...
	endif	

	ResetTimer( sFolder, "Convolve" )
	ResetTimer( sFolder, "Transfer" )		
	ResetTimer( sFolder, "Graphics" )		
	ResetTimer( sFolder, "OnlineAn" )		
	ResetTimer( sFolder, "CFSWrite" )		
	ResetTimer( sFolder, "Process" )		
	ResetTimer( sFolder, "TotalADDA" )		
...
//	SetFormula	$"root:uf:"+sFolder+":dlg:gnAcqStatus", "root:uf:aco:co:gbRunning + 2 * root:uf:aco:co:gbAcquiring"	// 041022
	SetFormula	$"root:uf:"+sFolder+":dlg:gnAcqStatus", "root:uf:aco:keep:gbRunning + 2 * root:uf:aco:co:gbAcquiring"	// 041022
...
//	endif
	...
	if ( ArmClockStart( nSmpInt, raTrigMode, hnd ) )	
...
static Function	ADDASwing() 
	...
	if ( !CEDHandleIsOpen() )						// MODE  WITHOUT  CED works well for testing
		...
		StopTimer( sFolder, "Convolve" )		
		StartTimer( sFolder, "Transfer" )		
		...
		StopTimer( sFolder, "Transfer" )		
 	else											// MODE  WITH  CED 
		// printf "\t\tADDASwing(A)  gnRep:%d ?<? nReps:%d \r", gnRep, nReps
		nDacReady =  CheckReadyDacPosition( sFolder, wG, "MEMDAC,P;", gnRep, gnChunk, hnd ) 
...
if (  cPROT_AWARE )
		...
		StartTimer( sFolder, "Process" )								// the current CED pointer is the only information this function gets
...
Static Function  CheckReadyDacPosition( sFolder, wG, command, nRep, nChunk, hnd )
...
Function		StopADDA_( strExplain , bDoApplyScript, hnd )
	...
	nvar		raTrigMode		= $"root:uf:"+sFolder+":dlg:raTrigMode"
	nvar		gbQuickAcqCheck	= $"root:uf:"+sFolder+":misc:AcqCheck0000"	
	nvar		gRadDebgGen		= root:uf:dlg:gRadDebgGen
	nvar		gRadDebgSel		= root:uf:dlg:gRadDebgSel
	nvar		pnDebgAcqDA		= root:uf:dlg:Debg:AcqDA
	nvar		pnDebgAcqAD		= root:uf:dlg:Debg:AcqAD
...
//		endif
	...
	if (  CEDHandleIsOpen() )
		...
		CEDSendStringCheckErrors( hnd, "ADCBST,K;" , 0 ) 						// 031111 kill the sampling process
		CEDSendStringCheckErrors( hnd, "MEMDAC,K;" , 0 ) 						// 031111 kill the stimulation process
		CEDSendStringCheckErrors( hnd, "DAC,0 1 2 3,0 0 0 0;" , 0 ) 				// 031111 set all DACs to 0 when aborting 
		CEDSendStringCheckErrors( hnd, "DIGTIM,K;" , 0 ) 						// 031111 kill the digital output process
		CEDSetAllDigOuts( hnd, 0 )											// 031111 Initialize the digital output ports with 0 : set to LOW
...
//	sprintf bf, "%s (hnd=%d) \r", strExplain,  xCEDGetHandle() ; Out( bf )
	...
	if ( gbQuickAcqCheck )								//  for quick testing the integrity of acquired data including telegraph channels
		QuickAcqCheck_( sFolder, wG, wIO, wRaw )
	endif
	...
	// 060206 ShowTimingStatistics_( sFolder, wG, wFix )
...
Function		QuickAcqCheck_( sFolder, wG, wIO, wRaw )
...
Function		ShowTimingStatistics_( sFolder, wG, wFix )
	...
	nvar		gMinReserve		= root:uf:aco:co:gMinReserve
	nvar		gMaxSmpPtspChan	= root:uf:aco:co:gMaxSmpPtspChan
	nvar		gbShowTimingStats	= $"root:uf:"+sFolder+":misc:TimeStats0000"
	...
	variable	TransferTime		= ReadTimer( sFolder, "Transfer" )
	variable	ConvolveTime		= ReadTimer( sFolder, "Convolve" )
	variable	GraphicsTime		= ReadTimer(sFolder, "Graphics" )
	variable	CFSWriteTime		= ReadTimer( sFolder, "CFSWrite" )
	variable	OnlineAnTime		= ReadTimer( sFolder, "OnlineAn" )
	//variable	FreeUseTime		= ReadTimer( sFolder, "FreeUse" )
	variable	ProcessTime		= ReadTimer( sFolder, "Process" )
	...
	StopTimer( sFolder, "TotalADDA" )		
	PrintAllTimers( sFolder, 0 )
	if ( gbShowTimingStats )
		...
		printf  "\t\tProtocol(total/stored):\t%d / %d ms  \tMeasured(routines): %d = %.1lf%% \t\tMeasured(overall): %d = %.1lf%% \r", ProtocolTotalTime,  ProtocolStoredTime, InRoutinesTime,  InRoutinesTime / ProtocolTotalTime * 100, ReadTimer( sFolder, "TotalADDA" ), ReadTimer( sFolder, "TotalADDA" )/ ProtocolTotalTime * 100
	endif
End
...
if ( cELIMINATE_BLANK )	// 031120
	...
	if ( ( gRadDebgSel == 2  ||  gRadDebgSel == 3 )  &&  pnDebgAcqAD )
		printf "\t\tDeConvAD( \tc:%d\t'%s'\tnChunk:%2d\tnRep:%2d\tCPRep:%2d\tnChs:%d\tPtpChk:%5d\tnHostOfs:%5d\tbpt:\t%11d\tept:\t%11d\tBigChk:\t%7d\tStore: %d\t \r", c, sRealWvNm, nChunk, nRep, nChunksPerRep, nChs, PtpChk, nHostOfs, begpt, endpt, nBigChunk, bStoreIt
	endif
...
// Returns wave name  (including folder)  for  telegraph wave when true channel number from script is given. This name must be different from the name for the true Adc wave
	string  	sFolder
	variable	ch
	return	"root:uf:" + sFolder + ":" + ksF_IO + ":" + TGNm_( ch ) 
End
...
static Function  /S	Factors( nPnts )	
...
//	endif
//	nvar		gnStoreCnt	= $"root:uf:" + sFolder + ":store:gnStoreCnt"		// THIS IS WRONG : MUST USE CURRENT SIZE
//	wave	wStoreTimes	= $"root:uf:" + sFolder + ":store:wStoreTimes"	
...
// In contrast to 'swSetTimes()'  this is not ordered by prot/block/frame/sweep . Here 1 period is stored in 1 index.
	...
	if (  ! waveExists( wStoreTimes ) )
		nStoreCnt	= cSTORE_INITIALSIZE
 		make /O /I /N = (nStoreCnt, 2)	$"root:uf:" + sFolder + ":store:wStoreTimes"		// the begin and the end  of  each 'Store' period  in total stimulus  given as a point index
		wave	wStoreTimes	= 	$"root:uf:" + sFolder + ":store:wStoreTimes"	
...
//  This behaviour could (at the expense of a rather large proramming effort) be improved by splitting chunks into Store=Transfer/NoStore=NoTransfer regions..(finer granularity). 
	...
	for ( t = 0; t < nStoretimes; t += 1 )
		...
		// printf "\t\t\tStoreChunkSet  b\tTimes \tt:\t%6d\t/%6d\tBg:\t%6d\tNd:\t%6d\tnEarlyOn:\t%6d\tnLateOff:\t%6d    \r", t , nStoretimes, wStoreTimes[ t ][ 0 ], wStoreTimes[ t ][ 1 ] , nEarlyOn, nLateOff 		
		for ( c = nEarlyOn; c < nLateOff; c += 1 )
			wStoreChunkOrNot[ c ] = TRUE
		endfor 
	endfor 
	for ( c = 0; c < nChunks; c += 1 )
		// printf "\t\t\tStoreChunkSet  c  \tChunks \tc:\t%6d\t/%6d\tStoreIt:\t%7d\t  \r", c , nChunks, wStoreChunkOrNot[ c ] 
	endfor 
End
...
static Function		CedSetEvent( sFolder, hnd, bMode )
	string  	sFolder
	variable	hnd, bMode
	nvar		raTrigMode	= $"root:uf:"+sFolder+":dlg:raTrigMode"
	variable	code			= 0
...
static Function  ArmDAC( sFolder, BufStart, BufPts, nrep, hnd )
//	wave  	wG
//	wave  /T	wIO
	string  	sFolder
...
static Function  ArmADC(  sFolder, BufStart, BufPts, nrep, hnd )
//	wave	wG
//	wave  /T	wIO
	string  	sFolder
	variable	BufStart, BufPts, nrep, hnd
...
static Function  		ArmDig( sFolder, OffsDO, hnd )
	string  	sFolder
//	wave	wG
	variable	OffsDO, hnd
	wave 	wG			= $"root:uf:" + sFolder + ":keep:wG"	  		// This  'wG'  	is valid in FPulse ( Acquisition )
	...
	if ( CEDHandleIsOpen() )
		...
		CEDSetAllDigOuts( hnd, 0 )												// 031110  Initialize the digital output ports with 0 : set to LOW
		...
		for ( n = 0; n < nSlices - 1 ; n +=1 )						// do not yet send the last slice because we must append the number of repeats 					
			...
			CEDSendStringCheckErrors( hnd, StringFromList( n, gsDigoutSlices ) + ";" ,  0 ) // 1  ) // each slice needs appr. 260 us to be sent 
		endfor
...
Function		ProcessDigitalOutputs( sFolder, wG, wVal, wFix, wEinCB, wELine, wE, wBFS, nEle )
	...
	variable	nPnts	= wG[ kPNTS ] 
	variable	nCode	= kOK
	ResetStartTimer( sFolder, "Digout" )
						...
						for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
							...
							if ( numtype(  nChan )  !=  kNUMTYPE_NAN )									// true channel number 
								...
								if ( ( nChan == 3  ||  nChan == 4 )   &&   ( b == 0  &&  f == 0  &&  s == 0  &&  e == 0 )  )
									nElemCnt = FillDigDispArrays( "AutoTrig", nSmpInt, nElemCnt, nChan, DgoBegPt-cTIME_DAC_TO_DIGOUT, DgoDur,  f, s, bfsPtr, e, BegPt, DgoDel, wDgoCh, wDgoBeg, wDgoDur )  
								...
								elseif ( ( nChan == 3  ||  nChan == 4 )   &&     e > 0   )	
									nElemCnt = FillDigDispArrays( "UserTrig", nSmpInt, nElemCnt, nChan, DgoBegPt, 						DgoDur, f, s, bfsPtr, e, BegPt, DgoDel,  wDgoCh, wDgoBeg, wDgoDur  )  
								// Pass all other channels in every block, frame, sweep
								elseif ( ! ( nChan == 3  ||  nChan == 4 ) )									
									nElemCnt  = FillDigDispArrays( "normal  ", nSmpInt, nElemCnt,  nChan, DgoBegPt, 						DgoDur, f, s, bfsPtr, e, BegPt, DgoDel,  wDgoCh, wDgoBeg, wDgoDur  )  
								endif			
							endif			
						endfor			
					endif
					...
					if ( numtype(BegPt)  != kNUMTYPE_NAN )		//  ..(if this is not checked, empty lines containing no digout information are processed...)  030513
						for ( ciDgo = 0; ciDgo < nDgoChs; ciDgo += 1)
							...
							if ( numtype(  nChan )  !=  kNUMTYPE_NAN )										// true channel number 
								...
								// Event 3 and 4 trigger the Adc / Dac : Pass the automatic (=InsertAdcDacDigoutTrigger)  pulse only once in 1.bl, 1.fr, 1.sweep (e==0, must be in 1.line) . 
								if ( ( nChan == 3  ||  nChan == 4 )   &&   ( b == 0  &&  f == 0  &&  s == 0  &&  e == 0 )  )
									nSlices  = FillDigTimArrays( "AutoTrig", nSmpInt, nSlices, nChan, DgoBegPt, 					   DgoDur, f, s, bfsPtr, e, BegPt, DgoDel, nPnts,wDgoTime, wDgoChan, wDgoChange )  
								// Event 3 and 4 trigger the Adc / Dac : Pass user pulses in chan 3 and 4 ((e>0, not in 1. line) in every block, frame, sweep
								elseif ( ( nChan == 3  ||  nChan == 4 )   &&     e > 0   )	
									nSlices  = FillDigTimArrays( "UserTrig", nSmpInt, nSlices, nChan, DgoBegPt + cTIME_DAC_TO_DIGOUT, DgoDur, f, s, bfsPtr, e, BegPt, DgoDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
								elseif ( ! ( nChan == 3  ||  nChan == 4 ) )									// Pass all other channels in every block, frame, sweep
									nSlices  = FillDigTimArrays( "normal   ", nSmpInt, nSlices, nChan, DgoBegPt + cTIME_DAC_TO_DIGOUT, DgoDur, f, s, bfsPtr, e, BegPt, DgoDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
								endif			
							endif			
						endfor		// ciDgo	
					endif
...
static Function	FillDigDispArrays( sText, nSmpInt, nElemCnt, nChan, DgoBegPt, DgoDur, f, s, bfsPtr, e, BegPt, DgoDel, wDgoCh, wDgoBeg, wDgoDur  )  
	string		sText
...
static Function	FillDigTimArrays( sText, nSmpInt, nSlices, nChan, DgoBegPt, DgoDur,  f, s, bfsPtr, e, BegPt, DgoDel, nPnts, wDgoTime, wDgoChan, wDgoChange )  
	string		sText
...
static Function		GetAndInterpretAcqErrors( hnd, sText1, sText2, chunk, nMaxChunks )
	string		sText1, sText2
...
static Function		ExplainCEDError( sErrorCodes, sCmd, chunk, nMaxChunks )
...
Function		buClose1401( ctrlName ) : ButtonControl
	...
	code	= xCedClose( hnd )
	CedHandleSet( CED_NOT_OPEN )
	printf "\tV3\tfClose1401 \t\treturns code : \t%d\t%s \r", code, CedErrorString( code ) 
End


static Function		CedDriverType( mode )
	variable	mode
...
Function		buType1401( ctrlName ) : ButtonControl
	...
	printf "\t\t\tCEDType:  "
	variable	nType	   = CEDType( hnd, MSGLINE_C )
	printf "\t\t\tCEDDriver:"
...
Function		buMemory1401( ctrlName ) : ButtonControl
	...
	nSize	= CEDGetMemSize( hnd, MSGLINE_C )	
	nCedType	= CEDType( hnd, MSGLINE_C )
...
static Function	CEDGetMemSize( hnd, mode )			
	variable	hnd, mode
	if ( hnd >= 0 )
...
Function		buStatus1401( ctrlName ) : ButtonControl
	...
	printf "\t\tfStatus1401 \tV3\t1401 has state :\t%d\t%s \r", state, CedErrorString( state ) 
End
...
static Function		CEDSetAllDigOuts( hnd, value )
...
static Function		CedHandleSet( hnd )
	variable	hnd
...
static Function		CEDHandleIsOpen()
// only the handle under program control (not the 1401 itself) is checked, if the open 1401 is accidentally turned off this function gives the wrong result 
