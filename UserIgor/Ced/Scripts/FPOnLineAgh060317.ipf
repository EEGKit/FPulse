...
// 051212
 constant		kOA_PH_BASE=0,  kOA_PH_PEAK=1,  kOA_PH_LATC0=2,  kOA_PH_LATC1=3,  kOA_PH_FIT0=4,  kOA_PH_FIT1=5, kOA_PH_MAX=6
...
// 060313	// Remove the OLA trace from the OLA window  A, B or C 
		sWnd	= AnalWndNm( w-1 )
		RemoveFromGraph  /Z  /W=$sWnd		$sOLANm  			// remove unconditionally to avoid an error if the user tries to remove a non-existing wave
...
// 060228 THIs is wrong : points are swallowed.................
	struct	WMCheckboxAction	&s
	string  	sFolders	= ksF_ACQ_PUL 
	 printf "\tfBlankPause()   control is '%s'    bValue:%d  sFolder:%s \r", s.Ctrlname, s.checked, sFolders
...
Static  Function		DrawAnalysisWndTrace( sWNm, sOLANm )
 ...
 printf "WRONG \t\t\tDrawAnalysisWndTrace( %s\t%s\tnRes: %d )   Rgb:\t%7d\t%7d\t%7d\t  Mode: %d    \t'%s' \tXAxis:%d BlankP:%d  \r", sWNm, pd( sOLANm,9),  rnType, Red, Green, Blue, nMode, "root:uf:acq:ola:wa:" + sOLANm + "_" + sWNm,  nXAxis, bBlankPause
...
// This function is called once per frame  during acquisition from the background task. It is called in parallel to 'DispDuringAcqCheckLag()'  and  'WriteDatasection()'  
	...
	if (  AnalysisIsOn() )										// If no cUSER region is defined we skip the whole analysis (this must be refined if there are analysis types requiring no cUSER region)
		...
		gOLAFrmCnt	+= 1	
		OLA1Point( sFolders, wG, wIO, pr, bl, fr, gOLAFrmCnt )			// 051210					
	endif
End												
...
// 051210 new
static Function	OLA1Point( sFolders, wG, wIO, pr, bl, fr, index )
...
//		// Add the data point to  the 1 dimensional (Frames) result wave used to display data 1 frame after the other
		...
		if ( MustCompute( wOLARes, ch, rg ) )
			...
			printf "\t\t\tOLA1Point(d)\tOFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\t'%s'  '%s'\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\tt:%2d/%2d\tleft:%.4g\twORsX:%8.4lf\tValue:%g \r", index, sType, pr, bl, fr, sFolders, sSrc, sType, sOlaNm, ch, rg, ph, t, nTypes,  Region( sFolders, ch, rg, ph, kOABE_BEG ) ,  wORsX[0], wOLARes[ ch ][ rg ][ rtp ]
			...
			if ( PkDir != kPEAK_OFF )
				...
				NewElementOlaDisp( sOLANm, index ) 							//  Returns TRUE if a new wave has just been constructed.
				SetElementOlaDisp( sOlaNm, index, wOLARes[ ch] [ rg ][ rtp ] )			// The  RESULT value is  always  added to the display wave. 
				printf "\t\t\tOLA1Point(e)\tOFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\t'%s'  '%s'\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\tt:%2d/%2d\tleft:%.4g\twORsX:%8.4lf\tValue:%g \r", index, sType, pr, bl, fr, sFolders, sSrc, sType, sOlaNm, ch, rg, ph, t, nTypes,  Region( sFolders, ch, rg, ph, kOABE_BEG ) ,  wORsX[0], wOLARes[ ch ][ rg ][ rtp ]
			endif
		endif
		...
		if ( rtp != kE_BASE  &&   rtp != kE_PEAK )
			...
			NewElementOlaDisp( sOLANm, index ) 							//  Returns TRUE if a new wave has just been constructed.
			SetElementOlaDisp( sOlaNm, index, wOLARes[ ch] [ rg ][ rtp ] )			// The  RESULT value is  always  added to the display wave. 
			 printf "\t\t\tOLA1Point(f)\tOFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\t'%s'  '%s'\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\tt:%2d/%2d\tleft:%.4g\twORsX:%8.4lf\tValue:%g \r", index, sType, pr, bl, fr, sFolders, sSrc, sType, sOlaNm, ch, rg, ph, t, nTypes,  Region( sFolders, ch, rg, ph, kOABE_BEG ) ,  wORsX[0], wOLARes[ ch ][ rg ][ rtp ]
		endif
