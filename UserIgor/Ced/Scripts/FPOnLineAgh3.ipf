...
//	make /O 	    /N = ( kMAXCHANS, kRG_MAX, kOA_PH_MAX, kOABE_MAX  )wCRegion	= nan		// region coordinates ...
	...
	make /O 	    /N = ( kMAXCHANS, kRG_MAX, nPH_MAX, 	   CN_MAX  )	wCRegion		= 1//nan	// region coordinates, drawing environment, number of phases in each region, ...
...
//		gnPeakAverMS	= 2							// Average (for testing) over a long time (including both sides of peak) to reduce noise influence on peak height
	...
	CopyToAllChansRegionsFromBase( ksF_ACQ_PUL )
	CopyToAllChansRegions( ksF_ACQ_PUL )
	SetPhaseColors( ksF_ACQ_PUL )			// colors the cursors
End
...
// 060313 wrong: the point should only be deleted if it appears in no other window
			if ( rtp != kNOTFOUND )// do not process fits, lats, ...
				Delete1EvalPointInAcqWnd( sFolders, ch, rg, rtp )				// Delete  'wAcqPt'  : Remove this SrcRegTyp from this acq window (W0, W1...) . Will trigger the  'Modified'  event in the acq window.
			endif
		endif
...
static Function		Delete1EvalPointInAcqWnd( sFolders, ch, rg, rtp )
// similar 'DisplayEvaluatedPoints()'
	...
	if ( waveExists( wORsX )  &&  waveExists( wORsY ) )
		...
		if ( WhichListItem( sYNm, sTNL, ";" )  != kNOTFOUND )		// ONLY if  wave is  in graph...
			// printf "\t\tDelete1EvalPointInAcqWnd\tch:%d  rg:%d  rtp:%d %s\t  t:\t%8.3lf\t  y:\t%8.3lf\t Shp:%2d\t%s\t(%d)\t APPENDING\t%s\t%s\t'%s'\tX:%g %g\tY:%g %g \r", ch, rg, rtp, pd( StringFromList(rtp, klstEVL_RESULTS),9), x, y, shp, pd(StringFromList( nColor, klstCOLORS),5), nColor, "root:uf:" + sFolders + sXNm, sYNm, sWnd, wORsX[0], wORsY[0], wORsX[1], wORsY[1]
			 printf "\t\tDelete1EvalPointInAcqWnd \tch:%d  rg:%d  rtp:%d %s\t  t:\t%8.3lf\t  y:\t%8.3lf\t Shp:%2d\t  \t  \t DELETING   \t%s\t  %s\t'%s'\tX:%g %g\tY:%g %g \r", ch, rg, rtp, pd( StringFromList(rtp, klstEVL_RESULTS),9), x, y, shp, sFoXNm, sYNm, sWnd, wORsX[0], wORsY[0], wORsX[1], wORsY[1]
...
static Function		Construct1EvalPointInAcqWnd( sFolders, ch, rg, rtp )
// Constructs the 2-point wave and trace containing the value (or 2 values for a line / slope) of the evaluated point in the ACQUISITION window (not in the OLA window) . 
// Set color and shape of the marker.  Igor will automatically update the display of the evaluated point whenever we update the value(s) in the wave.
// similar 'DisplayEvaluatedPoints()'
	string  	sFolders
	variable	ch, rg, rtp			// e.g.  kE_BASE  or  kE_DT50 , todo implement fit results
	...
	if ( ! waveExists( wORsX )  ||  ! waveExists( wORsY ) )
		...
		variable	nColor	= WhichListItem( RemoveWhiteSpace( StringFromList( rtp, klstEVL_COLORS ) ), klstCOLORS )
		 printf "\t\tConstruct1EvalPointInAcqWnd()\tch:%d  rg:%d  rtp:%d -> shp:%2d    Constructing folder '%s'  for  '%s'  and  '%s' \t \r", ch, rg, rtp, rtp, sFoldr, sFoXNm, sFoYNm
	...
	if ( WhichListItem( sYNm, sTNL, ";" )  == kNOTFOUND )			// ONLY if  wave is not in graph...
		 ...
		 printf "\t\tConstruct1EvalPointInAcqWnd()\tch:%d  rg:%d  rtp:%d %s\t  t:\t%8.3lf\t  y:\t%8.3lf\t Shp:%2d\tCo:%2d\t APPENDING\t%s\t  %s\t'%s'\tX:%g %g\tY:%g %g \r", ch, rg, rtp, pd( StringFromList(rtp, klstEVL_RESULTS),9), x, y, shp, nColor, sFoXNm, sYNm, sWnd, wORsX[0], wORsY[0], wORsX[1], wORsY[1]
	...
	// else
	// 	 printf "\t\tConstruct1EvalPointInAcqWnd\tch:%d  rg:%d  pt:%d %s\t  t:\t%8.3lf\t  y:\t%8.3lf\t  Shp:%2d\t%s\t(%d)\tWaves exist \t%s\t...%s\r", ch, rg, n, pd( StringFromList(n, klstEVL_RESULTS),9), x, y, shp, pd(StringFromList( nColor, klstCOLORS),5), nColor, "root:uf:eva" + sChRgNFolder + sXNm,  sYNm
	endif
End
...
// Design issue: the OLA is frame-oriented : it is executed once every frame, sweeps are ignored 
	...
	variable	BegPt		= FrameBegSave( sFolder, pr, bl, fr )		// could this simplify BResultXShift  ???? (see 070424)  SEE  SPREADCURSORS FOR A  DIFFERENT  APPROACH
...
//		if ( MustCompute( wOLARes, ch, rg ) )
			...
			ph					= PH_BASE
			PtLeft				= BegPt + Region( sFolders, ch, rg, ph, CN_BEG ) / SIFact	
			PtRight				= BegPt + Region( sFolders, ch, rg, ph, CN_END ) / SIFact	
			...
			if ( waveExists( wORsX )  &&  waveExists( wORsY ) )					// should exist but user may have killed the wave
				wORsX[ 0 ]	= Region( sFolders, ch, rg, ph, CN_BEG )	 	// or Eval( sFolders, ch, rg, kE_BASE, kTB... ) 
				wORsY[ 0 ]	= wOLARes[ ch ][ rg ][ rtp ]						// or Eval( sFolders, ch, rg, kE_BASE, kY ) 
				wORsX[ 1 ]	= Region( sFolders, ch, rg, ph, CN_END ) 
			...
			printf "\t\t\tOLA1Point(d) OFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\tn:%2d/%2d\t%s\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\trtp:%2d/%2d\tlft:\t%8.4lf\twORsX:%8.4lf\tValue:%g \r", index, sSrc, pr, bl, fr, n, nItems, pd( sOlaNmBP,13), sType, sType, ch, rg, ph, rtp, nTypes,  Region( sFolders, ch, rg, ph, CN_BEG ) ,  wORsX[0], wOLARes[ ch ][ rg ][ rtp ]
			...
			if ( PkDir != kPEAK_OFF )
				...
				NewElementOlaDisp( sOLANmBP, index ) 							//  Returns TRUE if a new wave has just been constructed.
				SetElementOlaDisp( sOlaNmBP, index, wOLARes[ ch] [ rg ][ rtp ] )			// The  RESULT value is  always  added to the display wave. 
				printf "\t\t\tOLA1Point(e) OFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\tn:%2d/%2d\t%s\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\trtp:%2d/%2d\tlft:\t%8.4lf\twORsX:%8.4lf\tValue:%g \r", index, sSrc, pr, bl, fr, n, nItems, pd( sOlaNmBP,13), sType, sType, ch, rg, ph, rtp, nTypes,  Region( sFolders, ch, rg, ph, CN_BEG ) ,  wORsX[0], wOLARes[ ch ][ rg ][ rtp ]
			endif
		endif
		...
		if ( rtp != kE_BASE  &&   rtp != kE_PEAK  &&  rtp != kNOTFOUND )
			...
			if ( rtp == kE_RISE20 )
				...
				// Add value to Acquisition display wave for visual check: draw a symbol
				if ( waveExists( wORsX )  &&  waveExists( wORsY ) )					// should exist but user may have killed the wave
					wORsX[ 0 ]	= ( Eval( sFolders, ch, rg, rtp, kT ) - BegPt ) * SIFact 
					wORsY[ 0 ]	= Eval( sFolders, ch, rg, rtp, kY )  //wOLARes[ ch ][ rg ][ rtp ]	
					wORsX[ 1 ]	= nan
					wORsY[ 1 ]	= nan
				endif
			endif
			...
			if ( rtp == kE_HALDU )
				wOLARes[ ch ][ rg ][ rtp ] 	= ( Eval( sFolders, ch, rg, kE_DEC50, kT ) - Eval( sFolders, ch, rg, kE_RISE50, kT )  ) * SIFact 
				// Add value to Acquisition display wave for visual check: draw a  LINE
				if ( waveExists( wORsX )  &&  waveExists( wORsY ) )					// should exist but user may have killed the wave
					wORsX[ 0 ]	= ( Eval( sFolders, ch, rg, kE_RISE50, kT ) - BegPt ) * SIFact 
					wORsY[ 0 ]	= Eval( sFolders, ch, rg, kE_RISE50, kY ) //wOLARes[ ch ][ rg ][ rtp ]	
					wORsX[ 1 ]	= ( Eval( sFolders, ch, rg, kE_DEC50, kT ) - BegPt ) * SIFact 
					wORsY[ 1 ]	= Eval( sFolders, ch, rg, kE_DEC50, kY )  //wOLARes[ ch ][ rg ][ rtp ]	
				endif
			endif

			if ( rtp == kE_RT2080 )
				wOLARes[ ch ][ rg ][ rtp ] 	= ( Eval( sFolders, ch, rg, kE_RISE80, kT ) - Eval( sFolders, ch, rg, kE_RISE20, kT )  ) * SIFact 
				// Add value to Acquisition display wave for visual check: draw a  LINE
				if ( waveExists( wORsX )  &&  waveExists( wORsY ) )					// should exist but user may have killed the wave
					wORsX[ 0 ]	= ( Eval( sFolders, ch, rg, kE_RISE20, kT ) - BegPt ) * SIFact 
					wORsY[ 0 ]	= Eval( sFolders, ch, rg, kE_RISE20, kY ) //wOLARes[ ch ][ rg ][ rtp ]	
					wORsX[ 1 ]	= ( Eval( sFolders, ch, rg, kE_RISE80, kT ) - BegPt ) * SIFact 
					wORsY[ 1 ]	= Eval( sFolders, ch, rg, kE_RISE80, kY )  //wOLARes[ ch ][ rg ][ rtp ]	
				endif
			endif
			...
			SetElementOlaDisp(   sOlaNm, index, wOLARes[ ch] [ rg ][ rtp ] )			// The  RESULT value is  always  added to the display wave. 
			 printf "\t\t\tOLA1Point(f)  OFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\tn:%2d/%2d\t%s\t'%s'\t[%s]\tch:%2d\trg:%2d\tph:%2d\trtp:%2d/%2d\tlft:\t%8.4lf\twORsX:%8.4lf\tValue:%g \r", index, sSrc, pr, bl, fr, n, nItems, pd( sOlaNm,13), sType, sType, ch, rg, -1, rtp, nTypes,  Region( sFolders, ch, rg, ph, CN_BEG ) ,  1.2345, wOLARes[ ch ][ rg ][ rtp ]
		endif
...
//			DisplayEvaluatedPoints( sFolders, ch, rg, sWNm )
//		endif
//		//  Do all the fitting 


		//  Do all the fitting 
		variable	fi	= PossiblyGetFitIndex( sOlaNm )							// extracts those starting with  'F0_...' , 'F1_...' ,   ...   'F9_...'
		if (  fi != kNOTFOUND )
			variable	nFitFunc		= FitFnc( sFolders, ch, rg, fi )
			string  	sFitPar		= StringFromList( 1, sOlaNm, "_" )			// assumption naming
			variable	par			= WhichParam( nFitFunc, sFitPar )
			if (  IsNewChannelRegionFit( ch, rg, fi )  ||  n == 0 )						// order is important :  IsNewChannelRegionFit()  MUST be executed independently of  'n' 
				OneFit( sFolders, wOrg, ch, rg, fi, BegPt, SIFact, lstFitResults )		// lstFitResults will be set
			endif
			value	= str2num( StringFromList( par, lstFitResults ) )
			 printf "\t\t\tOLA1Point(g) OFrC:%3d\t%s\tpr: %d   bl: %d  fr: %d\tn:%2d/%2d\t%s\t'%s'\t%s\tch:%2d\trg:%2d\tfit: %2d\tpar:%2d/%2d\tlft:\t%8.4lf\twORsX:%8.4lf\tValue:%8g \tlstFitResults:%s \r", index, sSrc, pr, bl, fr, n, nItems, pd( sOlaNm,13), sType, pd(sFitPar, 5), ch, rg, fi, par, nTypes,  Region( sFolders, ch, rg, ph, CN_BEG ) ,  1.2345, value, lstFitResults
			NewElementOlaDisp( sOlaNm, index ) 								//  Returns TRUE if a new wave has just been constructed.
...
static Function		IsNewChannelRegionFit( ch, rg, fi )
	...
	if ( ! nvar_exists( PrevCh )  ||  ! nvar_exists( PrevRg )  ||  ! nvar_exists( PrevFi ) )					
		...
		if ( ch == PrevCh  &&  rg == PrevRg  &&  fi == PrevFi )	// Is  NOT the first channel/region/fit..
			return	FALSE						// ..and is same as previous so we will not do the same fit again 
		else
