...
//	variable	rtp			= WhichListItemIgnoreWhiteSpace( sTyp, klstEVL_RESULTS ) 	// e.g. kE_Base=15,  kE_PEAK=25  todo fits......
	...
	variable	nInstn, bAutoscl, YZoom, YOfs
	...
	if ( s.eventCode == kLBE_MouseDown  &&  !( s.eventMod & kSHIFT )  &&  !( s.eventMod & kRIGHTCLICK ) )	// Only LEFT mouse clicks  without SHIFT  are interpreted here.  // ??? In this listbox only MouseDown is detected, whereas in the similar  DataSections  listbox  MouseUp  is detected???
		if ( w < 0 ) 														//  A  Mode/Range  column cell has been clicked  : ignore  clicks 
			...
			if ( ! ( nOldState & 3 ) ) 		// the cell must have been unselected, do not attempt to select a trace twice
				...
				sCurve	= BuildCurve( sChan, ra, mo, nInstn, bAutoscl, YOfs, YZoom, sRGB )
				...
				for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )					
					...
					ExtractCurve( sCurve, sChan, ra, mo, nInstn, bAutoscl, YOfs, YZoom, sRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
				...
				for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )						// Rebuild all traces in window
					...
					ExtractCurve( sCurve, sChan, ra, mo, nInstn, bAutoscl, YOfs, YZoom, sRGB )	// break 'sCurve' to get all traces to be displayed in window 'w'...
					Range2Pt( sFolder, ra, pr, bl, fr, sw, BegPt, EndPt, Pts, bIsFirst )
					DurAcqDrawTraceAndAxis( sFolder,  w, nCurve, sCurve, BegPt, Pts, bIsFirst, ra, nSmpInt, ResultXShift )
					DurAcqDrawXPositionOfYAxis( sFolder, w, nCurve, sWndAcDis, sChan, pts, nCurves, sRGB, LastDataPos )	
				endfor
...
// retrieve all window / trace arrangement variables (=the display configuration contained in WLoc and WA) from 'sFile' having the extension 'DCF' 
	...
	for ( w = 0; w < kMAX_ACDIS_WNDS; w += 1 )
		...
		variable	ra, mo, rnInstance, rAutoscl, rYOfs, rYZoom 
		string		sChan, sRGB
		variable	pr = 0,  bl = 0,  fr = 0,  sw = 0
		...
		for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )
			sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )		
			ExtractCurve( sCurve, sChan, ra, mo, rnInstance, rAutoscl, rYOfs, rYZoom, sRGB )// break 'sCurve' to get all traces to be displayed in window 'w'...
...
//????WHY is  Range2Pt()   OUTSIDE the curves loop????????? 
	...
	variable	nCurve, CurveCnt	= ItemsInList( sCurves, ksCURVSEP )
	for ( nCurve = 0; nCurve < CurveCnt; nCurve += 1 )			
	  	sCurve	= StringFromList( nCurve, sCurves, ksCURVSEP )
		DurAcqDrawTraceAndAxis( sFolder,  w, nCurve, sCurve, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift )
	endfor	
End
...
static  Function		DurAcqDrawTraceAndAxis( sFolder,  w, nCurve, sCurve, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift )	
// If the user changes the AxoPatch Gain the trace in the Acq Display should keep it's height but the Y scaling should adjust. This may occur any time during acquisition. The DAT file will reflect the changed gain only at the beginning of thr next data section.
	string  	sFolder, sCurve
	variable	w, nCurve, BegPt, Pts, bIsFirst, nRange, nSmpInt, ResultXShift

	string 	rsChan = "",  rsRGB = ""
	variable	ra, mo, rnInstance, rAuto, rYOfs, rYZoom
	string 	sWNm	= WndNm( w )							

	ExtractCurve( sCurve, rsChan, ra, mo, rnInstance, rAuto, rYOfs, rYZoom, rsRGB )			// parameters are changed
	...
	if (  Pts > 0   &&   waveExists( $sFolderTNm )   &&    nRange == ra )
		...
		gResultXShift =  ( ra ==  kRESULT )  ?  ResultXShift  :  0	// 040724
		 printf "\t\t\tDurAcqDrawTraceAndAxis()\tbIsFirst:%d  w:%2d/%d \tcv:%2d\tR:%d  M:%d  W:%s  \tO:%s \t'%s'\tpts: %d  smpint: %d  gResultXShift:%7.2lf \t'%s..' \r", bIsFirst, w, t,  nCurve, ra, mo, sWNm, rsChan, rsRGB, Pts, nSmpInt, gResultXShift, sCurve[0,80]
		DurAcqDrawTrace( sFolder, w, nCurve, sWNm, rsChan, BegPt, Pts, bIsFirst, mo, ra, nSmpInt, rsRGB, Gain ) 
		DurAcqRescaleYAxis( w, nCurve, sWNm, rsChan, rYOfs, rYZoom, Gain )										// Gain may change any time during acquisition. 
	endif
End
...
// Different  data are displayed under the same name, but any operation (e.g. resize window) destroys the data leaving only the last...
	...
	wave  /T	wYAxisNm		= root:uf:disp:wYAxisNm								// ..or (if it is not the 1. trace) the existing (not erased) trace is given new data....
	...
	if ( WhichListItem( sTNmUsedNF, TraceNameList( sWNm, ";", 1 ) ) == kNOTFOUND )		// For Redrawing outside acq ( e.g. changed zoom,ofs ) the traces are not erased, they exist : here  we avoid drawing them over and over #1, #2, #3... when the YOfs slider is moved 
		if ( nMode == kMANYSUPIMP   ||   ( nMode == kCURRENT && bIsFirst ) )			// in CURRENT mode the trace is appended only once : IGOR updates automatically
			wYAxisExists[ w ][ nCurve ] = TRUE									// 141102 mark this Y axis as 'displayed'  as  we can later act only on  'displayed'  Y axis
			...
			if ( nCurve == 0 ) 												// append the first trace with its Y Axis to the left....
				...
				// Here the connection is made between a certain trace and its accompanying axis name !  AxisInfo  will return  the name of the controlling wave 
				 AppendToGraph /Q /R=$wYAxisNm[ nCurve ]	/W=$sWNm  /C=( rnRed, rnGreen, rnBlue )	 $sTNmUsed 
			endif
			 printf "\t\t\t\tDurAcqDrawTrace(2) after appending \tmode: %d\tbIs1:%d\tWNm: '%s' \tsTNm:%s ->\t%s  \tUsedTrc[ w:%2d ] = %d \tvalid for nCurve != 0: AxNm: '%s'\t[cnt:%3d]\tTNL1: '%s'  \r", nMode, bIsFirst, sWNm, sTNm, pd(sTNmUsed,22),  w, nCurve+1, wYAxisNm[ nCurve ], ItemsInList( TraceNameList( sWNm, ";", 1 )  ),  TraceNameList( sWNm, ";", 1 )  
		endif
	endif
...
// 060603 new
static  Function		 DurAcqDrawXPositionOfYAxis( sFolder, w, nCurve, sWNm, sTNm, pts, nCurves, sRGB, LastDataPos )
...
// This leads to the complex code requiring  much bookkeeping with the help of  YAxisCnt(),   wYAxisBotAxEndMax, wYAxisExists
...
//	wave	wYAxisExists		= root:uf:disp:wYAxisExists
	...
	string		sAxisNm		= wYAxisNm[ nCurve ] 									// 030610
	// printf "\t\t\t\tDurAcqDrawXPositionOfYAxis()  w:%d  ut:%d   pts:%d  SI:%g  xScl:%g  v_min:%g   v_max:%g   nRightAxisCnt:%d   BotAxisEnd:%g  sAxisNm:'%s' \r", w, nCurve+1,  pts, nSmpInt, xScl, v_min, v_max, nRightAxisCnt,  BotAxisEnd, sAxisNm
...
// 060606
	...
	if ( nCurve > 0 )
		...
		//ModifyGraph	/W=$sWNm axisEnab( bottom ) = { 0, 1- ( nYAxis  >1 ) * .1 }			//  supplies a small margin to the right of the rightmost axis if there is at least  1 right Y axis for the rightmost Y axis numbers    
		 ...
		 printf "\t\t\t\tDurAcqDrawXPositionOfYAxis(2)\t'%s'\t%s\tut:%d/%d\tRiAxCnt:%d \tbottom axis   v_min:%g, v_Max:%g -> LastDataPos:%g  \tThisAxPos(%s):%4d\tBotAxEnd:%g / %g\r", sWNm, pd(sAxisNm,5), nCurve+1,  nYAxis, nRightAxisCnt, v_min,  1234,  LastDataPos, pad(wYAxisNm[ nCurve],5),YAxisXPos, BotAxisEnd, 12345//wYAxisBotAxEndMax[ w ]
...
static  Function		DurAcqRescaleYAxis( w, nCurve, sWNm, sChan, YOfs, YZoom, Gain )	
...
// 060607
	...
	if ( wYAxisLastNegLim[ w ][ nCurve ] != wYAxisNegLim[ w ][ nCurve ]   ||   wYAxisLastPosLim[ w ][ nCurve ] != wYAxisPosLim[ w ][ nCurve ] ) 
		 printf "\t\t\t\tDurAcqRescaleYAxis() \tw:%2d \tcv:%2d\tWNm:'%s'\t'%s'\t'%s'\tos:\t%7g\tzo:\t%7g\tyax:\t%7g\tgn:\t%7g\twLast:%g..\t%g\t-> \twLim:%10.2lf...\t%10.2lf\t  \r", w, nCurve, sWNm, wYAxisNm[ nCurve ], sChan, YOfs, YZoom, yAxis, Gain, wYAxisLastNegLim[ w ][ nCurve ],wYAxisLastPosLim[ w ][ nCurve ], wYAxisNegLim[ w ][ nCurve ],wYAxisPosLim[ w ][ nCurve ]
		wYAxisLastNegLim[ w ][ nCurve ]	= wYAxisNegLim[ w ][ nCurve ]
		wYAxisLastPosLim[ w ][ nCurve ]		= wYAxisPosLim[ w ][ nCurve ] 
		SetAxis /Z /W=$sWNm $wYAxisNm[ nCurve ],  wYAxisNegLim[ w ][ nCurve ], wYAxisPosLim[ w ][ nCurve ]  	
...
//		 printf "\t\t\t\tSetMultipleYAxisRange()\tcurve:%2d \t\tWNm:'%s'\tAxisNm[ nCurve:%d ] : '%s'\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\twLim:%10.2lf...\t%10.2lf [PosLim]  \r",  nCurve, sWNm, nCurve,  wYAxisNm[ nCurve ], NegLimit, PosLimit
//
//		if ( cmpstr( wYAxisNm[ 0 ], "left" ) )
//			FoAlert( ksACQ, kERR_IMPORTANT,  "Axis name error with  nCurve = 0   /   ut = 1 . Should be 'left'  but is  '" + wYAxisNm[ nCurve ] + "' . " )  
//		endif
//		SetAxis /Z /W=$sWNm $wYAxisNm[ nCurve ],	NegLimit, PosLimit 	
//
//	endif
//End
...
//	DisplayOffLineAllWindows( sFolder  )											// This is to display a changed Y axis in all windows which contain this AD channel   ( could probably be done simpler and more directly.....)
	...
	for ( w = 0; w < kMAX_ACDIS_WNDS; w += 1 )
		...
		if ( WinType( sWNm ) == kGRAPH )
			...
			string  	sMatchingTraces	= ListMatch( sTNL, sChan + "*" )				// e.g. 'Adc1SC_;Adc1FM_1000;Adc1xxxx;'  	
			variable	mt, nMatchingTraces	= ItemsInList( sMatchingTraces )
			string  	rsRGB												// parameters are set by  ExtractCurve()
			variable	ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom					// parameters are set by  ExtractMoRaName()  and  ExtractCurve()
			for ( mt = 0; mt < nMatchingTraces; mt += 1 )
				...
				ExtractCurve1( sCurve, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rsRGB )			// The remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rsRGB' are extracted from 'sCurve/sCurves' . 
				//ExtractCurve( sCurve, sChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	// Now all parameters extracted from 'sCurve' are valid as 'sCurve' is extracted from 'sCurves' which always contains currently valid values
				DurAcqRescaleYAxis( w, nCurve, sWNm, sChan, rYOfs, rYZoom, Gain )
			endfor
		endif
	endfor
End
...
// Executed whenever the mouse is moved within one of the acquisition windows. Gets mouse position in pixels and computes wave and cursor values in axis coordinates which are displayed in the Panel.
	...
	if ( s.eventCode	== kWHK_resize )
		...
		string  	sCurves, rsChan, rsRGB
		variable	nCurve, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom
		string  	sCurve 		= GetUserData( 	s.winName,  "",  "sCurve" )						// Get UD sCurve 
		ExtractCurve( sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )			// here only 'rsChan, ra, mo'  are extracted from 'sCurve' , the remaining parameters contain old invalid values (as 'sCurve' has been set only once when the controlbar was constructed)
		sCurves	= RetrieveCurves( w )
		...
		//ExtractCurve( sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )			// Now all parameters extracted from 'sCurve' are valid as 'sCurve' is extracted from 'sCurves' which always contains currently valid values
		ExtractCurve1( sCurve, rsChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rsRGB )					// The remaining (now valid) parameters 'rbAutoscl, rYOfs, rYZoom, rsRGB' are extracted from 'sCurve/sCurves' . 

		variable	bDisableCtrlOfs	= DisableCtrlZoomOfs(    rsChan, bAcqControlbar, rbAutoscl ) 
...
Function		UpdateCurves( w, nIndex, sValue, sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	
	variable	w, nIndex
	string  	sValue, sCurve
	string  	&rsChan, &rsRGB
	variable	&ra, &mo, &rnInstance, &rbAutoscl, &rYOfs, &rYZoom
	string 	sCurves
	variable	nCurve
	ExtractCurve( sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )			// here only 'rsChan, ra, mo'  are extracted from 'sCurve' , the remaining parameters contain old invalid values (as 'sCurve' has been set only once when the controlbar was constructed)
	sCurves	= RetrieveCurves( w )
	...
	switch ( nIndex )
		case	cbAUTOSCL:
			...
			string  	sFolder	= ksACQ
			variable	Gain		= GainByNmForDisplay( sFolder, rsChan )
			AutoscaleZoomAndOfs( w, rsChan, mo, ra, rbAutoscl, rYOfs, rYZoom, Gain )
	...
	if ( nCurve == kNOTFOUND )
	 ...
	 printf "\t\tUpdateCurves()  \tw:%2d  nCv:%2d\t%s\tAutoscale:%2d\tYzoom:\t%7.2lf\tYofs:\t%7.1lf\tGain:\t%7.1lf\t%s\t \r", w, nCurve, pd( StringFromList( nCurve,   RetrieveCurves( w ), ksCURVSEP ),52), rbAutoscl, rYZoom, rYOfs, Gain, rsRGB
	return	nCurve
End
...
static Function /S	  ExtractCurve1( sCurves, sTNm, ra, mo, rbAutoscl, rYOfs, rYZoom, rsRGB )			// sChan/mo/ra are known already....
// Extracts 4 entries (rbAutoscl, rYOfs, rYZoom, rsRGB)  when  'sCurves'  (containing all curves of 1 window)  and the trace name, the mode and the range are given. 
// There can only be 1 instance of each trace/mode/range  in 'sCurves'
	...
	//ExtractCurve( sCurve, sChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )			// sChan/mo/ra are known already so by doubling the code we simplify.......
...
static Function	  	ExtractCurve( sCurve, rsTNm, rnRange, rnMode, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB  )		// parameters are changed
// Extracts 8 entries including the trace name, the mode and the range when  'sCurve'  is given
	string		sCurve
...
static Function	  /S	BuildCurve( sTNm, nRange, nMode, nInstance, bAuto, yOfs, yZoom, sRGB )
...
//	nCurve	= WndUsersTrace( w )
	for ( nCurve = 0; nCurve < nCurves; nCurve += 1 )
		...
		variable	rnRange, rnMode, rnInstance, rbAutoscl, rYOfs, rYZoom 
		string		 rsTNm, rsRGB
		ExtractCurve( sCurve, rsTNm, rnRange, rnMode, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	// Get all traces/curves from this window...
		variable	bAcqControlbar	= AcqControlBar( w )
...
// depending on  'bAcqControlbar'  build and show or hide the controlbar  AND  show or hide the individual controls (buttons, listboxes..)
	...
	string  	sCurve	= ExtractCurve1( sCurves, sChan, ra, mo, rbAutoscl, rYOfs, rYZoom, rsRGB )	// sChan/mo/ra are known already so we extract only  rbAutoscl, rYOfs, rYZoom, rsRGB....
	...
	string  	sTNmMoRa	= sChan + " " + RangeNmLong( ra ) + " " + ModeNm( mo ) 	// e.g. 'Adc1 Frames M'	needs  cXTRACENMSIZE >= 120
	 printf "\tCreateControlBarInAcqWnd3( sWNm:%s %d ) \t%s\t mo:%d ra:%d\t%g\t%g\t%s \r",  sWNm, bAcqControlbar, pd( sTNmMoRa, 13), mo, ra , rYOfs, rYZoom, rsRGB 
...
// Executed only when the user changes the 'AutoScale' checkbox : If it is turned on, then YZoom and YOfs values are computed so that the currently displayed trace is fitted to the window.
	...
	variable	ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom							// parameters are set by  UpdateCurves()
	variable	w		= WndNr( s.Win )
	string  	sCurve 	= GetUserData( 	s.win,  "",  "sCurve" )							// Get UD sCurve 
	variable	nCurve	= UpdateCurves( w, cbAUTOSCL,  num2str( s.checked ),  sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	
	...
	string  	sFolder	= ksACQ
	variable	Gain		= GainByNmForDisplay( sFolder, rsChan )
	DurAcqRescaleYAxis( w, nCurve, s.Win, rsChan, rYOfs, rYZoom, Gain )
...
// Adjust   YZoom  and   YOffset  values  depending on the state of the  'Autoscale'  checkbox.  Return the changed  YZoom  and  YOfs  values  so that they can be stored in TWA  so that the next redraw will reflect the changed values.
	...
	string 	sTNm	= sChan + BuildMoRaName( ra, mo ) + sMORAPOINTSEP		// e.g.  'Adc0SC _'
	if ( mo == kMANYSUPIMP )
		sTNm	+= "0"												// e.g.  'Adc0SM _0'		!!! naming assumption : one superimposed trace ends with 0, but there can also be additional traces ending with any point number
	endif
...
Function 		CbPopmenuColors( s )
	...
	variable	w		= WndNr( s.Win )
	string  	sCurve 	= GetUserData( 	s.win,  "",  "sCurve" )							// Get UD sCurve 
	variable	nCurve	= UpdateCurves( w, csRGB, s.PopStr, sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	
	...
	string  	sTNm			= rsChan + BuildMoRaName( ra, mo ) + sMORAPOINTSEP	// e.g. 'Dac1FM_'	( in many/superimposed mode there is an integer point number appended, which we are not interested in)
	string  	sMatchingTraces	= ListMatch( sTNL, sTNm + "*" )						// e.g. 'Dac1FM_' ,  'Dac1FM_1000'  and  'Dac1FM_2000'			
	variable	mt, nMatchingTraces	= ItemsInList( sMatchingTraces )
	variable	nRed, nGreen, nBlue 
	ExtractColors( s.PopStr,  nRed, nGreen, nBlue )
	for ( mt = 0; mt < nMatchingTraces; mt += 1 )
		ModifyGraph	/W = $s.Win	rgb( $StringFromList( mt, sMatchingTraces ) ) = ( nRed, nGreen, nBlue )	
	endfor
	// Also change the color of the units which are displayed as a textbox right above the Y axis)
	wave  /T	wYAxisNm		= root:uf:disp:wYAxisNm							
	string		sAxisNm		= wYAxisNm[ nCurve ] 									
	TextBox 	/W=$s.win /C /N=$sAxisNm   /G=( nRed, nGreen, nBlue)  					// the textbox has the same name as it's axis
End
...
// Action proc executed when the user selects a zoom value from the listbox.  Update  TWA  and  change axis and traces immediately to give some feedback.
	...
	variable	w		= WndNr( s.Win )
	string  	sCurve 	= GetUserData( 	s.win,  "",  "sCurve" )							// Get UD sCurve
	variable	nCurve	= UpdateCurves( w, cYZOOM, s.PopStr, sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	
...
Function		CbSliderYOfs( s )
	...
	variable	w		= WndNr( s.Win )
	string  	sCurve 	= GetUserData( 	s.win,  "",  "sCurve" )							// Get UD sCurve
	variable	nCurve	= UpdateCurves( w, cYOFS, num2str( s.curval ), sCurve, rsChan, ra, mo, rnInstance, rbAutoscl, rYOfs, rYZoom, rsRGB )	
