...
// permanent, can not and should not be closed, can not and should not be minimised, are initialised and built in 1 step, often need recomputing/resizing/redrawing when e.g. the number of regions or fits changes  
	...
	if ( WinType( sWin ) != kPANEL )											// only if the panel does not yet  exist ..
		...
		yLocMax 	= UFCom_GetIgorAppPixelY() -  ySize - kY_MISSING_PIXEL_BOT			// the lowest panel position			// todo ???? ysize pixel
...
//  
	...
	for ( n = 0; n < nCnt; n += 1 )			// for each control in wPn[], no tab or any other expansions yet
		nType	= WhichListItem( UFCom_RemoveWhiteSpace( StringFromList( kTYPE, wPn[ n ], ksSEP_WPN ) ), lstTYPE )
		sType	= StringFromList( nType, lstTYPE )
		sName	=UFCom_RemoveWhiteSpace( StringFromList( kNAME,	wPn[ n ], ksSEP_WPN ) )
...
//  Draw the panel and the controls taking into account  Tab and Block grouping which was computed in  'PnSize()' . 
	...
	for ( n = 0; n < nCnt; n += 1 )											// for all controls (=all lines in 'wPn' )
		...
		sName	= UFCom_RemoveWhiteSpace( StringFromList( kNAME, wPn[ n ], ksSEP_WPN ) )			// or from llstCNames
		...
		sActProc	= UFCom_RemoveWhiteSpace( StringFromList( kACTPROC, wPn[ n ], ksSEP_WPN ) )
		nCiL		= str2num( StringFromList( kMXPO, wPn[ n ], ksSEP_WPN ) )
		nOvSz	= str2num( StringFromList( kOVS, wPn[ n ], ksSEP_WPN ) )
		xBodySz	= str2num( StringFromList( kXBODYSZ, wPn[ n ], ksSEP_WPN ) )
		...
		sFormEntry= UFCom_RemoveWhiteSpace( StringFromList( kFORMENTRY, wPn[ n ], ksSEP_WPN ) )
...
static Function		IsTrueTab( lstTabTi )	
	...
	// return	strlen( UFCom_RemoveWhiteSpace( ReplaceString( ksSEP_TAB, lstTabTi, "" ) ) ) > 0// FALSE if there is no title at all = all titles are empty or blank strings.  TRUE if there is at least 1 tab with a non-empty-non-blank title.
End
...
static Function		PanelPopup3( bDisable, bVisib, xPos,  xSize, yPos, sTitle, sProc, sFo, sWin, sFCNm, sName, lstTabTi, lstBlkTi, lstRowTi, lstColTi, nBodyWidth, sFormEntry, sHelpTopic )
	...
	if ( xPos+1 < 0  ||  yPos * kYLINEHEIGHT  < 0 )
		UFCom_DeveloperError( "Position clipping control " + sFCNm + " .   x : " + num2str( xPos ) + ",  y: "  + num2str( yPos * kYLINEHEIGHT ) )
	endif	
...
// Returns the number of blocks in  tab  'nTab' . 
	...
	variable	nTab
//	// printf "\t\t\t\tBlkCnt( \t%s\tnTab:%2d ) \t-> \t'%s'\t%s\t'%s' \t%d \t \r",  UFCom_pd(lstBlkTi,14),  nTab, ksSEP_TAB, StringFromList( nTab, lstBlkTi, ksSEP_TAB ), ksTAB_B_SEP, ItemsInList( StringFromList( nTab, lstBlkTi, ksSEP_TAB ), ksTAB_B_SEP )
