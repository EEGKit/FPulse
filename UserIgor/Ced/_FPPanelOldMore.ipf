...
//constant		kTYPE=0, kNXLN=1, kXPOS=2, kMXPO=3, kOVS=4, kTABS=5, kBLKS=6, 	kMODE=7,  kNAME=8,  kROWTI=9,   kCOLTI=10,  kACTPROC=11,  kXBODYSZ=12, kFORMENTRY=13, kINITVAL=14, kVISIB=15
constant	 kHELPTOPIC=16
...
//		PnInitVars( sF, sWin, $sPnWvNm )									// when increasing the number of blocks( e.g. =regions) or when loading another setting or script (number of chans=gains changes)  then create the additionally required controls (and their underlying variables) 
//		PnSize( sF, sWin, $sPnWvNm, xSize, ySize, lstPosX, lstPosY, lstTabGrp, lstTabcoBeg, lstTabcoEnd, llstTypes, llstCNames, lllstTabTi, llstBlkTi, lllstRowTi, lllstColTi, lllstVisibility, llstMode )	// most parameters are references 
...
//		// printf "\t\tfSetvar_struct3( sControlNm:'%s'   Value:%g , sVarName:'%s'  \t[will call if found: %s  ]\r", s.ctrlname, s.dval, s.vname,  pd(sProcNm+"()",26) 
...
//
//// 2009-10-22
////	if ( ! PnUserWantsHelp( s.win, s.ctrlName, s.eventcode, sEventName, s.eventmod, kCI_POPUPMENU, -1, sHelpTopic ) )	// anything else but left mouse button, e.g. right mouse button or left mouse and  ALT, CTRL or SHIFT
...
//	if ( ! bWantsHelp  &&  s.eventcode == 2 ) //UFCom_CBE_mouseup )								// 2009-07-02	React only on documented events. Otherwise e.g. this killing the control (as in PnDraw) would  trigger the action procedure 
//		SetTheSingleRadioCBGlobal( s.ctrlname, s.checked )
//		FUNCREF   fCbProc_struct3  fCbPrc = $( sProcNm ) 								// ...execute the action procedure ...
//		fCbPrc( s )																// ...if there is one defined, it can also be missing ( sProcNm =  "" ) 
//	endif
...
//	string  	sHelpTopic	= GetUserData(	s.win,  s.ctrlName,  "sHelpTopic" )
//	string  	sEventName	= ""
//// 2009-10-22 modify for Igor6     wrong
