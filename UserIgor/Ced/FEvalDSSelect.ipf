...
//	variable	ySizeNeed	= SctCnt * kLB_CELLY
	...
	LstPanels_Eva3Set( AddListItem( sDSPanelNm, LstPanels_Eva3() ) )	// ??? todo_c could prevent adding more than once....
							...
							for ( d = 0; d < nDerived; d += 1 )	
								...
								if ( cmpstr( sOneSelResult, sDirectTitle + sPostFix ) == 0 )			// we found a selected  direct title (d=0)  or  one of the derived titles (d=1..6) and we now know by index 'd' which it is
									...
									if ( nMode == kCOL_TITLES_ )
										...
										if ( ! bIsString )
											sprintf sItem, "%g", Eval( ch, rg, t, d ) * Eval_UnitFactor_( ch,  t, d )	// the numbers	
											//sprintf sItem,  "%s:%g  ", sOneSelResult, Eval( ch, rg, t, d )		// the numbers	
										else
