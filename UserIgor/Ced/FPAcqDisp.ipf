...
Function	DisplayOffLine( sFolder, wG, wIO, wFix, nWndBeg, nWndEnd )
	...
	for ( b  = 0; b < eBlocks( wG ); b += 1 )
		...
		for ( f = 0; f < fCnt; f += 1 )
			...
			for ( s = 0; s < sCnt; s += 1 )
				DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, s, 0,   kWNDDIVIDER_ - 1, nWndBeg, nWndEnd )		// 031008 p=0 : not really protocol aware
			endfor
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_FRAME,  0,	kWNDDIVIDER_ - 1, nWndBeg, nWndEnd )	
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_PRIM,	    0,	kWNDDIVIDER_ - 1, nWndBeg, nWndEnd )		
			DispDuringAcq( sFolder, wG, wIO, wFix, 0, b, f, kDISP_RESULT, 0, kWNDDIVIDER_ - 1, nWndBeg, nWndEnd )	
		endfor
	endfor
End
...
Function		DispDuringAcqCheckLag_( sFolder, wG, wIO, wFix, pr, bl, fr, sw )		// partially  PROTOCOL  AWARE
	...
	if ( gbDispAllDataLagging  ||  nLag < cLAG  ||  fr + sw == 0 )
		DispDuringAcq( sFolder, wG, wIO, wFix, pr, bl, fr, sw, 0,   kWNDDIVIDER_ - 1, nWndBeg, nWndEnd )
...
// Different  data are displayed under the same name, but any operation (e.g. resize window) destroys the data leaving only the last...
	...
	wave	wYAxisExists	= root:uf:aco:disp:wYAxisExists								// ..during acq the traces are regularly erased, so 'AppendToGraph' either actually draws the first trace in a blank window...
...
Function	/S	BuildTraceNmForAcqDisp( sFolder, sTNm, nMode, nRange, BegPt )
 	string		sFolder, sTNm
 	variable	nMode, nRange, BegPt
	return	"root:uf:" + ksACOld + ":dispFS:" + BuildTraceNmForAcqDispNoFolder( sTNm, nMode, nRange, BegPt )
End
...
Function		ConstructYAxis()
	...
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt() )			root:uf:aco:disp:wYAxUsedMax	  = 0		// maximum number of traces / curves used in a specific window
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:aco:disp:wYAxisExists		  = 0		// flag telling if the specified trace / curve is already displayed in the window
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:aco:disp:wYAxisNegLim	  = -5000	// minimum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:aco:disp:wYAxisLastNegLim = -5000	// minimum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:aco:disp:wYAxisPoslim	  =  5000	// maximum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt(), kMAXCHANS ) root:uf:aco:disp:wYAxisLastPosLim  =  5000	// maximum axis end value
	make	/O /N=( kMAXCHANS * ModeCnt() * RangeCnt() )			 root:uf:aco:disp:wYABotAxEndMax = -Inf	// bottom axis end point in a specific window
