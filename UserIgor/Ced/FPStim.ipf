...
//?  if channel number is missing a segment is ignored (reason: earlier, when chans/frames/elements are set)
	...
	make  /O	/N = (  eBlocks( wG ), eMaxFrames( wG, wFix ), eMaxSweeps( wG, wFix ) ) $"root:uf:" + sFolder + ":ar:wBFS" = 0	// storing the block / frame / sweep combination in 1 number is a...
	wave	wBFS												=    $"root:uf:" + sFolder + ":ar:wBFS"		// ..memory saving approach to overcome IGORs limitation of 4 wave dimensions 

	variable	 c = 0, b = 0, f = 0, s = 0, e, k,  l, nType, nLine, nLastK

	make /O /T  /N = ( vMaxLines( wVal ) ) 		$"root:uf:" + sFolder + ":ar:wLineRef"				// stores channel, block and element number for a given wVal line number 
	...
	make  /O	/N = ( maxDChs, bfsPtr, maxEle, cSIZE )	$"root:uf:" + sFolder + ":ar:wE" =	0	
	wave	wE	= 							$"root:uf:" + sFolder + ":ar:wE" 																
 		...
 		for ( b  = 0; b < eBlocks( wG ); b += 1 )
			for ( f  = 0; f < eFrames( wFix, b ); f += 1 )
				...
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )
					...
					// print "line1   ", nLine, "f:", f ,"c:", c , "b=", b , e , "storing type=", nType, "=?=", eV( wE, wBFS, c, b, f, 0, e, cTYP), "=retrieved"
				 	 // printf "\t\t\tExtractValSetEle() c:%d\tb:%d\tf:%d\ts:%d\te:%2d/%2d\tl:%d\tType:%s\t'%s' \r", c, b, f, s, e, eElems( wEinCB, c, b ), nLine,  pad(mS( eV( wE, wBFS, c, b,f,s,e, cTYP ) ),8) , GetScrLine( nLine ) 
					ExtractEValues( wVal, wELine, wE, wBFS, c, b, f, 0, e )	// break 'Dac' string into chan, dur, ddur, amp, damp
					SetLineRef( wLineRef, nLine, c, b, e )
				endfor
			endfor
		endfor
	endfor
		 	...
		 	for ( f = 0; f <  eFrames( wFix, b ); f += 1)			
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )
					for ( k =0; k < eKeys( wE ); k += 1)					// copy all subkeys including nType..
						variable	value = eV( wE, wBFS, c, b, f , 0 , e , k )			// ..from frame 0 / sweep 0.. 
					 	// printf "\t\t\tExtractValSetEle() \treading \tc:%d\tb:%d\tf:%d\ts:%d\te:%2d/%2d\tk:%2d\tType:%s\tvalue:%g    \r", c, b, f, 0, e, eElems( wEinCB, c, b ), k,  pd(mS( eV( wE, wBFS, c, b, f, 0, e, cTYP ) ),8) , value
						for ( s = 0; s < eSweeps( wFix, b ); s += 1 )		
							eSet( wE, wBFS,  c, b, f , s , e , k , value )	
						 // printf "\t\t\t\t\t\t\t\tcopying \tc:%d\tb:%d\tf:%d\ts:%d\te:%2d/%2d\tk:%2d\tType:%s\t    \r", c, b, f, s, e, eElems( wEinCB, c, b ), k,  pd(mS( eV( wE, wBFS, c, b, f, s, e, cTYP ) ),8)  
						endfor											
					endfor											
				endfor
			endfor
...
Static Function		SetLineRef( wLineRef, nLine, c, b, e )
...
Static Function		GetLineRef( wLineRef, nLine, c, b, e )
...
//	string	sScriptEntry
//
//variable nType =eV( wE, wBFS, c, b, f, s, e, cTYP)
...
//			// print  "*****", c, b, f, s, e, "->", n, vG( wVal, nLine, nskidx+1, f ), "mode:", eV( wE, wBFS, c, b, f, s, e, cMOD ), "del:", eV( wE, wBFS, c, b, f, s, e, cDEL )
//		endif
//	endfor
//	
//	endif //0805
...
//what if user forgets it???
	...
	// printf  "\t\tExtractValSetEle\t %s\tc:%d\tf:%d\ts:%d\te:%d\t%s\t--> Cha:%2d\t%1s:%d\tDel:%6.1lf\tDur:%5.1lf", pad(mS( nType ),9), c, b, f, s, e, pad(sScriptEntry,9), eV( wE, wBFS, c, b, f, s, e, cCHA ), sMode, eV( wE, wBFS, c, b, f, s, e, cMOD ), eV( wE, wBFS, c, b, f, s, e, cDEL ),eV( wE, wBFS, c, b, f, s, e, cDUR )
	...
	if (   nType  == mI( "Segment" ) ||  nType  == mI( "VarSegm" ) || nType  == mI( "Ramp" ) || nType  == mI( "Blank" )  ) 
		...
		// printf  "\tAmp:%5.1lf\tSca:%5.1lf \tDAm:%5.1lf\r ", eV( wE, wBFS, c, b, f, s, e, cAMP ), eV( wE, wBFS, c, b, f, s, e, cSCA_ ), eV( wE, wBFS, c, b, f, s, e, cDAM )
	endif
	if (   nType  == mI( "Expo" )  )
	...
	if (  ( nType  == mI( "StimWave" ) ) )
		...
		// printf  "\t\tExtractEValues( c, b, f, s, e ) StimWave \tDu1:%5.1lf \tSclA:\t%7.2lf\tOfsA:\t%7.2lf\tSclX:\t%7.2lf\t'%s' \r ", Dur, eV( wE, wBFS, c, b, f, s, e, cSCA_), eV( wE, wBFS, c, b, f, s, e, cDAM), eV( wE, wBFS, c, b, f, s, e, cAMP), ExtractStimwaveName( c, b, f, s, e )
	endif
End
...
// An InterBlockInterval  occurs  only  once for each frame: after the LAST element  of the LAST sweep. Set all others to 0 duration.
	...
	for ( b  = 0; b < eBlocks( wG ); b += 1 )
		for ( f = 0; f <  eFrames( wFix, b ); f += 1 )									// copy all frames 
			for ( s = 0; s < eSweeps( wFix, b ); s += 1 )		
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )
					if ( IsInterFrameInterval( wEinCB, c, b, f, s, e ) && s < eSweeps( wFix, b ) - 1 )	// an InterFrmInt is appended only after the last sweep...
...
// An InterBlockInterval  occurs  only  once for each block: after the LAST element  of the LAST sweep of the LAST frame. Set all others to 0 duration.
	...
	for ( b  = 0; b < eBlocks( wG ); b += 1 )
		for ( f = 0; f <  eFrames( wFix, b ); f += 1 )										// copy all frames 
			for ( s = 0; s < eSweeps( wFix, b ); s += 1 )		
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )
					if ( IsInterBlockInterval( wEinCB, c, b, f, s, e ) &&((s < eSweeps( wFix, b ) - 1)||( f < eFrames( wFix, b ) - 1)) )	// an InterBlockInt is appended only after the last frame...
...
static Function		IsInterFrameInterval( wEinCB, c, b, f, s, e )
...
// ..one InterFrameInterval (possibly of duration 0) 
	...
	if ( e == eElems( wEinCB, c, b )  - 2 )
		// printf "\t\tFound  InterFrameInterval ( c:%d  b:%d   f:%d   s:%d  [=e:%d ] )  with Dur:%g    Amp:%g  \r", c, b, f, s, e,  eV( wE, wBFS, c, b, f , s, e, cDUR ),  eV( wE, wBFS, c, b, f , s, e, cAMP )
		code	= TRUE
	endif
	return code
End
...
static Function	IsInterBlockInterval( wEinCB, c, b, f, s, e )
...
// ..one InterBlockInterval (possibly of duration 0)  
	...
	if ( e == eElems( wEinCB, c, b )  - 1 )
		// printf "\t\tFound  InterBlockInterval ( c:%d  b:%d   f:%d   s:%d  [=e:%d ] )  with Dur:%g    Amp:%g  \r", c, b, f, s, e,  eV( wE, wBFS, c, b, f , s, e, cDUR ),  eV( wE, wBFS, c, b, f , s, e, cAMP )
		code	= TRUE
	endif
	return code
End
					...
					for ( f  = 0; f < eFrames( wFix, b ); f += 1 ) 
						for ( s = 0; s < eSweeps( wFix, b ); s += 1 )			
							...
							for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )
								...
								Dur	   = eV( wE, wBFS, c, b, f, s, e, cDUR ) /  nSmpInt
								wSweepTime[ c ] +=  Dur
								sAmp	+= num2str(  eV( wE, wBFS, c, b, f, s, e, cAMP ) )+ "     \t"
								sDur	+= num2str(  Dur )+ "     \t"
								sTime	+= num2str( wSweepTime[ c ] ) + "   \t"
								//sBeg	+= num2str(  eV( wE, wBFS, c, b, f, s, e, cBEG ) )+ "  \t"
							endfor
							printf "%s\r", sAmp
							printf "%s\r", sDur
							printf "%s\r", sTime
					...
					for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )			// all wEl elements of sweep 0 (all sweeps have same number of elements, 
						...
						for ( s = 0; s < min( 2, eSweeps( wFix, b ) ); s += 1 )	// 2 Sweeps are enough because PoverN makes the 2., 3., 4..equal
							for ( k = 0; k <  eKeys( wE ); k += 1 )			// arrange all subkeys of sweep 0 and sweep 1 in one line
								sprintf  bf1, "%6.1lf\t ", eV( wE, wBFS, c, b, f, s, e, k )
								bf += bf1
							endfor
			...
			for ( f = 1; f < eFrames( wFix, b ); f += 1 )						// loop through all frames above frame 0, whose values are fixed
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )				// loop through all elements (=VarSegm,Segment,Ramp,StimWave..)
					...
					if ( nType == mI( "Blank" ) || nType == mI( "VarSegm" ) || nType == mI( "Segment" ) || nType == mI( "Ramp" ) ) // but use only Segments, Ramp....
						// 1210
						//	eSet( wE, wBFS,  c, b, f, 0, e, cAMP,			eV( wE, wBFS, c, b, f, 0, e, cAMP )    +	f * eV( wE, wBFS, c, b, f, 0, e, cDAM  ) )
						//	eSet( wE, wBFS,  c, b, f, 0, e, cDUR,   max( 0,	eV( wE, wBFS, c, b, f, 0, e, cDUR )   +  f * eV( wE, wBFS, c, b, f, 0, e, cDDU ) ) )	// durations must be positive
						
						if ( eV( wE, wBFS, c, b, f, 0, e, cDAM ) != 0 )					// 121002 allow deltas to change from frame to frame by adding delta of current frame  to value of previous frame	 
							eSet( wE, wBFS,  c, b, f, 0, e, cAMP,		   eV( wE, wBFS, c, b,  f - 1, 0, e, cAMP ) +  eV( wE, wBFS, c, b, f, 0, e, cDAM ) )	// add current delta to 	 	
							// print  "PULSEExpandFramesIncDec()", c, b, f, 0, e, " : "	,   eV( wE, wBFS, c, b,  f - 1, 0, e, cAMP ) ,"+ ",  eV( wE, wBFS, c, b, f, 0, e, cDAM )   ,"->" , eV( wE, wBFS, c, b, f, 0, e, cAMP )
						endif
						if ( eV( wE, wBFS, c, b, f, 0, e, cDDU ) != 0 )					// 121002 allow deltas to change from frame to frame by adding delta of current frame  to value of previous frame	
							eSet( wE, wBFS,  c, b, f, 0, e, cDUR,   max( 0, eV( wE, wBFS, c, b, f - 1, 0, e, cDUR) +  eV( wE, wBFS, c, b, f, 0, e, cDDU ) ) )// durations must be positive
						endif
	
						// printf "\t\t\tExpandFramesIncDec() reads c:%d f:0 s:0 e:%d  eTyp:%d~%s\tand sets f:%d s:0 Amp:%5g\t->%5g \tDur:%5g\t->%5g \r", c, e, eTyp(c,0,0,e), pad(mS(eTyp(c,0,0,e)),9), f, eV( wE, wBFS, c, 0, 0, e, cAMP ), eV( wE, wBFS, c, b, f, 0, e, cAMP ), eV( wE, wBFS, c, 0, 0, e, cDUR ), eV( wE, wBFS, c, b, f, 0, e, cDUR )
					endif
				...
				for ( s = 1; s < eSweeps( wFix, b ); s += 1 )							// compute all sweeps above the first from the first
					for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )						// loop through all elements (=Segments, Ramp..)
						...
						for ( k = 1; k <= eKeys( wE ); k += 1 )						// loop through all keys (=Dur, Amp, DDur.)..
							...
							if ( ! IsInterFrameInterval( wEinCB, c, b, f, s, e )   &&   ! IsInterBlockInterval( wEinCB, c, b, f, s, e )  )	
								eSet( wE, wBFS,  c, b, f, s, e, k,   eV( wE, wBFS, c, b, f, 0, e, k ) )		// at first copy all keys of all sweeps of all frames... 
								// printf "\t\tNormal Interval:\tcopying   \tvalue( c:%d  b:%d   f:%d   s=%d   e:%d   k:%2d ) :%5g\t-> s=%d \t(value:%g) \r", c, b, f, 0, e, k , eV( wE, wBFS, c, b, f, s, e, k ), s, eV( wE, wBFS, c, b, f, s, e, k )
							else										// ....except the InterFrmInterval / InterBlockInterval ) ...
								// printf "\t\tIFrmInt  IBlkInt:\talready set\tvalue( c:%d  b:%d   f:%d   s=%d   e:%d   k:%2d ) :%5g\t-> s=%d \t(value:%g) \r", c, b, f, 0, e, k , eV( wE, wBFS, c, b, f, s, e, k ), s, eV( wE, wBFS, c, b, f, s, e, k )
						...
						if ( ePoN( wFix, b )  &&   ! IsInterFrameInterval( wEinCB, c, b, f, s, e )   &&   ! IsInterBlockInterval( wEinCB, c, b, f, s, e )  )	
							eSet( wE, wBFS,  c, b, f, s, e, cSCA_,   -eV( wE, wBFS, c, b, f, 0, e, cSCA_ ) * eCorrAmp( wFix, b ) ) // then invert the scale key
							// printf "\t\tNormal with PoN\tinverting scl \tvalue( c:%d  b:%d   f:%d   s=%d   e:%d   k:%2d ) :%5g\t-> s=%d  \r", c, b, f, 0, e, cSCA_ , eV( wE, wBFS, c, b, f, s, e, cSCA_ ), s
						endif
					endfor
				endfor
			endfor
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
Static Function  CheckTimes( sFolder, wE, wBFS, c, b, f, s, e, SmpInt, DacMax, DacMin, DacGain )
...
// Amplitudes of Expo or Stimwave are not checked
	...
	string  	bf
	// printf "\tCheckTimes()  \tc:%d \tf:%d \ts:%d \te:%d  \tcDUR:\t%6.1lf \tcDDU:\t%6.1lf \tcOFS:\t%6.1lf \r", c, b, f, s, e, eV( wE, wBFS,  c, b, f, s, e, cDUR ), eV( wE, wBFS,  c, b, f, s, e, cDDU ), eV( wE, wBFS,  c, b, f, s, e, cDEL )
	...
	make	/O /N = 3  		$"root:uf:" + sFolder + ":ar:wIndicesToCheck"
	wave	wIndicesToCheck  =	$"root:uf:" + sFolder + ":ar:wIndicesToCheck"
	wIndicesToCheck[ 0 ] = cDUR
	wIndicesToCheck[ 1 ] = cDDU
	wIndicesToCheck[ 2 ] = cDEL
	for ( n = 0; n < numPnts( wIndicesToCheck ); n += 1 )
		vTime = eV( wE, wBFS, c, b, f, s, e, wIndicesToCheck[ n ] )
...
KillWaves	wIndicesToCheck	// 041014
	...
	vAmp = eV( wE, wBFS, c, b, f, s, e, cAMP )
			...
			for ( b  = 0; b < eBlocks( wG ); b += 1 )
				for ( f = 0; f < eFrames( wFix, b ); f += 1 )							// loop through all frames
					for ( s = 0; s < eSweeps( wFix, b ); s += 1 )						// compute all sweeps above the first from the first
						for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )					// loop through all elements (=Segments, Ramp..)
							if ( CheckTimes( sFolder, wE, wBFS, c, b, f, s, e, nSmpInt, DacMax, DacMin, DacGain  ) ) //    todo  1004   Digout    ,DODur,DODel,DODDur,DODDel" )
								return	kERROR
							endif
							nMode	= eV( wE, wBFS, c, b, f, s, e, cMOD )
							dur		= eV( wE, wBFS, c, b, f, s, e, cDUR ) 		// check if the current element has a subkey 'Dur'...
							del		= eV( wE, wBFS, c, b, f, s, e, cDEL ) 		// check if the current element has a subkey ...
// 050609
//							nLine	= wELine[ c ][ f ][ e ]					// retrieve original wVal line number for given frame and element
							...
							if ( nMode == cREL )
								GetLineRef( wLineRef, nLine - 1, c1, b1, e1 )										//  function changes c1, b1, e1
								Beg = eV( wE, wBFS, c1, b1, f, s, e1, cBEG ) + ( eV( wE, wBFS, c1, b1, f, s, e1, cDUR ) + del ) / nSmpInt 	// begin of preceding line  +  delay
								nWrongBegCnt = ( Beg != eV( wE, wBFS, c, b, f, s, e, cBEG ) ) ? nWrongBegCnt + 1 :  nWrongBegCnt 
							 	  printf "\t\t\t\tComputeTotalPoints REL\t%s\tMode:%d\tnPass:%d\tc:%d\tb:%d\tf:%d\ts:%d\te:%d\tLine:%d\tOfs:%6d \tBeg:%6d \t=?=%6d\tDur:%6d   c1:%2d   f1:%2d   s:%2d   e1:%2d\r", pad(mS( eTyp( wE, wBFS,  c, b, e ) ),8), nMode, nPass, c, b, f, s, e, nLine, del, Beg, eV( wE, wBFS, c, b, f, s, e, cBEG ) , dur / nSmpInt, c1, f1, s, e1
							endif				
...
variable	nPts, nStoreCnt	= 0
	...
	if ( gRadDebgSel > 0  &&  PnDebgOutElems )
		c = 0
		printf "\t\tOutElemsAllToWaves(2)\tc:%d \tTotDur[us]:%d    \t Swp00[us]:%d\t Frm0[us]:%d  TotalSweeps:%d    ( over %d Prots )\r", c, nPnts * nSmpInt, GetDurSweep( wEinCB, wE, wBFS, c, b, 0, 0 ), GetDurFrame( wFix, wEinCB, wE, wBFS, c, b, 0 ), wG[kTOTAL_SWPS], gnProts 
	endif	
	...
	for ( b  = 0; b < eBlocks( wG ); b += 1 )									// loop through all blocks
		for ( f = 0; f < eFrames( wFix, b ); f += 1 )							// loop through all frames
			for ( s = 0; s < eSweeps( wFix, b ); s += 1 )						// loop through all sweeps 
				...
				swSetTimes( sFolder, wFix, b, f, s, SWPBEG, nSwpBegPt, gnProts, nPnts )	// set true sweep begin point including 'Blanks' . Protocol aware 031007 
				...
				for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )				// loop through all elements (=Segments, Ramp..)
					...
					nPntCnt = SetStimulus( wVal, wELine, wE, wBFS, nSmpInt, c, b, f, s, e , wStimulus ) 
					if ( nType  == mI( "Blank" ) )
				...
				swSetTimes( sFolder, wFix, b, f, s, SWPBEGSTORE, nSwpBegPt, gnProts, nPnts )		//  protocol aware 031007 
				swSetTimes( sFolder, wFix, b, f, s, SWPLENSTORE, nSwpEndPt - nSwpBegPt , 1, 0 ) 	// nProts is always 1 : this could be simplified    length is not protocol aware 031007 
				swSetTimes( sFolder, wFix, b, f, s, SWPLEN, nPntCnt - nSwpBegInclBlank, 1, 0 )		// nProts is always 1 : this could be simplified    length is not protocol aware 031007 
...
//endif
			...
			for ( f = 0; f < eFrames( wFix, b ); f += 1 )					// loop through all frames
				for ( s = 0; s < eSweeps( wFix, b ); s += 1 )				// loop through all sweeps 
					for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )			// loop through all elements (=Segments, Ramp..)
						nType = eTyp( wE, wBFS,  c, b, e ) 
						if ( gRadDebgSel > 2  &&  PnDebgOutElems ) // very slow : even if nothing is printed this line takes about  12 us to fill  the buffer and execute Out( bf ), with printing it takes 40 us ! 
							printf  "\t\t\t\tOutElemsAllToWaves(8) c:%d b:%d f:%d s:%d e:%d\t%-12s\tstarting at %6d \tDur:%5d  \tAmp:%5.1lf \tx \r",c, b, f, s, e, mS(nType),  eV( wE, wBFS, c, b, f, s, e, cBEG),  eV( wE, wBFS, c, b, f, s, e, cDUR),  eV( wE, wBFS, c, b, f, s, e, cAMP)
						endif
						nPntCnt = SetStimulus( wVal, wELine, wE, wBFS, nSmpInt, c, b, f, s, e , wStimulus ) 
					endfor
				endfor 
	...
	// Version 1
	if ( eV( wE, wBFS, 0, 0, 0, 0, 0, cBEG ) > 0 ) 							// ch 0 = the first channel in the IO part ot the script must start at time 0
	...
	//	if ( eV( wE, wBFS, c, 0, 0, 0, 0, cBEG ) <  eV( wE, wBFS, 0, 0, 0, 0, 0, cBEG ) ) 	// ch 0 = the first channel in the IO part ot the script must have the earliest begin point of all Dac channels
...
variable	c = 0		//? only first Dac
	for ( seg = 0; seg < nSegments; seg += 1 )				// loop through all passed segments
		...
		//variable	SegBegPt = eV( wE, wBFS, c, b, f, s, e, cBEG )
		// print f,s,e, SegBegPt
//?eblocks( wG )
variable	b = 0
		SetStimulus( wVal, wELine, wE, wBFS, nSmpInt, c, b, f, s, e , wStimulus ) 
	endfor
End


static Function	SetStimulus( wVal, wELine, wE, wBFS, nSmpInt, c, b, f, s, e , wStim ) 
	wave	/T	wVal
	wave	wELine, wE, wBFS
	...
	if ( nType == mI( "StimWave" ) ) 		// cannot use switch because switch needs constants...
		nPntCnt	=  SetStimulusFromReadWave( wVal, wELine, wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	elseif ( nType  == mI( "Expo" ) )
		nPntCnt  = SetStimulusFromExpo( wVal, wELine, wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	elseif ( nType  == mI( "Ramp" ) )
		nPntCnt	= SetStimulusFromRamp( wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	else							// Segment, VarSegm, Blank
		nPntCnt	= SetStimulusFromSegments( wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	endif
	return nPntCnt
End

static  Function	SetStimulusFromRamp( wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	wave	wE, wBFS
	variable	c, b, f, s, e, nSmpInt
	wave	wStim
	variable	scla		= eV( wE, wBFS, c, b, f, s, e, cSCA_ )
	variable	amp		= eV( wE, wBFS, c, b, f, s, e, cAMP ) 
	variable 	BegPt	= eV( wE, wBFS, c, b, f, s, e, cBEG )
	variable 	pts		= eV( wE, wBFS, c, b, f, s, e, cDUR ) / nSmpInt	
...
static  Function	SetStimulusFromSegments( wE, wBFS, c, b, f, s, e, nSmpInt, wStim )
	wave	wE, wBFS
	variable	c, b, f, s, e, nSmpInt
	wave	wStim
	variable	scla		= eV( wE, wBFS, c, b, f, s, e, cSCA_ )
...
Static Function 	SetStimulusFromExpo( wVal, wELine, wE, wBFS,c, b, f, s, e, nSmpInt, wStim )
...
// test.... expo values are not bracketed.....but could be....
	...
	variable	ScalePoN	= eV( wE, wBFS, c, b, f, s, e, cSCA_ )
...
//
//Static  Function   SetNumberOfPointsFromReadWave( c, b, f, s, e )
...
Static  Function   SetStimulusFromReadWave( wVal, wELine, wE, wBFS, c, b, f, s, e, nSmpInt, wStimulus )
// reads wave file: data  values of wave found in wEleNam and in wEl at location frame f, element e
	...
	if ( pts )													// Zero has been stored in SetNumberOfPointsFromReadWave()  if  LoadWave has  failed at that time
		variable	SclA			= eV( wE, wBFS, c, b, f, s, e, cSCA_ ) 
		variable	OfsA			= eV( wE, wBFS, c, b, f, s, e, cDAM ) 				// 040519 !!! Rather than introducing new constants cOFSA and cSCLX...					
		variable	SclX			= eV( wE, wBFS, c, b, f, s, e, cAMP ) 				//...we misuse existing ones which are not used in StimWave (see also  'ExtractEValues()'  )				
		...
		if ( bFileExists )
			if ( FileHasExtension( sFileNameExt, ".IBW" ) )
				...
				ReadASCIIStimWave( sFileNameExt, nSmpInt, wStimulus, BegPt, pts, SclA, OfsA, SclX )	// 040519 !!! Rather than introducing new constants cOFSA and cSCLX we misuse existing ones which are not used in StimWave (see SetStimulusFrom...
			endif
		else
...
static Function  ReadASCIIStimWave( sFilePath, nSmpInt, wStimul, BegPt, pts, SclA, OfsA, SclX  )
			...
			for ( s = 0; s < eSweeps( wFix, b ); s += 1 )
				for ( e = 0; e < eElems( wEinCB, c, b); e += 1 )
					if ( cmpstr( mS( eTyp( wE, wBFS,  c, b, e ) ), sMainKey ) == 0 )	// the mainkey matches... 			
						OldValue = eV( wE, wBFS, c, b, f, s, e, cAMP ) 			// ..and the subkey...
...
static Function	GetDurFrame( wFix, wEinCB, wE, wBFS, c, b, f )
...
// plus 1 InterFrmInterval plus nSweeps * ( PreSwpInt + InterSwpInt )
	...
	variable	s, dur = 0
	for ( s = 0; s < eSweeps( wFix, b ); s += 1 )
		 dur += GetDurSweep( wEinCB, wE, wBFS, c, b, f, s )
		 dur += GetDurSweepBlank( wEinCB, wE, wBFS, c, b, f, s )
		 dur += GetDurFrameBlank( wEinCB, wE, wBFS, c, b, f, s )
	endfor
	return  dur
End

static Function	GetDurSweep( wEinCB, wE, wBFS, c, b, f, s )
...
static Function	GetDurSweepBlank( wEinCB, wE, wBFS, c, b, f, s )
// returns duration of ....
//     F0-PSI  F0-psi  F0-psi  F0-psi   IntFI    F1-PSI  F1-psi  F1-psi  F1-psi   IntFI    F2-PSI  F2-psi ...  
	...
	variable	e, dur = 0
	for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )								// loop through all elements (=Segments, Ramp..)
		if ( eTyp( wE, wBFS,  c, b, e )  == mI( "Blank" ) &&  ! IsInterFrameInterval( wEinCB, c, b, f, s, e ) )	// the InterFrmInterval does NOT belong... 
			dur += eV( wE, wBFS, c, b, f, s, e, cDUR )									// ..to any sweep, it belongs to the frame
		endif							
	endfor
	if ( gRadDebgSel > 1  &&  PnDebgOutElems )
...
static Function	GetDurFrameBlank( wEinCB, wE, wBFS, c, b, f, s )
...
//     F0-PSI  F0-psi  F0-psi  F0-psi   IntFI    F1-PSI  F1-psi  F1-psi  F1-psi   IntFI    F2-PSI  F2-psi ...  
	...
	variable	e, dur = 0
	for ( e = 0; e < eElems( wEinCB, c, b ); e += 1 )			// loop through all elements (=Segments, Ramp..)
		if ( IsInterFrameInterval( wEinCB, c, b, f, s, e ) )
			dur += eV( wE, wBFS, c, b, f, s, e, cDUR )
		endif							
	endfor
	if ( gRadDebgSel > 1  &&  PnDebgOutElems )
...
// returns value  from ' wE'[ nChan ][ nBlk ][ nFrm=0 ][ nSwp=0 ][ nEle ][ nKey=cTYP=0 ] which is the index of mainkey
	wave	wE, wBFS
	variable	c, b, e
	return	eV( wE, wBFS, c, b, 0, 0, e, cTYP )	
End 
...
Function  		eV( wE, wBFS, nChn, nBlk, nFrm, nSwp, nEle, nKey )
...
// sets value  wE[ nFrm ][ nSwp ][ nEle ][ nKey  ]
	...
	// if ( abs(eV( wE, wBFS, nChn, nBlk, nFrm, nSwp, nEle, nKey )) * .9999 >  abs(value)  || abs( value) > 1.0001 * abs(eV( wE, wBFS, nChn, nBlk, nFrm, nSwp, nEle, nKey ) ) )
	//	string errBf; sprintf errBf, "Saving value [ c:%d   \tb:%d\tf:%d\ts:%d   \te:%d\tk:%d ] = %12.12g \tfailed,  retrieving %12.12g ... ",  nChn, nBlk, nFrm, nSwp, nEle, nKey, value, eV( wE, wBFS, nChn, nBlk, nFrm, nSwp, nEle, nKey )
	//	InternalError( errBf )
	// endif
End 
...
static  Function  swSetTimes( sFolder, wFix, nBlk, nFrm, nSwp, nType, value, nProts, nPnts )	// set true sweep begin point including 'Blanks'
//  031007 stores start and length of each sweep (in total points and in points to be stored (=without blanks) )   being protocol aware
	...
	string		wNm		= "root:uf:" + sFolder + ":ar:wSwpTimeBlk" + num2str( nBlk )
...
//    031007 retrieves previously stored start and length of each sweep (in total points and in points to be stored (=without blanks) )      being protocol aware
	...
	wave  	wSwpTimes = $"root:uf:" + sFolder + ":ar:wSwpTimeBlk" + num2str( nBlk )
...
Function		DisplayOptionsStimulus_aco( xPos , yPos, nMode  )
	variable	xPos, yPos
	variable 	nMode
	...
	InitPanelDisplayStimulus_aco( sFBase + sFSub, sWin )					// Fills both big text waves  'sPnOptions' (=wPn)  in 'root:uf:aco:'  and  in  'root:uf:evo:'  with all information about the controls necessary to build the panel
...
Function		InitPanelDisplayStimulus_aco( sF, sPnOptions )
...
//Function		std_gnDspBlock( s )									// valid for  kbSUBFOLDER_IN_ACTIONPROC_NM_SV = 1 
	...
	string  	sF		= ksACOld	
	string  	sWin		= ksPN_NAME_SDAO
	nvar		gnDspBlock = $"root:uf:" + sF + ":" + sWin + ":gnDspBlock0000"
	...
	// print  "TODO........gnDspBlock( s )  " + "root:uf:" + sF + ":keep:wG"
	DisplayStimulus1( sF, sWin, kNOINIT )
	return	0
End
...
// Called  when  'Display all blocks'  is switched on and off.  Store temporarily all panel settings which are OK in the  '1 block' mode but which make no sense in the 'all blocks' mode so that they can automatically be restored when the user switches back.
	...
	variable	bValue
	nvar		raRangeFS  	= $"root:uf:" + sFolder + ":" + sWin + ":raRangeFS00"				// The 'Range' setting  'All frames, all sweeps'  makes sense if the user wants to see all blocks so we automatically set it (although this is not required by the program)
	nvar		raCatStck	 	= $"root:uf:" + sFolder + ":" + sWin + ":raCatStck00"				// The 'Mode'   setting  'Cat frames + sweeps'  makes sense if the user wants to see all blocks so we automatically set it (although this is not required by the program)
	// Used like static, should be hidden within this function but must keep it's value between calls
	nvar	/Z 	 gPrevRangeFS = $"root:uf:" + sFolder + ":" + sWin + ":gPrevRangeFS00"			//  We also automatically set the 'Range' setting to 'All frames, all sweeps'  but we will restore the setting to the previous state if the user wants to see single blocks again
	nvar	/Z 	 gPrevCatStack	= $"root:uf:" + sFolder + ":" + sWin + ":gPrevCatStack00"			//  We also automatically set the 'Mode'  setting to 'catenate frames + sweeps'  but we will restore the setting to the previous state if the user wants to see single blocks again
	if ( ! nvar_Exists( gPrevRangeFS ) )			
		variable 	/G 		   $"root:uf:" + sFolder + ":" + sWin + ":gPrevRangeFS00" = raRangeFS// Used like static, should be hidden within this function but must keep it's value between calls
		variable	/G		   $"root:uf:" + sFolder + ":" + sWin + ":gPrevCatStack00" = raCatStck	// Used like static, ...
		//variable /G $"root:uf:" + sFolder + ":std:gPrevCatenation"	= gbCaten			// Used like static
		nvar	 gPrevRangeFS	 = $"root:uf:" + sFolder + ":" + sWin + ":gPrevRangeFS00"			//  We also automatically set the 'Range' setting to 'All frames, all sweeps'  but we will restore the setting to the previous state if the user wants to see single blocks again
		nvar	 gPrevCatStack	 = $"root:uf:" + sFolder + ":" + sWin + ":gPrevCatStack00"			//  We also automatically set the  'Mode'  setting to 'catenate frames + sweeps'  but we will restore the setting to the previous state if the user wants to see single blocks again
	endif
...
// Todo: checkbox to allow the user to alternatively select a 'true-but-slow' drawing mode: no decimation = step is always 1 = pulses/spikes have and keep their correct duration even when zooming 
	...
	nvar		gbDisplay			= $"root:uf:" + sFolder + ":" + sWin + ":gbDisplay0000"
	nvar		gbAllBlocks		= $"root:uf:" + sFolder + ":" + sWin + ":bAllBlocks0000"
	nvar		gnDspBlock		= $"root:uf:" + sFolder + ":" + sWin + ":gnDspBlock0000"		// the block which is to be displayed
	nvar		raRangeFS		= $"root:uf:" + sFolder + ":" + sWin + ":raRangeFS00"			// the radio button lin index variable is autobuilt by truncating the last 2 digits
	nvar		raCatStck			= $"root:uf:" + sFolder + ":" + sWin + ":raCatStck00"			// the radio button lin index variable is autobuilt by truncating the last 2 digits
	nvar		gbShowBlank		= $"root:uf:" + sFolder + ":" + sWin + ":bShowBlank0000"
	nvar		gbSameYAx		= $"root:uf:" + sFolder + ":" + sWin + ":gbSameYAx0000"
...
//		string		sDgoChans	= gsDigOutChans
		...
		//make  /O  /N = ( nDacPts )  	$"root:uf:" + sFolder + ":stim:" + DispWave( "SV" )  =  Nan // wSvWv cannot be an integer wave as an integer wave does not have Nan needed to blank out points	
		make /O /N=( nDacPts / step ) 	$"root:uf:" + sFolder + ":stim:" + DispWave( "SV" ) =  Nan // wSvWv cannot be an integer wave as an integer wave does not have Nan needed to blank out points	

		// Step 2: Set points which are not to be  displayed to Nan in  Save/NoSave    and   store minima and maxima
		make	/O /N=( MAXDACS, 2 )	$"root:uf:" + sFolder + ":stim:wDMinMax"
		wave	wDMinMax		= 	$"root:uf:" + sFolder + ":stim:wDMinMax"						
		for ( b = nBlkBeg; b < nBlkEnd; b += 1 )
			nFrm	  = ( raRangeFS == cOneFAllS  ||  raRangeFS == cOneFOneS ) ?  1 :  eFrames( wFix, b )
			nSwp = ( raRangeFS == cAllFOneS  ||  raRangeFS == cOneFOneS ) ?  1 : eSweeps( wFix, b )

			wave wSVWv	= $"root:uf:" + sFolder + ":stim:" + DispWave( "SV" )		
			// printf "\t\tDispStimulus...'%s'    exists:%d  \r", "root:uf:" + sFolder + ":stim:" + DispWave( "SV" ), waveexists( 	wSVWv )	
...
//					wSVWv[ nStoreStart, nStoreEnd - 1 ] 	= 0  									// the SAVE / NOSAVE periods : mark as 'Save'  (default is nosave = Nan)
		...
		for ( cd = 0; cd < nDgoChs; cd += 1 )	
			if ( step > 1 )
				sDONm		=  DispWaveDgo( gsDigOutChans, cd )  
				make /O /B /N	= ( nDacPts / step ) 	$"root:uf:" + sFolder + ":stim:" + sDONm  				// BYTE wave
				wave  wDOWv	=  				$"root:uf:" + sFolder + ":stim:" + sDONm 				// ..for the display of the digital output
				...
				SetScale /P X, 0, nSmpInt / kXSCALE * step , ksXUNIT, $"root:uf:" + sFolder + ":stim:" + DispWaveDgoFull( gsDigOutChans, cd )	//???!!!			// expand in x by number AND prevent IGOR from scaling  11,12... kms  (..8000,9000 ms is OK )
			endif
	MarkPerfTestTime 825	// InterpretScript: DisplayStimulus Step2 decimate DO
		...
		for ( c = 0; c < nCntDA; c += 1)		
			...
			if ( step > 1 )
				...
				make /O /N=( nDacPts / step ) 	$"root:uf:" + sFolder + ":stim:" + sDacNm	 // wSvWv cannot be an integer wave as an integer wave does not have Nan needed to blank out points	
				wave	wStimulus		=   	$"root:uf:" + sFolder + ":stim:" + sDacNm 
// 040831
...
//				SetScale /P X, 0, nSmpInt / kXSCALE * step, ksXUNIT, wStimulus	//???!!!			// expand in x by number
	...
	MarkPerfTestTime 826	// InterpretScript: DisplayStimulus Step2 decimate DA
		...
		string  /G					$"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMin"	= ""
		string  /G					$"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMax"	= ""
		svar	   	sUnitsDacMin	= 	$"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMin"
		svar	   	sUnitsDacMax	=	$"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMax"
		for ( c = 0; c < nCntDA; c += 1)		
			// printf "\t\t\t\tDisplayStimulus(3)   c:%d   \t%s\tmin:%g   \tmax%g \r", c, ios( wIO, nIO, c , cIOUNIT ), wDMinMax[ c ][ 0 ],  wDMinMax[ c ][ 1 ] 
			SetDacMinMaxForSameUnits( sFolder, ios( wIO, nIO, c , cIOUNIT ), wDMinMax[ c ][ 0 ],  wDMinMax[ c ][ 1 ] )
		endfor
			...
			for ( f = 0; f < nFrm; f += 1 )										// loop through all frames   (only ONE for catenated wave mode)
				...
				for ( s = 0; s < nSwp; s += 1)								// loop through all sweeps (only ONE for catenated wave mode)
					...
					for ( cd = 0; cd < nDgoChs; cd += 1 )	
						...
						if ( step > 1 )
							wave  wDOFull	=  $"root:uf:" + sFolder + ":stim:" + sDONm			// the name of the full wave ( all points )
							sDONm		=  DispWaveDgo( gsDigOutChans, cd )  				// the name of the decimated wave 
							MyDecimate1( wDOFull,  "root:uf:" + sFolder + ":stim:" + sDONm , step, 2, TRUE, nStartPos, nStartPos + nSwpPts )
						endif
...
//						wave  wDOWv	=  $"root:uf:" + sFolder + ":stim:" + sDONm 					// ..for the display of the digital output
					...
					for ( c = 0; c < nCntDA; c += 1)		
						...
						if (  step > 1 )
							...
							wave	wStimulus		= $"root:uf:" + sFolder + ":stim:" + sDacNm 
							MyDecimate1( wStimulusFull,  "root:uf:" + sFolder + ":stim:" + sDacNm, step, 2, TRUE, nStartPos, nStartPos + nSwpPts )
							//MyDecimate1( wStimulusFull,  "root:uf:" + sFolder + ":stim:" + sDacNm, step, 2, FALSE, nStartPos, nStartPos + nSwpPts )
						else
							wave	wStimulus		= $FldAcqioio( sFolder, wIO, nIO, c, cIONM ) 
						endif
						sDacNm 			+=  sSeg
						...
						if (  c > 0 )
							if ( gbSameYAx )
								GetDacMinMaxForSameUnits( sFolder, ios( wIO, nIO, c , cIOUNIT ), rMin, rMax )
							else
...
static Function  /S  DispWaveDgo( sDgoChans, cd )
	string 	sDgoChans
	variable	cd
...
static Function	SetDacMinMaxForSameUnits( sFolder, sUnits, vMin, vMax )
	string	  	sFolder, sUnits
	variable	vMin, vMax
//	svar	   	sUnitsDacMin	= $"root:uf:" + sFolder + ":sUnitsDacMin"
//	svar	   	sUnitsDacMax	= $"root:uf:" + sFolder + ":sUnitsDacMax"
	svar	   	sUnitsDacMin	= $"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMin"
	svar	   	sUnitsDacMax	= $"root:uf:" + sFolder + ":" + ksF_IO + ":sUnitsDacMax"
...
static Function	GetDacMinMaxForSameUnits( sFolder, sUnits, rMin, rMax )
	string	  	sFolder, sUnits
	variable	&rMin, &rMax
//	svar	   	sUnitsDacMin	= $"root:uf:" + sFolder + ":sUnitsDacMin"
//	svar	   	sUnitsDacMax	= $"root:uf:" + sFolder + ":sUnitsDacMax"
