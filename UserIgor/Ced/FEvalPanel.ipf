...
// 050728 unnamed window hook
//	SetWindow	$sWin  , hook(PnEvaluation1) = fHookPnEvaluation1 				// WM,JP: using the one and only unnamed hook should avoid the crash but it does NOT!..... prevent the user to accidentally minimize, maximize or close the panel
...
////// 2009-10-28
////Beep
////print "There is a menu item to quit FEval.\r"
...
// 2009-10-28 alles weg
// // Static Function		Cleanup( sFo )
//		Function		Cleanup( sFo )
//	string  	sFo
...
//	PossiblyKillPanel( "PnEvalDetails" ) 
//	PossiblyKillPanel( SDNamee( sFo) ) //"PnDispStim" + sFo )		// the StimDisp panel in Eval
//	PossiblyKillNotebook( ksSCRIPTEXTRACTED_NB_WNDNAME_ )
////	KillDataFolderUnconditionally( "root:uf:evo:cfsr" )
//	KillDataFolderUnconditionally( "root:uf:" + sFo )	
...
//	n += 1;	tPn[ n ] =	"SV:	   0:	3:	4:	0:	LstChan():	LstReg():	1°1°1°1°1°1°:	svSDevFact:	SDevF:		:			:			40:		%.1lf;1,100,0:	~2:			:		:	"		//  	1 dim  SetVariable( 1 row)
	...
	n += 1;	tPn[ n ] =	"CB:	   1:	0:	3:	0:	LstChan():	LstReg():	1,1,1,°1,1,1,°:	cbFit:		fFitRowTitles():	:			fFit():			:		:			fFitOnOff():		:		:	"		// !!! the # of row titles ~ # of fits
...
//	fSetvar_struct1( s )										// sets the global variable
	...
	Panel3Main(   "de", "Evaluation Details3", "root:uf:" + ksEVO_, 2, 95 ) // Compute the location of panel controls and the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
...
// Computes the alignment time according to the user settings in the 'Alignment' popupmenu
	...
	for ( ch = 0; ch < gChannels; ch += 1)				// ...selected a setting different from 'no align' . For this reason  'kALIGN_NONE'  must  NOT  appear in the 'if cascade!'
		...
		for ( rg = 0; rg < rgCnt;  rg += 1 )	
			...
			if 	( nAlign == kALIGN_BR )
				Val	= EvT( ch, rg, kE_BRISE ) -   Eval( 0, 0, kE_BEG, kVAL ) 	; chAl = ch	; rgAl = rg 
			elseif ( nAlign == kALIGN_RT5 )
				Val	= EvT( ch, rg, kE_RISE50 ) -  Eval( 0, 0, kE_BEG, kVAL ) 	; chAl = ch	; rgAl = rg  
			elseif ( nAlign == kALIGN_PK )
				Val	= EvT( ch, rg, kE_PEAK )	 -  Eval( 0, 0, kE_BEG, kVAL ) 	; chAl = ch	; rgAl = rg  
			elseif ( nAlign == kALIGN_DT5 ) 	
				Val	= EvT( ch, rg, kE_DEC50 )	 -  Eval( 0, 0, kE_BEG, kVAL ) 	; chAl = ch	; rgAl = rg  
			elseif ( nAlign == kALIGN_RSLP )
				Val	= EvT( ch, rg, kE_RISSLP ) -  Eval( 0, 0, kE_BEG, kVAL ) 	; chAl = ch	; rgAl = rg  
			endif	
		endfor
	endfor
	// printf "\t\t\tAlignment() \tchAl:%.0lf\trgAl:%.0lf\tnAlign :%2d \t-> \t[ Beg:] \t%7g\t%g ms  \r", chAl, rgAl, nAlign,  Eval( 0, 0, kE_BEG, kVAL )  * 1000, Val * 1000
	return 	Val
End
...
// 060216
static strconstant		ksEVO_HELP_WND	= "Eval Help"   
static strconstant		ksEVO_HELP_PATH	= "FEvalHelp.txt"
...
Function		fEvHelp_( s )
	...
	if (  s.eventCode == kCCE_mouseup ) 
		...
		if ( cmpstr( sPath[0], ":" ) == 0 )
			InternalError( "Could not locate file '" + ksEVO_HELP_PATH + "' ." )
		...
		sPath = ParseFilePath( 1, sPath, ":", 1,0 ) + ksEVO_HELP_PATH// Create path to the help file.
		printf "\t\tfEvHelp( s ) \thelp path: '%s' \r", sPath
	
		OpenNotebook	/K=1	/V=1	/N=$ksEVO_HELP_WND   sPath	// visible, could also use /P=symbpath...
		MoveWindow /W=$ksEVO_HELP_WND	1, 1, 1, 1			// restore from minimised to old size
...
// Called when a button is pressed.  ALSO CALLED when moving the mouse over a button.  See  'fButtonProc_struct()'  on how to avoid this event.
	...
	if (  s.eventCode == kCCE_mouseup ) 
		...
		Panel3Main(   sSavePn, "Evaluation Details3", "root:uf:" + ksEVO_, 2, 95 ) // Compute the location of panel controls and the panel dimensions. Draw the panel displaying and hiding needed/unneeded controls
