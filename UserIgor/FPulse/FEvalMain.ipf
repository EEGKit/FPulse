...
// 2012-02-05  included again  ONLY to avoid linker errors  when starting  FEval  before or without  FPulse
#include "FPAcqCed"					// for  ProcessDigitalOutputs		unfortunately we must include this file even in EVAL to avoid linker errors although the code is actually never used
#include "FPAcqScript"					// for  SweepLenAll				unfortunately we must include this file even in EVAL although very few functions are actually used
#include "FPAcqDisp"					// for  SweepLenAll				unfortunately we must include this file even in EVAL although very few functions are actually used
#include "FPCfsWrite"					// for  InitializeCFSDescriptors_		unfortunately we must include this file even in EVAL although very few functions are actually used
#include "FPOnlineA"					// for  FinishAnalysisFile_			unfortunately we must include this file even in EVAL although very few functions are actually used
...
// 2009-12-10	Even in EVAL  there is a data folder 'aco'  containing  the subfolders  'dlg'  and  'script' .  Unfortinately it seems not easy to distribute to 'evo' ...
	...
	NewDataFolder	/O		$"root:uf:" + sFo				// creates  'root:uf:evo' .  	Holds subfolders which are cleared whenever............(acquis:  a new script is loaded.)
...
//	NewDataFolder  /O	 	$"root:uf:" + sFo + ":" + ksVAR		// required early e.g. for the panel list 'lstPanels'
