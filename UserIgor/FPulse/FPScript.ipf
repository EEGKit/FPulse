...
// creates all folder-specific variables. To be used once from CreateGlobals() as the current data folder is changed and not restored
	NewDataFolder  /O  /S root:uf:aco
	NewDataFolder  /O  /S root:uf:aco:script		// make a new data folder and use as CDF,  clear everything
...
// 2013-01-15 made static
static Function	stCreateGlobalsInFolder_Co( sFolder )
// creates all folder-specific variables. To be used once from CreateGlobals() as the current data folder is changed and not restored
	string  	sFolder
	string		sDFSave		= GetDataFolder( 1 )				// remember CDF in a string.

	NewDataFolder  /O  /S  $ksROOTUF_ + sFolder + ":co"							// acquisition: create a new data folder and use as CDF,  clear everything
...
// the Adc should have 'mV' and not 'pA' as default units to display TGChan correctly (they do not not have their own units) [or leave pA and give them selectively mV]   
	...
	variable	n	= -1 
	make /O /T /N=( 30 )	   root:uf:aco:script:wMK, root:uf:aco:script:wSK
	wave  /T	wMK		= root:uf:aco:script:wMK
	wave  /T	wSK		= root:uf:aco:script:wSK
...
static Function	stMKMustOccurOnce( nMK ) 
...
//	variable	nMK, nSK
//	string	sSubKeys
//	return	WhichListItem( SKNm( nMK, nSK ), sSubKeys, "," ) == kNOTFOUND ? FALSE : TRUE 
//End
...
// For faster access to mainkey and subkey data:  Build mainkey string  'sMainKey'  containing all mainkey  keywords from program template supplied  'wMK'  and  'wSK'
	string  /G	root:uf:aco:script:gsMainKey	= ""
	svar		gsMainKey	= root:uf:aco:script:gsMainKey
	variable	n
	for ( n = 0; n < MKCnt(); n += 1)											// for all mainkeys found in template 
...
// Cave: the value  nIO = kNOTFOUND (=-1) is passed  e.g. in ComputeAverage() when 'Aver' is not defined in script
	wave  /T	wIO
	variable	nIO
	variable	ioUse	= str2num( wIO[ nIO ][ 0 ][ cIOUSED ] )	
	return	( numtype( ioUse ) == kNUMTYPE_NAN )  ?  0  :  ioUse		// returns 0 rather than Nan
...
// Returns  IO string   (including folder)    when  IO type , index into ioType  and  data type to be retrieved is given
	...
	wave  /T	wIO
	variable	nIO, c, nData
	return	ksROOTUF_ + sFolder + ":" + ksF_IO + ":" + ios( wIO, nIO, c, nData )
End
...
Function	/S	FldAcqioPoNio( sFolder, wIO, nIO, c, nData )	 
	...
	wave  /T	wIO
	variable	nIO, c, nData
	return	ksROOTUF_ + sFolder + ":" + ksF_IO + ":" +  "PoN" + sIOCHSEP + ios( wIO, nIO, c, nData )
End
...
Function		vMakeWVal( sFolder, nLines, nKeys, maxFrm )
	...
	make	/O /T /N	= ( nKeys, nLines ) $ksROOTUF_ + sFolder + ":ar:wVal"	
End
...
Static Function	stWhichValScriptLine( wVal, nType )
...
// computing with vGetS()  is very slow : takes 300 us
	...
	variable	nLine	= stWhichValScriptLine( wVal, nMain )
...
Function 	InterpretScript_( sFolder, sWin, bDoAcq )
...
// this routine is very slow when the script loaded contains MANY FRAMES and few sweeps/blocks (e.g 100 and  1/1)  compared to 10/10/1  or 10/1/10.  
	string		sFolder, sWin						// subfolder of root (root:'sF':...) used to discriminate between multiple instances of the  InterpretScript    e.g. from FPulse and from FEval
	variable	bDoAcq
	string		bf	
	variable	nPnts, maxDChs, nBlk, maxFrm, maxEle, nKeys, MainDac

	NewDataFolder  /O	$ksROOTUF_ + sFolder + ":ar"				// for the arrays  e.g.  wIO , wVal , wE
	NewDataFolder  /O	$ksROOTUF_ + sFolder + ":dig"			// for the digital IO strings and waves which control the Ced1401

	string	  /G			$ksROOTUF_ + sFolder + ":dig:gsDigOutChans"	= ""
	string   /G			$ksROOTUF_ + sFolder + ":dig:gsDigoutSlices"		= ""
	variable /G		$ksROOTUF_ + sFolder + ":dig:gnJumpBack"		= 0

	NewDataFolder  /O	$ksROOTUF_ + sFolder + ":stim"			// for the Save/NoSave and Digout display waves on screen
	NewDataFolder  /O	$ksROOTUF_ + sFolder + ":store"			// for the data controlling the storing of periods and the skipping of blanks 
	NewDataFolder  /O	$ksROOTUF_ + sFolder + ":" + ksF_IO	// for the large basic IO waves  e.g. 'Dac0'  ,  'Adc1' , 'PoN1'  and  for  'sDacUnits..'
// test 041028
if ( bDoAcq )
	stCreateGlobalsInFolder_Co( sFolder )						// creates  'root:uf:aco:co'
	NewDataFolder	/O	$ksROOTUF_ + sFolder + ":dispFS"			// root:uf:aco:dispFS  (acquisition) : for the whole bunch of similar display waves Adc..., Dac... with different FrmSwp-suffices
endif

	//ResetStartTimer( "OverAll" )

	// Step 1: Processing 'wLine' 
	stShowLines( sFolder, " original  ")

//keep:wG
	make /O/N = (kMAX)  $ksROOTUF_ + sFolder + ":keep:wG"  = 0	// for all general acquisition variables	e.g. SmpInt, CntAD, Pnts [must not be deleted on 'Apply' after 'Start' as used in XOP]
	wave  	wG	     = 	 $ksROOTUF_ + sFolder + ":keep:wG"  	// There are 2  instances of  'wG :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 

	if ( stExtractLinesIntoIO( sFolder, wG ) )					// Construct wIO
		return kERROR
	endif
	wave  /T	wIO	    = 	 $ksROOTUF_ + sFolder + ":ar:wIO"  	// There are 2  instances of  'wIO' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	
	if ( stCheckMandatorySubkeys( wIO ) )
		return kERROR
	endif
// test 041028
if ( bDoAcq )
	if ( stTelegraphConnect( wG, wIO ) )					// set wG[ nCntxx,...] 
		return kERROR
	endif
endif

	if ( stCheckPresenceRequiredSrcChans( wIO ) )			// step 7: check if the 'Src' channels required by the computed channels are present
		return	kERROR
	endif
	
	stShowIO( wIO ) 			   						// for Debugging, do not delete
	stShowKeys()

	if ( stCheckScriptSetSI( sFolder, wG ) )
		return kERROR
	endif
	
 	nBlk		= stCheck_Frm_Swp_EndSwp_EndFrm( sFolder )	// preliminary block count (not used)...
	if ( nBlk == kERROR )						
		return kERROR							
	endif

 	stExpandLines( sFolder, "Loop", "EndLoop" )
	stShowLines( sFolder, " after loop expansion  ")

	// extract structure of text wave and set values in ' wVal'
	nKeys	= SKMax()								// counts only, does not set.(returns one more, for 'nType').
	maxDChs	= stGetNumberOfDacChannels( sFolder )		//todo eliminate
	
	MainDac	= stExtractFixSweepData( sFolder, wG )			// Construct wFix for FixSweepData, sets eBlocks, eMaxFrames, eMaxSweeps 
	wave  	wFix	 = $ksROOTUF_ + sFolder + ":ar:wFix"  		// There are 2  instances of  'wFix' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 

	if ( stPossiblyInsertIntervals( sFolder, MainDac ) )
		return	kERROR
	endif 
	stShowLines( sFolder, "after inserting IFI / IBI ")	

	if ( stCheckCombinationPoNSweeps( sFolder, wG, wFix ) )
		return	kERROR
	endif
	stAdjustMissingCorrAmp( wG, wFix )
	maxFrm	= eMaxFrames( wG, wFix )
	maxEle	= stSetNumberOfElemsInSweeps( sFolder, wG )	// Construct  wELine
	wave	wEinCB	= $ksROOTUF_ + sFolder + ":ar:wEinCB"	// There are 2  instances of  'wEinCB' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	wave  	wELine	= $ksROOTUF_ + sFolder + ":ar:wELine"  	// There are 2  instances of  'wELine' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	if ( maxEle == kERROR )
		return kERROR
	endif
	...
	wave  /T	wVal	= $ksROOTUF_ + sFolder + ":ar:wVal"  		// There are 2  instances of  'wVal' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	
	if ( stExtractLinesIntoVal( sFolder, wVal, maxFrm ) )
		return kERROR
	endif
	...
	wave  /T	wBFS 	= $ksROOTUF_ + sFolder + ":ar:wBFS"  	// There are 2  instances of  'wBFS' :	in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	wave  	wE	 	= $ksROOTUF_ + sFolder + ":ar:wE"  		// There are 2  instances of  'wE' :  	in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	...
	if ( nPnts <= 0 )
		return kERROR						// no points  or  kERROR because  durations not consistent with sample interval
	endif	

	// Step 4: Processing 'wDGO' Digital outputs
	if ( ProcessDigitalOutputs( sFolder, wG, wVal, wFix, wEinCB, wELine, wE, wBFS, maxEle ) )				// CED1401 dependent
		return kERROR
	endif
...
//endif
	...
	DisplayStimulus( sFolder, sWin, wG, wIO, wFix, kDOINIT )			// displays partial waves with automatic names,  kDOINIT= -1 enforces initialization 

	if ( bDoAcq )						// not if called from FEval
		if ( CEDInitialize_( sFolder ) ) 
			return kERROR 			// executing the Ced initialization already here (and not later when the acquistion is started)  saves valuable 'PreStart'-time 
		endif
	endif
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//   Advance from  'wLine'  to  wIO
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static Function	stExtractLinesIntoIO( sFolder, wG )				
// Read script  wLine (again), get all values from IO section of  the script  and store them in  wIO
// wIO[ nData : Name, Units, YZoom, IsIO... ]  stores script data and computed data  as a strings  AND stores the number of used channels of the given IO type
	...
	variable	nError		= 0
	make  /T /O	/N=(kMAXCHANS*3)	wSortedIOLines
	...
	// Step2 : Copy the sorted IO and Comp lines from 'wSortedIOLines'  back into 'wLine' 
	// after this step 'wLine' contains all  IO and Comp lines in a sorted order between  nFirstLineSorted  and  nLastLineSorted . This simplifies access in the following steps.
	...
	make   /O /T /N=( kIO_MAX, nIOLines, cIOLAST )  $ksROOTUF_ + sFolder + ":ar:wIO" = ""	// nIOLines are too many but will be redimensioned below
	wave	/T 		wIO					=  $ksROOTUF_ + sFolder + ":ar:wIO"		// "" sets the initial count of each IO type to Nan = 0 = missing
	//wIO	= ""
	...
	for ( l = nFirstIOLine; l <= nLastIOLine ; l += 1 )	
		...
		if ( numType( iov( wIO, nIO, 0, cIOUSED ) ) == kNUMTYPE_NAN )
	...
	for ( l = nFirstIOLine; l <= nLastIOLine ; l += 1 )	
		...
		for ( k = 0; k < nKeys; k += 1 )
			...
			if ( strsearch( sData, sVALSEP, 0 ) != kNOTFOUND )
				Alert( kERR_FATAL,  "Could not interpret '" + sData + "' in line '" + sLine + "'. " )
				return	kERROR
			endif
	...
	stPrintIO( wIO, "ReadValuesAndDefaults()" )
	...
	for ( nIO = 0; nIO < kIO_MAX; nIO += 1 )
		cCnt	= ioUse( wIO, nIO )
		for ( c = 0; c < cCnt; c += 1 )
			sData	= stBuildIONm( wIO, nIO, c )							// use script supplied  'Name'  if possible  or else build  from  Nm, Chan, Src
			ioSet( wIO, nIO, c, cIONM, sData )								// store  'Name'  general entries from script or defaults
		endfor
	endfor
	stPrintIO( wIO, "BuildIONm()" )
	
	return	nError
End

static Function   /S	stBuildIONm( wIO, nIO, c )
...
static Function	stCheckMandatorySubkeys( wIO )
	...
	cCnt	= ioUse( wIO, nIO )
	for ( c = 0; c < cCnt; c += 1 )
		nError	+=	stCheckMissing( wIO, nIO, c, cIOCHAN, kERR_FATAL, "After  'Dac:'  the specification   'Chan = n'   is required. " )
		nError	+=	stCheckMissing( wIO, nIO, c, cIOGAIN,  kERR_FATAL, "After  'Dac:'  the specification   'Gain = n'   is required. " )
	endfor

	nIO	= kIO_ADC
	cCnt	= ioUse( wIO, nIO )
	for ( c = 0; c < cCnt; c += 1 )
		nError	+=	stCheckMissing(   wIO, nIO, c, cIOCHAN, kERR_FATAL, "After  'Adc:'  the specification   'Chan = n'   is required. " )
		...
		if (   stIsMissingVal( wIO, nIO, c, cIOGAIN )   &&   stIsMissingVal( wIO, nIO, c, cIOTGCH )   &&   stIsMissingVal( wIO, nIO, c, cIOTGMCCH ) )	
			Alert( kERR_LESS_IMPORTANT, "After  'Adc:'   the specification  'Gain = n'  or  'TGChan = n'  or  'TGMCChan = n'  is expected. Using Adc default gain = " + num2str(cADCGAIN_DEFAULT) )
			ioSet( wIO, nIO, c, cIOGAIN, num2str( cADCGAIN_DEFAULT ) )
		endif
		if ( (  ! stIsMissingVal( wIO, nIO, c, cIOGAIN )   &&  ! stIsMissingVal( wIO, nIO, c, cIOTGCH ) )  ||  (  ! stIsMissingVal( wIO, nIO, c, cIOGAIN )   &&  ! stIsMissingVal( wIO, nIO, c, cIOTGMCCH ) )  ||  (  ! stIsMissingVal( wIO, nIO, c, cIOTGCH )   &&  ! stIsMissingVal( wIO, nIO, c, cIOTGMCCH ) ) )
			Alert( kERR_FATAL, "After  'Adc:'  only 1 of the specifications   'Gain = n'  or  'TGChan = n'  or  'TGMCChan = n'  is allowed. " )
			nError	+= 1
		endif
	endfor
	...
	for ( nIO = kIO_PON; nIO < kIO_MAX; nIO += 1 )
		cCnt	= ioUse( wIO, nIO )
		for ( c = 0; c < cCnt; c += 1 )
			if ( stIsMissingStr( wIO, nIO, c, cIOSRC ) )
				Alert( kERR_FATAL, "After  'PoN:'  or  'Aver:'  or  'Sum:'  the specification   'Src = n'  is required. " )
				nError	+= 1
			endif	
		endfor
	endfor
	stPrintIO( wIO, "CheckMandatorySubkeys()" )
	return	nError
End


static Function	stCheckMissing( wIO, nIO, c,  nData, nErrorLevel, sText )
	wave  /T	wIO
	variable	nIO, c,  nData, nErrorLevel
	string 	sText
	variable	nCode	= 0
	if ( stIsMissingVal( wIO, nIO, c, nData ) )				
		Alert( nErrorLevel, sText )
		if ( nErrorLevel >= kERR_FATAL )
			nCode	= 1
		endif
	endif
	return	nCode
...
static Function	stIsMissingVal( wIO, nIO, c,  nData )
	wave  /T	wIO
	variable	 nIO, c,  nData
	return	numtype( iov( wIO, nIO, c, nData ) ) == kNUMTYPE_NAN  
End

static Function	stIsMissingStr( wIO, nIO, c,  nData )
	wave  /T	wIO
	variable	nIO, c,  nData
...
static Function	stTelegraphConnect( wG, wIO )
...
// Returns  TRUE = 1  or  FALSE = 0  depending on whether the Adc channel specified by linear index in script  'ioch'  is  telegraph controlled by AxoPatch 200 voltage signal
	wave  /T	wIO
	variable	nIO, c
	return   	numtype( iov( wIO, nIO, c, cIOTGCH ) )  !=  kNUMTYPE_NAN  
End
...
// Returns  TRUE = 1  or  FALSE = 0  depending on whether the Adc channel specified by linear index in script  'ioch'  is  telegraph controlled by AxoPatch MultiClamp
	wave  /T	wIO
	variable	 nIO, c 
	return   	numtype( iov( wIO, nIO, c , cIOTGMCCH ) )  !=  kNUMTYPE_NAN
End

Function		TGChan( wIO, nIO, c )
// returns telegraph channel number if linear index  'c'  has a corresponding telegraph channel, else 'kNOTFOUND'
	wave  /T	wIO
	variable	nIO, c									// ioch is linear index in script     
	return	HasTG( wIO, nIO, c ) ? iov( wIO, nIO, c, cIOTGCH ) :  kNOTFOUND
End

Function		TGMCChan( wIO, nIO, c )
// returns MC telegraph channel number if linear index  'c'  has a corresponding MC telegraph channel, else 'kNOTFOUND'
	wave  /T	wIO
	variable	nIO, c									// ioch is linear index in script     
	return	HasTGMC( wIO, nIO, c ) ? iov( wIO, nIO, c, cIOTGMCCH ) :  kNOTFOUND
End

Function	/S	TGMCChanS_( wIO, nIO, c )
// returns MC telegraph 700AB-extended channel number if linear index  'c'  has a corresponding MC telegraph channel, else 'kNOTFOUND'
	wave  /T	wIO
	variable	nIO, c									// ioch is linear index in script     
	// print "TGMCChanS_( ioch )", ios( ioch, cIOTGMCCH )
...
static Function	stCheckPresenceRequiredSrcChans( wIO )
	...
	for ( nIO = kIO_PON; nIO < kIO_MAX; nIO += 1 )				// for all   Comp IO types  e.g. Pon, Sum, Aver
		...
		for ( c = 0; c < cCnt; c += 1 )
			...
			for ( n = 0; n < nSrc; n += 1 )
				...
				string  	lstIOChans	= ioChanList( wIO )
				if ( WhichListItem( sOneSrc, lstIOChans ) == kNOTFOUND )
		 			Alert( kERR_FATAL,  "Source channel '" + sOneSrc + "' required by '" + ioTNm( nIO ) + "' not found in '" + lstIOChans + "' . [CheckPresenceOfRequiredSrcChans]" )
					return	kERROR
				endif
			endfor
		endfor
...
static Function	stPrintIO( wIO, sText )
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	PROCESSING  SCRIPT ~ ' wLine'
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static Function	stShowKeys()
// prints mainkey and subkey data supplied by program in wMK and wSK
	...
	if ( gRadDebgSel > 1  &&  PnDebgShowKeys )	// we save time by skipping this function right here if it is turned off anyway
		for ( nMK = 0; nMK <MKCnt(); nMK += 1 )
			string	sSKs	= SKStr( nMK )
			printf  "\t\t\tShowKeys() \tnMK:%2d/%2d \tT:%2d  I:%2d \tNm:%12s\tSKs:%s \r", nMK, MKCnt(),  MkTyp( nMK ), stMkMustOccurOnce( nMK ),  MkNm( nMK ), sSKs
...
static Function stCheckScriptSetSI( sFolder, wG )
...
// extensive script file error checking is possible
	...
	variable	nMK, l, n, nMatch, LineCnt	= nLines( sFolder ) 
	...
	for ( nMK = 0; nMK < MKCnt(); nMK += 1 )				//  LOOP THROUGH ALL MAINKEYS supplied by programm...
		sMK =  MkNm( nMK )						
		nMatch = 0
		for ( l = 0; l < LineCnt; l += 1)						// Loop through all script lines, get mainkey from script line...
	...
	for ( l = 0; l < LineCnt; l += 1)							// LOOP THROUGH ALL SCRIPT LINES  and get...
		sLine = GetScrLine( sFolder, l )							//...mainkey from script line..
		sMK = StringFromList( 0, sLine, sMAINKEYSEP )	// ...and compare it against entries
 		if ( stCheckUnknownMainkeys( sMK ) )				// ...in 'sMainKey'   supplied by  program
 			return	kERROR
 		endif
 	endfor
	...
	for ( l = 0; l < LineCnt; l += 1)							// LOOP THROUGH ALL SCRIPT LINES  and get...
		sLine = GetScrLine( sFolder, l )					
		if ( stCheckUnknownSubkeys( sLine ) )				// Find and complain about unknown subkeys
			return	kERROR
 		endif
 	endfor 
	...
	for ( l = 0; l < nLines( sFolder ); l += 1)							// LOOP THROUGH ALL SCRIPT LINES  and get...
		sLine = GetScrLine( sFolder, l )					
		stCheckSmpIntsAreEqual( sLine, rnSmpIntMin, rnSmpIntMax )	// Find and complain about  differing  sample intervals
 	endfor 
	if ( rnSmpIntMin == rnSmpIntMax )
		wG[ kSI ]	= rnSmpIntMin
	else
		Alert( kERR_FATAL, "All sample intervals must be equal. Found '" + num2str( rnSmpIntMin ) + "' and '" + num2str( rnSmpIntMax ) + "'" )
		return	kERROR
	endif
...
static	Function	CheckNrOfIntendedMKOccur( nMatch, nMK ) 
	...
	if ( nMatch != 1 &&  stMKMustOccurOnce( nMK ) )
		Alert( kERR_FATAL,  "Mainkey  '" +  MKNm( nMK ) + "'  must occur exactly once, found " + num2str( nMatch ) + " times." )
		return kERROR
	endif
	return 0
End

static Function	stCheckUnknownMainkeys( sMK )
	string	 	sMK
	if ( mI( sMK ) != kNOTFOUND )		
		// print "CheckUnknownMainkeys", sMK,  mI( sMK )
		return 0	
	else
		Alert( kERR_FATAL,  "Could not interpret  main key  '"+ sMK + "'" )
		return kERROR
	endif
End

static Function stCheckUnknownSubkeys( sLine )
	...
	for ( nS = 0; nS < nScriptSKs; nS += 1 )
		...
		if ( WhichListItem( sScriptSk, SKList( sMK, ";" ), ";" ) == kNOTFOUND  )
			Alert( kERR_FATAL,  "Could not interpret  subkey '" + sScriptSKInfo + "' in line '" + sLine + "'" )
			...
			if ( cmpstr( sScriptSKInfo[ 0, 4 ] , "YAxis" ) == 0 )			// as this used to be a valid keyword : just ignore it now and give the user a hint to remove it, but continue processing
				//Alert( kERR_FATAL,  "From V2.24 on there is a change in script keyword and behaviour :\r\tReplace in your script all occurences of \r\t'YAxis = value'  by  'YZoom =  10000 / value'  " )
				Alert( kERR_FATAL,  "From V2.24 the keyword 'YAxis' is obsolete and should be removed.\r\tYou can now store different YZoom values for each trace :\r\t'Preferences'  'Trace / window control bar'  , set Zoom\r\tthen  'Acquis window options'  'Save  display config'  " )
			elseif	 ( cmpstr( sScriptSKInfo[ 0, 4 ] , "YZoom" ) == 0 )		//as this used to be a valid keyword : just ignore it now and give the user a hint to remove it, but continue processing
				Alert( kERR_FATAL,  "From V2.30 the keyword 'YZoom' is obsolete and should be removed.\r\tYou can now store different YZoom values for each trace :\r\t'Preferences'  'Trace / window control bar'  , set Zoom\r\tthen  'Acquis window options'  'Save  display config'  " )
			else
				return kERROR								// this was a typographical error or some other nonsense : do NOT let the user continue
			endif
		endif
	endfor
	return 0	
...
static Function stCheckSmpIntsAreEqual( sLine, rnSmpIntMin, rnSmpIntMax )
// loop  for given mainkey  through all subkeys provided in this script line 
...
static Function	stShowLines( sFolder, sText )
...
static Function	stExpectComplain( sExpectKey, sComplainKeys, nLine, sLine )
	...
	for ( k = 0; k < ItemsInList( sComplainKeys ); k += 1 )
		if ( cmpstr(  StringFromList( k, sComplainKeys ), StringFromList( 0, sLine, sMAINKEYSEP ) ) == 0 )
			Alert( kERR_FATAL,  "Expecting keyword  '" + sExpectKey + "'  in line " + num2str( nLine ) + " : " + sLine )
			return kERROR
		endif
	endfor
	return	0
...
static Function	stCheck_Frm_Swp_EndSwp_EndFrm( sFolder )
...
// .. e.g 'Segment' between Blocks or between Frames and Sweeps. This is checked later in PossiblyInsertIntervals()
	...
	for ( l = 0; l < LineCnt; l += 1)
		...
		if ( depth == 0 )
			if ( cmpstr( "Frames", StringFromList( 0, sLine , sMAINKEYSEP ) ) == 0 )
				...
				continue
			elseif ( stExpectComplain( "Frames", "Sweeps;EndSweep;EndFrame", l, sLine ) )
				return	kERROR
			endif
		endif
		if ( depth == cINFRAME  &&  bMovingIn )
			if ( cmpstr( "Sweeps", StringFromList( 0, sLine, sMAINKEYSEP ) ) == 0 )
				...
				bMovingIn = TRUE
				continue
			elseif ( stExpectComplain( "Sweeps", "Frames;EndSweep;EndFrame", l, sLine ) )
				return	kERROR
			endif
		endif
		if ( depth == cINFRAME  &&  ! bMovingIn )
			if ( cmpstr( "EndFrame", StringFromList( 0, sLine, sMAINKEYSEP ) ) == 0 )
				...
				bMovingIn = TRUE
				continue
			elseif ( stExpectComplain( "EndFrame", "Frames;Sweeps;EndSweep", l, sLine ) )
				return	kERROR
			endif
		endif
		if ( depth == cINSWEEP )
			if ( cmpstr( "EndSweep", StringFromList( 0, GetScrLine( sFolder, l ), sMAINKEYSEP ) ) == 0 )
				...
				bMovingIn = FALSE
				continue
			elseif ( stExpectComplain( "EndSweep", "Frames;Sweeps;EndFrame", l, sLine ) )
				return	kERROR
			endif
		endif
	endfor
	...
	if ( depth == cINSWEEP )
		Alert( kERR_FATAL,  "Expecting terminating 'EndSweep'  and 'EndFrame'  lines following : " + sLine )
		return kERROR
	endif
	if ( depth == cINFRAME )
		Alert( kERR_FATAL,  "Expecting keyword 'EndFrame'  after line " + num2str( l ) + " : " + sLine )
		return kERROR
	endif
...
static Function	stExpandLines( sFolder, sLoopBegKey, sLoopEndKey )
// Step 1: read lines (again), expand 'Loop' - 'EndLoop' 
	...
	// Copy   the  loop expanded temporary text wave into  the  original  script  (overwriting it)
	ConvertToScrLines( sFolder, twLinesExpanded )		// 050205
	killwaves		twLinesExpanded	twOneLoop
End

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static Function	stGetNumberOfDacChannels( sFolder )
// scans ' wLine'  to get count of the 'Dac' keywords. Todo: complain about doublettes
	string  	sFolder
...
static Function	stExtractFixSweepData( sFolder, wG )
// assumes and relies on a syntactically correct Frames/Sweeps structure in the script (must be expanded and checked first...)
	...
	make /O /N = ( b +1 , cMAXFrmSwpPon )	$ksROOTUF_ + sFolder + ":ar:wFix"	= 0	// one more (=the last index) is provided to hold the maximum value 
	wave  	wFix					 = 	$ksROOTUF_ + sFolder + ":ar:wFix"  			// There are 2  instances of  'wELine' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	...
	for ( l = 0; l < nLines( sFolder ); l += 1 )												// loop through all ' wLine' 
...
static Function	stPossiblyInsertIntervals( sFolder, MainDac ) 
// we supply a 'Blank' line   (=InterBlockInterval)	with zero duration and zero ampitude between the blocks if the script does...
// we supply a 'Blank' line  (=InterFrameInterval)	with zero duration and zero ampitude between the frames if the script does...
...
//!  THIS  FUNCTION  DETERMINES  THE  ORDER  OF  THE  ENTRIES  IN  THE  SCRIPT  OR  DEPENDS ON  IT
	...
	if ( stPossiblyInsertBetween( sFolder,  "EndSweep" , "EndFrame" , sInsertLine ) ) 
		return	kERROR
	endif
	if ( stPossiblyInsertBetween( sFolder,  "EndFrame" , "Frames" , sInsertLine ) )
		return	kERROR
	endif
End

static Function	stPossiblyInsertBetween( sFolder, sThisKey, sKeyInNextLine, sInsertLine )
...
// Flaw: no error checking ( 2 Blank lines or wrong keyword is not flagged as error), ' sKeyInNextLine' is ignored
	...
	for ( l = 0; l < LineCnt; l += 1)
		...
		if ( cmpstr( sThisKey, StringFromList( 0, sLine, sMAINKEYSEP ) ) == 0 )	
			...
			if ( gRadDebgSel > 2  &&  PnDebgExpand )
				printf  "\t\t\tExpandLines()  \tPossiblyInsertInterval()  reading '%s' , looking for following 'Blank'  line, found '%s'  \r  ",  sLine, sNextLine
			endif
			if ( cmpstr( "Blank" , StringFromList( 0, sNextLine, sMAINKEYSEP ) ) )					// keyword in next line is NOT 'Blank' ..(could be 'EndFrame or Frames'  or  wrong key  or  last line)
				if ( gRadDebgSel > 2  &&  PnDebgExpand )
					printf  "\t\t\tExpandLines()  \tPossiblyInsertInterval()  inserting  '%s'  between  '%s'  and  '%s'  [%s]\r", sInsertLine, sLine, sNextLine,StringFromList( 0, sNextLine, sMAINKEYSEP ) 
				endif
				l  += 1															// .. so we insert the blank line 
				LineCnt += 1
				InsertScrLines( sFolder, l, 1,   sInsertLine )
			endif
		endif
	endfor
	return	0
...
static Function	stCheckCombinationPoNSweeps( sFolder, wG, wFix )
	string  	sFolder
	wave	wG, wFix
	...
	for ( b = 0; b < eBlocks( wG ); b += 1 )
		if ( ePoN( wFix, b ) )
			...
			if ( eSweeps( wFix, b ) == 1 )
				Alert( kERR_FATAL,  "P over N  correction requested which cannot be executed because number of sweeps = 1. " )
				code	= kERROR
			endif
		endif
	endfor
	if ( bFoundPoN  &&  ! bFoundPoNInStimPart )
		Alert( kERR_LESS_IMPORTANT,  "P over N  display  requested but no PoN correction specified in stimulus part. " )
	endif
	return	code
End

static Function	stAdjustMissingCorrAmp( wG, wFix )
	wave	wG, wFix
	variable	code	= 0
	variable	b, Sweeps
	for ( b = 0; b < eBlocks( wG ); b += 1 )
		variable	CorrAmp, CorrAmpScr	= eCorrAmp( wFix, b )
		if ( numType( CorrAmpScr ) == kNUMTYPE_NAN )		// CorrAmp is missing in script
...
static Function	stSetNumberOfElemsInSweeps( sFolder, wG ) 
// scans ' wLine'  to get count of  the lines of each sweep 
...
// this extraction of  Dac channel information is preliminary  and  is done again more extensively  in .....????? ExtractIO() 
	...
	for ( l = 0; l < nLines( sFolder ); l += 1)
		nIODacChan		= stGetDacChannelFromIO( sFolder, l )
		if ( WhichListItem( num2str( nIODacChan ), sDacChanList ) != kNOTFOUND )
			Alert( kERR_FATAL,  "Dac channel " + num2str( nIODacChan ) + " declared multiple times." )
		endif 
		if ( nIODacChan < kNOTFOUND )
			Alert( kERR_FATAL,  "Illegal Dac channel " + num2str( nIODacChan ) + " in IO section." )
		endif
		if ( nIODacChan != kNOTFOUND )									// skip non-'Dac'-lines
	...
	for ( l = 0; l < nLines( sFolder ); l += 1 )														// loop through all ' wLine'
		nStimDacChan	= stGetDacChannelFromStim( sFolder, l )						
		if ( nStimDacChan < kNOTFOUND )
			Alert( kERR_FATAL,  "***Illegal Dac channel " + num2str( nStimDacChan ) + " in STIM section." )
		endif
		if ( nStimDacChan != kNOTFOUND )
	...
	if ( strlen( sStimTmp ) )	
		Alert( kERR_FATAL,  "Dac channel(s) " + sStimTmp[ 0, strlen( sStimTmp ) - 2 ] + " used but not declared in IO section." )
		return	kERROR
	endif
	...
	if ( strlen( sStimTmp ) )	
		...
		Alert( kERR_FATAL,   "Dac channel(s) " + sStimTmp[ 0, strlen( sStimTmp ) - 2 ] + "  declared in IO section but not used in any segment. " )
		return	kERROR
	endif
	...
	make /O /N = ( MAXDACS, nBlk ) 	$ksROOTUF_ + sFolder + ":ar:wEinCB"	= 0	// wElements_In_Chans_And_Blocks : less channels might be sufficient ?...
	wave	wEinCB    			   = 	$ksROOTUF_ + sFolder + ":ar:wEinCB"
	b = 0
	for ( l = 0; l < nLines( sFolder ); l += 1 )
		if ( l >1  &&  cmpstr( StringFromList( 0, GetScrLine( sFolder, l - 2 ), sMAINKEYSEP ), "EndFrame" ) == 0 )		// the IBI after EndFrame still belongs to the last block
	...
	make  /O /N = ( nChans, nBlk, nMaxElems )   $ksROOTUF_ + sFolder + ":ar:wELine" 	// stores original wVal line number for given frame and element
	wave  	wELine	 				= $ksROOTUF_ + sFolder + ":ar:wELine"  	// There are 2  instances of  'wELine' :  in FPulse  in folder  'Aco'   and in FEval  in folder  'evo' 
	b = 0
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//   NEXT  STAGE  : Advance from  'wLine'  to  wVal  
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static Function	stExtractLinesIntoVal( sFolder, wVal, nFrm  )
// Step 2: read lines (again), get all values
	string  	sFolder
	wave	/T	wVal
	variable	nFrm
	variable	l, nMK, code = 0, LineCnt = nLines( sFolder )
	string		bf 
...
MarkPerfTestTime 200	// ExtractLinesIntoVal: mI
		...
		code += stExtractValue( wVal, nMk, sLine, l, nFrm )					// allow collection of multiple errors
MarkPerfTestTime 203	// ExtractLinesIntoVal: ExtractValue
	...
	// code = code ? kERROR : 0									// return kERROR, no matter how many errors occured	
	return code												// anything != 0 is an error
End


static Function	stExtractValue( wVal, nType, sPulsLine, nEle, nFrm )
...
MarkPerfTestTime 310	// ExtractValue: Init
	for ( k = 0; k < nKeys; k += 1 )
		...
		if ( strsearch( sData, sVALSEP, 0 ) != kNOTFOUND )
			Alert( kERR_FATAL,  "Could not interpret '" + sData + "' in line '" + sPulsLine + "'. " )
			return	kERROR
		endif
...
// return corrected frame number (but don't use it, use 'nFrm' for array size to keep array rectangular)
	...
	if ( nFrames > nFrm )				// more entries in this line than determined by Frames key word
		Alert( kERR_FATAL,  "'" + ms( nTyp ) + "' contains more frames (=" + num2str( nFrames ) + ") than determined by 'Frames' key word (=" + num2str( nFrm ) + "). " )
	endif
	return nFrm
End
...
// Check the intermediate data structure ' wVal' current variables on screen
	...
	if ( gRadDebgSel > 1  &&  PnDebgShowVal )	// we save (a little bit of) time by skipping this function right here if it is turned off anyway
		...
		for  ( l = 0; l < vMaxLines( wVal ); l += 1 )
			stPrintValLine( wVal, l, nFrm )
		endfor
	endif
End

static Function  stPrintValLine( wVal, nLine, nFrm )
	wave	/T	wVal
	variable	nLine, nFrm
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//   INTERFACE  FOR  SCRIPT LINES   = TEXT WAVE  ' wLine'
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static constant	bNEWSCRIPT = 1		// 050205       set to 1 after testing
...
//		printf "nLines() : %d\t != \t%d\r",  ItemsInList( gsCoScript, "\r" ) , numpnts( wLines ) 
//	endif
	if ( bNEWSCRIPT )
		return	ItemsInList( gsCoScript, "\r" )
	else
...
// Get one script line
	...
	if ( nLine < 0 )
		print "GetScrLine index < 0"
		nLine =0
	endif
	if ( ! bNEWSCRIPT )
		if ( nLine > nLines( sFolder ) -1  )	
			print "GetScrLine index  too large", nLine, " >", nLines( sFolder ) - 1 
			nLine = min( nLines( sFolder ) - 1, nLine )
		endif
	endif
//	if ( cmpstr ( StringFromList( nLine, gsCoScript, "\r" ), wLines[nLine ] ) )
//		printf "error GetScrLine  line:%d  '%s' \t\t'%s' \r",  nLine, StringFromList( nLine, gsCoScript, "\r" ), wLines[nLine ]
//	endif
	if ( bNEWSCRIPT )
		return	StringFromList( nLine, gsCoScript, "\r" )
	else
		return	wLines[ nLine ]
	endif
...
// Set one script line, the indexed line must already exist, the list item will possibly be appended.
	string  	sFolder
	variable	nLine
	string		sLine
	wave  /T	wLines	= $ksROOTUF_ + sFolder + ":keep:wLines"
	...
	if ( nLine < 0 )
		print "SetScrLine index < 0"
		nLine =0
	endif
	if ( ! bNEWSCRIPT )
		if ( nLine > nLines( sFolder ) -1  )	
			print "SetScrLine index  too large", nLine, " >", nLines( sFolder ) - 1 
			nLine = min( nLines( sFolder ) - 1, nLine )
		endif
	endif
	gsCoScript	= ReplaceListItem1( sLine, gsCoScript, "\r", nLine )
	wLines[ nLine ] = sLine
	
	if ( cmpstr ( StringFromList( nLine, gsCoScript, "\r" ), wLines[nLine ] ) )
		printf "error SetScrLine  line:%d  '%s' \t\t'%s' \r", nLine,  StringFromList( nLine, gsCoScript, "\r" ), wLines[nLine ]
	endif
End
...
// Make new line at  the end and fill it with 'sText' (wave size will increase)
	...
	svar		gsCoScript	= $ksROOTUF_ + sFolder + ":gsCoScript"
	gsCoScript	= AddListItem( sText, gsCoScript, "\r", nLine )
	printf "InsertScrLineA  line:%d  '%s' \t\t%d\t =?=\t%d \r", nLine,  sText, ItemsInList( gsCoScript, "\r" ),  numPnts(wLines)
	...
	printf "InsertScrLineB  line:%d  '%s' \t\t%d\t =?=\t%d \r", nLine,  sText, ItemsInList( gsCoScript, "\r" ),  numPnts(wLines)

End

Function		InsertScrLines( sFolder, pos, cnt, sInsertLine )
// inserts 'cnt'  lines containing 'sInsertLine'  after line 'pos'  in 'wLine' , increases 'wLine' 
	string  	sFolder
	variable	pos, cnt
	string		sInsertLine
	...
	for ( l = OldLineCnt-1; l >= pos; l -=1 )			// 050205
...
Function		ConvertToScrLines( sFolder, twave )
// inserts 'cnt'  lines containing 'sInsertLine'  after line 'pos'  in 'wLine' , increases 'wLine' 
	string  	sFolder
	wave  /T	twave

	variable	n
	for ( n = 0; n < numPnts( twave ); n += 1 )
		SetScrLine( sFolder, n, twave[ n ] )
	endfor		

	duplicate /O 	twave	$ksROOTUF_ + sFolder + ":keep:wLines"

End

// 050205
//Function  		bLinesOK( sFolder )
//// Check if  wLine has  already been defined
//	string  	sFolder
//	wave /Z /T  wLines	= $ksROOTUF_ + sFolder + ":keep:wLines"
...
//	variable	nLine
//	wave  /T	wLines	= $ksROOTUF_ + sFolder + ":keep:wLines"
//	// print "AppendScrLine1  n:", nLine, numpnts( $ksROOTUF_ + sFolder + ":keep:wLines" )
//	wLines[ nLine ]  = { "" }				// this indeed make the text wave 1 bigger...(without redimensioning?)
//	// print "AppendScrLine2  n:", nLine, numpnts( $ksROOTUF_ + sFolder + ":keep:wLines" )
//End
...
// 021125 could and should??? replace all occurrences of GetScrValS()
	...
	if ( numType( value ) == kNUMTYPE_NAN )					// subkey not found in this script line 
...
static Function	stGetDacChannelFromIO( sFolder, l )
	string  	sFolder
	variable	l
	variable	nIODacChan	= kNOTFOUND
...
static Function	stGetDacChannelFromStim( sFolder, l )
// //!  THIS  FUNCTION  DETERMINES  THE  ORDER  OF  THE  ENTRIES  IN  THE  SCRIPT  OR  DEPENDS ON  IT
	string  	sFolder
	variable	l
	...
	// print	numType( Cha ) == kNUMTYPE_NAN ? kNOTFOUND : Cha	// was correct  line (= correct subkey)  but  channel entry was empty
	return	numType( Cha ) == kNUMTYPE_NAN ? kNOTFOUND : Cha	// was correct  line (= correct subkey)  but  channel entry was empty
End
