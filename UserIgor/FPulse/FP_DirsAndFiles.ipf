...
static Function   /S	stExtractTrailingCharacters_( sPathAndFile, cnt )
// extracts the last 'cnt'  characters from a file name ignoring the extension 
	string 	sPathAndFile
	variable	cnt
	variable	nPos	= strsearch( sPathAndFile,  "." ,  0 )
...
static Function	stLettersToIdx( sLetters, NamingMode )
...
static Function  /S	stIdxToLetters( nIndex, NamingMode )	
...
static Function   /S	stBuildNextPathFile( sPathAndFileOld, nIndex, NamingMode )
// builds the path and file of the next file. Starts with  sPathAndFile  and replaces the 2-letter-combination by  new 2 letters given by nIndex
	...
	sPathAndFile[ nPos-nCharacters, nPos-1 ]  = stIdxToLetters( nIndex, NamingMode )	// replace the 2 characters found by the letters given by the new index
...
// Possible flaws in this version : Processing might be confused  1. mixing naming modes in 1 directory and  2. by mixing small and capital letter file names (user may have renamed the files..) 
	...
	nIndex = stLettersToIdx( stExtractTrailingCharacters_( sPathAndFile, nCharacters ), NamingMode )
	do
		nIndex	+= step
		...
		sNextUsedFile = stBuildNextPathFile( sPathAndFile, nIndex, NamingMode ) 
...
// As we are processing files the processing must be and is case-insensitive, although (for historical reasons)  the appended postfixes are small or capiatal letters..
	...
	string  	sHighestFile
	if ( nFiles == 0 )
		sNextUsedFile	= sFileBase + stIdxToLetters( 0, NamingMode ) + sExt				// create the first file by appending 'a', '0' or 'AA'
	else
		...
		nIndex 		= stLettersToIdx( stExtractTrailingCharacters_( sHighestFile, nCharacters ), NamingMode ) + 1
		...
		sNextUsedFile 	= stBuildNextPathFile( sHighestFile, nIndex, NamingMode ) 
	endif
