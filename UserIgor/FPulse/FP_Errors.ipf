...
//strconstant		lstWARNING_LEVEL	= "message;less important;important;severe;fatal;"	//  Cave ordered the other way . 	Popmenus need semicolon separators
...
Function	/S	PrefPanelName( sFolder ) 
	string  	sFolder
	return	"prf" + sFolder[ 0, 0 ] 				// 'prfe'   in Eval    or  'prfa'  in  FPULSE   .  If this is changed all occurences of  'prfa'  and  'prfe'  must also be changed.
End


Function		WarningLevel( sFolder )
	string  	sFolder
	nvar		nWarningLevel	= $ksROOTUF_ + sFolder + ":" + PrefPanelName( sFolder ) + ":gnWarnLevl0000"
...
Function		WarningBeep( sFolder )
	string  	sFolder
	nvar		bWarningBeep	= $ksROOTUF_ + sFolder + ":" + PrefPanelName( sFolder ) + ":gbWarnBeep0000"
	return	bWarningBeep
End


// Simple version without  global 'gWarningLevel'
 Function	Alert1( nPrgWarnLevel, sMessage )
...
Function		Alert( nPrgWarnLevel, sMessage )
...
Function		FoAlert( sFolder, nPrgWarnLevel, sMessage )
// Display warning message and possibly beep. The user may set the warning level separately in the program parts FPulse (sFolder='aco') and FEval (sFolder='evo') . 
	variable	nPrgWarnLevel
	string 	sFolder, sMessage
	variable   	nWarningLevel	= WarningLevel( sFolder )
	variable   	bWarningBeep	= WarningBeep( sFolder )
	if ( nPrgWarnLevel == kERR_FATAL )				
		DoAlert	0, sMessage
		sMessage = "++++Error: " + sMessage
		printf "%s\r", sMessage
...
Function 		DeveloperError( sMessage )
...
Function 		InternalError( sMessage )
...
Function		 InternalWarning( sMessage )
	string 	sMessage
