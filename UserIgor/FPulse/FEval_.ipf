			...
			for ( ch = 0; ch < gChannels; ch += 1)
				...
				for ( rg = 0; rg < rgCnt;  rg += 1 )	
					...
					if ( nLatOp != kLAT_OFF_ )
						...
						// printf "\t\tAllLatenciesCheck() \tl:%d/%d\t   BegEnd:%d   ch:%d   rg:%d\t%s Latc:%2d\t(%s)   \r", lc, LatCnt, BegEnd, ch, rg,  SelectString( BegEnd, "beg: " , "\t\t\t\t -> end: " ), nLatOp, StringFromList( nLatOp, _klstLATC )
					endif
				endfor
			endfor
		endfor
...
// 060109 Latency RiseSlp
						...
						//printf "\t\tAllLatencies() \tl:%d/%d\t   BegEnd:%d   ch:%d   rg:%d\t%s Latc:%2d\t(%s)   \r", lc, LatCnt, BegEnd, ch, rg,  SelectString( BegEnd, "beg: " , "\t\t\t\t -> end: " ), nLatOp, StringFromList( nLatOp, _klstLATC )
...
//  the idea is to use as few globals as possible and to hide as much of the internals to the calling functions (like 'AddToAverage() '  or  'AddDirectlyToFile() ' )
	...
	sFilePath	= GetNextFileNm( ksfEVO, sFilePath, kSEARCH_FREE, kUP, NamingMode )	// find the next unused file name
	return	sFilePath
End
...
End	
	 ...
	 Function		Evalu( ch, rg, pt, nType )			// calling this  'Evaleval(()'  leads to conflicts with the window name  'Eval'
...
//	 Function		SetEval( ch, rg, pt, nType, Value ) 
	...
	// printf "\t\tSetEval( \tch:%d  rg:%d  pt:%d  nType:%d )  \tstores \t%g    =?= %g  =?= %g \r", ch, rg, pt, nType, value, Evalu( ch, rg, pt, nType ), wEval[ ch ][ rg ][ pt ][ nType ]
End	
...
// creates windows for  Eval  data
	...
	string  	lstCfsWindows	= WinList( ksfEVO_WNM + "*", ";" , "WIN:1" )			// all graphs starting with 'Eval' , in this case 'Eval0' , Eval1' , Eval2' ...
	...
	// print  "\t\tDebug1 : \t\t\tGraphs 'Eval..':", WinList( ksfEVO_WNM + "*", ";" , "WIN:1" ) , "all windows 'Eval..' except Graphs (should be none): ", WinList( ksfEVO_WNM + "*", ";" , "WIN:214" )
...
Function	/S	EvalWndNm( ch )
	variable	ch
	return	ksfEVO_WNM + num2str( ch ) 			 					//   window names must not contain '('  or  ')'
End
