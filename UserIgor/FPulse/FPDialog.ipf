...
Function	PnXsize( twPanel )
	wave /T	twPanel
	return	stMaxXLen( twPanel ) + 2 * kXMARGIN
End

Function	PnYsize( twPanel )
	wave /T	twPanel
	variable	nEmptySeps	= stCountEmptySeparators( twPanel )	
	variable	yTotalTcHeight	=stTotalHeightOfAllTabControls( twPanel )
	// if there is no separator text we make it lower to gain screen space
	variable	ControlCntNoTC	= stControlCntNoTabControl( twPanel )
	variable	yPixHeight	= ControlCntNoTC * ( kYHEIGHT + kYMARGIN ) - stDecreaseSeparatorHeight( nEmptySeps ) + yTotalTcHeight + kYMARGIN		// if there is no separator text we make it lower to gain screen space
...
static Function	stTotalHeightOfAllTabControls( tw )	
	wave  /T	tw
...
static Function	stControlCntNoTabControl( tw )	
...
static Function	stCountEmptySeparators( tw )	
	wave  /T	tw
...
static Function	stDecreaseSeparatorHeight( nEmptySeps )
	variable	nEmptySeps 
...
static Function	stGetXSize( sText  ) 
...
// 031027  TODO (minor importance) : the field size of the PN_DICOLTXT  field  (and consequently the total line length) is not in all cases adjusted properly to the strlen of the 1. text item in the possibility list 
		nTitleLen		= stTextLenToPixel( StringFromList( 0, StringFromList( TXT, sControl ), ksTILDE_SEP ) )	// the text in the button  or  the 1. string item (up to ksTILDE_SEP ~ ) in the possibility list
		//nTitleLen	= stTextLenToPixel( StringFromList( TXT, sControl ) )							// the text in the button  or  total length of all string items in the possibility list  // 031027 old
		...
		if ( cmpstr( sType, "PN_TABCTRL" )  == 0 )					
			...
			nTitleLen = stTextLenToPixel( sTabs ) + nTabs * 18			// supply extra X width for the tabs
		endif
...
Function		UpdatePanel(  sPnWndNm, sPnTitle, sF, sPnOptions )
	...
	if ( WinType( sPnWndNm ) == kPANEL )							// only if the panel does already exist then ...
		...
		string		sPanelWvNm = ksROOTUF_ + sF + sPnOptions
...
Function		DrawPanel( sF, tw, XSize, xLoc, yLoc, sPanelTitle )
	...
	stDrawPanelControls( sF, tw, XSize - 2 * kXMARGIN, sPanelTitle )
End


static Function	stDrawPanelControls( sThisF, tw, nXsize, sPanelTitle )
...
// CAVE   pd()  is sometimes not padding correctly here........
	wave  /T	tw
	variable	nXsize
	string		sThisF				// ksfACO  or  ksEVAL
	string	  	sPanelTitle
	...
	string  	sDisable		= ExtractAndClean( kDLG_DISABLE, sControl )	// 050221
			...
			elseif ( cmpstr( sType, "PN_BUTPICT" )  == 0 )										
				...
				CheckBox	$sFo_Name,	value	= stFolderGetV( sF, sName, OFF )
...
// print "kika", sLen, nXSize,nXSize/ ControlsInLine,(nXSize/ ControlsInLine-7 -str2num( sLen ) ) /3, pd( sTitle, (nXSize/ ControlsInLine-7 -str2num( sLen ) ) /3 ) 
			...
			elseif (  cmpstr( sType, "PN_SEPAR" ) == 0  ||  cmpstr( sType, "kSEPAR" ) == 0 )		
				if ( strlen( RemoveWhiteSpace( sTitle ) ) ) 
					...
					nYpos	= nYpos - stDecreaseSeparatorHeight( 1 )					// if there is no separator text then make it lower
				endif	
...
Function		PanelCheckbox( c, ControlsInLine, nXpos,  nYpos, nXsize, nYsize, nOverSize, sCleanName, sTitle, sPanelTitle, sProc, sThisF, sF, sCNm, sControl, sType, sDefProc )
	...
	CheckBox	$sCNm,	value	= stFolderGetV( sF, sName, OFF )
...
Function		PanelRadio( c, ControlsInLine, nXpos,  nYpos, nXsize, nYsize, nOverSize, sCleanName, sTitle, sPanelTitle, sProc, sThisF, sF, sCNm, sControl, sType, sDefProc )
	...
	CheckBox $sCNm,	proc		= $sProcNm	
	CheckBox	$sCNm,	value 	= stRadioButtonBoolValue( sCNm )// sFo_Name )
...
//return -1
	...
	elseif (  s.eventCode == kCCE_draw ) 
		...
		if ( kWINDOWS_LIKE_BUTTON )
			...
			//DrawRect   	2, 1, xSize-2, ySize - 2			// the rectangle drawn covers only the interior of the button
			if ( s.nVal )
				stColorButtonDrawLeftTop( xSize, ySize, 40000 )
				stColorButtonDrawRightBot( xSize, ySize, 60000 )
			else
				stColorButtonDrawLeftTop( xSize, ySize, 60000 )
				stColorButtonDrawRightBot( xSize, ySize, 40000 )
			endif
			...
			if ( cmpstr( sCtrlNm, s.ctrlName ) == 0 )				// for unknown reasons  all controls are drawn without this code, so we must mask out all controls except the one we are interested in
				if ( bOverControl  &&  s.nVal )				// !!! this is bad and fragile code as we have to store and retrieve the control's (global) name at appropriate times
					...
					SetDrawEnv  	linethick = 0 ,  fillfgc = ( str2num( StringFromList( 0, sOneColor, "," ) ), str2num( StringFromList( 1, sOneColor, "," ) ), str2num( StringFromList( 2, sOneColor, "," ) ) )
					DrawRect   	1, 1, xSize-1,   ySize-1     				// the rectangle drawn covers the most of the inside but leaves a margin
					stColorButtonDrawLeftTop( xSize, ySize, 20000 )
					stColorButtonDrawRightBot( xSize, ySize, 64000 )
				endif
				if ( bOverControl  &&  !s.nVal )
					stColorButtonDrawLeftTop( xSize, ySize, 64000 )
					stColorButtonDrawRightBot( xSize, ySize, 20000 )
				endif
				if ( bPressed )
					stColorButtonDrawLeftTop( xSize, ySize, 20000 )
					stColorButtonDrawRightBot( xSize, ySize, 64000 )
...
static Function	stColorButtonDrawLeftTop( xSize, ySize, nColor )
...
static Function	stColorButtonDrawRightBot( xSize, ySize, nColor )
	variable	xSize, ySize, nColor
	SetDrawEnv  	linethick = 1 ,  linefgc = ( nColor, nColor, nColor ) , save
	...
	for ( i = 0; i < cnt;  i += 1 )								// 050223 if sType2 == PN_POPUP  then  1 function name is passed instead of  sTitleList !!!
		...
		for ( j = 0; j < cntHV; j += 1 )
			...
			if ( cmpstr( sType2, "PN_CHKBOX" ) == 0  ||  cmpstr( sType2, "PN_CHKBOXT" ) == 0 )
				...
				stPossiblyBuild1SubFolderAndVar( sVarNm , TRUE, bState )
			elseif (cmpstr( sType2, "PN_RADIO" ) == 0  ||  cmpstr( sType2, "PN_RADIOT" ) == 0 ) 	
				...
				sVarNm	= stRadButVarNm( nDims, j, sFolderVarNmBase , sTitle, ItemsInList( sTitleListHV ) ) // double indexing: the 1.dim of 2  is stored in the base name,  thus the old functions processing only 1 dim can be used
			elseif (cmpstr( sType2, "kRADIO" ) == 0  ||  cmpstr( sType2, "kRADIOT" ) == 0 ) 
				...
				sVarNm	= ksROOTUF_ +  sF + ":"  + stRadButVarNm( nDims, j, sFolderVarNmBase , sTitle, ItemsInList( sTitleListHV ) ) 	// double indexing: the 1.dim of 2  is stored in the base name,  thus the old functions processing only 1 dim can be used
			endif
...
//		if ( cmpstr( sType2, "PN_RADIO" ) == 0  ||  cmpstr( sType2, "PN_RADIOT" ) == 0 )	
		if ( cmpstr( sType2, "PN_RADIO" ) == 0  ||  cmpstr( sType2, "PN_RADIOT" ) == 0  ||  cmpstr( sType2, "kRADIO" ) == 0  ||  cmpstr( sType2, "kRADIOT" ) == 0 )	
			stPossiblyBuild1SubFolderAndVar( sVarNm , FALSE, bState )			// do NOT build the variable and ignore  'bState'
...
static Function	stPossiblyBuild1SubFolderAndVar( sVarNm , bBuildVariable, bState )
...
Static Function	/S	stRadButVarNm( nDims, i, sFolderVarNmBase, sVTitle, cnt )
...
// ...and there is only one help topic (=the sSepText line above the buttons) which is connected separately to each button
	...
	string  	sProc	= stRadioButtonProcNm( sControlNm, nDims )
...
// ...and there is only one help topic (=the sSepText line above the buttons) which is connected separately to each button
	...
	string 	sProc 
	RadioCheckUncheck( sControlNm, Value ) 						// check / uncheck all the radio buttons of this group
	sProc		    = stRadioButtonProcNm( sControlNm, nDims )
...
Static Function  	 stRadioButtonCnt( sFolderIndexedCtrlName )
...
Static Function 	 stRadioButtonIndex( sFolderIndexedCtrlName )
...
static Function  /S	stRadioButtonProcNm( sControlNm, nDims )
// delete the indices of the radio button checkboxes: extract the one and only global variable(including folder)  for this radio button group (=the index of the ON button)  
	string  	sControlNm
	variable	nDims
	string  	sFolderBaseName
...
static Function 	stRadioButtonBoolValue( sFolderIndexedCtrlName )
...
// check and uncheck the radio buttons of this group 
	string		ctrlName
	variable	bValue			
	variable 	n, cnt = stRadioButtonCnt( ctrlName )
	...
	for ( n = 0; n < cnt; n += 1 )
		...
		if ( cmpstr( sRadio, ctrlName ) == 0 )						// check (=turn on) the one clicked button... 
			...
			stFolderSetV( rsFolder, rsVar, n )						// store state of radio button group in ONE global variable
