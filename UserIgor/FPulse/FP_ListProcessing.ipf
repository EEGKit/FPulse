...
Function	/S	RemoveLastListItems( cnt, sList, sSep )
...
Function  /S	ReplaceListItem( sItem, sList, sListSep, nItem )
...
Function  /S	ReplaceListItem1( sItem, sList, sListSep, nItem )
// Replaces the list item at position 'nItem' .  'sListSep'  may be or may be missing at the end of the list. 
...
// ????? empty list and nItem=0???
	...
	// printf "\t\tReplaceListItem1()\tListitems: %d \t ->\t%d .\tReplacing item %d  \tby %s\t : \t%s\t ->\t%s \r",  nOldItems, ItemsInList( sList, sListSep), nItem, pd(sItem,9), sOldList, sList
	return	sList
End

Function	/S	PossiblyAddListItem( sItem, sList )
// Adds item to end of list, but only if item is not already in the list
	...
	if ( WhichListItem( sItem, sList )  ==  kNOTFOUND )
...
Function	/S	LastListItem( sList, sSep )
...
Function		WhichListItemNoCase( sItem, sList )
...
Function		ItemsInList2( str, sSep1, sSep2 )	
...
Function	/S	StringFromList2( n, str, sSep1, sSep2 )	
...
Function	/S	SortListExt( lstListToSort, sSkipLeading, nSortOrder )	
...
// The order of list entries not starting  with  'sSkipLeading' is undefined  but all are together at the end (or beginning) of the list
	...
	for ( n = 0; n < nItems; n += 1 )
		...
		if ( cmpstr( sItem[0, len - 1 ] , sSkipLeading ) == 0 )  						// if  the wnd name starts with 'W' it is probably an acquis window...
			lstSorted = AddListItem( StringFromList( n, lstListToSort )[len,Inf] , lstSorted )	//..which we add to our acquis wnd list after the 'W' has been removed.
