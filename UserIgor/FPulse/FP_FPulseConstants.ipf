...
strconstant	ksfACO_			= "aco:"			// the subfolder for the  'acquisition'  variables. Do not change as action proc names (and globals in panels) depend on it.
strconstant	ksfEVO_			= "evo:"			// the subfolder for the  'evaluation'  variables. Do not change as action proc names rely on it. At most 3 letters or else control action proc names will be too long)
strconstant	ksF_IO_			= "io:"

strconstant	ksfACO			= "aco"			// the subfolder for the  'acquisition'  variables. Do not change as action proc names (and globals in panels) depend on it.
strconstant	ksfEVO			= "evo"			// the subfolder for the  'evaluation'  variables. Do not change as action proc names rely on it. At most 3 letters or else control action proc names will be too long)
strconstant	ksF_IO			= "io"

//strconstant	ksVAR			= "vars"			// sub directory for  FPulse 
// 060127
//strconstant	ksKEEP 			= "pul"			// DO NOT CAHANGE unless VERY MANY variables (and control names .._aco_pul_.. ) are also changed.  Valid for   FPULS   and   Eva  .  Must be a  folder which is NOT cleared when a new script is loaded.
strconstant	ksKEEPwr 		= "Raw"			// for 'wRaw' .	Valid for   FPULS   and   Eva  .  Must be a  folder which is NOT cleared when a new script is loaded 
strconstant	ksKEEPwl 		= "Lines"			// for 'wLines' .	Valid for   FPULS   and   Eva  .  Must be a  folder which is NOT cleared when a new script is loaded 
strconstant	ksKPwg 			= "G"			// for 'wG' . 	Valid for   FPULS   and   Eva  .  Must be a  folder which is NOT cleared when a new script is loaded 
...
// Constants used when loading a script ( needed in FPULS and Eval)
constant		cPROT_AWARE 		= 0			// 030926..031014	cPROT_AWARE=0 : Normal, memory saving mode.    cPROT_AWARE=1 : all Prots are stored during acquis in BIG Adc Dac waves		
...
constant		cELIMINATE_BLANK 	= 1			// 031120  1 : do NOT transfer 'Blank' periods (which are not to be stored in the CFS file)  between computer and host. Improves loading time and acquis data rates.  DOES PROBABLY NOT WORK WITH cPROT_AWARE = 1
...
strconstant	ksfEVO_WNM		= "FEva"		// do not  name this  'Eval'   which will lead to name conflicts with FEval   
