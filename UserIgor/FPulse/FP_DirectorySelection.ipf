...
Function	/S	DirectoryDialog( sPath )				
...
// always returns a valid path: If the user CANCELS the initially passed path is returned unchanged
 ...
 printf "xUtilxUtil\t\tDirectoryDialog(4) \txUtilFileDialog(  %d,  '%s',  '' ,  '%s' )  returns  sSelectedPath: '%s' \t \r",  cDIRINDEX, sPath,  csDIRDUMMY , sSelectedPath	
	...
	sPath = SelectString( bSelectionOK, sPath, StripFileAndExtension( sSelectedPath )	)// only if  NOT CANCELLED we overwrite the selected path  (after removing the dummy file '_' )  for returning it
	// printf "\t\tDirectoryDialog() \tDialog box returned: %s  \t-> we return: %s\t%s \r", pd(sSelectedPath,24),  pd( sPath,24), SelectString( bSelectionOK, "User cancelled", "Selection OK" )
	return	sPath
End

Function	/S	DirectoryCheckAndPossiblyDialog( sPath )					
...
// Returns an empty path  if the user makes nonsense input or cancels
	...
	variable	bDirExists		= FPDirectoryExists( sPath )	// Check if the passed  'sPath'  is already a  valid directory....
	// printf "\t\tDirectoryCheckAnd...1() \tsPath: %s\t-> Dir exists:%d   -> %s \r", pd( sPath,30 ), bDirExists, SelectString( bDirExists, "search parent dir and  display dialog", "return this dir" )
	if ( ! bDirExists ) 									// .. if it is then use it directly without  displaying the dialog box
		do										// walk back in the directory hierarchy level for level towards the root...
			...
			bDirExists	= FPDirectoryExists( sPath )
			// printf "\t\tDirectoryCheckAnd...2() \tsPath: %s\t-> Dir exists:%d\tSelectP: %s \t \r", pd( sPath,30 ), bDirExists, pd( sSelectedPath,30)	
		while ( ! bDirExists )							// ...until a valid directory is found : Use this as a starting directory for the dialog box below

 printf "\t\txUtilxUtilDirectoryCheckAnd...2() \tsPath: %s\t-> Dir exists:%d\tSelectP: %s \t \r", pd( sPath,30 ), bDirExists, pd( sSelectedPath,30)	
		sPath	= xUtilFileDialog( csDIRPROMPT, csDIRFILTER, cDIRINDEX, sPath, "", csDIRDUMMY )	// sample for  directory selection dialog
		sPath	= StripFileAndExtension( sPath )
	endif
...
Function	/S	FileDialog( sPath, sFilter )				
	...
	string 	sSelectedPath		= ""					// temporary storage for  dialog selection, will be empty  string  if user cancelled
	 ...
	 printf "\t\xUtilxUtilFileDialog(1) receives\tsPath: '%s'      sFilter: '%s'  \r", pd( sPath,30 ), sFilter
	...
	if ( strlen( sSelectedPath ) )
	 ...
	 printf "\t\tFileDialog(2) returns\tsPath: %s\tThe dialog box returned   sSelectedPath: '%s' \t \r", pd( sPath,30 ), sSelectedPath	
	return	sPath 
End
